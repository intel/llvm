...
UR_APIEXPORT ur_result_t UR_APICALL urMemBufferCreate(
  ...
  // CL_MEM_USE_HOST_PTR requires alignment to CL_DEVICE_MEM_BASE_ADDR_ALIGN
  // and unified host memory. When either is missing, fall back to
  // CL_MEM_COPY_HOST_PTR so the driver allocates its own aligned storage.
  // Write-back to the user pointer is the responsibility of the caller
  // (e.g. SYCL's scheduler via urEnqueueMemBufferRead).
