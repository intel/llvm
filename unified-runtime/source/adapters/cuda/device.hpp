...
public:
  ur_device_handle_t_(native_type cuDevice, CUcontext cuContext, CUevent evBase,
      ...
      : CuDevice(cuDevice), CuContext(cuContext), EvBase(evBase), RefCount{1},
        ...
        MemProvidersDevice(cuContext, cuDevice, UMF_MEMORY_TYPE_DEVICE),
        MemProvidersShared(cuContext, cuDevice, UMF_MEMORY_TYPE_SHARED) {
  ...
  // get a UMF memory pool for the given CUDA allocation flags
  CUDAFlagsUMFVendor MemProvidersDevice;
  umf_memory_pool_t *getMemoryPoolDevice(unsigned flags = 0);

  // UMF CUDA memory provider for the shared memory (UMF_MEMORY_TYPE_SHARED)
  CUDAFlagsUMFVendor MemProvidersShared;
  umf_memory_pool_t *getMemoryPoolShared(unsigned flags = 0);
};
