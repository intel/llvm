...
UR_APIEXPORT ur_result_t UR_APICALL urEnqueueKernelLaunchWithArgsExp(
  ...
  for (uint32_t i = 0; i < numArgs; i++) {
    ...
    case UR_EXP_KERNEL_ARG_TYPE_MEM_OBJ: {
      ...
      hKernel->Args.addMemObjArg(i, ArgMem, pArgs[i].arg.memObjTuple.flags);
      void *HIPPtr = std::get<BufferMem>(ArgMem->Mem).getPtr(Device);
      hKernel->setKernelArg(pArgs[i].index, sizeof(void *), (void *)&HIPPtr);
      break;
    }
