...
ur_result_t urSamplerCreate(
  ...
  //
  ur_device_handle_t Device = asInterface(Context)->getDevices()[0];

  ze_sampler_handle_t ZeSampler;
  ZeStruct<ze_sampler_desc_t> ZeSamplerDesc;
  ze_api_version_t ZeApiVersion = asInterface(Context)->getPlatform()->ZeApiVersion;
  UR_CALL(ur2zeSamplerDesc(ZeApiVersion, Props, ZeSamplerDesc));

  ZE2UR_CALL(zeSamplerCreate, (asInterface(Context)->getZeHandle(), Device->ZeDevice,
                               &ZeSamplerDesc, // TODO: translate properties
                               &ZeSampler));

  try {
    ur_sampler_handle_t_ *UrSampler =
        new ur_sampler_handle_t_(ZeSampler, Context);
...
ur_result_t urSamplerRelease(
    ...
    ur_sampler_handle_t Sampler) {
  auto *pSampler = cast(Sampler);
  if (!pSampler->RefCount.release())
    return UR_RESULT_SUCCESS;

  if (checkL0LoaderTeardown()) {
    auto ZeResult = ZE_CALL_NOCHECK(zeSamplerDestroy, (pSampler->ZeSampler));
