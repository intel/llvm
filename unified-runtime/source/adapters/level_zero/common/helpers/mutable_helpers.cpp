...
ur_result_t updateCommandBufferUnlocked(
  ...
  for (uint32_t i = 0; i < NumKernelUpdates; i++) {
    ...
    auto Command =
        reinterpret_cast<kernel_command_handle *>(CommandDesc.hCommand);

    // Both v1's and v2's ur_kernel_handle_t_ inherit from a handle base that
    // lays out `ddi_table` then `Mutex` at offset 0 (required by the loader
    // intercept layer). Reach `Mutex` through that ABI-stable layout without
    // depending on either adapter's full struct definition.
