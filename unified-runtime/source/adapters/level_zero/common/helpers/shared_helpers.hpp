...
#define ZE2UR_CALL(ZeName, ZeArgs)                                             \
  {                                                                            \
    ...
    if (auto Result = ::ur::level_zero::ZeCall().doCall(ZeResult, #ZeName,     \
                                                        #ZeArgs, true))        \
      return ::ur::level_zero::ze2urResult(Result);                            \
  }
...
#define ZE2UR_CALL_THROWS(ZeName, ZeArgs)                                      \
  {                                                                            \
    ...
    if (auto Result = ::ur::level_zero::ZeCall().doCall(ZeResult, #ZeName,     \
                                                        #ZeArgs, true)) {      \
      throw ::ur::level_zero::ze2urResult(Result);                             \
    }                                                                          \
  }

// Perform traced call to L0 without checking for errors
#define ZE_CALL_NOCHECK(ZeName, ZeArgs)                                        \
  ::ur::level_zero::ZeCall().doCall(ZeName ZeArgs, #ZeName, #ZeArgs, false)

#define ZE_CALL_NOCHECK_NAME(ZeName, ZeArgs, callName)                         \
  ::ur::level_zero::ZeCall().doCall(ZeName ZeArgs, callName, #ZeArgs, false)
