...
ur_result_t urPhysicalMemCreate(
  ...
  ZE2UR_CALL(zePhysicalMemCreate,
             (asInterface(hContext)->getZeHandle(), hDevice->ZeDevice,
              &PhysicalMemDesc, &ZePhysicalMem));
  try {
    *phPhysicalMem = reinterpret_cast<::ur_physical_mem_handle_t>(
        new ur_physical_mem_handle_t_(ZePhysicalMem, hContext));
...
ur_result_t urPhysicalMemRetain(ur_physical_mem_handle_t hPhysicalMem) {
  cast(hPhysicalMem)->RefCount.retain();
  return UR_RESULT_SUCCESS;
}

ur_result_t urPhysicalMemRelease(ur_physical_mem_handle_t hPhysicalMem) {
  auto *pPhysMem = cast(hPhysicalMem);
  if (!pPhysMem->RefCount.release())
    return UR_RESULT_SUCCESS;

  if (checkL0LoaderTeardown()) {
    ZE2UR_CALL(zePhysicalMemDestroy,
               (asInterface(pPhysMem->Context)->getZeHandle(),
                pPhysMem->ZePhysicalMem));
  }
  delete pPhysMem;
...
ur_result_t urPhysicalMemGetInfo(ur_physical_mem_handle_t hPhysicalMem,
  ...
  case UR_PHYSICAL_MEM_INFO_REFERENCE_COUNT: {
    return ReturnValue(cast(hPhysicalMem)->RefCount.getCount());
  }
  default:
