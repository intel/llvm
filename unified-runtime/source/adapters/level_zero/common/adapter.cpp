...
// Weak fallbacks for the v1 and v2 adapter sentinels. When v1's or v2's
// TUs are linked alongside common/, the strong definition from that
// adapter's adapter.cpp overrides the weak one here. In single-adapter
// builds the missing adapter's weak symbol stays at nullptr, so any
// shared code that probes `Adapter == GlobalAdapterVx` correctly sees
// "the other adapter isn't present".
__attribute__((weak)) ur_adapter_handle_t_ *GlobalAdapter = nullptr;
__attribute__((weak)) ur_adapter_handle_t_ *GlobalAdapterV2 = nullptr;
...
ur_result_t urAdapterGetInfo(ur_adapter_handle_t hAdapter,
  ...
  case UR_ADAPTER_INFO_VERSION:
    return ReturnValue(hAdapter->version);
  default:
