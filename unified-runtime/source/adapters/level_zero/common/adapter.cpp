...
ur::level_zero::ZeUSMImportExtension ur::level_zero::ZeUSMImport;
...
// GlobalAdapter used by both L0v1 and L0v2. Only one of the adapters is
// selected at the runtime.
ur_adapter_handle_t_ *GlobalAdapter = nullptr;
std::mutex GlobalAdapterMutex;
...
ur_result_t adapterStateTeardown() {
  // On Windows the adapter library is unloaded before UMF, so tear UMF
  // down here.
#if defined(_WIN32)
  umfTearDown();
#endif
...
ur_result_t urAdapterRetain(::ur_adapter_handle_t hAdapterOpque) {
  if (!hAdapterOpque)
    return UR_RESULT_ERROR_INVALID_NULL_HANDLE;
  common_cast(hAdapterOpque)->RefCount.retain();
...
ur_result_t urAdapterSetLoggerCallback(
    ::ur_adapter_handle_t hAdapterOpque, ur_logger_callback_t pfnLoggerCallback,
...
ur_result_t urAdapterSetLoggerCallbackLevel(::ur_adapter_handle_t hAdapterOpque,
  ...
  if (hAdapterOpque) {
    common_cast(hAdapterOpque)->logger.setCallbackLevel(level);
  }
