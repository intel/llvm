...
ur_result_t urPlatformGetInfo(
  ...
  case UR_PLATFORM_INFO_ADAPTER:
    // A platform belongs to whichever adapter's DDI table it carries.
    // Test positively against GlobalAdapterV2 so we never dereference
    // GlobalAdapter, which is nullptr in v2-only builds (v1's
    // urAdapterGet never runs there).
    if (GlobalAdapterV2 &&
        Platform->ddi_table == GlobalAdapterV2->ddi_table) {
