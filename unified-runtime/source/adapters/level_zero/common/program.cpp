...
ur_result_t urProgramCreateWithBinary(
  ...
  try {
    for (uint32_t i = 0; i < numDevices; i++) {
      ...
      UR_ASSERT(asInterface(hContext)->isValidDevice(phDevices[i]),
...
ur_result_t urProgramBuild(
    ...
    const char *Options) {
  std::vector<ur_device_handle_t> Devices = asInterface(Context)->getDevices();
...
ur_result_t urProgramBuildExp(
  ...
  // UR_ASSERT(Program->asInterface(Context)->isValidDevice(Devices[0]),
  ...
  for (uint32_t i = 0; i < numDevices; i++) {
    ...
    ze_context_handle_t ZeContext = asInterface(hProgram->Context)->getZeHandle();
...
ur_result_t urProgramCompile(
    ...
    /// [in][optional] pointer to build options null-terminated string.
    const char *Options) {
  auto devices = asInterface(Context)->getDevices();
  ...
  try {
    ...
    *phProgram = reinterpret_cast<ur_program_handle_t>(UrProgram);
    for (uint32_t i = 0; i < numDevices; i++) {
      ...
      ze_device_handle_t ZeDevice = phDevices[i]->ZeDevice;
      ze_context_handle_t ZeContext = asInterface(hContext)->getZeHandle();
...
ur_result_t urProgramDynamicLinkExp(
  ...
  try {
    ...
    for (ur_device_handle_t Device : asInterface(hContext)->getDevices()) {
      for (uint32_t I = 0; I < count; ++I) {
...
ur_program_handle_t_::ur_program_handle_t_(state St,
    ...
    : Context{Context}, NativeProperties{nullptr}, OwnZeModule{true},
      AssociatedDevices(::asInterface(Context)->getDevices()), ILCode{new uint8_t[Length]},
  ...
  for (auto &Device : ::asInterface(Context)->getDevices()) {
...
ur_program_handle_t_::ur_program_handle_t_(ur_context_handle_t Context)
    : Context{Context}, NativeProperties{nullptr}, OwnZeModule{true},
      AssociatedDevices(::asInterface(Context)->getDevices()) {
  ddi_table = ::ddiTableOf(Context);
}
...
ur_program_handle_t_::ur_program_handle_t_(state, ur_context_handle_t Context,
    ...
    : Context{Context}, NativeProperties{nullptr}, OwnZeModule{true},
      AssociatedDevices({::asInterface(Context)->getDevices()[0]}),
...
ur_program_handle_t_::ur_program_handle_t_(state St,
    ...
    : Context{Context}, NativeProperties{nullptr}, OwnZeModule{true},
      ErrorMessage{ErrorMessage}, AssociatedDevices(::asInterface(Context)->getDevices()) {
  ddi_table = ::ddiTableOf(Context);
  for (auto &Device : ::asInterface(Context)->getDevices()) {
    DeviceData &PerDevData = DeviceDataMap[Device->ZeDevice];
    PerDevData.State = St;
