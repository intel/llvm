...
ur_result_t bindlessImagesCreateImpl(ur_context_handle_t hContext,
  ...
  if (Sampled) {
    ze_api_version_t ZeApiVersion =
        asInterface(hContext)->getPlatform()->ZeApiVersion;
    UR_CALL(ur::level_zero::common::ur2zeSamplerDesc(ZeApiVersion, pSamplerDesc,
                                                     ZeSamplerDesc));
  ...
  ze_context_handle_t zeCtx = asInterface(hContext)->getZeHandle();
...
ur_result_t urUSMPitchedAllocExp(ur_context_handle_t hContext,
  ...
  ZE2UR_CALL(
      ...
      (asInterface(hContext)->getZeHandle(), ZeDeviceTranslated, Width, height,
       elementSizeBytes, &RowPitch));
  *pResultPitch = RowPitch;

  size_t Size = height * RowPitch;
  // Dispatch through the context's DDI table so each adapter's own USM
  // implementation is reached (v1 / v2).
  auto *ddi = asInterface(hContext)->ddi_table;
  UR_CALL(ddi->USM.pfnDeviceAlloc(hContext, hDevice, pUSMDesc, pool, Size,
                                  ppMem));
...
ur_result_t urBindlessImagesImageAllocateExp(
  ...
  UR_CALL(createUrImgFromZeImage(asInterface(hContext)->getZeHandle(), hDevice->ZeDevice,
...
ur_result_t urBindlessImagesImportExternalSemaphoreExp(
  ...
  auto UrPlatform = asInterface(hContext)->getPlatform();
...
ur_result_t urBindlessImagesMapExternalLinearMemoryExp(
  ...
  ze_result_t zeResult = ZE_CALL_NOCHECK(
      zeMemAllocDevice, (asInterface(hContext)->getZeHandle(), &allocDesc, size, 0,
                         hDevice->ZeDevice, &mappedMemory));
  if (zeResult != ZE_RESULT_SUCCESS) {
    return UR_RESULT_ERROR_OUT_OF_RESOURCES;
  }

  zeResult = zeContextMakeMemoryResident(asInterface(hContext)->getZeHandle(),
                                         hDevice->ZeDevice, mappedMemory, size);
  if (zeResult != ZE_RESULT_SUCCESS) {
    ZE_CALL_NOCHECK(zeMemFree, (asInterface(hContext)->getZeHandle(), mappedMemory));
...
ur_result_t urBindlessImagesFreeMappedLinearMemoryExp(
  ...
  ZE2UR_CALL(zeMemFree, (asInterface(hContext)->getZeHandle(), pMem));
  return UR_RESULT_SUCCESS;
}
...
ur_result_t urBindlessImagesGetImageMemoryHandleTypeSupportExp(
  ...
  UR_ASSERT(std::find(asInterface(hContext)->getDevices().begin(),
                      asInterface(hContext)->getDevices().end(),
                      hDevice) != asInterface(hContext)->getDevices().end(),
