...
ur_result_t urDeviceGetInfo(
  ...
  case UR_DEVICE_INFO_GRAPH_RECORD_AND_REPLAY_SUPPORT_EXP: {
    // The ZeGraphExt.Supported flag is populated by initPlatforms in
    // both adapters; if the underlying L0 driver doesn't expose graphs
    // we early-return false.
  ...
  case UR_DEVICE_INFO_ENQUEUE_HOST_TASK_SUPPORT_EXP:
    return ReturnValue(
        static_cast<ur_bool_t>(Device->Platform->ZeHostTaskExt.Supported));
