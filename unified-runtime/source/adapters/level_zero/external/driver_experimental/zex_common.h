...
#define ZE_SYNCHRONIZED_DISPATCH_EXP_NAME                                      \
  "ZE_experimental_synchronized_dispatch"
...
typedef enum _ze_synchronized_dispatch_exp_version_t {
  ZE_SYNCHRONIZED_DISPATCH_EXP_VERSION_1_0 =
...
typedef enum _ze_synchronized_dispatch_exp_flag_t {
  ZE_SYNCHRONIZED_DISPATCH_DISABLED_EXP_FLAG =
      ZE_BIT(0), ///< Non-synchronized dispatch. Must synchronize only with
                 ///< other synchronized dispatches
  ZE_SYNCHRONIZED_DISPATCH_ENABLED_EXP_FLAG =
      ZE_BIT(1), ///< Synchronized dispatch. Must synchronize with all
                 ///< synchronized and non-synchronized dispatches
...
///////////////////////////////////////////////////////////////////////////////
/// @brief Command queue or command list descriptor for synchronized dispatch.
/// This structure may be
///        passed as pNext member of ::ze_command_queue_desc_t. or
...
typedef struct _ze_synchronized_dispatch_exp_desc_t {
  ...
  ze_synchronized_dispatch_exp_flags_t
      flags; ///< [in] mode flags.
...
typedef struct _ze_intel_media_communication_desc_t {
  ...
  void *controlSharedMemoryBuffer; ///< [in] control shared memory buffer
                                   ///< pointer, must be USM address
  uint32_t
      controlSharedMemoryBufferSize; ///< [in] control shared memory buffer size
  void *controlBatchBuffer; ///< [in] control batch buffer pointer, must be USM
                            ///< address
...
/// @details Handle of the doorbell. This structure is passed as argument of
...
typedef enum _ze_intel_device_media_exp_flag_t {
  ZE_INTEL_DEVICE_MEDIA_SUPPORTS_ENCODING_EXP_FLAG =
      ZE_BIT(0), ///< Supports encoding
  ZE_INTEL_DEVICE_MEDIA_SUPPORTS_DECODING_EXP_FLAG =
      ZE_BIT(1), ///< Supports decoding
...
typedef enum _zex_counter_based_event_version_t {
  ...
  ZEX_COUNTER_BASED_EVENT_VERSION_CURRENT =
      ZE_MAKE_VERSION(1, 0), ///< latest known version
...
typedef struct _zex_ipc_counter_based_event_handle_t {
  char data[ZE_MAX_IPC_HANDLE_SIZE]; ///< [out] Opaque data representing an IPC
                                     ///< handle
...
typedef enum _zex_counter_based_event_exp_flag_t {
  ZEX_COUNTER_BASED_EVENT_FLAG_IMMEDIATE = ZE_BIT(
      0), ///< Counter-based event is used for immediate command lists (default)
  ZEX_COUNTER_BASED_EVENT_FLAG_NON_IMMEDIATE = ZE_BIT(
      1), ///< Counter-based event is used for non-immediate command lists
  ZEX_COUNTER_BASED_EVENT_FLAG_HOST_VISIBLE =
      ZE_BIT(2), ///< Signals and waits are also visible to host
  ZEX_COUNTER_BASED_EVENT_FLAG_IPC =
      ZE_BIT(3), ///< Event can be shared across processes for waiting
  ZEX_COUNTER_BASED_EVENT_FLAG_KERNEL_TIMESTAMP =
      ZE_BIT(4), ///< Event contains kernel timestamps
  ZEX_COUNTER_BASED_EVENT_FLAG_KERNEL_MAPPED_TIMESTAMP =
...
typedef struct _zex_counter_based_event_desc_t {
  ...
  zex_counter_based_event_exp_flags_t
      flags; ///< [in] counter based event flags.
             ///< Must be 0 (default) or a valid combination of
             ///< ::zex_counter_based_event_exp_flag_t
  ze_event_scope_flags_t
      signalScope; ///< [in] defines the scope of relevant cache hierarchies to
                   ///< flush on a signal action before the event is triggered.
                   ///< must be 0 (default) or a valid combination of
                   ///< ::ze_event_scope_flag_t; default behavior is
                   ///< synchronization within the command list only, no
                   ///< additional cache hierarchies are flushed.
  ze_event_scope_flags_t
      waitScope; ///< [in] defines the scope of relevant cache hierarchies to
                 ///< invalidate on a wait action after the event is complete.
                 ///< must be 0 (default) or a valid combination of
                 ///< ::ze_event_scope_flag_t; default behavior is
                 ///< synchronization within the command list only, no
...
///////////////////////////////////////////////////////////////////////////////
/// @brief Initial Counter Based Event synchronization parameters. This
/// structure may be
...
typedef struct _zex_counter_based_event_external_sync_alloc_properties_t {
  ...
  const void *pNext;       ///< [in][optional] must be null or a pointer to an
                           ///< extension-specific
  uint64_t *deviceAddress; ///< [in] device address for external synchronization
                           ///< allocation
  uint64_t *hostAddress;   ///< [in] host address for external synchronization
                           ///< allocation
  uint64_t completionValue; ///< [in] completion value for external
...
typedef struct _zex_counter_based_event_external_storage_properties_t {
  ...
  uint64_t *deviceAddress; ///< [in] device address that would be updated with
                           ...
                           ///< device USM memory
  uint64_t incrementValue; ///< [in] value which would by atomically added upon
                           ///< each completion
  uint64_t completionValue; ///< [in] final completion value, when value under
