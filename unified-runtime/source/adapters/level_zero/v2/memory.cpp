  ...
  if (!p2pAccessible) {
    ...
    if (cmdList) {
      ...
      if (isImmediate) {
        ...
        // zeCommandListHostSynchronize is only valid for immediate command
        // lists. For non-immediate (batched) command lists fall back to the
        // same synchronous host-staged migration used when cmdList is null.
