...
/*
The modified wait_list_view is introduced in order to solve deadlock problems in
events created by batched queues.

Passed events might be created in batched queues, which use regular
command lists (batches). Since regular command lists are not executed explicitly
but only after enqueueing on immediate lists, it is necessary to enqueue the
...
the current queue has created the given event. As onWaitListView() tries to
acquire the lock for the batch associated with the given event, double locking
on the same queue would cause a deadlock.
*/
wait_list_view::wait_list_view(const ur_event_handle_t *phWaitEvents,
  ...
  // empty vector.
  // Explicit handling of nullptr prevents driver crashes
...
wait_list_view::wait_list_view(const ur_event_handle_t *phWaitEvents,
  ...
  if (phWaitEvents == nullptr) {
    ...
    waitList.resize(max_size);
    for (uint32_t i = 0; i < numWaitEvents; i++) {
      // We have already acquired the lock for the current queue
...
void wait_list_view::addEvent(ur_event_handle_t additionalEvent) {
  if (additionalEvent) {
    if (handles) {
      assert(num != max_size);
...
ur_result_t ur_command_list_manager::appendGenericCommandListsExp(
  ...
  waitListView.addEvent(additionalWaitEvent);
  auto [pWaitEvents, numWaitEvents, _] = waitListView;
