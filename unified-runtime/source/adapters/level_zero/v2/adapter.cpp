...
ur_result_t urAdapterGet(uint32_t NumEntries, ur_adapter_handle_t *Adapters,
  ...
  if (!GlobalAdapterV2) {
    ...
    auto *v2Ddi = ur::level_zero::v2::ddi_getter::value();
    GlobalAdapterV2->ddi_table = v2Ddi;
    for (auto &platform : GlobalAdapterV2->Platforms)
      platform->ddi_table = v2Ddi;
  }

  // Decline if hardware/env preference says V2 is not the right choice
  ...
  // version probe inside shouldUseV2Adapter() can enumerate drivers).
