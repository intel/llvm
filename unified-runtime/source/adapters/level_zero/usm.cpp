...
umf_result_t L0MemoryProvider::open_ipc_handle(void *IpcData, void **Ptr) {
  ...
  if (zeIpcData->pid != ur_getpid()) {
    ...
    if (fdLocal == -1) {
      UR_LOG(ERROR, "duplicating file descriptor from IPC handle failed");
      return UMF_RESULT_ERROR_MEMORY_PROVIDER_SPECIFIC;
    }
...
MakeProvider(ProviderParams *Params = nullptr) {
  if constexpr (std::is_same_v<ProviderParams, l0_memory_provider_params_t>) {
    ...
    case UR_USM_TYPE_SHARED:
      ...
      break;
    default:
      UR_LOG(ERROR, "urUSMPoolCreate: invalid USM type found");
...
ur_usm_pool_handle_t_::ur_usm_pool_handle_t_(ur_context_handle_t Context,
  ...
  for (auto &Desc : Descriptors) {
    ...
    if (Ret) {
      UR_LOG(ERROR, "urUSMPoolCreate: failed to store USM pool tag");
      throw UsmAllocationException(Ret);
    }

    Ret = PoolManager.addPool(Desc, std::move(usmPool));
    if (Ret) {
      UR_LOG(ERROR, "urUSMPoolCreate: failed to store UMF pool");
      throw UsmAllocationException(Ret);
    }
  }
}
