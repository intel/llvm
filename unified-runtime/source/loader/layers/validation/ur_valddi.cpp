...
__urdlllocal ur_result_t UR_APICALL urEventCreateExp(
  ...
  if (getContext()->enableParameterValidation) {
    ...
    if (NULL != pProperties &&
        UR_EXP_EVENT_CREATE_FLAGS_MASK & pProperties->flags)
      return UR_RESULT_ERROR_INVALID_ENUMERATION;

    if (pProperties != NULL &&
        pProperties->flags & ~UR_EXP_EVENT_CREATE_FLAG_PROFILING_ENABLE)
      return UR_RESULT_ERROR_INVALID_VALUE;
  }
...
UR_DLLEXPORT ur_result_t UR_APICALL urGetCommandBufferExpProcAddrTable(
  ...
  pDdiTable->pfnAppendKernelLaunchWithArgsExp =
      ur_validation_layer::urCommandBufferAppendKernelLaunchWithArgsExp;
...
UR_DLLEXPORT ur_result_t UR_APICALL urGetGraphExpProcAddrTable(
  ...
  pDdiTable->pfnSetDestructionCallbackExp =
      ur_validation_layer::urGraphSetDestructionCallbackExp;
