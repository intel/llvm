...
__urdlllocal ur_result_t UR_APICALL urAdapterSetLoggerCallbackLevel(
  ...
  uint64_t instance =
      getContext()->notify_begin(UR_FUNCTION_ADAPTER_SET_LOGGER_CALLBACK_LEVEL,
...
urBindlessImagesGetImageMemoryHandleTypeSupportExp(
  ...
  getContext()->notify_end(
      UR_FUNCTION_BINDLESS_IMAGES_GET_IMAGE_MEMORY_HANDLE_TYPE_SUPPORT_EXP,
      "urBindlessImagesGetImageMemoryHandleTypeSupportExp", &params, &result,
      instance);
...
__urdlllocal ur_result_t UR_APICALL urBindlessImagesFreeMappedLinearMemoryExp(
  ...
  uint64_t instance = getContext()->notify_begin(
      UR_FUNCTION_BINDLESS_IMAGES_FREE_MAPPED_LINEAR_MEMORY_EXP,
      "urBindlessImagesFreeMappedLinearMemoryExp", &params);

  auto &logger = getContext()->logger;
  UR_LOG_L(logger, INFO, "   ---> urBindlessImagesFreeMappedLinearMemoryExp\n");

  ur_result_t result = pfnFreeMappedLinearMemoryExp(hContext, hDevice, pMem);

  getContext()->notify_end(
      UR_FUNCTION_BINDLESS_IMAGES_FREE_MAPPED_LINEAR_MEMORY_EXP,
      "urBindlessImagesFreeMappedLinearMemoryExp", &params, &result, instance);

  if (logger.getLevel() <= UR_LOGGER_LEVEL_INFO) {
    std::ostringstream args_str;
    ur::extras::printFunctionParams(
        args_str, UR_FUNCTION_BINDLESS_IMAGES_FREE_MAPPED_LINEAR_MEMORY_EXP,
        &params);
    UR_LOG_L(logger, INFO,
...
__urdlllocal ur_result_t UR_APICALL urQueueInstantiateGraphExp(
  ...
  auto pfnInstantiateGraphExp =
      getContext()->urDdiTable.QueueExp.pfnInstantiateGraphExp;
  ...
  ur_queue_instantiate_graph_exp_params_t params = {&hGraph, &phExecutableGraph,
                                                    &pNext};
  uint64_t instance =
      getContext()->notify_begin(UR_FUNCTION_QUEUE_INSTANTIATE_GRAPH_EXP,
                                 "urQueueInstantiateGraphExp", &params);

  auto &logger = getContext()->logger;
  UR_LOG_L(logger, INFO, "   ---> urQueueInstantiateGraphExp\n");

  ur_result_t result = pfnInstantiateGraphExp(hGraph, phExecutableGraph, pNext);

  getContext()->notify_end(UR_FUNCTION_QUEUE_INSTANTIATE_GRAPH_EXP,
                           "urQueueInstantiateGraphExp", &params, &result,
                           instance);

  if (logger.getLevel() <= UR_LOGGER_LEVEL_INFO) {
    std::ostringstream args_str;
    ur::extras::printFunctionParams(
        args_str, UR_FUNCTION_QUEUE_INSTANTIATE_GRAPH_EXP, &params);
    UR_LOG_L(logger, INFO, "   <--- urQueueInstantiateGraphExp({}) -> {};\n",
             args_str.str(), result);
  }
...
__urdlllocal ur_result_t UR_APICALL urGraphIsEmptyExp(
  ...
  ur_result_t result = pfnIsEmptyExp(hGraph);
...
__urdlllocal ur_result_t UR_APICALL urGetBindlessImagesExpProcAddrTable(
  ...
  pDdiTable->pfnGetImageMemoryHandleTypeSupportExp =
      ur_tracing_layer::urBindlessImagesGetImageMemoryHandleTypeSupportExp;
...
__urdlllocal ur_result_t UR_APICALL urGetQueueExpProcAddrTable(
  ...
  pDdiTable->pfnInstantiateGraphExp =
      ur_tracing_layer::urQueueInstantiateGraphExp;
