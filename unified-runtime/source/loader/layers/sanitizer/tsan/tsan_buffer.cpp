...
ur_result_t MemBuffer::getHandle(ur_device_handle_t Device, char *&Handle) {
  ...
  if (!Allocation) {
    ...
    if (URes != UR_RESULT_SUCCESS) {
      UR_LOG_LOGGER(getContext()->logger, ERR, "Failed to allocate {} bytes memory for buffer {}", Size,
                this);
      return URes;
    }
    ...
    if (HostPtr) {
      ...
      if (URes != UR_RESULT_SUCCESS) {
        UR_LOG_LOGGER(getContext()->logger, ERR,
...
ur_result_t MemBuffer::free() {
  for (const auto &[_, Ptr] : Allocations) {
    ...
    if (URes != UR_RESULT_SUCCESS) {
      UR_LOG_LOGGER(getContext()->logger, ERR, "Failed to free buffer handle {}", Ptr);
      return URes;
    }
  }
  Allocations.clear();
