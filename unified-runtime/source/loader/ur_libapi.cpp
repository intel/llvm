...
ur_result_t UR_APICALL urQueueInstantiateGraphExp(
  ...
  auto pfnInstantiateGraphExp =
      ur_lib::getContext()->urDdiTable.QueueExp.pfnInstantiateGraphExp;
  if (nullptr == pfnInstantiateGraphExp)
    return UR_RESULT_ERROR_UNINITIALIZED;

  return pfnInstantiateGraphExp(hGraph, phExecutableGraph, pNext);
} catch (...) {
  return exceptionToResult(std::current_exception());
}
...
ur_result_t UR_APICALL urQueueAppendGraphExp(
  ...
  auto pfnAppendGraphExp =
      ur_lib::getContext()->urDdiTable.QueueExp.pfnAppendGraphExp;
  if (nullptr == pfnAppendGraphExp)
    return UR_RESULT_ERROR_UNINITIALIZED;

  return pfnAppendGraphExp(hQueue, hGraph, pNext, hSignalEvent, numWaitEvents,
                           phWaitEvents);
} catch (...) {
  return exceptionToResult(std::current_exception());
}
