...
// This header verifies that in-order queue (IOQ) semantics are preserved
// when using the discard_events optimization. It submits a sequence of 5
// kernel stages interspersed with USM copy and fill operations. Each kernel
// stage validates that previous stages have executed in order by checking
