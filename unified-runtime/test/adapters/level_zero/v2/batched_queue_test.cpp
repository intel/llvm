...
struct urBatchedQueueTest : uur::urContextTest {
  ...
  void vectorOfSubmittedBatchesIsClearedHelper() {
    ...
    // Initially, the vector of batches submitted for execution is empty. After every iteration, each queueFlush results in submitting the current batch for execution, then pushing the submitted batch to the vector of submitted batches. Therefore, after initialSlotsForBatches we reach the maximum arbitrarily chosen capacity (initialSlotsForBatches)
    ...
    // The maximum arbitrarily set capacity is reached, but the vector is not cleared
...
When q2 performs an operation O2 which must wait on events from q1, O2 would never be executed if the batch from q1 is not enqueued during wait_list_view construction.
*/
...
enqueueSth(q1, ..., getEvent2) // access to batchNr - check if the batch has been submitted for execution for the second time
getEvent2->getBatch == getEvent->getBatch

event statuses in L0v2 are only UR_EVENT_STATUS_SUBMITTED and UR_EVENT_STATUS_COMPLETE
*/
TEST_P(urBatchedQueueTest, RunBatchOnlyWhenNeededSimple) {
  ...
  // Event1 is from the batch from q1, which has been already submitted for execution
...
TEST_P(urBatchedQueueTest, IncreaseGenerationNumberAfterQueueFinish) {
  ...
  ASSERT_SUCCESS(urEnqueueMemBufferWrite(queue1, buffer, /* isBlocking */ false,
                                         ...
                                         nullptr, &event1));
  ASSERT_NE(event1->getBatch(), std::nullopt);
  ASSERT_NO_FATAL_FAILURE(isEventSubmitted(event1));
...
//   e1 should be the initial generation number - the current batch is not run when empty

//   enqueue cmdbuf
//   e2 = enqueue sth (q1)
//   e2.generation_number should be e1.generation_number + 1 since cmdbuff was enqueued on non-empty batch

// When a command buffer is enqueued, the current batch is closed and submitted for execution on an immediate command list, then the command buffer is submitted for execution on the same immediate command list as the current batch. After that, a new batch with an incremented generation number is opened for enqueueing operations.
...
TEST_P(urBatchedQueueTest, RunBatchIfNeededCommandBuffer) {
  ...
  // Command buffers are submitted to an immediate command list instead of the regular commnd list (the current batch)
...
TEST_P(urBatchedQueueTest, RunBatchWhenNeededSameQueue) {
  ...
  ASSERT_NE(event2->getBatch(), std::nullopt);
  ASSERT_NO_FATAL_FAILURE(isEventSubmitted(event2));
  // wait_list_view is constructed before passing arguments to command list manager functions. Therefore, if the batch from the current queue might have been submitted for execution, the generation number of the event passed as an argument for the enqueue command must have already been increased. However, there is no need to submit batches assigned to events from the same queue, since the operations are executed in-order: either from different consecutive batches or as part of the same batch.
  ASSERT_EQ(event1->getBatch(), event2->getBatch());
...
TEST_P(urBatchedQueueTest, RunBatchWhenNeededQueueFlush) {
  ...
  ASSERT_NO_FATAL_FAILURE(isEventSubmitted(eventEmpty));
  ...
  ASSERT_NO_FATAL_FAILURE(isEventSubmitted(eventNonEmpty));
...
// Submitting batches for execution and eventually clearing the vector of submitted batches is triggered by queueFlush.
...
TEST_P(urBatchedQueueTest, VectorOfSubmittedBatchesIsClearedQueueFinish) {
  ...
  // After queueFlush, the batch should be pushed onto the vector of submitted batches, which is cleared in queueFinish. Command lists are returned to the command list cache in their destructors. The current batch is reset during queueFinish, therefore only one command list is returned to the command list cache
