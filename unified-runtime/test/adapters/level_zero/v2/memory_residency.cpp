...
struct urMemoryMultiResidencyTest : uur::urMultiDeviceContextTestTemplate<2> {
	...
	void SetUp() override {
		UUR_RETURN_ON_FATAL_FAILURE(uur::urMultiDeviceContextTestTemplate<2>::SetUp());

		for (std::size_t i = 0; i < 2; i++) {
			ur_bool_t usm_p2p_support = false;
			ASSERT_SUCCESS(urDeviceGetInfo(devices[i], UR_DEVICE_INFO_USM_P2P_SUPPORT_EXP,
	...
	void TearDown() override {
		UUR_RETURN_ON_FATAL_FAILURE(uur::urMultiDeviceContextTestTemplate<2>::TearDown());
	}
