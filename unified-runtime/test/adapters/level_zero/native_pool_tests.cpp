...
struct NativePoolTestParam {
  AllocFunc USMAllocFunc;
  CheckSupportFunc checkUSMSupportFunc;
};

std::ostream &operator<<(std::ostream &os, NativePoolTestParam param) {
...
struct urL0NativePoolTest : uur::urQueueTestWithParam<NativePoolTestParam> {
...
UUR_DEVICE_TEST_SUITE_WITH_PARAM(
    urL0NativePoolTest,
    ::testing::ValuesIn({NativePoolTestParam{urUSMHostAllocWrapped,
                                             uur::GetDeviceUSMHostSupport},
                         NativePoolTestParam{
                             urUSMSharedAlloc,
                             uur::GetDeviceUSMSingleSharedSupport},
                         NativePoolTestParam{urUSMDeviceAlloc,
                                             uur::GetDeviceUSMDeviceSupport}}),
    uur::deviceTestWithParamPrinter<NativePoolTestParam>);

TEST_P(urL0NativePoolTest, SuccessRepeat) {
  ...
  nativePoolDesc.pMem = bufPtr;
  nativePoolDesc.size = bufSize;
  nativePoolDesc.memType = AllocFuncsToUSMType[USMAllocFunc];
  nativePoolDesc.device = hDevice;
