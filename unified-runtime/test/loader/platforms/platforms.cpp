...
int main(int, char *[]) {
  ...
  if (status != UR_RESULT_SUCCESS) {
    UR_LOG_LOGGER(out, ERR, "urLoaderInit failed with return code: {}", status);
    return 1;
  }
  UR_LOG_LOGGER(out, INFO, "urLoaderInit succeeded.");

  uint32_t adapterCount = 0;
  std::vector<ur_adapter_handle_t> adapters;
  status = urAdapterGet(0, nullptr, &adapterCount);
  if (status != UR_RESULT_SUCCESS) {
    UR_LOG_LOGGER(out, ERR, "urAdapterGet failed with return code: {}", status);
    return 1;
  }
  ...
  if (status != UR_RESULT_SUCCESS) {
    UR_LOG_LOGGER(out, ERR, "urPlatformGet failed with return code: {}", status);
    goto out;
  }
  UR_LOG_LOGGER(out, INFO, "urPlatformGet found {} platforms", platformCount);
  ...
  for (auto p : platforms) {
    ...
    if (status != UR_RESULT_SUCCESS) {
      UR_LOG_LOGGER(out, ERR, "urPlatformGetInfo failed with return code: {}", status);
      goto out;
    }
    ...
    UR_LOG_LOGGER(out, INFO, "Found {} ", name);
