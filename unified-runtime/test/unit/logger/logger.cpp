...
TEST_F(DefaultLoggerWithFileSink, DefaultLevelNoOutput) {
  UR_LOG_LOGGER(*logger, INFO, "This should not be printed: {}", 42);
  test_msg.clear();
}

TEST_F(DefaultLoggerWithFileSink, MultipleLines) {
  UR_LOG_LOGGER(*logger, WARN, "Test message: {}", "success");
  UR_LOG_LOGGER(*logger, DEBUG, "This should not be printed: {}", 42);
  UR_LOG_LOGGER(*logger, ERR, "Test message: {}", "success");
...
TEST_F(DefaultLoggerWithFileSink, SetLevelDebug) {
  ...
  UR_LOG_LOGGER(*logger, DEBUG, "Test message: {}", "success");
...
TEST_F(DefaultLoggerWithFileSink, SetLevelInfo) {
  ...
  logger->setLevel(level);
  logger->setFlushLevel(level);
  UR_LOG_LOGGER(*logger, INFO, "Test message: {}", "success");
  UR_LOG_LOGGER(*logger, DEBUG, "This should not be printed: {}", 42);
...
TEST_F(DefaultLoggerWithFileSink, SetLevelError) {
  ...
  UR_LOG_LOGGER(*logger, ERR, "Test message: {}", "success");
  UR_LOG_LOGGER(*logger, WARN, "This should not be printed: {}", 42);
...
TEST_P(FileSinkLoggerMultipleThreads, Multithreaded) {
  ...
  for (int i = 0; i < thread_count; i++) {
    threads.emplace_back([&]() {
      for (int j = 0; j < message_count; ++j) {
        UR_LOG_LOGGER(local_logger, WARN, "Test message: {}", "it's a success");
      }
    });
  }
  ...
  for (int i = 0; i < thread_count; i++) {
    threads.emplace_back([&]() {
      for (int j = 0; j < message_count; ++j) {
        UR_LOG_LOGGER(local_logger, ERR, "Flushed test message: {}", "it's a success");
      }
    });
  }
