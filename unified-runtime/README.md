...
> Note: `filecheck` requires Python >= 3.10; if you get errors claiming that `filecheck` is not available, update your Python version or only install `lit`. If `filecheck` is not available, tests that don't depend on it will still run.
