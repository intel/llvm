...
def _get_etor_value(value, prev):
    ...
    if value:
        if value_traits.is_ver(value):
            return (
                value_traits.get_major_ver(value) << 16
            ) + value_traits.get_minor_ver(value)
...
def _validate_doc(f, d, tags, line_num, meta):
    ...
    def __validate_version(d, prefix="", base_version=default_version):
        if "version" in d:
            if not isinstance(d["version"], str):
                raise Exception(
                    prefix + "'version' must be a string: '%s'" % type(d["version"])
                )
        ...
        if isinstance(desc, dict):
            for k, v in desc.items():
                if not isinstance(k, str):
                    raise Exception(
                        prefix + "'version' must be a string: '%s'" % type(k)
                    )
    ...
    def __validate_name(d, key, tags, case="lower", prefix=""):
        if not isinstance(d[key], str):
            raise Exception(
                prefix + "'%s' must be a string: '%s'" % (key, type(d[key]))
            )
                    ...
                    if isinstance(item, dict):
                        for key in item:
                            if not isinstance(key, str):
                                raise Exception(
                                    prefix + "must be a string: '%s'" % type(key)
                                )
                            ...
                            for j, val in enumerate(item[key]):
                                ...
                                if not isinstance(val, str):
                                    raise Exception(
                                        prefix2 + "must be a string: '%s'" % type(val)
                                    )
    ...
    def __validate_etors(d, tags):
        ...
        for i, item in enumerate(d["etors"]):
            ...
            if type_traits.is_flags(d["name"]) and not value_traits.is_bit(
                item.get("value")
            ):
                raise Exception(prefix + "'value' must use BIT macro: %s" % value)
            elif not type_traits.is_flags(d["name"]) and value_traits.is_bit(
                item.get("value")
            ):
            ...
            if value >= 0x7FFFFFFF:
                raise Exception(
                    prefix + "'value' must be less than 0x7fffffff: %s" % value
                )
            ...
            if ver < max_ver:
                raise Exception(
                    prefix + "'version' must be increasing: %s" % item["version"]
                )
            max_ver = ver
    ...
    def __validate_base(d):
        ...
        if d["name"] not in valid_names:
            if type_traits.is_descriptor(d["name"]) and not d.get("base", "").endswith(
                "base_desc_t"
            ):
                raise Exception(
                    "'base' must be '%s_base_desc_t': %s" % (namespace, d["name"])
                )

            elif type_traits.is_properties(d["name"]) and not d.get(
                "base", ""
            ).endswith("base_properties_t"):
                raise Exception(
                    "'base' must be '%s_base_properties_t': %s" % (namespace, d["name"])
                )
    ...
    def __validate_struct_range_members(name, members, meta):
        ...
        for m in members:
            ...
            if type_traits.is_struct(m["type"], meta):
                member_members = type_traits.get_struct_members(m["type"], meta)
                # We can't handle a range of structs with handles within a range of structs
                if param_traits.is_range(m) and has_handle(member_members, meta):
                    raise Exception(
    ...
    def __validate_members(d, meta):
        ...
        for i, item in enumerate(d["members"]):
            ...
            if item["type"].endswith("flag_t"):
                raise Exception(
                    prefix + "'type' must not be '*_flag_t': %s" % item["type"]
                )

            if d["type"] == "union" and item.get("tag") is None:
                raise Exception(
                    prefix
                    + f"union member {item['name']} must include a 'tag' annotation"
                )

            if type_traits.is_struct(item["type"], meta) and param_traits.is_range(
                item
            ):
                member_members = type_traits.get_struct_members(item["type"], meta)
                __validate_struct_range_members(item["name"], member_members, meta)

            if type_traits.is_handle(item["type"]) and param_traits.is_output(item):
                raise Exception(
                    prefix
                    + f"struct member {item['name']} is an object handle, so it must not be have the [out] tag"
                )
    ...
    def __validate_params(d, meta):
        ...
        for i, item in enumerate(d["params"]):
            ...
            if (
                d.get("decl") != "static"
                and i == 0
                and not type_traits.is_handle(item["type"])
            ):
                raise Exception(
                    prefix + "'type' must be '*_handle_t': %s" % item["type"]
                )
            ...
            if (
                type_traits.is_pointer(item["type"])
                and "_handle_t" in item["type"]
                and "[in]" in item["desc"]
            ):
    ...
    try:
        ...
        if "header" == d["type"]:
            ...
            if "class" in d:
                __validate_name(
                    {"name": d["class"] + d["name"]}, "name", tags, case="camel"
                )
            else:
...
def _generate_meta(d, ordinal, meta):
    ...
    if "class" != type:
        ...
        if "enum" == type:
            ...
            if type_traits.is_flags(name):
                meta[type][name]["max"] = (
                    hex((max_value << 1) - 1) if max_value else "0"
                )
                if bit_mask != 0:
            ...
            for p in d["params"]:
                meta[type][name]["params"].append({"type": p["type"]})
    ...
    if re.match(r"struct|union", obj["type"]):
        ...
        if base in meta["struct"]:
            for i, m in enumerate(meta["struct"][base]["members"]):
                ...
                if m["name"] == "stype":
                    m["init"] = re.sub(
                        r"(\$[a-z]+)(\w+)_t", r"\1_STRUCTURE_TYPE\2", obj["name"]
                    ).upper()
...
def _generate_returns(obj, meta):
    ...
    if re.match(r"function", obj["type"]):
        ...
        def append_nullchecks(param, accessor: str):
            if type_traits.is_pointer(param["type"]) or type_traits.is_array(param["type"]):
                _append(
                    rets,
                ...
                type_traits.is_handle(param["type"])
                and not type_traits.is_ipc_handle(item["type"])
                and not type_traits.is_native_handle(item["type"])
            ):
                _append(
                    rets,
        ...
        def append_enum_checks(param, accessor: str):
            ...
            if re.match(r"stype", param["name"]):
                _append(
                    ...
                    % (
                        re.sub(
                            r"(\$\w)_(.*)_t.*", r"\1_STRUCTURE_TYPE_\2", typename
                        ).upper(),
                        accessor,
                    ),
                )
        ...
        for item in obj["params"]:
            ...
            if type_traits.is_enum(item["type"], meta) and not type_traits.is_pointer(
                item["type"]
            ):
                append_enum_checks(item, item["name"])

            if type_traits.is_descriptor(item["type"]) or type_traits.is_properties(
                item["type"]
            ):
                ...
                for m in meta["struct"][typename]["members"]:
                    ...
                    if type_traits.is_enum(
                        m["type"], meta
                    ) and not type_traits.is_pointer(m["type"]):
...
def _validate_ext_enum_range(extension, enum) -> bool:
    try:
        existing_values = [
            _get_etor_value(etor.get("value"), None) for etor in enum["etors"]
        ]
...
def _extend_enums(enum_extensions, specs, meta):
    ...
    enum_groups = [
        (k, list(g))
        for k, g in itertools.groupby(enum_extensions, key=lambda x: x["name"])
    ]
    ...
    for f in files:
        ...
        for i, d in enumerate(docs):
            ...
            if re.match(r"header", d["type"]):
                ...
                header["ordinal"] *= (
                    1000
                    if re.match(r"extension", header.get("desc", "").lower())
                    else 1
                )
                header["ordinal"] *= (
                    1000
                    if re.match(r"experimental", header.get("desc", "").lower())
                    else 1
                )
