    ...
    for iline, line in iter:
        ...
        if re.match(RE_INVALID_TAG_FORMAT, line):
            print(
                ...
                % (fin, iline + 1, re.sub(RE_INVALID_TAG_FORMAT, r"\1", line))
            )
            error = True

        newline = (
            line  # new line will contain proper tags for reStructuredText if needed.
        )
                ...
                if symbol:
                    ...
                    if code_block and "function" == symbol_type:
                        ...
                        try:
                            ...
                            print(
                                f"Function {line[:100]} was not terminated by a ';' character."
                            )
                            error = True
                        ...
                        if len(words) != len(meta["function"][symbol]["params"]):
                            print(
                                ...
                                % (
                                    ...
                                    symbol,
                                    len(words),
                                    len(meta["function"][symbol]["params"]),
                                )
                            )
                            print("line = %s" % line)
                            error = True
...
def generate_rst(docpath, section, namespace, tags, ver, rev, specs, meta, fast_mode):
    ...
    for fin in util.findFiles(srcpath, "*.rst"):
        ...
        loc += _generate_valid_rst(
            os.path.abspath(fin), fout, namespace, tags, ver, rev, meta, fast_mode
        )
...
def generate_common(dstpath, sections, ver, rev):
    ...
    if (
        re.search(
            r"warning: explicit link request to \'.*\' could not be resolved", output
        )
...
def generate_html(dstpath):
    ...
    result = subprocess.run(
        ["sphinx-build", "-M", "html", sourcepath, "../docs"], stderr=subprocess.PIPE
    )
...
def generate_pdf(dstpath):
    ...
    result = subprocess.run(
        ["sphinx-build", "-b", "pdf", sourcepath, "../docs/latex"],
