...
; RUN: sycl-post-link -split-esimd -symbols -reduce-memory-usage=true -S %s -o %t-red.table
; RUN: FileCheck %s -input-file=%t-red.table
; RUN: FileCheck %s -input-file=%t-red_0.sym --check-prefixes CHECK-SYCL-SYM
; RUN: FileCheck %s -input-file=%t-red_esimd_0.sym --check-prefixes CHECK-ESIMD-SYM
