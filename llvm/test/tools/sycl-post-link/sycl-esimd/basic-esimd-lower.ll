...
; RUN: sycl-post-link -split-esimd -lower-esimd -reduce-memory-usage=true -S %s -o %t.table
; RUN: FileCheck %s -input-file=%t_esimd_0.ll --check-prefixes CHECK-O2
...
; RUN: sycl-post-link -split-esimd -lower-esimd -O2 -reduce-memory-usage=true -S %s -o %t.table
; RUN: FileCheck %s -input-file=%t_esimd_0.ll --check-prefixes CHECK-O2
...
; RUN: sycl-post-link -split-esimd -lower-esimd -O0 -reduce-memory-usage=true -S %s -o %t.table
; RUN: FileCheck %s -input-file=%t_esimd_0.ll --check-prefixes CHECK-O0
