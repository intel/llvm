...
; RUN: sycl-post-link -split=source -reduce-memory-usage=true -S %s -o %t-red.table
; RUN: FileCheck %s -input-file=%t-red.table
; RUN: FileCheck %s -input-file=%t-red_0.ll --check-prefixes CHECK-IR-0
; RUN: FileCheck %s -input-file=%t-red_1.ll --check-prefixes CHECK-IR-1
