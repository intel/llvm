...
; RUN: sycl-post-link -spec-const=default -reduce-memory-usage=true %s -S -o %t-red.table
; RUN: FileCheck %s -check-prefixes=CHECK,CHECK-DEF < %t-red_0.ll
; RUN: FileCheck %s --check-prefixes=CHECK-PROPS,CHECK-PROPS-DEF < %t-red_0.prop
; RUN: sycl-post-link -spec-const=rt -reduce-memory-usage=true %s -S -o %t-red.table
; RUN: FileCheck %s -check-prefixes=CHECK,CHECK-RT < %t-red_0.ll
; RUN: FileCheck %s --check-prefixes=CHECK-PROPS < %t-red_0.prop
