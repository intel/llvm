...
; RUN: llc -opaque-pointers -mtriple=x86_64 < %s | FileCheck %s

;; Check that we accept .Ldsolocal$local: below the function label.
; RUN: llc -opaque-pointers -mtriple=x86_64 -relocation-model=pic < %s | FileCheck %s --check-prefix=PIC

;; Check that we accept .seh_proc below the function label.
; RUN: llc -opaque-pointers -mtriple=x86_64-windows -relocation-model=pic < %s | FileCheck %s --check-prefix=WIN
; endif
