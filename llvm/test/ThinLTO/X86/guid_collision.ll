...
; RUN: llvm-lto2 run -lto-opaque-pointers -opaque-pointers %t.bc %t2.bc -o %t.out -save-temps \
; RUN: -r=%t.bc,H,px -r=%t.bc,G, -r=%t2.bc,G,px
; RUN: llvm-dis -o - %t.out.1.3.import.bc | FileCheck %s

; RUN: llvm-lto2 run -lto-opaque-pointers -opaque-pointers %t.bc %t2.bc -o %t.out -thinlto-distributed-indexes \
; RUN: -r=%t.bc,H,px -r=%t.bc,G, -r=%t2.bc,G,px
