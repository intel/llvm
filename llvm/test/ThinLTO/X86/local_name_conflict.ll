...
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers -module-summary -module-hash %p/Inputs/local_name_conflict1.ll -o %t2.bc
; Added -opaque-pointers.
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers -module-summary -module-hash %p/Inputs/local_name_conflict2.ll -o %t3.bc
; Added -opaque-pointers.
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
...
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: llvm-lto -opaque-pointers -thinlto-action=import %t.bc -thinlto-index=%t4.bc -o - | llvm-dis -opaque-pointers -o - | FileCheck %s --check-prefix=IMPORT
...
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: llvm-lto -opaque-pointers -thinlto-action=promote %t2.bc -thinlto-index=%t4.bc -o - | llvm-dis -opaque-pointers -o - | FileCheck %s --check-prefix=NOEXPORTSTATIC
...
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: llvm-lto -opaque-pointers -thinlto-action=promote %t3.bc -thinlto-index=%t4.bc -o - | llvm-dis -opaque-pointers -o - | FileCheck %s --check-prefix=EXPORTSTATIC
