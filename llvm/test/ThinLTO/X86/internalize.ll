...
; RUN: llvm-lto -thinlto-action=thinlink -opaque-pointers -o %t.index.bc %t2.bc %t1.bc
; RUN: llvm-lto -thinlto-action=internalize -thinlto-index -opaque-pointers %t.index.bc %t1.bc -o - | llvm-dis -o - | FileCheck %s --check-prefix=REGULAR
; RUN: llvm-lto -thinlto-action=internalize -thinlto-index -opaque-pointers %t.index.bc %t1.bc -o -  --exported-symbol=_foo | llvm-dis -o - | FileCheck %s --check-prefix=INTERNALIZE
...
; RUN: llvm-lto -thinlto-action=internalize -thinlto-index -opaque-pointers %t.index.bc %t1.bc \
