...
; RUN: llvm-lto2 run -lto-opaque-pointers -opaque-pointers %t1.bc -o %t.o -save-temps \
...
; This makes sure indices are not marked as internallinkage and therefore
; internalization does not happen.
; RUN: llvm-lto2 run -lto-opaque-pointers -opaque-pointers %t1.bc -o %t.o -save-temps -enable-lto-internalization=false \
; RUN:     -r=%t1.bc,_foo,pxl \
; RUN:     -r=%t1.bc,_bar,pl \
; RUN:     -r=%t1.bc,_linkonce_func,pl \
; RUN:     -r=%t1.bc,_weak_func_prevailing,pl \
