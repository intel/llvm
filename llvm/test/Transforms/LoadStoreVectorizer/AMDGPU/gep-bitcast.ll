; RUN: opt -S -mtriple=amdgcn-amd-amdhsa -passes=load-store-vectorizer < %s | FileCheck %s
; RUN: opt -S -mtriple=amdgcn-amd-amdhsa -passes='function(load-store-vectorizer)' < %s | FileCheck %s
