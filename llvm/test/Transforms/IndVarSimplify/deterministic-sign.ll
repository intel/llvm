...
; pointers by default
; RUN: opt -opaque-pointers "-passes=loop-rotate,indvars" -S < %s | FileCheck %s
; Added -opaque-pointers.
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers "-passes=loop-rotate" < %s | opt -opaque-pointers "-passes=indvars" -S - | FileCheck %s
