...
; pointers by default
; RUN: opt -opaque-pointers -module-summary %s -o %t.main.bc
; Added -opaque-pointers.
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers -module-summary %p/Inputs/noinline.ll -o %t.inputs.noinline.bc
...
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers -passes=function-import -summary-file %t.summary.thinlto.bc %t.main.bc -S 2>&1 \
; RUN:   | FileCheck -check-prefix=NOIMPORT %s
; Added -opaque-pointers.
; FIXME: Align with the community code when project is ready to enable opaque
; pointers by default
; RUN: opt -opaque-pointers -passes=function-import -force-import-all -summary-file %t.summary.thinlto.bc \
