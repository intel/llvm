...
; RUN: opt -opaque-pointers -passes=simplifycfg --switch-to-lookup -S %s | FileCheck %s
; RUN: opt -opaque-pointers '-passes=simplifycfg<switch-to-lookup>' -data-layout="e" -S %s | FileCheck %s
;; If basic integer types are natively supported, the value is generated inline instead of using a global
; RUN: opt -opaque-pointers '-passes=simplifycfg<switch-to-lookup>' -data-layout="e-n8:16:32:64" -S %s | FileCheck %s --check-prefix=INLINE
