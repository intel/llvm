...
; INTEL RUN: opt -opaque-pointers %s -o %t.bc
; INTEL RUN: opt -opaque-pointers %p/Inputs/funcimport.ll -o %t2.bc
; INTEL RUN: llvm-link -opaque-pointers %t.bc -summary-index=%t.bc -S
...
; INTEL RUN: llvm-link -opaque-pointers %t.bc -summary-index=%t3.thinlto.bc -S | FileCheck %s --check-prefix=EXPORTSTATIC
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=globalfunc1:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOB1
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=globalfunc2:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOB2
...
; as an alias.
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=globalfunc1:%t.bc -import=globalfunc2:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOB3
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=globalfunc2:%t.bc -import=globalfunc1:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOB4
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=linkoncefunc:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOB5
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=referencestatics:%t.bc -S | FileCheck %s --check-prefix=IMPORTSTATIC
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=referenceglobals:%t.bc -S | FileCheck %s --check-prefix=IMPORTGLOBALS
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=referencecommon:%t.bc -S | FileCheck %s --check-prefix=IMPORTCOMMON
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=callfuncptr:%t.bc -S | FileCheck %s --check-prefix=IMPORTFUNCPTR
...
; INTEL RUN: llvm-link -opaque-pointers %t2.bc -summary-index=%t3.thinlto.bc -import=callweakfunc:%t.bc -import=weakfunc:%t.bc -S 2>&1 | FileCheck %s --check-prefix=IMPORTWEAKFUNC
