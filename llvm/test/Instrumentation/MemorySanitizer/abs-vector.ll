...
; RUN: opt -opaque-pointers %s -S -msan-check-access-address=0 -passes=msan 2>&1 | FileCheck %s
; RUN: opt -opaque-pointers %s -S -msan-check-access-address=0 -msan-track-origins=2 -passes=msan 2>&1 | FileCheck %s --check-prefixes=CHECK,ORIGIN
