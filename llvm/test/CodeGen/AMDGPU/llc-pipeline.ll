...
; RUN: llc -O0 -mtriple=amdgcn-amd-amdhsa -disable-verify -debug-pass=Structure < %s 2>&1 \
; RUN:   | FileCheck -match-full-lines -strict-whitespace -check-prefix=GCN-O0 %s
; RUN: llc -O1 -mtriple=amdgcn-amd-amdhsa -disable-verify -debug-pass=Structure < %s 2>&1 \
; RUN:   | FileCheck -match-full-lines -strict-whitespace -check-prefix=GCN-O1 %s
; RUN: llc -O1 -mtriple=amdgcn-amd-amdhsa -disable-verify -amdgpu-scalar-ir-passes -amdgpu-sdwa-peephole \
...
; RUN: llc -O2 -mtriple=amdgcn-amd-amdhsa -disable-verify -debug-pass=Structure < %s 2>&1 \
; RUN:   | FileCheck -match-full-lines -strict-whitespace -check-prefix=GCN-O2 %s
; RUN: llc -O3 -mtriple=amdgcn-amd-amdhsa -disable-verify -debug-pass=Structure < %s 2>&1 \
