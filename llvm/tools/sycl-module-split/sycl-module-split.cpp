...
int main(int argc, char *argv[]) {
  ...
  auto PostSplitCallback = [&SplitModules,
                            Settings, OutputAssemblyAfterSplit, OutputPrefix](std::unique_ptr<ModuleDesc> MD) -> Error {
    ...
    Expected<SplitModule> SplitImageOrErr =
        sycl_post_link::saveModuleDesc(*MD, OutIRFileName, OutputAssemblyAfterSplit);
    if (!SplitImageOrErr)
