...
using sycl_post_link::TargetFilenamePair;
...
Error saveDeviceLibModule(
  ...
  Expected<sycl_post_link::FinalModule> FM =
      sycl_post_link::generateFinalModule(
          DeviceLibMD, DoSymGen, I, OutputPrefix, "", OutputAssembly,
          AllowDeviceImageDependencies, GlobProps, SplitMode);
  if (!FM)
    return FM.takeError();
...
processInputModule(std::unique_ptr<Module> M, const StringRef OutputPrefix) {
  ...
  while (Splitter->hasMoreSplits()) {
    ...
    if (IROutputOnly) {
      ...
      ExitOnErr(sycl_post_link::saveModuleIR(
          MMs.front()->getModule(), OutputFiles[0].Filename, OutputAssembly));
      return Tables;
    }
    ...
    for (const std::unique_ptr<module_split::ModuleDesc> &IrMD : MMs) {
      ...
      Expected<sycl_post_link::FinalModule> FM =
          sycl_post_link::generateFinalModule(
              *IrMD, DoSymGen, ID, OutputPrefix, OutIRFileName, OutputAssembly,
              AllowDeviceImageDependencies, GlobProps, SplitMode);
      if (!FM)
    ...
    if (!MMsWithDefaultSpecConsts.empty()) {
      for (size_t i = 0; i != MMsWithDefaultSpecConsts.size(); ++i) {
        ...
        Expected<sycl_post_link::FinalModule> FM =
            sycl_post_link::generateFinalModule(
                *IrMD, DoSymGen, ID, OutputPrefix, OutIRFileName,
                OutputAssembly, AllowDeviceImageDependencies, GlobProps,
                SplitMode);
        if (!FM)
          ExitOnErr(FM.takeError());
