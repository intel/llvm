...
Error writeToFile(StringRef Filename, StringRef Content) {
...
Error writePropertiesToFile(StringRef Filename,
...
Error llvm::sycl_post_link::saveModuleIR(Module &M, const StringRef Filename,
  ...
  if (EC)
    return createStringError(
        formatv("can't open the file. filename: {0}", Filename), EC);
...
void llvm::sycl_post_link::addTableRow(
    util::SimpleTable &Table,
    const sycl_post_link::IrPropSymFilenameTriple &RowData) {
...
Expected<sycl_post_link::FinalModule> llvm::sycl_post_link::generateFinalModule(
    module_split::ModuleDesc &MD, bool GenerateSymbols, int I,
    const Twine &OutputPrefix, StringRef IRFilename, bool OutputAssembly,
    bool AllowDeviceImageDependencies, sycl::GlobalBinImageProps GlobalProps,
    module_split::IRSplitMode SplitMode) {
  sycl_post_link::FinalModule Module = {MD};
  StringRef Suffix = MD.isESIMD() ? "_esimd" : "";
  MD.saveSplitInformationAsMetadata();

  if (!IRFilename.empty()) {
    // Don't save IR, just record the filename.
    Module.IR = IRFilename.str();
  } else {
    if (!MD.isSYCLDeviceLib()) {
    ...
    Module.IR = (OutputPrefix + Suffix + "_" + Twine(I) + IRExtension).str();
    if (Error E = saveModuleIR(MD.getModule(), Module.IR, OutputAssembly))
      return std::move(E);
  }
  ...
  Module.Properties = computeModuleProperties(MD, GlobalProps, SplitMode,
                                              AllowDeviceImageDependencies);
  return Module;
}

Error llvm::sycl_post_link::saveFinalModuleForEveryTarget(
    ...
    ArrayRef<TargetFilenamePair> OutputFiles, bool DoPropGen,
    StringRef Suffix) {

  std::string SymbolsPath;
  if (GenerateSymbols) {
    SymbolsPath = (OutputPrefix + Suffix + "_" + Twine(I) + ".sym").str();
    if (Error E = writeToFile(SymbolsPath, FM.Symbols))
      return E;
  }
  ...
  for (const auto &[Table, OutputFile] : zip_equal(OutTables, OutputFiles)) {
    ...
    if (DoPropGen) {
      ...
      if (Error E = writePropertiesToFile(CopyTriple.Prop, PropSet))
        return std::move(E);
