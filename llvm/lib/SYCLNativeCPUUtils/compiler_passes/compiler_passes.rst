...
* **vecz** has a test tool **veczc** and associated **lit** tests. This tool if
  ...
  requires `NATIVE_CPU_BUILD_VECZ_TEST_TOOLS` cmake option to build. It is also
  not added to any check targets so would require running `llvm-lit` directly on the
  directory `lib/SYCLNativeCPUUtils/compiler_passes/vecz/test/lit` under the
  build directory, after building `veczc`.
