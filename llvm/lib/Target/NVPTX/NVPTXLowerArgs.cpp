...
void copyByValParam(Function &F, Argument &Arg) {
  ...
  // Must create ArgInParamAS before replacing uses of Arg.
  // createNVVMInternalAddrspaceWrap needs to use Arg as an operand.
  Value *ArgInParamAS = createNVVMInternalAddrspaceWrap(IRB, Arg);

  // Replace all uses of Arg with AllocA, except the use in ArgInParamAS.
  // Note: we can't use replaceAllUsesWith because it would replace
  // ArgInParamAS's operand too, creating a circular dependency.
  SmallVector<Use *, 8> UsesToReplace;
  for (Use &U : Arg.uses()) {
    if (U.getUser() != ArgInParamAS)
