...
// Wrapper to use the pass with legacy pass manager
...
PreservedAnalyses GlobalOffsetPass::run(Module &M, ModuleAnalysisManager &) {
  ...
  // FIXME: use something more prescise
...
std::pair<Function *, Value *> GlobalOffsetPass::addOffsetArgumentToFunction(
    Module &M, Function *Func, Type *ImplicitArgumentType,
    bool KeepOriginal) {
...
llvm::DenseMap<Function *, MDNode *>
GlobalOffsetPass::getEntryPointMetadata(Module &M) {
  auto NvvmMetadata = M.getNamedMetadata("nvvm.annotations");
  assert(NvvmMetadata && "IR compiled to PTX must have nvvm.annotations");
...
INITIALIZE_PASS(GlobalOffsetLegacy, "globaloffset", "SYCL Global Offset",
                false, false)

ModulePass *llvm::createGlobalOffsetLegacyPass() {
