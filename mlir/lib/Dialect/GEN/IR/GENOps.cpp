...
LogicalResult GEN::MatrixDPASOp::verify() {
  ...
  return TypeSwitch<Type, LogicalResult>(AElemTy)
      .Case<Float32Type>([&](auto ty) -> LogicalResult {
        if (precision != GEN::PrecisionType::TF32)
      ...
      .Case<BFloat16Type>([&](auto ty) -> LogicalResult {
        if (precision != GEN::PrecisionType::BF16)
          return this->emitOpError(
      ...
      .Case<Float16Type>([&](auto ty) -> LogicalResult {
        if (precision != GEN::PrecisionType::FP16)
...
LogicalResult GEN::Matrix2DBlockLoadOp::verify() { return verifyInput(*this); }

//===----------------------------------------------------------------------===//
// gen.matrix.2Dblockstore
//===----------------------------------------------------------------------===//

LogicalResult GEN::Matrix2DBlockStoreOp::verify() { return verifyInput(*this); }
