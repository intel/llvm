...
int main() {
  ...
  constexpr int N = 32;
  max_generic_test<double>(q, N);

  // Include long tests if they are 64 bits wide
  if constexpr (sizeof(long) == 8) {
    max_generic_test<long>(q, N);
    max_generic_test<unsigned long>(q, N);
  }

  // Include long long tests if they are 64 bits wide
  if constexpr (sizeof(long long) == 8) {
    max_generic_test<long long>(q, N);
    max_generic_test<unsigned long long>(q, N);
  }
