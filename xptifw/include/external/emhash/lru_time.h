...
#define NEW_KVALUE(key, value, bucket)                                         \
  new (_pairs + bucket) PairT(key, value, bucket, _time_out), _num_filled++
...
public:
  ...
  lru_cache(uint32_t bucket = 4, uint32_t max_bucket = 1 << 24,
            int timeout = 3600 * 24 * 365) {
    init();
    _time_out = timeout;
  ...
  void clone(const lru_cache &other) {
    ...
    if (std::is_pod<KeyT>::value && std::is_pod<ValueT>::value) {
      ...
      _pairs[_num_buckets + 0].timeout = _pairs[_num_buckets + 1].timeout =
          INACTIVE;
    }
  }
  ...
  bool check_timeout(uint32_t bucket) {
    ...
    if (IS_TIMEOUT(_pairs, bucket) ||
        hash_bucket(EMH_KEY(_pairs, bucket)) == bucket) {
      //_pairs[bucket].~PairT();
      clear_bucket(bucket);
      return true;
    }
  ...
  int get_bucket_info(const uint32_t bucket, uint32_t steps[],
  ...
  void dump_statis() const {
    ...
    if (sumb == 0)
      return;
    printf("    _num_filled/bucket_size/packed collision/cache_miss/hit_found "
  ...
  //    std::pair<iterator, bool> insert(const value_pair& value) { return
  ...
  void clear() {
    if (is_notrivially() || sizeof(PairT) > EMHASH_CACHE_LINE_SIZE ||
  ...
  bool reserve(uint32_t num_elems) {
    const auto required_buckets =
        (uint32_t)(((uint64_t)num_elems) * _loadlf >> 27);
  ...
  void rehash(uint32_t required_buckets) {
    ...
    NEXT_BUCKET(_pairs, _num_buckets) = NEXT_BUCKET(_pairs, _num_buckets + 1) =
        0;
    _pairs[_num_buckets + 0].timeout = _pairs[_num_buckets + 1].timeout =
        INACTIVE;
    ...
    for (uint32_t src_bucket = 0; old_num_filled > 0; src_bucket++) {
      ...
      old_num_filled--;
      if (old_pairs[src_bucket].timeout > now_ts &&
          _num_filled < _max_buckets) {
        auto &key = EMH_KEY(old_pairs, src_bucket);
        const auto bucket = find_unique_bucket(key);
        NEW_KVALUE(std::move(key), std::move(EMH_VAL(old_pairs, src_bucket)),
                   bucket);
