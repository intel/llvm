...
protected:
  template <class K> size_t _find_key(const K &key, size_t hashval) {
    auto seq = probe(hashval);
    while (true) {
      Group g{ctrl_ + seq.offset()};
      for (uint32_t i : g.Match((h2_t)H2(hashval))) {
...
public:
  ...
  emplace_decomposable_with_hash(const K &key, size_t hashval, Args &&...args) {
    ...
    if (offset == (size_t)-1 && m.switch_to_unique()) {
      // we did an unlock/lock, and another thread could have inserted the same
      // key, so we need to do a find() again.
  ...
  bool if_contains(const key_arg<K> &key, F &&f) const {
    return const_cast<parallel_hash_set *>(this)
        ->template modify_if_impl<K, F, SharedLock>(key, std::forward<F>(f));
  }
  ...
  bool lazy_emplace_l(const key_arg<K> &key, FExists &&fExists,
    ...
    if (std::get<2>(res)) {
      ...
      // key found. Call fExists lambda. In case of the set, non "key" part of
      // value_type can be changed
      auto it =
...
public:
  ...
  bool try_emplace_l(K &&k, F &&f, Args &&...args) {
    ...
    if (std::get<2>(res)) {
      ...
      auto it =
          this->iterator_at(inner, inner->set_.iterator_at(std::get<1>(res)));
      // call lambda. in case of the set, non "key" part of value_type can be
      // changed
