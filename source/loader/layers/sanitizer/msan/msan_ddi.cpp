...
ur_result_t urAdapterGet(
    ...
    ///< will be returned.
    ur_adapter_handle_t *phAdapters, ///< [out][optional][range(0, NumEntries)]
                                     ///< array of handle of adapters.
    ...
    uint32_t *pNumAdapters ///< [out][optional] returns the total number of
                           ///< adapters available.
) {
...
ur_result_t urProgramCreateWithNativeHandle(
    ...
    const ur_program_native_properties_t
        *pProperties, ///< [in][optional] pointer to native program properties
                      ///< struct.
...
ur_result_t urEnqueueKernelLaunch(
    ...
    const size_t *pGlobalWorkOffset, ///< [in] pointer to an array of workDim
                                     ///< unsigned values that specify the
    ///< offset used to calculate the global ID of a work-item
    const size_t *pGlobalWorkSize, ///< [in] pointer to an array of workDim
                                   ///< unsigned values that specify the
    ///< number of global work-items in workDim that will execute the kernel
    ///< function
    const size_t *pLocalWorkSize, ///< [in][optional] pointer to an array of
                                  ///< workDim unsigned values that
    ...
    ///< event.
    ur_event_handle_t
        *phEvent ///< [out][optional] return an event object that identifies
...
ur_result_t urContextCreateWithNativeHandle(
    ur_native_handle_t hNativeContext, ///< [in][nocheck] the native handle of
                                       ///< the getContext()->
    ur_adapter_handle_t hAdapter,
    uint32_t numDevices, ///< [in] number of devices associated with the context
    const ur_device_handle_t
        *phDevices, ///< [in][range(0, numDevices)] list of devices associated
                    ///< with the context
    const ur_context_native_properties_t
        *pProperties, ///< [in][optional] pointer to native context properties
                      ///< struct
...
ur_result_t
urMemGetInfo(ur_mem_handle_t
                 hMemory, ///< [in] handle to the memory object being queried.
             ur_mem_info_t propName, ///< [in] type of the info to retrieve.
             size_t propSize, ///< [in] the number of bytes of memory pointed to
                              ///< by pPropValue.
             void *pPropValue, ///< [out][optional][typename(propName,
                               ///< propSize)] array of bytes holding the info.
             ///< If propSize is less than the real number of bytes needed to
             ///< return the info then the ::UR_RESULT_ERROR_INVALID_SIZE error
             ///< is returned and pPropValue is not used.
             size_t *pPropSizeRet ///< [out][optional] pointer to the actual
                                  ///< size in bytes of the queried propName.
) {
...
ur_result_t urEnqueueMemBufferReadRect(
    ur_queue_handle_t hQueue, ///< [in] handle of the queue object
    ur_mem_handle_t hBuffer,  ///< [in][bounds(bufferOrigin, region)] handle of
                              ///< the buffer object
    bool blockingRead, ///< [in] indicates blocking (true), non-blocking (false)
...
ur_result_t urEnqueueMemBufferCopy(
    ur_queue_handle_t hQueue,   ///< [in] handle of the queue object
    ur_mem_handle_t hBufferSrc, ///< [in][bounds(srcOffset, size)] handle of the
                                ///< src buffer object
    ur_mem_handle_t hBufferDst, ///< [in][bounds(dstOffset, size)] handle of the
                                ///< dest buffer object
...
ur_result_t urEnqueueMemBufferCopyRect(
    ur_queue_handle_t hQueue,   ///< [in] handle of the queue object
    ur_mem_handle_t hBufferSrc, ///< [in][bounds(srcOrigin, region)] handle of
                                ///< the source buffer object
    ur_mem_handle_t hBufferDst, ///< [in][bounds(dstOrigin, region)] handle of
                                ///< the dest buffer object
    ur_rect_offset_t srcOrigin, ///< [in] 3D offset in the source buffer
    ur_rect_offset_t dstOrigin, ///< [in] 3D offset in the destination buffer
    ur_rect_region_t region, ///< [in] source 3D rectangular region descriptor:
                             ///< width, height, depth
...
ur_result_t UR_APICALL urEnqueueUSMMemcpy(
    ...
    void *pDst, ///< [in][bounds(0, size)] pointer to the destination USM memory
                ///< object
    const void *
...
ur_result_t UR_APICALL urEnqueueUSMFill2D(
    ...
    void *pMem,   ///< [in][bounds(0, pitch * height)] pointer to memory to be
                  ///< filled.
    ...
    const void
        *pPattern, ///< [in] pointer with the bytes of the pattern to set.
    size_t width,  ///< [in] the width in bytes of each row to fill. Must be a
                  ///< multiple of patternSize.
...
ur_result_t UR_APICALL urEnqueueUSMMemcpy2D(
    ...
    void *pDst, ///< [in][bounds(0, dstPitch * height)] pointer to memory where
                ///< data will be copied.
    size_t dstPitch,  ///< [in] the total width of the source memory including
                      ///< padding.
    const void *pSrc, ///< [in][bounds(0, srcPitch * height)] pointer to memory
                      ///< to be copied.
