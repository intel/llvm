...
; CHECK-LLVM: fmul float %a, %b
; CHECK-LLVM: fmul nnan float %a, %b
; CHECK-LLVM: fmul ninf float %a, %b
; CHECK-LLVM: fmul nsz float %a, %b
; CHECK-LLVM: fmul arcp float %a, %b
; CHECK-LLVM-DEFAULT: fmul fast float %a, %b
; CHECK-LLVM-FC2: fmul reassoc nnan ninf nsz arcp contract float %a, %b
; CHECK-LLVM: fmul nnan ninf float %a, %b
; CHECK-LLVM-DEFAULT: fmul float %a, %b
; CHECK-LLVM-FC2: fmul contract float %a, %b
; CHECK-LLVM-DEFAULT: fmul float %a, %b
; CHECK-LLVM-FC2: fmul reassoc contract float %a, %b
