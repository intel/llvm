...
checkContextSupports(const std::shared_ptr<detail::context_impl> &ContextImpl,
  ...
  Plugin->call<UrApiKind::urContextGetInfo>( ContextImpl->getHandleRef(), InfoQuery,
...
std::optional<std::array<size_t, 3>> handler::getMaxWorkGroups() {
  ...
  auto Ret = Dev->getPlugin()->call_nocheck<UrApiKind::urDeviceGetInfo>( Dev->getHandleRef(),
      UrInfoCode<
