...
device device_selector::select_device() const {
  ...
  for (const auto &dev : devices) {
    ...
    if (detail::pi::trace(detail::pi::TraceLevel::PI_TRACE_ALL)) {
      ...
      std::cout << "SYCL_PI_TRACE[1]: select_device(): -> score = " << score
                << std::endl
                << "SYCL_PI_TRACE[1]:   platform: " << PlatformVersion
                << std::endl
                << "SYCL_PI_TRACE[1]:   device: " << DeviceName << std::endl;
    }
  ...
  if (res != nullptr) {
    if (detail::pi::trace(detail::pi::TraceLevel::PI_TRACE_BASIC)) {
      ...
      std::cout << "SYCL_PI_TRACE[1]: Selected device ->" << std::endl
                << "SYCL_PI_TRACE[1]:   platform: " << PlatformVersion
                << std::endl
                << "SYCL_PI_TRACE[1]:   device: " << DeviceName << std::endl;
    }
