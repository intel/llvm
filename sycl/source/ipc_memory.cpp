...
openIPCPhysMemHandle(const std::byte *HandleData, size_t HandleDataSize,
  ...
  try {
    ...
    // Query the IPC virtual address established by zeMemOpenIpcHandle.
    // This is the address the SYCL consumer must use to access the memory;
    // it is returned by physical_mem_impl::map() for IPC-opened handles.
    void *IpcVirtualAddress = nullptr;
    Adapter.call<sycl::detail::UrApiKind::urPhysicalMemGetInfo>(
        PhysMemHandle, UR_PHYSICAL_MEM_INFO_IPC_VIRTUAL_ADDRESS, sizeof(void *),
        &IpcVirtualAddress, nullptr);
    if (!IpcVirtualAddress)
      throw sycl::exception(
          sycl::make_error_code(errc::runtime),
          "urIPCOpenPhysMemHandleExp succeeded but returned a null IPC virtual "
          "address.");

    auto PhysMemImpl = std::make_shared<sycl::detail::physical_mem_impl>(
        *getSyclObjImpl(Dev), Ctx, NumBytes, PhysMemHandle, IpcVirtualAddress);
...
get(const ext::oneapi::experimental::physical_mem &PhysMem) {
  ...
  if (UrRes != UR_RESULT_SUCCESS) {
    ...
    if (UrRes == UR_RESULT_ERROR_UNSUPPORTED_FEATURE)
      throw sycl::exception(
          sycl::make_error_code(errc::feature_not_supported),
          "The driver does not support IPC for physical memory objects.");
    Adapter.checkUrResult(UrRes);
  }
  if (HandlePtr == nullptr)
    throw sycl::exception(
