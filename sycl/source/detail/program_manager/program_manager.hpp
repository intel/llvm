...
public:
  ...
  RTDeviceBinaryImage &getDeviceImage(std::string_view KernelName,
  ...
  bool kernelUsesAssert(std::string_view KernelName) const;
...
protected:
  ...
  /// Access must be guarded by the m_KernelIDsMutex mutex.
  std::unordered_multimap<std::string_view, RTDeviceBinaryImage *>
      m_ServiceKernels;
  ...
  std::set<std::string_view> m_KernelUsesAssert;
