...
static void enableITTAnnotationsIfNeeded(const sycl::detail::pi::PiProgram &Prog,
...
createBinaryProgram(const ContextImplPtr Context, const device &Device,
  ...
  const sycl::detail::pi::PiDevice PiDevice = getSyclObjImpl(Device)->getHandleRef();
...
static sycl::detail::pi::PiProgram createSpirvProgram(const ContextImplPtr Context,
...
sycl::detail::pi::PiProgram ProgramManager::createPIProgram(const RTDeviceBinaryImage &Img,
  ...
  sycl::detail::pi::PiProgram Res = Format == PI_DEVICE_BINARY_TYPE_SPIRV
...
std::pair<sycl::detail::pi::PiProgram, bool> ProgramManager::getOrCreatePIProgram(
...
#undef __SYCL_ASPECT
  ...
  auto BuildResult = getOrBuild<sycl::detail::pi::PiProgram, compile_program_error>(
...
std::tuple<sycl::detail::pi::PiKernel, std::mutex *, const KernelArgMask *, sycl::detail::pi::PiProgram>
...
ProgramManager::getPiProgramFromPiKernel(sycl::detail::pi::PiKernel Kernel,
  ...
  Plugin->call<PiApiKind::piKernelGetInfo>(
      Kernel, PI_KERNEL_INFO_PROGRAM, sizeof(sycl::detail::pi::PiProgram), &Program, nullptr);
  return Program;
}

std::string ProgramManager::getProgramBuildLog(const sycl::detail::pi::PiProgram &Program,
                                               const ContextImplPtr Context) {
  size_t PIDevicesSize = 0;
  const PluginPtr &Plugin = Context->getPlugin();
  Plugin->call<PiApiKind::piProgramGetInfo>(Program, PI_PROGRAM_INFO_DEVICES, 0,
                                            nullptr, &PIDevicesSize);
  std::vector<sycl::detail::pi::PiDevice> PIDevices(PIDevicesSize / sizeof(sycl::detail::pi::PiDevice));
...
static sycl::detail::pi::PiProgram loadDeviceLibFallback(const ContextImplPtr Context,
  ...
  sycl::detail::pi::PiResult Error = Plugin->call_nocheck<PiApiKind::piProgramCompile>(
      LibProg,
...
getDeviceLibPrograms(const ContextImplPtr Context, const sycl::detail::pi::PiDevice &Device,
...
ProgramManager::build(ProgramPtr Program, const ContextImplPtr Context,
  ...
  if (LinkPrograms.empty() && !ForceLink) {
    ...
    sycl::detail::pi::PiResult Error = Plugin->call_nocheck<PiApiKind::piProgramBuild>(
  ...
  sycl::detail::pi::PiResult Error = Plugin->call_nocheck<PiApiKind::piProgramLink>(
...
static bool compatibleWithDevice(RTDeviceBinaryImage *BinImage,
  ...
  sycl::detail::pi::PiResult Error = Plugin->call_nocheck<PiApiKind::piextDeviceSelectBinary>(
...
setSpecializationConstants(const std::shared_ptr<device_image_impl> &InputImpl,
                           sycl::detail::pi::PiProgram Prog, const PluginPtr &Plugin) {
...
ProgramManager::compile(const device_image_plain &DeviceImage,
  ...
  // device is OK.
  sycl::detail::pi::PiProgram Prog = createPIProgram(*InputImpl->get_bin_image_ref(),
...
device_image_plain ProgramManager::build(const device_image_plain &DeviceImage,
  ...
  uint32_t ImgId = Img.getImageID();
  const sycl::detail::pi::PiDevice PiDevice = getRawSyclObjImpl(Devs[0])->getHandleRef();
  auto CacheKey =
      std::make_pair(std::make_pair(std::move(SpecConsts), ImgId),
                     std::make_pair(PiDevice, CompileOpts + LinkOpts));
  ...
  for (size_t Idx = 1; Idx < Devs.size(); ++Idx) {
    ...
    getOrBuild<sycl::detail::pi::PiProgram, compile_program_error>(Cache, GetCachedBuildF,
                                                     CacheOtherDevices);
    // getOrBuild is not supposed to return nullptr
    assert(BuildResult != nullptr && "Invalid build result");
  }
