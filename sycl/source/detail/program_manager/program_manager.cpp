...
convertMatrixTypeStringMatrixTypeEnumValue(std::string MatrixTypeString) {
  ...
  if ("matrix_type::bf16" == MatrixTypeString)
    return matrix_ext::matrix_type::bf16;
  else if ("matrix_type::fp16" == MatrixTypeString)
    return matrix_ext::matrix_type::fp16;
  else if ("matrix_type::tf32" == MatrixTypeString)
    return matrix_ext::matrix_type::tf32;
  else if ("matrix_type::fp32" == MatrixTypeString)
    return matrix_ext::matrix_type::fp32;
  else if ("matrix_type::fp64" == MatrixTypeString)
    return matrix_ext::matrix_type::fp64;
  else if ("matrix_type::sint8" == MatrixTypeString)
    return matrix_ext::matrix_type::sint8;
  else if ("matrix_type::sint16" == MatrixTypeString)
    return matrix_ext::matrix_type::sint16;
  else if ("matrix_type::sint32" == MatrixTypeString)
    return matrix_ext::matrix_type::sint32;
  else if ("matrix_type::sint64" == MatrixTypeString)
    return matrix_ext::matrix_type::sint64;
  else if ("matrix_type::uint8" == MatrixTypeString)
    return matrix_ext::matrix_type::uint8;
  else if ("matrix_type::uint16" == MatrixTypeString)
    return matrix_ext::matrix_type::uint16;
  else if ("matrix_type::uint32" == MatrixTypeString)
    return matrix_ext::matrix_type::uint32;
  else if ("matrix_type::uint64" == MatrixTypeString)
    return matrix_ext::matrix_type::uint64;
  return std::nullopt;
}

bool isMatrixSupportedByHW(std::string MatrixTypeStrUser, size_t RowsUser,
                           size_t ColsUser,
  ...
  return (MatrixTypeUserOpt ? (MatrixTypeUserOpt.value() == MatrixTypeRuntime)
                            : false) &&
         ((RowsRuntime != 0) ? (RowsUser == RowsRuntime)
                             : (RowsUser <= MaxRowsRuntime)) &&
         ((ColsRuntime != 0) ? (ColsUser == ColsRuntime)
...
checkDevSupportDeviceRequirements(const device &Dev,
  ...
  if (JointMatrixPropIt) {
    ...
    std::cout << ">>> JointMatrixByteArrayToStr: " << JointMatrixByteArrayToStr
              << "\n";
  ...
  if (JointMatrixMadPropIt) {
    ...
    std::cout << ">>> JointMatrixMadByteArrayToStr: "
    ...
    while (std::getline(JointMatrixMadStrStream, SingleJointMatrixMad, ';')) {
      ...
      if (JointMatrixMadVec.size() == 7) {
        std::string MatrixTypeAStrUser = JointMatrixMadVec[0];
        std::string MatrixTypeBStrUser = JointMatrixMadVec[1];
        std::string MatrixTypeCStrUser = JointMatrixMadVec[2];
        std::string MatrixTypeDStrUser = JointMatrixMadVec[3];
        ...
        for (const auto &Combination : Combinations) {
          if (MatrixTypeAUserOpt && MatrixTypeBUserOpt && MatrixTypeCUserOpt &&
            ...
            IsMatrixMadCompatible =
                ...
                ((Combination.msize != 0)
                     ? (MSizeUser == Combination.msize)
                     : (MSizeUser <= Combination.max_msize)) &&
                ((Combination.ksize != 0)
                     ? (KSizeUser == Combination.ksize)
                     : (KSizeUser <= Combination.max_ksize)) &&
                ((Combination.nsize != 0)
                     ? (NSizeUser == Combination.nsize)
