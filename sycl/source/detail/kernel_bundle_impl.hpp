...
public:
  ...
  kernel_bundle_impl(
    ...
    // Posion the images based on whether the corresponding device supports it.
  ...
  build_from_source(const std::vector<device> Devices,
    ...
    return std::make_shared<kernel_bundle_impl>(
        MContext, Devices, std::move(NewDevImgs), bundle_state::executable,
        std::move(NewDeviceBinaries));
  }
  ...
  compile_from_source(const std::vector<device> Devices,
    ...
    return std::make_shared<kernel_bundle_impl>(
        MContext, Devices, std::move(NewDevImgs), bundle_state::object,
        std::move(NewDeviceBinaries));
  }
