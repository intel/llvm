...
context_impl::context_impl(const std::vector<sycl::device> Devices,
  ...
  getPlugin()->call<UrApiKind::urContextCreate>( DeviceIds.size(), DeviceIds.data(),
                    nullptr, &MContext);
...
context_impl::context_impl(ur_context_handle_t UrContext,
  ...
  if (!MDevices.empty()) {
    ...
    Plugin->call<UrApiKind::urContextGetInfo>( MContext, UR_CONTEXT_INFO_NUM_DEVICES,
                 sizeof(DevicesNum), &DevicesNum, nullptr);
    DeviceIds.resize(DevicesNum);
    // TODO catch an exception and put it to list of asynchronous exceptions
    Plugin->call<UrApiKind::urContextGetInfo>( MContext, UR_CONTEXT_INFO_DEVICES,
...
cl_context context_impl::get() const {
  ...
  getPlugin()->call<UrApiKind::urContextGetNativeHandle>( MContext, &nativeHandle);
