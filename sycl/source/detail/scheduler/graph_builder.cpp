...
Scheduler::GraphBuilder::insertMemoryMove(MemObjRecord *Record,
  ...
  AllocaCommandBase *AllocaCmdSrc =
      findAllocaForReq(Record, Req, Record->MCurContext.get());
  if (!AllocaCmdSrc && IsSuitableSubReq(Req)) {
    ...
    auto IsSuitableAlloca = [Record](AllocaCommandBase *AllocaCmd) {
      bool Res =
          isOnSameContext(Record->MCurContext.get(), AllocaCmd->getQueue()) &&
  ...
  if (AllocaCmdSrc->MLinkedAllocaCmd == AllocaCmdDst) {
    ...
    if ((Req->MAccessMode == access::mode::discard_write) ||
      ...
      Record->MCurContext = Context ? Context->shared_from_this() : nullptr;
      return nullptr;
    } else {
...
Command *Scheduler::GraphBuilder::remapMemoryObject(
  ...
  std::set<Command *> Deps =
      findDepsForReq(Record, Req, Record->MCurContext.get());
...
Scheduler::GraphBuilder::addHostAccessor(Requirement *Req,
  ...
  if (isOnSameContext(Record->MCurContext.get(), HostAllocaCmd->getQueue())) {
...
Command *Scheduler::GraphBuilder::addCG(
  ...
  for (Requirement *Req : Reqs) {
...

    {
      ...
      isSameCtx = isOnSameContext(Record->MCurContext.get(), QueueForAlloca);
    }
    ...
    if (isSameCtx) {
      ...
      if (isInteropTask) {
        ...
        if (!isOnSameContext(Record->MCurContext.get(), HT.MQueue.get())) {
...
Command *Scheduler::GraphBuilder::addCommandGraphUpdate(
  ...
  for (Requirement *Req : Requirements) {
...

    {
      ...
      isSameCtx = isOnSameContext(Record->MCurContext.get(), Queue);
    }
