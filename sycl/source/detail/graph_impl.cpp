...
void exec_graph_impl::createCommandBuffers(
  ...
  ur_result_t Res = Plugin->call_nocheck<sycl::detail::UrApiKind::urCommandBufferCreateExp>( ContextImpl->getHandleRef(),
  ...
  Res = Plugin->call_nocheck<sycl::detail::UrApiKind::urCommandBufferFinalizeExp>( OutCommandBuffer);
  if (Res != UR_RESULT_SUCCESS) {
    throw sycl::exception(errc::invalid,
  ...
  try {
    ...
    for (const auto &Partition : MPartitions) {
      ...
      for (const auto &Iter : Partition->MCommandBuffers) {
        if (auto CmdBuf = Iter.second; CmdBuf) {
          ur_result_t Res =
              Plugin->call_nocheck<sycl::detail::UrApiKind::urCommandBufferReleaseExp>( CmdBuf);
          (void)Res;
    ...
    for (auto &Iter : MCommandMap) {
      if (auto Command = Iter.second; Command) {
        ur_result_t Res =
            Plugin->call_nocheck<sycl::detail::UrApiKind::urCommandBufferReleaseCommandExp>( Command);
        (void)Res;
        assert(Res == UR_RESULT_SUCCESS);
      }
    }
  ...
  for (uint32_t currentPartitionsNum = 0;
    ...
    if (CommandBuffer) {
      ...
      if (CGData.MRequirements.empty() && CGData.MEvents.empty()) {
        ...
        ur_result_t Res = Queue->getPlugin()->call_nocheck<sycl::detail::UrApiKind::urCommandBufferEnqueueExp>( CommandBuffer, Queue->getHandleRef(), 0,
            nullptr, &UREvent);
...
void exec_graph_impl::updateImpl(std::shared_ptr<node_impl> Node) {
  ...
  if (NDRDesc.LocalSize[0] != 0)
    ...
    Plugin->call<sycl::detail::UrApiKind::urKernelGetGroupInfo>( UrKernel, DeviceImpl->getHandleRef(),
  ...
  ur_result_t Res = Plugin->call_nocheck<sycl::detail::UrApiKind::urCommandBufferUpdateKernelLaunchExp>(
                                         Command, &UpdateDesc);
