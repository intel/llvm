...
__SYCL_INLINE_NAMESPACE(cl) {
  ...
  public:
    ...
    /// Clean up the command associated with the event. Assumes that the task
    /// this
    /// event is associated with has been completed.
    ///
    /// \param Self is a pointer to this event.
    void
    cleanupCommand(std::shared_ptr<cl::sycl::detail::event_impl> Self) const;
    ...
    /// \return a native handle or initializes if it is defaultly constructed
    /// examplar.
    ...
    /// Performs a flush on the queue associated with this event if the user
    /// queue
    /// is different and the task associated with this event hasn't been
    /// submitted
    /// to the device yet.
  ...
  private:
    // When instrumentation is enabled emits trace event for event wait begin
    // and
    ...
    // handled by post enqueue cleanup yet and has to be deleted by cleanup
    // after
    // wait.
