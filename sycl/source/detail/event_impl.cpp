...
__SYCL_INLINE_NAMESPACE(cl) {
  ...
  void *event_impl::instrumentationProlog(std::string &Name, int32_t StreamID,
    ...
    if (!xptiTraceEnabled())
      return TraceEvent;
    // Use a thread-safe counter to get a unique instance ID for the wait() on
    // the
    // event
  ...
  void
  event_impl::wait(std::shared_ptr<cl::sycl::detail::event_impl> Self) const {
    if (MState == HES_Discarded)
      throw sycl::exception(
          make_error_code(errc::invalid),
  ...
  event_impl::get_profiling_info<info::event_profiling::command_end>() const {
    ...
    if (!MHostEvent) {
      if (MEvent)
        return get_event_profiling_info<
            info::event_profiling::command_end>::get(this->getHandleRef(),
                                                     this->getPlugin());
      return 0;
    }
    if (!MHostProfilingInfo)
