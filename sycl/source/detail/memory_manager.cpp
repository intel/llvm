...
void MemoryManager::releaseMemObj(context_impl *TargetContext,
  ...
  memReleaseHelper(TargetContext->getAdapter(),
                   ur::cast<ur_mem_handle_t>(MemAllocation));
}
...
void *MemoryManager::allocateImageObject(context_impl *TargetContext,
  ...
  TargetContext->getAdapter().call<UrApiKind::urMemImageCreate>(
...
MemoryManager::allocateBufferObject(context_impl *TargetContext, void *UserPtr,
  ...
  memBufferCreateHelper(TargetContext->getAdapter(),
                        TargetContext->getHandleRef(), CreationFlags, Size,
                        &NewMem, &AllocProps);
  return NewMem;
}
