...
public:
  ...
  buildFromSource(const std::vector<device> &Devices,
    ...
    if (MRTCBinInfo->MLanguage == syclex::source_language::sycl)
      return createSYCLImages(Devices, bundle_state::executable,
                                 BuildOptions, LogPtr, RegisteredKernelNames,
                                 OutDeviceBinaries);
...
private:
  ...
  std::vector<std::shared_ptr<device_image_impl>> createSYCLImages(
    ...
    for (int I = 0; I < Binaries->NumDeviceBinaries; I++) {
      ...
      // Mark the image as input so the program manager will bring it into the
      // right state.
