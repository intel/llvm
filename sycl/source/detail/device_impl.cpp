...
std::vector<device> device_impl::create_sub_devices(
  ...
  const PluginPtr &Plugin = getPlugin();
  Plugin->call<sycl::errc::invalid>(urDevicePartition,
      MUrDevice, &Properties, 0, nullptr, &SubDevicesCount);
...
uint64_t device_impl::getCurrentDeviceTime() {
  ...
  if (!MDeviceHostBaseTime.second || Diff > TimeTillRefresh) {
    ...
    auto Result =
        Plugin->call_nocheck(urDeviceGetGlobalTimestamps,
            MUrDevice, &MDeviceHostBaseTime.first, &MDeviceHostBaseTime.second);
