...
device_impl::device_impl(ur_native_handle_t InteropDeviceHandle,
  ...
  if (Device == nullptr) {
    ...
    Plugin->call<UrApiKind::urDeviceCreateWithNativeHandle>(
        InteropDeviceHandle, Plugin->getUrAdapter(), nullptr, &MDevice);
...
std::vector<device> device_impl::create_sub_devices(
  ...
  const PluginPtr &Plugin = getPlugin();
  Plugin->call<sycl::errc::invalid, UrApiKind::urDevicePartition>(
...
bool device_impl::has(aspect Aspect) const {
  ...
  case aspect::ext_intel_device_id:
    return getPlugin()->call_nocheck<UrApiKind::urDeviceGetInfo>(
  ...
  case aspect::ext_intel_device_info_uuid: {
    auto Result = getPlugin()->call_nocheck<UrApiKind::urDeviceGetInfo>(
  ...
  case aspect::ext_oneapi_cuda_async_barrier: {
    ...
    bool call_successful =
        getPlugin()->call_nocheck<UrApiKind::urDeviceGetInfo>(
...
uint64_t device_impl::getCurrentDeviceTime() {
  ...
  if (!MDeviceHostBaseTime.second || Diff > TimeTillRefresh) {
    ...
    auto Result = Plugin->call_nocheck<UrApiKind::urDeviceGetGlobalTimestamps>(
