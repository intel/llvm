...
void handleInvalidWorkGroupSize(const device_impl &DeviceImpl,
  ...
  Plugin->call<UrApiKind::urKernelGetGroupInfo>( Kernel, Device,
               UR_KERNEL_GROUP_INFO_COMPILE_WORK_GROUP_SIZE, sizeof(size_t) * 3,
               CompileWGSize, nullptr);

  size_t MaxWGSize = 0;
  Plugin->call<UrApiKind::urDeviceGetInfo>( Device, UR_DEVICE_INFO_MAX_WORK_GROUP_SIZE,
  ...
  if (HasLocalSize) {
    ...
    Plugin->call<UrApiKind::urDeviceGetInfo>( Device, UR_DEVICE_INFO_MAX_WORK_ITEM_SIZES,
...
void handleInvalidValue(const device_impl &DeviceImpl,
  ...
  Plugin->call<UrApiKind::urDeviceGetInfo>( Device, UR_DEVICE_INFO_MAX_WORK_GROUPS_3D,
