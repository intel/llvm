  ...
  case kernel_param_kind_t::kind_accessor: {
    ...
    case access::target::local: {
      ...
      addArgsForLocalAccessor(LAccImpl, Index, IndexShift,
                              IsKernelCreatedFromSource, MArgs, MKernelIsESIMD);
      break;
    }
...
void KernelData::extractArgsAndReqsFromLambda(const NDRDescT &NDRDesc) {
  size_t IndexShift = 0;
  clearArgs();
  MArgs.reserve(MaxNumAdditionalArgs * MKernelNumArgs);

  for (size_t I = 0; I < MKernelNumArgs; ++I) {
    detail::kernel_param_desc_t ParamDesc = MKernelParamDescGetter(I);
