...
#pragma once
#include <sycl/ext/oneapi/matrix/joint-matrix-cuda-impl.hpp>
#include <sycl/ext/oneapi/matrix/matrix-tensor-cores.hpp>
// #include <sycl/ext/oneapi/matrix/matrix-intel-impl....hpp>
...
// intel joint_matrix_fill_intel_impl
#endif // defined(__NVPTX__)
#else
    std::ignore = res;
    std::ignore = v;
    throw runtime_error("The matrix extension is only currently supported on "
                        "Intel and Nvidia devices",
...
#if defined(__SYCL_DEVICE_ONLY__)
#if defined(__NVPTX__)
    sycl::ext::oneapi::detail::load_accumulator_cuda(res, src, stride,
                                                     LayoutAcc);
#elif defined(__SPIR__)
// load_accumulator_intel
#endif // defined(__NVPTX__)
#else
    std::ignore = sg;
    std::ignore = res;
...
#if defined(__SYCL_DEVICE_ONLY__)
#if defined(__NVPTX__)
    sycl::ext::oneapi::detail::joint_matrix_store_cuda_impl<T, NumRows, NumCols,
                                                            Space>{}
        .store(src, dst, stride, LayoutAcc);
#elif defined(__SPIR__)
// joint_matrix_store_intel_impl
#endif // defined(__NVPTX__)
#else
    std::ignore = sg;
    std::ignore = src;
  ...
  template <
      typename Group, typename Ta, typename Tb, typename Tc, std::size_t M,
      std::size_t K, std::size_t N,
      layout LayoutA = sycl::ext::oneapi::experimental::matrix::layout::dynamic,
      layout LayoutB = sycl::ext::oneapi::experimental::matrix::layout::dynamic>
  joint_matrix<Tc, matrix_use::accumulator, M, N,
               sycl::ext::oneapi::experimental::matrix::layout::dynamic, Group>
  joint_matrix_mad(
      Group sg, joint_matrix<Ta, matrix_use::a, M, K, LayoutA, Group> &A,
    ...
    if constexpr (std::is_same<Ta, Tb>::value) {
      joint_matrix<Tc, matrix_use::accumulator, M, N,
                   ...
                   Group>
          D;
      sycl::ext::oneapi::detail::joint_matrix_mad_cuda_impl<Ta, Tc, Tc, M, K, N,
                                                            LayoutA, LayoutB>{}
          .mad(D, A, B, C);
      return D;
    } else {
