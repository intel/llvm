...
template <size_t NumRows, size_t NumCols,
          ...
          sycl::ext::oneapi::experimental::matrix::layout Layout,
          typename Group>
class wi_element<sycl::detail::half_impl::half, NumRows, NumCols, Use, Layout,
                 Group> {
  sycl::ext::oneapi::experimental::matrix::joint_matrix<
      Group, sycl::detail::half_impl::half, Use, NumRows, NumCols, Layout> &M;
  std::size_t idx;

public:
  wi_element(sycl::ext::oneapi::experimental::matrix::joint_matrix<
                 Group, sycl::detail::half_impl::half, Use, NumRows, NumCols,
                 Layout> &Mat,
             std::size_t i)
      : M(Mat), idx(i) {}
  ...
  operator float() {
#ifdef __SYCL_DEVICE_ONLY__
sycl::detail::half_impl::half *InsertP =
        __spirv_AccessChain<sycl::detail::half_impl::half,
                            sycl::detail::half_impl::half, NumRows, NumCols,
                            spv_matrix_use_traits<Use>::value,
                            spv_scope_traits<Group>::value>(&M.spvm, idx);
  return (float)*InsertP;
  ...
  explicit operator bool() {
    ...
    sycl::detail::half_impl::half *ExtractP =
        __spirv_AccessChain<sycl::detail::half_impl::half,
                            ...
                            spv_matrix_use_traits<Use>::value,
                            spv_scope_traits<Group>::value>(&M.spvm, idx);
    sycl::detail::half_impl::half Elem = *ExtractP;
    return sycl::fabs(static_cast<float>(Elem)) >=
           std::numeric_limits<float>::epsilon();
#else
    throw exception(make_error_code(errc::runtime),
  ...
  wi_element &operator=(const sycl::detail::half_impl::half &rhs) {
#ifdef __SYCL_DEVICE_ONLY__
    sycl::detail::half_impl::half *InsertP =
        __spirv_AccessChain<sycl::detail::half_impl::half,
                            sycl::detail::half_impl::half, NumRows, NumCols,
  ...
  wi_element &operator=(const wi_element<sycl::detail::half_impl::half, NumRows,
                                         NumCols, Use, Layout, Group> &rhs) {
#ifdef __SYCL_DEVICE_ONLY__
    sycl::detail::half_impl::half *ExtractP =
        __spirv_AccessChain<sycl::detail::half_impl::half,
...
#if __SYCL_DEVICE_ONLY__
#define OP(opassign, op)                                                       \
  wi_element &operator opassign(const sycl::detail::half_impl::half &rhs) {      \
    sycl::detail::half_impl::half *ExtractP =                                    \
        __spirv_AccessChain<sycl::detail::half_impl::half,                       \
                            sycl::detail::half_impl::half, NumRows, NumCols,     \
                            spv_matrix_use_traits<Use>::value,                 \
...
#define OP(type, op)                                                           \
  friend type operator op(                                                     \
      const wi_element<sycl::detail::half_impl::half, NumRows, NumCols, Use,     \
                       Layout, Group> &lhs,                                    \
      const sycl::detail::half_impl::half &rhs) {                                \
    sycl::detail::half_impl::half *ExtractP =                                    \
        __spirv_AccessChain<sycl::detail::half_impl::half,                       \
                            sycl::detail::half_impl::half, NumRows, NumCols,     \
                            spv_matrix_use_traits<Use>::value,                 \
  ...
  friend type operator op(                                                     \
      const sycl::detail::half_impl::half &lhs,                                  \
      const wi_element<sycl::detail::half_impl::half, NumRows, NumCols, Use,     \
                       Layout, Group> &rhs) {                                  \
    sycl::detail::half_impl::half *ExtractP =                                    \
        __spirv_AccessChain<sycl::detail::half_impl::half,                       \
...
#define OP(type, op)                                                           \
  friend type operator op(                                                     \
      const wi_element<sycl::detail::half_impl::half, NumRows, NumCols, Use,     \
                       Layout, Group> &,                                       \
      const sycl::detail::half_impl::half &) {                                   \
    throw exception(make_error_code(errc::runtime),                            \
                    "joint matrix is not supported on host.");                 \
  }                                                                            \
  friend type operator op(                                                     \
