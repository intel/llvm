...
public:
  memory_pool_impl(const sycl::context &ctx, const sycl::device &dev,
                   const sycl::usm::alloc kind, const size_t threshold,
                   const size_t maxSize, const property_list &props);
  memory_pool_impl(const sycl::context &ctx, const sycl::device &dev,
...
public:
  ...
  // Copy constructible/assignable, move constructrible/assignable
  ...
  size_t get_max_size() const { return impl->get_max_size(); }
  size_t get_threshold() const { return impl->get_threshold(); }
  void set_new_threshold(size_t newThreshold);
