...
template <typename Group, std::size_t Extent> class group_with_scratchpad {
  Group g;
  __sycl_ns::span<std::byte, Extent> scratch;

public:
  group_with_scratchpad(Group g_, __sycl_ns::span<std::byte, Extent> scratch_)
      : g(g_), scratch(scratch_) {}
  Group get_group() const { return g; }
  __sycl_ns::span<std::byte, Extent> get_memory() const { return scratch; }
};
...
public:
  ...
  default_sorter(__sycl_ns::span<std::byte, Extent> scratch_,
  ...
  void operator()(Group g, Ptr first, Ptr last) {
    ...
    using T = typename __sycl_ns::detail::GetValueType<Ptr>::type;
    if (scratch_size >= memory_required<T>(Group::fence_scope, last - first))
      __sycl_ns::detail::merge_sort(g, first, last - first, comp, scratch);
  ...
  template <typename Group, typename T> T operator()(Group g, T val) {
    ...
    if (scratch_size >= memory_required<T>(Group::fence_scope, range_size)) {
      ...
      __sycl_ns::detail::merge_sort(g, temp, range_size, comp,
  ...
  template <typename T>
  static constexpr std::size_t memory_required(__sycl_ns::memory_scope,
                                               std::size_t range_size) {
    return range_size * sizeof(T) + alignof(T);
  }

  template <typename T, int dim = 1>
  static constexpr std::size_t memory_required(__sycl_ns::memory_scope scope,
                                               __sycl_ns::range<dim> r) {
