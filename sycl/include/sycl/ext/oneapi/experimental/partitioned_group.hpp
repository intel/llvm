//==------ partitioned_group.hpp --- SYCL extension for non-uniform groups -----==//
//
...
template <size_t PartitionSize, typename ParentGroup> class partitioned_group;
...
inline std::enable_if_t<sycl::is_group_v<std::decay_t<Group>> &&
                        ...
                        partitioned_group<PartitionSize, Group>>
get_partitioned_group(Group group);
...
protected:
  ...
  friend partitioned_group<PartitionSize, ParentGroup>
  get_partitioned_group<PartitionSize, ParentGroup>(ParentGroup g);
};
...
get_partitioned_group(Group group) {
  (void)group;
#ifdef __SYCL_DEVICE_ONLY__
  return partitioned_group<PartitionSize, sycl::sub_group>();
#else
  throw runtime_error("Non-uniform groups are not supported on host device.",
                      PI_ERROR_INVALID_DEVICE);
#endif
...
struct is_user_constructed_group<partitioned_group<PartitionSize, ParentGroup>>
    : std::true_type {};
