//==----- fpga_kernel_attributes.hpp - SYCL properties associated with FPGA kernel attributes ---==//
//
...
template <> struct is_property_key<pipelined_key> : std::true_type {};

template <typename T, typename PropertyListT>
struct is_property_key_of<streaming_interface_key,
                          fpga_kernel_attribute<T, PropertyListT>> : std::true_type {};
template <typename T, typename PropertyListT>
struct is_property_key_of<register_map_interface_key, fpga_kernel_attribute<T, PropertyListT>>
    : std::true_type {};
template <typename T, typename PropertyListT>
struct is_property_key_of<pipelined_key, fpga_kernel_attribute<T, PropertyListT>>
    : std::true_type {};

namespace detail {
template <> struct PropertyToKind<streaming_interface_key> {
  static constexpr PropKind Kind = PropKind::StreamingInterface;
};
template <> struct PropertyToKind<register_map_interface_key> {
  static constexpr PropKind Kind = PropKind::RegisterMapInterface;
};
template <> struct PropertyToKind<pipelined_key> {
  static constexpr PropKind Kind = PropKind::Pipelined;
};

template <>
struct IsCompileTimeProperty<streaming_interface_key> : std::true_type {};
template <> struct IsCompileTimeProperty<register_map_interface_key> : std::true_type {};
template <> struct IsCompileTimeProperty<pipelined_key> : std::true_type {};

template <streaming_interface_options_enum Stall>
struct PropertyMetaInfo<streaming_interface_key::value_t<Stall>> {
  static constexpr const char *name = "sycl-streaming-interface";
  static constexpr streaming_interface_options_enum value = Stall;
};
template<register_map_interface_options_enum Wait>
struct PropertyMetaInfo<register_map_interface_key::value_t<Wait>> {
  static constexpr const char *name = "sycl-register-map-interface";
  static constexpr register_map_interface_options_enum value = Wait;
};
template <int Value>
struct PropertyMetaInfo<pipelined_key::value_t<Value>> {
