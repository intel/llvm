...
private:
  ...
  friend class __sycl_ns::program;

public:
  template <typename V = T>
  typename __sycl_ns::detail::enable_if_t<std::is_arithmetic<V>::value, V>
  ...
  template <typename V = T>
  typename __sycl_ns::detail::enable_if_t<std::is_class<V>::value &&
                                              std::is_pod<V>::value,
                                          V>
  get() const { // explicit access.
#ifdef __SYCL_DEVICE_ONLY__
