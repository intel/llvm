...
__esimd_media_block_load(unsigned modififer, TACC handle, unsigned plane,
  ...
  for (int row = 0; row < M; row++) {
    for (int col = 0; col < N; col += vpp) {
      ...
      auto coords = sycl::cl_int2(xoff, yoff);
      sycl::cl_uint4 data = handle.read(coords);
      ...
      constexpr int refN = sizeof(sycl::cl_uint4) / sizeof(Ty);
      unsigned int stride = sizeof(sycl::cl_uint4) / bpp;
...
inline void __esimd_media_block_store(unsigned modififer, TACC handle,
  ...
  for (int row = 0; row < M; row++) {
    for (int col = 0; col < N; col += vpp) {
      constexpr int Sz = sizeof(sycl::cl_uint4) / sizeof(Ty);
