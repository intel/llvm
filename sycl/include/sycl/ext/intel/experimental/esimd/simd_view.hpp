...
public:
  ...
  template <int Size, int Stride, typename T = simd_view,
            typename = __sycl_internal::detail::enable_if_t<T::is1D()>>
  ...
  template <int SizeY, int StrideY, int SizeX, int StrideX,
            ...
            typename = __sycl_internal::detail::enable_if_t<T::is2D()>>
...
#undef DEF_UNARY_OP
  ...
  template <
      ...
      typename = __sycl_internal::detail::enable_if_t<std::is_integral<T1>::value, T2>>
  uint16_t any() {
    return read().any();
  }
