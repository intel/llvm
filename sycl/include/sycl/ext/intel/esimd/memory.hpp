...
/// simd<T, N> gather(const T *p, simd_view<OffsetObjT, OffsetRegionT>
/// byte_offsets,
///                   simd_mask<N> mask, simd_view<T, N> pass_thru,
///                   PropertyListT props = {});                   // (usm-ga-9)
/// simd<T, N> gather(const T *p, simd_view<OffsetT, N> byte_offsets,
///                   simd_mask<N> mask, PropertyListT props = {});//(usm-ga-10)
/// simd<T, N> gather(const T *p, simd_view<OffsetT, N> byte_offsets,
///                   PropertyListT props = {});                   //(usm-ga-11)
...
/// For each \c i, ((byte*)p + byte_offsets[i]) must be element size aligned.
...
/// template <typename T, int N, int VS = 1, typename OffsetObjT, typename
/// OffsetRegionT,
///           typename PropertyListT = empty_properties_t>
/// simd<T, N> gather(const T *p, simd<OffsetT, N / VS> byte_offsets,
///                   PropertyListT props = {});                   // (usm-ga-9)
