...
#include <sycl/detail/memcpy.hpp>         // for bit_cast
...
public:
  ...
  // Operator float
  __SYCL_CONSTEXPR_HALF operator float() const volatile {
    return half2Float(const_cast<uint16_t &>(Buf));
  }
...
public:
  ...
  constexpr volatile half &operator=(const half &rhs) volatile {
    std::ignore = (Data = rhs.Data); // std::ignore to avoid compiler warnings
    return *this;
  }
...
#undef OP
  ...
  // Operator float
  __SYCL_CONSTEXPR_HALF operator float() const volatile {
    return static_cast<float>(const_cast<StorageT &>(Data));
  }
