...
// float to int
#define __SYCL_GENERATE_CONVERT_IMPL(SPIRVOp, DestType, DestTypeSpelling,      \
  ...
  convertImpl(T Value) {                                                       \
    OpenCLT OpValue = sycl::detail::convertDataToType<T, OpenCLT>(Value);      \
    return __spirv_Convert##SPIRVOp##_R##DestTypeSpelling##_##RoundingMode(    \
        OpValue);                                                              \
  }

#define __SYCL_GENERATE_CONVERT_IMPL_FOR_ROUNDING_MODE(RoundingMode,           \
                                                       RoundingModeCondition)  \
  __SYCL_GENERATE_CONVERT_IMPL(FToS, int, int, RoundingMode,                   \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToS, char, char, RoundingMode,                 \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToS, short, short, RoundingMode,               \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToS, long, long, RoundingMode,                 \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToU, unsigned int, uint, RoundingMode,         \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToU, unsigned char, uchar, RoundingMode,       \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToU, unsigned short, ushort, RoundingMode,     \
                               RoundingModeCondition)                          \
  __SYCL_GENERATE_CONVERT_IMPL(FToU, unsigned long, ulong, RoundingMode,       \
                               RoundingModeCondition)
...
template <typename T, int N, typename V> struct VecStorage {
  static_assert(isValidVectorSize(N),
                "Incorrect number of elements for sycl::vec: only 2, 3, 4, 8 "
                "or 16 are supported");
