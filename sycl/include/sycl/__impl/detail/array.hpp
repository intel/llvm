...
public:
  ...
  operator __sycl_internal::__v1::id<dimensions>() const {
    __sycl_internal::__v1::id<dimensions> result;
  ...
  operator __sycl_internal::__v1::range<dimensions>() const {
    __sycl_internal::__v1::range<dimensions> result;
    for (int i = 0; i < dimensions; ++i) {
      result[i] = common_array[i];
    }
    return result;
...
protected:
  ...
  __SYCL_ALWAYS_INLINE void check_dimension(int dimension) const {
    ...
    if (dimension >= dimensions || dimension < 0) {
      throw __sycl_internal::__v1::invalid_parameter_error("Index out of range",
                                              PI_INVALID_VALUE);
    }
#endif
    (void)dimension;
