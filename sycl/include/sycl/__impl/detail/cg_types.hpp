...
class ArgDesc {
public:
  ArgDesc(__sycl_internal::detail::kernel_param_kind_t Type, void *Ptr, int Size,
          int Index)
      : MType(Type), MPtr(Ptr), MSize(Size), MIndex(Index) {}

  __sycl_internal::detail::kernel_param_kind_t MType;
  void *MPtr;
  int MSize;
  int MIndex;
};
...
public:
  ...
  template <int Dims_> void set(__sycl_internal::range<Dims_> NumWorkItems) {
  ...
  // offset.
  template <int Dims_>
  void set(__sycl_internal::range<Dims_> NumWorkItems, __sycl_internal::id<Dims_> Offset) {
    for (int I = 0; I < Dims_; ++I) {
      GlobalSize[I] = NumWorkItems[I];
      LocalSize[I] = 0;
  ...
  template <int Dims_> void set(__sycl_internal::nd_range<Dims_> ExecutionRange) {
    for (int I = 0; I < Dims_; ++I) {
  ...
  void set(int Dims_, __sycl_internal::nd_range<3> ExecutionRange) {
    for (int I = 0; I < Dims_; ++I) {
      GlobalSize[I] = ExecutionRange.get_global_range()[I];
      LocalSize[I] = ExecutionRange.get_local_range()[I];
      GlobalOffset[I] = ExecutionRange.get_offset()[I];
  ...
  template <int Dims_> void setNumWorkGroups(__sycl_internal::range<Dims_> N) {
    for (int I = 0; I < Dims_; ++I) {
      GlobalSize[I] = 0;
  ...
  __sycl_internal::range<3> GlobalSize;
  __sycl_internal::range<3> LocalSize;
  __sycl_internal::id<3> GlobalOffset;
  ...
  /// zero
  __sycl_internal::range<3> NumWorkGroups;
  size_t Dims;
};
...
class InteropTask {
  std::function<void(__sycl_internal::interop_handler)> MFunc;

public:
  InteropTask(function_class<void(__sycl_internal::interop_handler)> Func)
      : MFunc(Func) {}
  void call(__sycl_internal::interop_handler &h) { MFunc(h); }
};
...
class HostKernel : public HostKernelBase {
  using IDBuilder = __sycl_internal::detail::Builder;
  KernelType MKernel;
...
public:
  ...
  template <class ArgT = KernelArgType>
  typename detail::enable_if_t<std::is_same<ArgT, __sycl_internal::id<Dims>>::value>
  runOnHost(const NDRDescT &NDRDesc) {
    using KI = detail::KernelInfo<KernelName>;
    constexpr bool StoreLocation = KI::callsAnyThisFreeFunction();

    __sycl_internal::range<Dims> Range(InitializedVal<Dims, range>::template get<0>());
    __sycl_internal::id<Dims> Offset;
    __sycl_internal::range<Dims> Stride(
        InitializedVal<Dims, range>::template get<1>()); // initialized to 1
    __sycl_internal::range<Dims> UpperBound(
    ...
    detail::NDLoop<Dims>::iterate(
        ...
        [&](const __sycl_internal::id<Dims> &ID) {
          __sycl_internal::item<Dims, /*Offset=*/true> Item =
          ...
          runKernelWithArg<const __sycl_internal::id<Dims> &>(MKernel, ID);
        });
  }
  ...
  runOnHost(const NDRDescT &NDRDesc) {
    ...
    __sycl_internal::id<Dims> ID;
    __sycl_internal::range<Dims> Range(InitializedVal<Dims, range>::template get<0>());
    for (int I = 0; I < Dims; ++I)
      Range[I] = NDRDesc.GlobalSize[I];

    detail::NDLoop<Dims>::iterate(Range, [&](const __sycl_internal::id<Dims> ID) {
      __sycl_internal::item<Dims, /*Offset=*/false> Item =
          IDBuilder::createItem<Dims, false>(Range, ID);
      __sycl_internal::item<Dims, /*Offset=*/true> ItemWithOffset = Item;

      if (StoreLocation) {
        store_id(&ID);
        store_item(&ItemWithOffset);
      }
      runKernelWithArg<__sycl_internal::item<Dims, /*Offset=*/false>>(MKernel, Item);
    });
  }
  ...
  runOnHost(const NDRDescT &NDRDesc) {
    ...
    detail::NDLoop<Dims>::iterate(
        ...
        [&](const __sycl_internal::id<Dims> &ID) {
          ...
          runKernelWithArg<__sycl_internal::item<Dims, /*Offset=*/true>>(MKernel, Item);
        });
  }
  ...
  runOnHost(const NDRDescT &NDRDesc) {
    ...
    __sycl_internal::range<Dims> GroupSize(InitializedVal<Dims, range>::template get<0>());
    for (int I = 0; I < Dims; ++I) {
      if (NDRDesc.LocalSize[I] == 0 ||
          NDRDesc.GlobalSize[I] % NDRDesc.LocalSize[I] != 0)
        throw __sycl_internal::nd_range_error("Invalid local size for global size",
                                   PI_INVALID_WORK_GROUP_SIZE);
      GroupSize[I] = NDRDesc.GlobalSize[I] / NDRDesc.LocalSize[I];
    }

    __sycl_internal::range<Dims> LocalSize(InitializedVal<Dims, range>::template get<0>());
    __sycl_internal::range<Dims> GlobalSize(
        InitializedVal<Dims, range>::template get<0>());
    __sycl_internal::id<Dims> GlobalOffset;
    for (int I = 0; I < Dims; ++I) {
    ...
    detail::NDLoop<Dims>::iterate(GroupSize, [&](const id<Dims> &GroupID) {
      __sycl_internal::group<Dims> Group = IDBuilder::createGroup<Dims>(
      ...
      detail::NDLoop<Dims>::iterate(LocalSize, [&](const id<Dims> &LocalID) {
        ...
        const __sycl_internal::item<Dims, /*Offset=*/true> GlobalItem =
            IDBuilder::createItem<Dims, true>(GlobalSize, GlobalID,
                                              GlobalOffset);
        const __sycl_internal::item<Dims, /*Offset=*/false> LocalItem =
            IDBuilder::createItem<Dims, false>(LocalSize, LocalID);
        const __sycl_internal::nd_item<Dims> NDItem =
        ...
        runKernelWithArg<const __sycl_internal::nd_item<Dims>>(MKernel, NDItem);
      });
    });
  }

  template <typename ArgT = KernelArgType>
  enable_if_t<std::is_same<ArgT, __sycl_internal::group<Dims>>::value>
  runOnHost(const NDRDescT &NDRDesc) {
    __sycl_internal::range<Dims> NGroups(InitializedVal<Dims, range>::template get<0>());
    ...
    detail::NDLoop<Dims>::iterate(NGroups, [&](const id<Dims> &GroupID) {
      __sycl_internal::group<Dims> Group =
          IDBuilder::createGroup<Dims>(GlobalSize, LocalSize, NGroups, GroupID);
      runKernelWithArg<__sycl_internal::group<Dims>>(MKernel, Group);
    });
  }
