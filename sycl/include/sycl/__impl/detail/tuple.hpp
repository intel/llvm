...
template <typename T, typename... Ts> struct tuple<T, Ts...> {
  ...
  tuple(const std::tuple<T, Ts...> &RHS)
      : holder(std::get<0>(RHS)), next(__sycl_internal::detail::get_tuple_tail(RHS)) {}
  ...
  // Convert std::tuple to __sycl_internal::detail::tuple
  template <typename UT, typename... UTs>
  tuple &operator=(const std::tuple<UT, UTs...> &RHS) {
    holder.value = std::get<0>(RHS);
    next = __sycl_internal::detail::get_tuple_tail(RHS);
    return *this;
  }
...
get(__sycl_internal::detail::tuple<Types...> &Arg) noexcept {
  return __sycl_internal::detail::get<I>()(Arg);
}

template <size_t I, typename... Types>
constexpr typename tuple_element<I, tuple<Types...>>::type const &
get(const __sycl_internal::detail::tuple<Types...> &Arg) noexcept {
  return __sycl_internal::detail::get<I>()(Arg);
}
