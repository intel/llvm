...
// __sycl_internal::buffer and __sycl_internal::image. For example, each accessor used in a kernel,
...
public:
  LocalAccessorBaseDevice(__sycl_internal::range<Dims> Size)
      : AccessRange(Size),
...
private:
  friend class __sycl_internal::ext::intel::experimental::esimd::detail::
      AccessorPrivateProxy;
};

class __SYCL_EXPORT LocalAccessorImplHost {
public:
  LocalAccessorImplHost(__sycl_internal::range<3> Size, int Dims, int ElemSize)
      : MSize(Size), MDims(Dims), MElemSize(ElemSize),
        MMem(Size[0] * Size[1] * Size[2] * ElemSize) {}

  __sycl_internal::range<3> MSize;
  int MDims;
  int MElemSize;
...
public:
  LocalAccessorBaseHost(__sycl_internal::range<3> Size, int Dims, int ElemSize) {
    impl = shared_ptr_class<LocalAccessorImplHost>(
        new LocalAccessorImplHost(Size, Dims, ElemSize));
  }
  __sycl_internal::range<3> &getSize() { return impl->MSize; }
  const __sycl_internal::range<3> &getSize() const { return impl->MSize; }
