...
#define __SYCL_INVOKE_SPIRV_CALL_ARG1(call)                                    \
  template <typename R, typename T1> inline R __invoke_##call(T1 ParT1) {      \
    ...
    return __sycl_internal::__v1::detail::convertDataToType<Ret, R>(RetVar);                \
  }
...
static void __invoke__ImageWrite(ImageT Img, CoordT Coords, ValT Val) {
  ...
  using TmpValT = __sycl_internal::__v1::detail::ConvertToOpenCLType_t<ValT>;
  using TmpCoordT = __sycl_internal::__v1::detail::ConvertToOpenCLType_t<CoordT>;

  TmpCoordT TmpCoord =
      __sycl_internal::__v1::detail::convertDataToType<CoordT, TmpCoordT>(Coords);
  TmpValT TmpVal = __sycl_internal::__v1::detail::convertDataToType<ValT, TmpValT>(Val);
...
static RetType __invoke__ImageRead(ImageT Img, CoordT Coords) {
  ...
  // Convert from sycl types to builtin types to get correct function mangling.
  using TempRetT = __sycl_internal::__v1::detail::ConvertToOpenCLType_t<RetType>;
  using TempArgT = __sycl_internal::__v1::detail::ConvertToOpenCLType_t<CoordT>;

  TempArgT Arg = __sycl_internal::__v1::detail::convertDataToType<CoordT, TempArgT>(Coords);
  TempRetT Ret = __spirv_ImageRead<TempRetT, ImageT, TempArgT>(Img, Arg);
  return __sycl_internal::__v1::detail::convertDataToType<TempRetT, RetType>(Ret);
}
...
static RetType __invoke__ImageReadSampler(ImageT Img, CoordT Coords,
  ...
  using TempArgT = __sycl_internal::__v1::detail::ConvertToOpenCLType_t<CoordT>;
  using SampledT =
      typename __sycl_internal::__v1::detail::sampled_opencl_image_type<ImageT>::type;

  TempArgT TmpCoords =
      __sycl_internal::__v1::detail::convertDataToType<CoordT, TempArgT>(Coords);
