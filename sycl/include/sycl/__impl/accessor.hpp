...
#include <sycl/__impl/detail/image_accessor_util.hpp>
#include <sycl/__impl/detail/image_ocl_types.hpp>
#include <sycl/__impl/exception.hpp>
#include <sycl/__impl/id.hpp>
#include <sycl/__impl/image.hpp>
#include <sycl/__impl/pointers.hpp>
#include <sycl/__impl/properties/accessor_properties.hpp>
#include <sycl/__impl/property_list.hpp>
#include <sycl/__impl/property_list_conversion.hpp>
...
/// buffer, multidimensional access using __sycl_internal::id, conversions to various
...
private:
  ...
  __sycl_internal::vec<int, Dimensions> getRangeInternal() const {
    return __invoke_ImageQuerySize<__sycl_internal::vec<int, Dimensions>, OCLImageTy>(
        MImageObj);
  }
  ...
  __sycl_internal::vec<int, Dimensions> getRangeInternal() const {
    ...
    throw runtime_error("image::getRangeInternal() is not implemented for host",
                        PI_INVALID_OPERATION);
    return __sycl_internal::vec<int, Dimensions>{1};
  }
...
class __image_array_slice__ {
  ...
  __sycl_internal::vec<CoordElemType, AdjustedDims>
  getAdjustedCoords(const CoordT &Coords) const {
    CoordElemType LastCoord = 0;

    if (std::is_same<float, CoordElemType>::value) {
      __sycl_internal::vec<int, Dimensions + 1> Size = MBaseAcc.getRangeInternal();
      LastCoord =
    ...
    __sycl_internal::vec<CoordElemType, Dimensions> LeftoverCoords{LastCoord};
    __sycl_internal::vec<CoordElemType, AdjustedDims> AdjustedCoords{Coords,
                                                          LeftoverCoords};
...
protected:
  ...
  __sycl_internal::range<AdjustedDim> &getSize() { return impl.MemRange; }
  const __sycl_internal::range<AdjustedDim> &getSize() const { return impl.MemRange; }
...
public:
  template <typename AllocatorT>
  accessor(__sycl_internal::image<Dimensions, AllocatorT> &Image,
...
public:
  template <typename AllocatorT>
  accessor(__sycl_internal::image<Dimensions + 1, AllocatorT> &Image,
           handler &CommandGroupHandler)
...
template <typename DataT, int Dimensions, __sycl_internal::access::mode AccessMode,
          ...
          __sycl_internal::access::placeholder IsPlaceholder>
struct hash<__sycl_internal::accessor<DataT, Dimensions, AccessMode, AccessTarget,
                               IsPlaceholder>> {
  using AccType = __sycl_internal::accessor<DataT, Dimensions, AccessMode,
  ...
  size_t operator()(const AccType &A) const {
    ...
    auto AccImplPtr = __sycl_internal::detail::getSyclObjImpl(A);
...
namespace sycl {
  using __sycl_internal::__v1::accessor;
  using __sycl_internal::__v1::host_accessor;
}
