...
template <typename T_Src, typename T_Dst, int Dims,
          __sycl_internal::access::mode AccessMode,
          __sycl_internal::access::target AccessTarget,
          __sycl_internal::access::placeholder IsPlaceholder>
class __copyAcc2Ptr;
...
template <typename T_Src, int Dims_Src, __sycl_internal::access::mode AccessMode_Src,
          __sycl_internal::access::target AccessTarget_Src, typename T_Dst,
          int Dims_Dst, __sycl_internal::access::mode AccessMode_Dst,
          __sycl_internal::access::target AccessTarget_Dst,
          __sycl_internal::access::placeholder IsPlaceholder_Src,
          __sycl_internal::access::placeholder IsPlaceholder_Dst>
class __copyAcc2Acc;
...
///
/// \code{.cpp}
/// __sycl_internal::queue::submit([](handler &CGH){
...
private:
  ...
  void throwIfActionIsCreated() {
    if (detail::CG::NONE != getType())
      throw __sycl_internal::runtime_error("Attempt to set multiple actions for the "
  ...
  template <typename LambdaNameT> bool lambdaAndKernelHaveEqualName() {
    ...
    // for parallel_for with __sycl_internal::kernel and lambda/functor together
  ...
  /// returns __sycl_internal::event object representing the command group.
  ...
  void StoreLambda(KernelType KernelFunc) {
    ...
    using KI = __sycl_internal::detail::KernelInfo<KernelName>;
  ...
  void parallel_for_lambda_impl(range<Dims> NumWorkItems,
    ...
    using LambdaArgType = __sycl_internal::detail::lambda_arg_type<KernelType, item<Dims>>;

    // If 1D kernel argument is an integral type, convert it to __sycl_internal::item<1>
...
#define _KERNELFUNCPARAM(a) const KernelType &a
  ...
  void parallel_for(nd_range<Dims> ExecutionRange,
    ...
    using NameT =
        typename detail::get_kernel_name_t<KernelName, KernelType>::name;
    using LambdaArgType =
        __sycl_internal::detail::lambda_arg_type<KernelType, nd_item<Dims>>;
    (void)ExecutionRange;
    kernel_parallel_for_wrapper<NameT, LambdaArgType>(KernelFunc);
#ifndef __SYCL_DEVICE_ONLY__
  ...
  /// object. This version uses fast __sycl_internal::atomic operations to update reduction
  ...
  parallel_for(nd_range<Dims> Range, Reduction Redu,
    ...
    if (Range.get_local_range().size() > MaxWGSize)
      throw __sycl_internal::runtime_error("The implementation handling parallel_for with"
  ...
  void parallel_for_work_group(range<Dims> NumWorkGroups,
    ...
    using NameT =
        typename detail::get_kernel_name_t<KernelName, KernelType>::name;
    using LambdaArgType =
        __sycl_internal::detail::lambda_arg_type<KernelType, group<Dims>>;
    (void)NumWorkGroups;
...
private:
  ...
  /// Storage for a __sycl_internal::kernel object.
...
namespace sycl {
  using __sycl_internal::__v1::handler;
}
