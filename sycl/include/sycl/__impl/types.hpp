...
#define __SYCL_GENERATE_CONVERT_IMPL(DestType)                                 \
  ...
  detail::enable_if_t<is_sint_to_sint<T, R>::value &&                          \
                      ...
                      R>                                                       \
  convertImpl(T Value) {                                                       \
    OpenCLT OpValue = __sycl_internal::detail::convertDataToType<T, OpenCLT>(Value);  \
...
public:
  ...
  using EnableIfNotHostHalf = typename detail::enable_if_t<
      !std::is_same<DataT, __sycl_internal::detail::half_impl::half>::value ||
          !std::is_same<__sycl_internal::detail::half_impl::StorageT,
                        __sycl_internal::detail::host_half_impl::half_v2>::value,
      T>;
  template <typename T = void>
  ...
  // ext_vector_type is used as an underlying type for __sycl_internal::vec on device.
...
#define __SYCL_DECLARE_TYPE_VIA_CL_T(type)                                     \
  using __##type##_t = __sycl_internal::cl_##type;                                    \
  using __##type##2_vec_t =                                                    \
      __sycl_internal::cl_##type __attribute__((ext_vector_type(2)));                 \
  using __##type##3_vec_t =                                                    \
      __sycl_internal::cl_##type __attribute__((ext_vector_type(3)));                 \
  using __##type##4_vec_t =                                                    \
      __sycl_internal::cl_##type __attribute__((ext_vector_type(4)));                 \
  using __##type##8_vec_t =                                                    \
      __sycl_internal::cl_##type __attribute__((ext_vector_type(8)));                 \
  using __##type##16_vec_t =                                                   \
      __sycl_internal::cl_##type __attribute__((ext_vector_type(16)));

#define __SYCL_DECLARE_TYPE_T(type)                                            \
  using __##type##_t = __sycl_internal::type;                                         \
  using __##type##2_vec_t =                                                    \
      __sycl_internal::type __attribute__((ext_vector_type(2)));                      \
  using __##type##3_vec_t =                                                    \
      __sycl_internal::type __attribute__((ext_vector_type(3)));                      \
  using __##type##4_vec_t =                                                    \
      __sycl_internal::type __attribute__((ext_vector_type(4)));                      \
  using __##type##8_vec_t =                                                    \
      __sycl_internal::type __attribute__((ext_vector_type(8)));                      \
  using __##type##16_vec_t =                                                   \
      __sycl_internal::type __attribute__((ext_vector_type(16)));
...
using __half_t = __sycl_internal::detail::half_impl::StorageT;
using __half2_vec_t = __sycl_internal::detail::half_impl::Vec2StorageT;
using __half3_vec_t = __sycl_internal::detail::half_impl::Vec3StorageT;
using __half4_vec_t = __sycl_internal::detail::half_impl::Vec4StorageT;
using __half8_vec_t = __sycl_internal::detail::half_impl::Vec8StorageT;
using __half16_vec_t = __sycl_internal::detail::half_impl::Vec16StorageT;
