...
using namespace ::__sycl_internal::detail;

using memory_order = __sycl_internal::ONEAPI::memory_order;
using memory_scope = __sycl_internal::ONEAPI::memory_scope;

template <typename T>
using IsValidAtomicType =
    bool_constant<std::is_arithmetic<T>::value || std::is_pointer<T>::value>;

template <__sycl_internal::access::address_space AS>
...
template <> struct bit_equal<float> {
  bool operator()(const float &lhs, const float &rhs) {
    auto LhsInt = __sycl_internal::bit_cast<uint32_t>(lhs);
    auto RhsInt = __sycl_internal::bit_cast<uint32_t>(rhs);
...
template <> struct bit_equal<double> {
  bool operator()(const double &lhs, const double &rhs) {
    auto LhsInt = __sycl_internal::bit_cast<uint64_t>(lhs);
    auto RhsInt = __sycl_internal::bit_cast<uint64_t>(rhs);
    return LhsInt == RhsInt;
  }
};
