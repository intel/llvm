...
private:
  ...
  friend class __sycl_internal::program;

public:
  template <typename V = T>
  typename __sycl_internal::detail::enable_if_t<std::is_arithmetic<V>::value, V>
  ...
  template <typename V = T>
  typename __sycl_internal::detail::enable_if_t<std::is_class<V>::value &&
                                         std::is_pod<V>::value,
                                     V>
  get() const { // explicit access.
#ifdef __SYCL_DEVICE_ONLY__
