...
#include <sycl/__impl/detail/tuple.hpp>
...
using __sycl_internal::detail::bool_constant;
using __sycl_internal::detail::enable_if_t;
using __sycl_internal::detail::queue_impl;
using __sycl_internal::detail::remove_AS;
...
using IsReduOptForFastAtomicFetch =
    ...
    bool_constant<false>;
#else
    bool_constant<__sycl_internal::detail::is_sgeninteger<T>::value &&
                  __sycl_internal::detail::IsValidAtomicType<T>::value &&
                  (__sycl_internal::detail::IsPlus<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsMinimum<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsMaximum<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsBitOR<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsBitXOR<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsBitAND<T, BinaryOperation>::value)>;
#endif
...
using IsReduOptForFastReduce =
    ...
    bool_constant<((__sycl_internal::detail::is_sgeninteger<T>::value &&
                   ...
                   __sycl_internal::detail::is_sgenfloat<T>::value) &&
                  (__sycl_internal::detail::IsPlus<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsMinimum<T, BinaryOperation>::value ||
                   __sycl_internal::detail::IsMaximum<T, BinaryOperation>::value)>;
#endif
...
// Thus __sycl_internal::detail::tuple is used instead.
// Switching from __sycl_internal::device::tuple to std::tuple can be done by re-defining
// the ReduTupleT type and makeReduTupleT() function below.
template <typename... Ts> using ReduTupleT = __sycl_internal::detail::tuple<Ts...>;
template <typename... Ts> ReduTupleT<Ts...> makeReduTupleT(Ts... Elements) {
  return __sycl_internal::detail::make_tuple(Elements...);
}
...
public:
  ...
  enable_if_t<__sycl_internal::detail::IsPlus<_T, BinaryOperation>::value &&
              __sycl_internal::detail::is_geninteger<_T>::value>
  operator++() {
  ...
  template <typename _T = T>
  enable_if_t<__sycl_internal::detail::IsMultiplies<_T, BinaryOperation>::value>
  operator*=(const _T &Partial) {
    combine(Partial);
  }

  template <typename _T = T>
  enable_if_t<__sycl_internal::detail::IsBitOR<_T, BinaryOperation>::value>
  operator|=(const _T &Partial) {
    combine(Partial);
  }

  template <typename _T = T>
  enable_if_t<__sycl_internal::detail::IsBitXOR<_T, BinaryOperation>::value>
  operator^=(const _T &Partial) {
    combine(Partial);
  }

  template <typename _T = T>
  enable_if_t<__sycl_internal::detail::IsBitAND<_T, BinaryOperation>::value>
  operator&=(const _T &Partial) {
    combine(Partial);
  }
...
/// __sycl_internal::atomic::fetch_add().
//
...
public:
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              ...
              __sycl_internal::detail::IsPlus<T, _BinaryOperation>::value>
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              IsReduOptForFastAtomicFetch<T, _BinaryOperation>::value &&
              __sycl_internal::detail::IsBitOR<T, _BinaryOperation>::value>
  atomic_combine(_T *ReduVarPtr) const {
    atomic<T, access::address_space::global_space>(global_ptr<T>(ReduVarPtr))
        .fetch_or(MValue);
  }
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              IsReduOptForFastAtomicFetch<T, _BinaryOperation>::value &&
              __sycl_internal::detail::IsBitXOR<T, _BinaryOperation>::value>
  atomic_combine(_T *ReduVarPtr) const {
    atomic<T, access::address_space::global_space>(global_ptr<T>(ReduVarPtr))
        .fetch_xor(MValue);
  }
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              IsReduOptForFastAtomicFetch<T, _BinaryOperation>::value &&
              __sycl_internal::detail::IsBitAND<T, _BinaryOperation>::value>
  atomic_combine(_T *ReduVarPtr) const {
    atomic<T, access::address_space::global_space>(global_ptr<T>(ReduVarPtr))
        .fetch_and(MValue);
  }
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              IsReduOptForFastAtomicFetch<T, _BinaryOperation>::value &&
              __sycl_internal::detail::IsMinimum<T, _BinaryOperation>::value>
  atomic_combine(_T *ReduVarPtr) const {
    atomic<T, access::address_space::global_space>(global_ptr<T>(ReduVarPtr))
        .fetch_min(MValue);
  }
  ...
  enable_if_t<std::is_same<typename remove_AS<_T>::type, T>::value &&
              IsReduOptForFastAtomicFetch<T, _BinaryOperation>::value &&
              __sycl_internal::detail::IsMaximum<T, _BinaryOperation>::value>
  atomic_combine(_T *ReduVarPtr) const {
    atomic<T, access::address_space::global_space>(global_ptr<T>(ReduVarPtr))
        .fetch_max(MValue);
  }
...
public:
  ...
  reduction_impl(buffer<_T, 1, AllocatorT> Buffer, handler &CGH,
    ...
    if (Buffer.size() != 1)
      throw __sycl_internal::runtime_error("Reduction variable must be a scalar.",
                                PI_INVALID_VALUE);
  }
...
struct get_reduction_main_kernel_name_t {
  using name = __sycl_reduction_main_kernel<
      typename __sycl_internal::detail::get_kernel_name_t<Name, Type>::name, B1, B2, T3>;
};
...
struct has_known_identity
    : __sycl_internal::has_known_identity<BinaryOperation, AccumulatorT> {};
...
// ---- known_identity
template <typename BinaryOperation, typename AccumulatorT>
struct known_identity : __sycl_internal::known_identity<BinaryOperation, AccumulatorT> {};
