...
#include <sycl/__impl/detail/defines_elementary.hpp> // for __SYCL_INLINE_NAMESPACE
#include <sycl/__impl/detail/export.hpp>             // for __SYCL_EXPORT
...
class online_compile_error : public __sycl_internal::exception {
public:
  online_compile_error() = default;
  online_compile_error(const string_class &Msg) : __sycl_internal::exception(Msg) {}
};
...
public:
  ...
  online_compiler(compiled_code_format fmt = compiled_code_format::spir_v)
      : OutputFormat(fmt), OutputFormatVersion({0, 0}),
        DeviceType(__sycl_internal::info::device_type::all), DeviceArch(device_arch::any),
  ...
  online_compiler(__sycl_internal::info::device_type dev_type, device_arch arch,
  ...
  // features.
  online_compiler(const __sycl_internal::device &)
  ...
  online_compiler<Lang> &setTargetDeviceType(__sycl_internal::info::device_type type) {
    DeviceType = type;
    return *this;
  }
...
private:
  ...
  /// Target device type
  __sycl_internal::info::device_type DeviceType;
