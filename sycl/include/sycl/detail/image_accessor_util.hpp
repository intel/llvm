...
// Possible DataT are vec<opencl::cl_int, 4>, cl_uint4, vec<opencl::cl_float,
// 4>, cl_half4;
...
DataT imageReadSamplerHostImpl(const CoordT &Coords, const sampler &Smpl,
  ...
  // opencl::cl_int2, vec<opencl::cl_int, 4>, opencl::cl_float,
  // opencl::cl_float2 and vec<opencl::cl_float, 4> into Coordinates of kind
  // vec<opencl::cl_float, 4> with no loss of precision. For pixel_coordinates
  // already in vec<opencl::cl_float, 4> format, the function returns the same
  // values. This conversion is done to enable implementation of one common
  // function getPixelCoordXXXMode, for any datatype of CoordT passed.
