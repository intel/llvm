...
namespace std {
template <> struct hash<__sycl_ns::event> {
  size_t operator()(const __sycl_ns::event &e) const {
    return hash<std::shared_ptr<__sycl_ns::detail::event_impl>>()(
        __sycl_ns::detail::getSyclObjImpl(e));
  }
};
} // namespace std
