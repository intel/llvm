...
/// \return a valid instance of OpenCL cl_event.
    ...
    /// event
    /// is waiting on executions from, then call that context's asynchronous
    /// error
    ...
    /// called
    /// due to incompletion of command groups associated with the event, then
    /// the
    ...
    backend_return_t<Backend, event> get_native() const {
      return reinterpret_cast<backend_return_t<Backend, event>>(
          lazyInit(Backend));
    }
