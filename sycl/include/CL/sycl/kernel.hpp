...
namespace std {
template <> struct hash<__sycl_ns::kernel> {
  size_t operator()(const __sycl_ns::kernel &Kernel) const {
    return hash<std::shared_ptr<__sycl_ns::detail::kernel_impl>>()(
        __sycl_ns::detail::getSyclObjImpl(Kernel));
  }
};
} // namespace std
