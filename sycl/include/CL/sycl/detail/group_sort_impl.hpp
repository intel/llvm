...
// following to functions could be useless if std::[lower|upper]_bound worked well
...
template <typename Acc, typename Value, typename Compare>
std::size_t
my_upper_bound(Acc acc, const std::size_t first, const std::size_t last, const Value& value, Compare comp)
{
    return my_lower_bound(acc, first, last, value, [comp](auto x, auto y){return !comp(y, x);});
}
...
void
merge(const std::size_t offset, InAcc& in_acc1, OutAcc& out_acc1,
            ...
            Compare comp, const std::size_t chunk)
{
    ...
    const std::size_t local_end_1 = sycl::min(static_cast<std::size_t>(local_start_1 + chunk), end_1);
    const std::size_t local_start_2 = sycl::min(static_cast<std::size_t>(offset + start_2), end_2);
    const std::size_t local_end_2 = sycl::min(static_cast<std::size_t>(local_start_2 + chunk), end_2);

    const std::size_t local_size_1 = local_end_1 - local_start_1;
    const std::size_t local_size_2 = local_end_2 - local_start_2;

    // TODO: process cases where all elements of 1st sequence > 2nd, 2nd > 1st to improve performance
    ...
    if (local_start_1 < local_end_1)
    {
        ...
        std::size_t l_search_bound_2 = my_lower_bound(in_acc1, start_2, end_2, local_l_item_1, comp);
        ...
        if (local_size_1 > 1)
        {
            ...
            r_search_bound_2 = my_lower_bound(in_acc1, l_search_bound_2, end_2, local_r_item_1, comp);
        ...
        for (std::size_t idx = local_start_1 + 1; idx < local_end_1 - 1; ++idx)
        {
            ...
            // we shouldn't seek in whole 2nd sequence. Just for the part where the 1st sequence should be
            l_search_bound_2 = my_lower_bound(in_acc1, l_search_bound_2, r_search_bound_2, intermediate_item_1, comp);
    ...
    if (local_start_2 < local_end_2)
    {
        ...
        std::size_t l_search_bound_1 = my_upper_bound(in_acc1, start_1, end_1, local_l_item_2, comp);
        ...
        if (local_size_2 > 1)
        {
            ...
            r_search_bound_1 = my_upper_bound(in_acc1, l_search_bound_1, end_1, local_r_item_2, comp);
        ...
        for (auto idx = local_start_2 + 1; idx < local_end_2 - 1; ++idx)
        {
            ...
            // we shouldn't seek in whole 1st sequence. Just for the part where the 2nd sequence should be
            l_search_bound_1 = my_upper_bound(in_acc1, l_search_bound_1, r_search_bound_1, intermediate_item_2, comp);
...
void
bubble_sort(Iter first, const std::size_t begin, const std::size_t end, Compare comp)
{
    if (begin < end)
    {
        ...
        std::uint8_t* scratch, Id id)
{
    ...
    {
        const std::size_t start_1 = sycl::min(2 * sorted_size * chunk * (idx / sorted_size), n);
