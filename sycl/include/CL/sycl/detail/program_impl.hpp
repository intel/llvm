...
public:
  ...
  program_impl(vector_class<std::shared_ptr<program_impl>> ProgramList,
    ...
    if (!is_host()) {
      ...
      Err = PI_CALL_RESULT(
          piProgramLink, detail::getSyclObjImpl(Context)->getHandleRef(),
  ...
  void link(string_class LinkOptions = "") {
    ...
    if (!is_host()) {
      ...
      vector_class<RT::PiDevice> Devices(get_pi_devices());
      RT::PiResult Err;
      Err = PI_CALL_RESULT(piProgramLink,
...
private:
  ...
  void compile(const string_class &Options) {
    ...
    vector_class<RT::PiDevice> Devices(get_pi_devices());
    RT::PiResult Err = PI_CALL_RESULT(piProgramCompile, Program, Devices.size(),
                                      Devices.data(), Options.c_str(), 0,
  ...
  void build(const string_class &Options) {
    ...
    vector_class<RT::PiDevice> Devices(get_pi_devices());
    RT::PiResult Err =
        PI_CALL_RESULT(piProgramBuild, Program, Devices.size(), Devices.data(),
  ...
  RT::PiKernel get_pi_kernel(const string_class &KernelName) const {
    RT::PiKernel Kernel;
    RT::PiResult Err;
    Err = PI_CALL_RESULT(piKernelCreate, Program, KernelName.c_str(), &Kernel);
