...
private:
  template <bool B>
  using EnableIfPitchT = typename std::enable_if_t<B, range<Dimensions - 1>>;
...
public:
  ...
  typename std::enable_if_t<B, range<1>> get_pitch() const {
  ...
  typename std::enable_if_t<B, range<2>> get_pitch() const {
