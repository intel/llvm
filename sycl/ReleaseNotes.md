...
- Added support for querying device architecture [1ad69e59eb13]
- Added esimd as an optional device feature [a5f471ea1535]
- Added `ext_oneapi_non_uniform_groups` aspect [de92299c2c09]

### Documentation
- Updated `sycl_ext_oneapi_complex` from proposed to experimental [ea6aea37c6dd]
- Added SYCLcompat documentation [180a92ad707b]
- Added draft forward progress extension [67461148b169]
- Added `sycl_ext_oneapi_kernel_compiler` extension spec [5e64f96d20c2]
- Added `sycl_ext_oneapi_enqueue_functions` extension spec [ebc3ddb9a60b]
- Added `sycl_ext_oneapi_graph_fusion` extension spec [8c4cad45faba]
- Added `sycl_ext_intel_fpga_datapath` extension spec [a162179381fd]
- Added `sycl_ext_intel_cache_controls` extension spec [deb020bf740b]
- Added design doc for SYCL-Graph Extension [1713ed19f71c]

## Improvements
### SYCL Compiler
- Diagnose SYCL options in non-sycl mode. [bf128c873e74]
- Modified SYCL offloading flow to use `clang-linker-wrapper` [d7a1a4146eb2] [7b907cbcaea9]
...
- Adjusted debug format for SYCL device on Windows [73bba79a3ffb]
- Disabled parallel for range rounding at `-O0` [2c117d7a6104]
- Enableed `SPV_INTEL_optnone` extension for non-fpga targets [993fe5b31b43]
- Relaxed `LowerESIMDSlmReservation` assert [7897517c3a5e]
...
- Implemented fallback path for profiling info support [7cfa951303eb]
- Fixed require alignment in buffers [093dae1f7214]
- Improved error message when running using `sycl_ext_oneapi_graph` extension on unsupported device [8d8d3f4f52b0]
...
- Improve error message the CGF object parameter accepts the wrong type of parameter [5801970585b2]
...
- Added missing `rintf` [3c327c736ced]
- Cache the max local mem size in CUDA adapter [1c892f952cae]
- Added support for local accessors to lsc API [9dce2d227b31]
- Replaced USM allocator with UMF abstractions in Level Zero backend [7b9b8ce4e4c5]
- Made SYCL-Graph functions thread-safe [c8c64a673eeb]
- Improveed diagnostic for nd_item passed to parallel_for with range [e600d7922489]
- Implemented missing exceptions defined by SYCL-Graphs specification [77b794bcaad6]
...
- Improveed handling of spec constants with padding in the middle of structs [292309c826a3]
- Added support for CUDA backend with graph extension [367b662a49ba]
- Added support for specialization constants with graph extension [fca7f5e2e3fb]
...
- Added get-coord API and general query example to `sycl_ext_intel_matric` [579f1dec0b5b] [38ac2124c1e8]
...
- Added queue requirement when replay a graph [2c53815c6c1e]
...
- Fixed a bug when `-enable-global-offset=false` for AMDGU and NVPTX targets [00cf4c29740b]

### SYCL Library
- Fixed stack corruption in `sycl::modf` for `sycl::half` type [7c6541dc18e3]
- Fixed nextafter with half on host [1f10f3588e8c]
- Fixed identity values for (sub)group collectives min/max [248389371932]
- Improved handling of large `reqd_work_group_size` values [25c3666dffa9]
- Fixed hip `prefetch`/`mem_advise` (noop) for rocm < 5.0 [7e4cc1113987]
- Fixed `range` to allow default constructor to 0s [0289d2ac94f9]
- Fixed propagation of aspects from global variables [fb368a130448]
- Missing return statement in the `multi_ptr<void..>` conversion operator [836469ba88e2]
- Fixed `convert` to `sycl::vec<bool, N>` [9dfaf274e2cb]
- Ensured `queue_impl` is not unnecessarily referenced in kernel fusion [a33b0a950ae6]
- Fixed int overflow for OffsetInBytes in accessors [2fb0b140ad3b]
...
- Fixed issues with `sycl_ext_oneapi_graph` subgraphs [92ddf8d562d7]
- Fixed ambiguous `log2` call in host code
- Added device copyable trait for `annotated_arg` [9f65e61a2dbf]
