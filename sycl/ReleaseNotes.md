...
### SYCL Library
  ...
  - Added a mode for the L0 plugin where only last command in each batch yields a host-visible event. Enabled this mode by default. [c6b7b8e]
  ...
  - Added support for an experimental L0 API for host pointer import into USM. The feature can be enabled using `SYCL_USM_HOSTPTR_IMPORT` env variable. [844d7b6]
  ...
  - Added `SYCL_RT_WARNING_LEVEL` env variable which allows to control amount of warnings and performance hints the runtime library may print. [2741010]
  ...
  - Added support for round-robin submissions to multiple compute CCS for the L0 backend. Disabled by default, can be controlled using `SYCL_PI_LEVEL_ZERO_USE_COMPUTE_ENGINE`. [a836c87]
  - Added support for buffer migration for contexts with multiple devices in the L0 plugin. [7baf152]
  - Added mode where the L0 plugin uses immediate commandlists instead of standard commandlists. This mode is disabled by default, can be enabled using `SYCL_PI_LEVEL_ZERO_USE_IMMEDIATE_COMMANDLISTS` env variable. [b9cb1d1]
...
### Documentation
  ...
  - Added a design document for [device global feature](doc/DeviceGlobal.md) [8c22ef1]
  - Added a design document for [`SYCL_EXT_ONEAPI_PROPERTY_LIST` extension](doc/CompileTimeProperties.md) [912572f]
...
### Tools
  ...
  - Add support for tracing Level Zero API calls using XPTI and updated sycl-trace tool to be able to display both PI and L0 calls simultaneously. [fc9cf52]

## Improvements
### SYCL Compiler
  - Added a diagnostic on attempt to use zero lenght arrays in the device code [52e8f58]
...
### SYCL Library
  ...
  - Relax the mutex lock duration in queue finish for the L0 backend to allow working with the queue from other threads. [8573935]
  ...
  - Improved get_kernel_bundle performance. [4817b3f]
  - Changed USM pooling parameters for the L0 backend to boost performance. [57f8a44][b000db8]
  - Exposed value_type and min_capacity from SYCL pipes extension class. [e1619fa]
  - Improved thread-safety of the L0 plugin by guarding access to the PI objects. [3321141] [a37c10b] [bd80f34] [8f97fe2]
  ...
  - Improved performance by caching the result of `zeKernelGetName` in the L0 plugin. [40cece3]
  ...
  - Enabled caching of native OpenCL and L0 executable binaries. [f0283fc]
...
### Documentation
  ...
  - Structured sycl/doc directory:
    ...
    - Moved the proposed extensions to the "sycl/doc/extensions/proposed". [eb350ee]
  ...
  - Clarified the C++ attribute `[[sycl_detail::uses_aspects()]]` usage in the doc. [27cc930]
...
### SYCL Library
  ...
  - Workarounded two bugs in the L0 driver related to static linking extension. [2930a94]
  - Fixed return type of `get_nativesycl::backend::opencl(event)` from `cl_event` to `vector<cl_event>`. [a2189c6]
  - Modified L0 plugin support for copy engines to address scenario when main copy engine is not available. [478a576]
  ...
  - Fixed incorrect handling of queue indexing for L0 backend. [1f4c9df]
  ...
  - Fixed thread-safety issue caused by parallel access to the command list cache in the L0 plugin. [1f531c0]
  - Fixed device code outlining for static local variables to avoid invalid device code generation. [66e207e]
  - Fixed dynamic batching in the L0 plugin. [d6f115c]
  ...
  - Fixed kernel timestamp calculation in the L0 plugin. [c228f12] [7efb3e6] [76a3898]
  - Fixed usage of copy-engines in the L0 interoperability queue. [847f8b6]
  - Fixed kernel execution hang under large memory consumption by workarounding a bug in the L0 runtime. [b831bd0]
  - Fixed the L0 plugin to honor `property::queue::enable_profiling`. [fb27c65]
  - Fixed memory leak which existed when program build failed for the L0 backend. [beb7277]
  ...
  - Fixed BDF format on PCI query for the L0 backend. [ad6253e]
  ...
  - Fixed make_queue interoperability API for the L0 to accept device argument to properly associate queue with the right device. [29a5369]
  ...
  - Fixed sporadic failure of in-order queue due to non-closed batch on the L0 backend. [e8bff05]
  ...
  - Fixed API to get maximum width/height/depth of an image for the L0 backend. [9ecc74b]
