...
- Added a support of compilation of the `.cu` files containing CUDA, and SYCL codes in the same translation unit [31608c24] [f5126d28]
- Added a support of `amd-gpu-gfx1034` as an acceptable value for `-fsycl-targets` [5e86a41d]
...
- Partially implemented [`sycl_ext_codeplay_kernel_fusion`](doc/extensions/experimental/sycl_ext_codeplay_kernel_fusion.asciidoc) extension for Linux: Intel CPU and GPU devices are fully supported, except specialization constants, streams, images and reductions [89e82e3f] [5d7e86d4] [ab0c46bd] [b5d5aada] [325bc4e2] [cf245f62]
...
- Added initialization of `sycl::ext::oneapi::experimental::device_global` without `device_image_scope` property [c4617790]
...
- Added `rbegin()`, `rend()`, `crbegin()` and `crend()` methods to `sycl::accessor`,
`sycl::host_accessor` and `sycl::local_accessor` [baa3d739]
...
- Introduced an API to get thread ID and subdevice ID for ESIMD extension [dd6145e8]
- Implemented `kernel_bundle::native_specialization_constant()` [4a412307]
- Implemented SYCL 2020 identities without a known or specified identity [648845fb] [1404ff1d]
- Implemented basic functionality for the following group types: `ballot_group`, `cluster_group`, `tangle_group`, `opportunistic_group` as part of [`sycl_ext_oneapi_non_uniform_groups`](doc/extensions/proposed/sycl_ext_oneapi_non_uniform_groups.asciidoc) extension [86894205]
...
### Documentation
- Added [`sycl_ext_oneapi_annotated_ptr`](doc/extensions/experimental/sycl_ext_oneapi_annotated_ptr.asciidoc), [`sycl_ext_intel_fpga_kernel_arg_properties`](doc/extensions/proposed/sycl_ext_intel_fpga_kernel_arg_properties.asciidoc), and [`sycl_ext_oneapi_kernel_arg_properties`](doc/extensions/proposed/sycl_ext_oneapi_kernel_arg_properties.asciidoc) extention specifications [b423e1db]
- Reintroduced `device_ptr` and `host_ptr` under SYCL namespace for [`sycl_ext_intel_usm_address_spaces`](doc/extensions/supported/sycl_ext_intel_usm_address_spaces.asciidoc) [ea8a0e56]
...
- Added an initial draft of non-uniform sub-groups proposal [cc7cbd28] [a5d62c8a]
- Added a draft of 16-bit atomics extension [1b973aed]
- Added an extension specification for USM allocation functions with properties support [3600f922]
- Added `sycl_ext_oneapi_peer_access` extention specification [b5bce772] [37960cda]
...
### SYCL Library
- Started to use const qualifier in several accessor iteratoration operators [e64433c9]
...
- Improved `group_local_memory_for_overwrite` (part of [`sycl_ext_oneapi_local_memory`](doc/extensions/supported/sycl_ext_oneapi_local_memory.asciidoc)) by defailt initializing it [37a43f93]
...
- Fixed return type of identityless reduction as it used an unexpected value for the deprecated placeholder template parameter in accessor [c64f88ec]
...
- Fixed `sycl::handler::get_specialization_constant` segmentation fault happened in certain cases likely due to strict aliasing violations [86c08b3c]
