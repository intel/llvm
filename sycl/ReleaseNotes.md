...
## New Features
### SYCL Compiler
- Add `-fpreview-breaking-changes` option [08febcfcb64a] [99b97eef4278]
- Enable early AOT abilities when creating objects [6f248088541c]
- Add support for `-faltmathlib` [32c3e642d644]
- Add support for `-ftarget-register-alloc-mode` [8e5275af77af]
- Add `-fsycl-add-default-spec-consts-image` command line option to Clang Driver [3847c7c6c5c7]

### SYCL Library
- Add experimental implementation of SYCL bindless images extension [58a8f2001d8b] [380453deeb51] [b1aab046f408]
- Add support for local accessors to `copy_from`/`copy_to` API [474461cb2e1d]
- Add support for local accessors to atomic update API [8974cb944b3c]
- Add new `raw_send` APIs moving compile time params to template params [1a5e53216b00]
...
- Implement `sycl_ext_oneapi_auto_local_range` [a6d4c2120b5c]
- Implement `sycl_ext_oneapi_sub_group_mask` version 2 [3bd09b9089c5]
- Implement `sycl_ext_oneapi_queue_priority` [0c33fea5fac0]
- Implement optional kernel feature for `joint_matrix` and `joint_matrix_mad` [946b1f236167] [c38dfcf64681]
- Implement `sycl_ext_oneapi_prefetch` [e7139b0f15d0] [0229456e19d5] [b5d69df53620]
- Add support for querying device architecture [1ad69e59eb13]
- Add esimd as an optional device feature [a5f471ea1535]
- Added `ext_oneapi_non_uniform_groups` aspect [de92299c2c09]

### Documentation
- Update `sycl_ext_oneapi_complex` from proposed to experimental [ea6aea37c6dd]
- Add SYCLcompat documentation [180a92ad707b]
- Add draft forward progress extension [67461148b169]
- Add `sycl_ext_oneapi_kernel_compiler` extension spec [5e64f96d20c2]
- Add `sycl_ext_oneapi_enqueue_functions` extension spec [ebc3ddb9a60b]
- Add `sycl_ext_oneapi_graph_fusion` extension spec [8c4cad45faba]
- Add `sycl_ext_intel_fpga_datapath` extension spec [a162179381fd]
- Add `sycl_ext_intel_cache_controls` extension spec [deb020bf740b]
- Add design doc for SYCL-Graph Extension [1713ed19f71c]

## Improvements
### SYCL Compiler
- Diagnose SYCL options in non-sycl mode. [bf128c873e74]
- Modify SYCL offloading flow to use `clang-linker-wrapper` [d7a1a4146eb2] [7b907cbcaea9]
- Enable `INTEL_SYCL_OPAQUEPOINTER_READY` [a47705f2d865] [9dded21c5984]
- Support multiple SYCL targets with `native_cpu` in the same compiler invocation [aab5d747c46e]
- Make `-fintelfpga` flag imply `-fsycl`. [ad2f959d8b88] [6d48bc13c456]
- Adjust debug format for SYCL device on Windows [73bba79a3ffb]
- Disable parallel for range rounding at `-O0` [2c117d7a6104]
- Enable `SPV_INTEL_optnone` extension for non-fpga targets [993fe5b31b43]
- Relax `LowerESIMDSlmReservation` assert [7897517c3a5e]

### SYCL Library
- vec abi unification and trivially copyable [531aabfdee91] [f4ed132f243a]
- Support missing types with ldg extension [aec8a35788c1]
- Stop throwing exception when passing empty accessor to `handler::require()` [7bf1f57bc26e]
- Move `addc()` and `subb()` functions out of experimental namespace [9e3852bc1c2e]
- Move BFN function from experimental namespace [c9da5c7ff7e8]
- Report source code info(file/line number) where null pointer exception occurred in memcpy [626c4b5078c1]
- Improve accuracy of double `tanpi` host implementation [041a74c0cbda]
- Add more support for AMD subgroup functions [288aeaef03a9]
- `annotated_ptr` API fixes [954730e7a823] [fbad42a398ef] [43336a15fcaf]
- Implement fallback path for profiling info support [7cfa951303eb]
- Fix require alignment in buffers [093dae1f7214]
- Throw an exception when unsupported features used in a graph [8d8d3f4f52b0]
- Support `get_multi_ptr` in ESIMD context [2f0f3814c541]
- Accept integral type in parallel_for with offsets [ecfc49c825c6]
- Support `assert()` in ESIMD kernels [545a043ebaf5]
- Add missing Intel archs to `sycl_ext_oneapi_device_architecture` [c4db251179dc]
- Enable range rounding for unnamed lambdas [4f30e6648d9e]
- Replaced `LoadLibraryA()` with `LoadLibraryEx()` [713442327d40]
- Don't use legacy ANSI-only Windows API for loading plugins [5c30815d4821]
- Use SFINAE to reject wrong signature of CGF object parameter [5801970585b2]
- `fill()` use native functions instead of custom kernels [0249a55f810e]
- Add defaults for streaming and register_map fpga_kernel_properties [23a6f389c1e4]
- Move GRF property conflict check to compile time [4fb92d35c7be]
- Add error when using invoke_simd with callables with struct arguments [03c13fd53a24]
- Add missing `rintf` [3c327c736ced]
- Cache the max local mem size in CUDA adapter [1c892f952cae]
- Add support for local accessors to lsc API [9dce2d227b31]
- Replace USM allocator with UMF abstractions in Level Zero backend [7b9b8ce4e4c5]
- Make SYCL-Graph functions thread-safe [c8c64a673eeb]
- Improve diagnostic for nd_item passed to parallel_for with range [e600d7922489]
- Implement missing exceptions defined by SYCL-Graphs specification [77b794bcaad6]
- Implemented supported make_device, make_queue, make_event interop functions for HIP backend [5e9d07b14a47]
- Add missing nearbyint for CUDA [0ef26d3ee808]
- Implement runtime query for matrix combinations [a09491f4e389]
- Support basic kernel launches over 64-bit global ranges [192c3016e76b]
- Clarify runtime error for nested kernel submissions [7afc2d03fd00]
- Improve handling of spec constants with padding in the middle of structs [292309c826a3]
- Add support for CUDA backend with graph extension [367b662a49ba]
- Add support for specialization constants with graph extension [fca7f5e2e3fb]
- Improve `vec::convert` implementation to use a single instruction instead of per-element [f634cc9bf799]

### Documentation
- Update `sycl_ext_oneapi_local_memory` [457251feceb8]
- Add get-coord API and general query example to `sycl_ext_intel_matric` [579f1dec0b5b] [38ac2124c1e8]
- Update design doc for new offloading model [115808f5128f]
- Update immediate command list usage [ad973f3383e2]
- Add overloads with `sycl::queue` for some functions in `sycl_ext_oneapi_bindless_images` [4d0c552985bd]
- Add queue requirement when replay a graph [2c53815c6c1e]

## Bug Fixes
### SYCL Compiler
- Fix backend option passing issues with multiple targets [12ca2db0e30b]
- Fix `-fgpu-rdc` option for CUDA [f7595ac7527e]
- Fix triple in offload mismatch warning [0fd9a4eb17a2]
- Fix a bug when `-enable-global-offset=false` for AMDGU and NVPTX targets [00cf4c29740b]

### SYCL Library
- Fix stack corruption in `sycl::modf` for `sycl::half` type [7c6541dc18e3]
- Fix nextafter with half on host [1f10f3588e8c]
- Fix identity values for (sub)group collectives min/max [248389371932]
- Improve handling of large `reqd_work_group_size` values [25c3666dffa9]
- Fix hip `prefetch`/`mem_advise` (noop) for rocm < 5.0 [7e4cc1113987]
- Fix `range` to allow default constructor to 0s [0289d2ac94f9]
- Fix propagation of aspects from global variables [fb368a130448]
- Missing return statement in the `multi_ptr<void..>` conversion operator [836469ba88e2]
- Fix `convert` to `sycl::vec<bool, N>` [9dfaf274e2cb]
- Ensure `queue_impl` is not unnecessarily referenced in kernel fusion [a33b0a950ae6]
- Fix int overflow for OffsetInBytes in accessors [2fb0b140ad3b]
- Fix `std::enable_if` typo for `marray` and `multi_ptr` [0ef25ec3bdf8]
- Fix integer type overflow in `SYCLDeviceRequirements` [cee07d3d2652]
- Fix UB in group sort scratch memory access [ed4c01301a7c]
- Use primary context in HIP adapter [d1c92cb95b29]
- Allow passing default constructed accessors to kernels [fb8ddbf7b0ea]
- Fix issues with `sycl_ext_oneapi_graph` subgraphs [92ddf8d562d7]
- Fix ambiguous `log2` call in host code
- Add device copyable trait for `annotated_arg` [9f65e61a2dbf]

### Documentation
- Update `device_image_scope` property  in `sycl_ext_oneapi_device_global` [be8e031cd078]

## API/ABI breakages
- Deprecate experimental functions: quot,div,mod,dp*,line,lrp [76976a22ba2e]
- Remove ESIMD Emulator plugin [1ece6da6b113]
- Deprecate `sycl::abs(genfloat)` [d15d4409b2cf]
- Don't include `<complex>/<cmath>` from `sycl.hpp`, unless `-fpreview-breaking-changes` [6ed0ab887d18] [d22c9c58d2e7]
