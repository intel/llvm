...
backend=lit_config.params.get('SYCL_BE', "opencl")
...
config.substitutions.append( ('%BE_RUN_PLACEHOLDER', "env SYCL_DEVICE_FILTER={SYCL_BE} ".format(SYCL_BE=backend)) )
config.substitutions.append( ('%HOST_RUN_PLACEHOLDER', "env SYCL_DEVICE_FILTER=host ") )
...
if getDeviceCount("cpu")[0]:
    ...
    cpu_run_substitute = "env SYCL_DEVICE_FILTER={SYCL_BE}:cpu ".format(SYCL_BE=backend)
    cpu_check_substitute = "| FileCheck %s"
    config.available_features.add('cpu')
    if platform.system() == "Linux":
        cpu_run_on_linux_substitute = "env SYCL_DEVICE_FILTER={SYCL_BE}:cpu ".format(SYCL_BE=backend)
...
if gpu_count > 0:
    ...
    gpu_run_substitute = " env SYCL_DEVICE_FILTER={SYCL_BE}:gpu ".format(SYCL_BE=backend)
    ...
    if platform.system() == "Linux":
        gpu_run_on_linux_substitute = "env SYCL_DEVICE_FILTER={SYCL_BE}:gpu ".format(SYCL_BE=backend)
...
if getDeviceCount("accelerator")[0]:
    ...
    acc_run_substitute = " env SYCL_DEVICE_FILTER={SYCL_BE}:acc ".format(SYCL_BE=backend)
