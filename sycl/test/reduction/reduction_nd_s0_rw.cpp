...
int main() {
  ...
  test<class KernelName_OtxLfwQuVfGAUEXMIs, int, 0, intel::plus<int>>(0, 49,
                                                                      49 * 5);

  // Try some power-of-two work-group sizes.
  test<class KernelName_lMJpe, int, 0, intel::plus<int>>(0, 2, 64);
  test<class KernelName_jikSUrEuFUxYGGfXNet, int, 0, intel::plus<int>>(0, 4,
                                                                       64);
  test<class KernelName_cByxQmddzgEGUeboDDbO, int, 0, intel::plus<int>>(0, 8,
                                                                        128);
  ...
  test<class KernelName_eWffIBPdwvvUwPFZFeG, int, 0, intel::plus<int>>(0, 256,
                                                                       256);
  ...
  test<class KernelName_jdixaAPjypPSGPCbXIw, int, 0, intel::bit_xor<int>>(0, 8,
                                                                          256);
  test<class KernelName_FNGt, int, 0, intel::bit_and<int>>(~0, 8, 256);
  test<class KernelName_KPtKKagKhZzwSibEl, int, 0, intel::minimum<int>>(
      (std::numeric_limits<int>::max)(), 8, 256);
  test<class KernelName_xdNhx, int, 0, intel::maximum<int>>(
      (std::numeric_limits<int>::min)(), 8, 256);

  // Check with various types.
  test<class KernelName_IxDwu, float, 0, std::multiplies<float>>(1, 8, 256);
  test<class KernelName_NpYzX, float, 0, intel::minimum<float>>(
      getMaximumFPValue<float>(), 8, 256);
  test<class KernelName_dofjVNlXWgJ, float, 0, intel::maximum<float>>(
