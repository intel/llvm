...
// RUN: env SYCL_PARALLEL_FOR_RANGE_ROUNDING_TRACE=1 %RUN_ON_HOST %t.out
...
int try_rounding_off(size_t size) {
  ...
  int Counter = 0;
  {
    ...
    std::cout
        << "Run parallel_for with SYCL_DISABLE_PARALLEL_FOR_RANGE_ROUNDING"
        << std::endl;
...
// CHECK:  Run parallel_for with SYCL_DISABLE_PARALLEL_FOR_RANGE_ROUNDING
// CHECK-NOT: parallel_for range adjusted from 10 to 32
