...
int main() {
  ...
  queue testQueue;
  range<size> ndRng(size);
  int kernelResult;
  {
    buffer<int, size> buffer(&kernelResult, ndRng);
    ...
    testQueue.submit([&](handler &cgh) {
      ...
      cgh.single_task<class kernel>([=]() {
        ...
        auto local_acc_multi_ptr =
            local_ptr.get_multi_ptr<access::decorated::yes>();
        static_assert(std::is_same_v<decltype(acc_multi_ptr),
                                     typename accessor_t::template accessor_ptr<
                                         access::decorated::yes>>);
        static_assert(
            std::is_same_v<decltype(local_acc_multi_ptr),
                           typename local_accessor_t::template accessor_ptr<
                               access::decorated::yes>>);
        *local_acc_multi_ptr.get() = 5;
        *acc_multi_ptr.get() = local_ptr[0];
      });
    });
  }
