...
//
// RUN: sycl-post-link %t.bc -spec-const=rt -reduce-memory-usage=true -o %t-split-red.txt
// RUN: cat %t-split-red_0.prop | FileCheck %s
// RUN: llvm-spirv -o %t-split-red_0.spv -spirv-max-version=1.1 -spirv-ext=+all %t-split-red_0.bc
//
//==----------- spec_const.cpp ---------------------------------------------==//
//
