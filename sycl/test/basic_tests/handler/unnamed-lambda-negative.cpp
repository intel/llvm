...
int main() {
  ...
  // expected-error-re@sycl/kernel_launch_helper.hpp:* {{unnamed type '{{.*}}' is invalid; provide a kernel name, or use '-fsycl-unnamed-lambda' to enable unnamed kernel lambdas}}
  // expected-note@+1 {{in instantiation of function template}}
  q.single_task([=](){});

  // expected-error-re@sycl/kernel_launch_helper.hpp:* {{unnamed type 'sycl::detail::RoundedRangeKernel<{{.*}}>' is invalid; provide a kernel name, or use '-fsycl-unnamed-lambda' to enable unnamed kernel lambdas}}
  // expected-error-re@sycl/kernel_launch_helper.hpp:* {{unnamed type '{{.*}}' is invalid; provide a kernel name, or use '-fsycl-unnamed-lambda' to enable unnamed kernel lambdas}}
  // expected-note@+1 {{in instantiation of function template}}
