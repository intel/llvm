// XUN: %if preview-breaking-changes-supported %{  %clangxx -fsycl
// -fpreview-breaking-changes -fsyntax-only -Xclang -verify %s -Xclang
// -verify-ignore-unexpected=note,warning %}
// RUN: %clangxx -fsycl -fpreview-breaking-changes -fsyntax-only -Xclang -verify %s -Xclang -verify-ignore-unexpected=note,warning
#include <string>
