// RUN: %clangxx -fpreview-breaking-changes -fsycl -O2 -fno-fast-math -S -emit-llvm -o - %s | FileCheck %s
...
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef float @_Z15__spirv_ocl_cosf(float noundef %{{.*}})
// CHECK-NEXT:   ret float %{{.*}}
// CHECK-NEXT: }
// CHECK-EMPTY:
// CHECK:      define weak_odr dso_local spir_func noundef double @_Z8test_cosIdEDcT_(double noundef %{{.*}}) {{.*}} {
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef double @_Z15__spirv_ocl_cosd(double noundef %{{.*}})
...
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef half @_Z15__spirv_ocl_cosDF16_(half noundef %{{.*}})
...
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef <2 x float> @_Z15__spirv_ocl_cosDv2_f(<2 x float> noundef %{{.*}})
...
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef <3 x float> @_Z15__spirv_ocl_cosDv3_f(<3 x float> noundef %{{.*}})
...
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = load <4 x float>, ptr %{{.*}}, align 16
// CHECK-NEXT:   %{{.*}} = tail call spir_func noundef <4 x float> @_Z15__spirv_ocl_cosDv4_f(<4 x float> noundef %{{.*}})
// CHECK-NEXT:   store <4 x float> %{{.*}}, ptr addrspace(4) %{{.*}}, align 16
// CHECK-NEXT:   ret void
// CHECK-NEXT: }
// CHECK-EMPTY:
...
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call noundef float @_ZN4sycl3_V110__cos_implEf(float noundef %{{.*}})
// CHECK-NEXT:   ret float %{{.*}}
// CHECK-NEXT: }
// CHECK-EMPTY:
// CHECK:      define weak_odr dso_local noundef double @_Z8test_cosIdEDcT_(double noundef %{{.*}}) {{.*}} {
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call noundef double @_ZN4sycl3_V110__cos_implEd(double noundef %{{.*}})
// CHECK-NEXT:   ret double %{{.*}}
// CHECK-NEXT: }
// CHECK-EMPTY:
...
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call float @_ZN4sycl3_V110__cos_implENS0_3vecIfLi1EEE(float %{{.*}})
// CHECK-NEXT:   ret float %{{.*}}
// CHECK-NEXT: }
// CHECK-EMPTY:
// CHECK:      define weak_odr dso_local <2 x float> @_Z8test_cosIN4sycl3_V13vecIfLi2EEEEDcT_(<2 x float> %{{.*}}) {{.*}} {
// CHECK-NEXT: entry:
// CHECK-NEXT:   %{{.*}} = tail call <2 x float> @_ZN4sycl3_V110__cos_implENS0_3vecIfLi2EEE(<2 x float> %{{.*}})
