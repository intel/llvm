...
// RUN: %clangxx -O0 -fsycl -D_DEBUG -shared %s -nostdlib -Xclang --dependent-lib=msvcrtd -fms-runtime-lib=dll_dbg  &> %t.compile.log
...
// CHECK-NOT: UnsupportedVarArgFunction: Variadic functions other than 'printf' are not supported in SPIR-V.
// CHECK-NOT: clang{{.*}} error: llvm-spirv command failed with exit code 23{{.*}}
