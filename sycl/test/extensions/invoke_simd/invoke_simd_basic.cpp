// RUN: %clangxx -fsycl %s -DINVOKE_SIMD=1 -Xclang -emit-llvm -O3 -g0 -c -fsycl-device-only -o %t.ll
// TODO add IR matching
// TODO add other callables
...
simd<float, VL> SIMD_CALLEE(float *A, simd<float, VL> b,
                            int i) SYCL_ESIMD_FUNCTION {
  // TODO need conversion functions from esimd::simd to oneapi::simd
  esimd::simd<float, VL> b1(
      static_cast<typename simd<float, VL>::raw_storage_type>(b));
  esimd::simd<float, VL> res1 = ESIMD_CALLEE(A, b1, i);
  auto y = b1 > res1; // unused code for testing purposes
  return simd<float, VL>{res1.data()};
}
