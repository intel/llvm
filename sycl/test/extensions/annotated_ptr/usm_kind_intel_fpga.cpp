...
// Tests that the address space of `annotated_ptr` kernel argument is refined when:
// 1. `usm_kind` property is specified in annotated_ptr type, and
// 2. both `__SYCL_DEVICE_ONLY__` and `__ENABLE_USM_ADDR_SPACE__` are turned on (i.e. equiv to flags "-fsycl-device-only -fsycl-targets=spir64_fpga")
...
using annotated_ptr_t1 =
    annotated_ptr<int, decltype(properties(buffer_location<0>, usm_kind_device))>;
using annotated_ptr_t2 =
    annotated_ptr<int, decltype(properties(buffer_location<1>, usm_kind_host))>;
using annotated_ptr_t3 =
    annotated_ptr<int, decltype(properties(buffer_location<2>, usm_kind_shared))>;


struct MyIP {

// CHECK-IR: spir_kernel void @_ZTS4MyIP(ptr addrspace(5) {{.*}} %_arg_a, ptr addrspace(6) {{.*}} %_arg_b, ptr addrspace(6) {{.*}} %_arg_c)
  annotated_ptr_t1 a;
  annotated_ptr_t2 b;
  annotated_ptr_t3 c;
