// RUN: %clangxx -fsycl-device-only -S -Xclang -emit-llvm -Wno-deprecated %s -o - | FileCheck %s --check-prefix CHECK-IR
// RUN: %clangxx -fsycl -fsyntax-only -Xclang -verify %s -Wno-deprecated
