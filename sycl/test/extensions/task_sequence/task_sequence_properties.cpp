...
static task_sequence<dummyTask, decltype(properties(balanced, response_capacity<0>, invocation_capacity<0>, pipelined<-1>, use_stall_free_clusters))> TaskSequence1;
static task_sequence<dummyTask, decltype(properties(response_capacity<2>, invocation_capacity<3>, pipelined<1>, use_stall_enable_clusters))> TaskSequence2;
static task_sequence<dummyTask, decltype(properties(pipelined<0>))> TaskSequence3;
...
template <typename T> void checkIsPropertyOf() {
  ...
  static_assert(is_property_value_of<decltype(invocation_capacity<1>), T>::value);
  static_assert(is_property_value_of<decltype(pipelined<1>), T>::value);
  static_assert(is_property_value_of<decltype(fpga_cluster<fpga_cluster_options_enum::stall_enable>), T>::value);
}
...
int main() {
  ...
  static_assert(TaskSequence1.get_property<response_capacity_key>() == response_capacity<0>);
  static_assert(TaskSequence1.get_property<invocation_capacity_key>() == invocation_capacity<0>);
  static_assert(TaskSequence1.get_property<pipelined_key>() == pipelined<-1>);
  static_assert(TaskSequence1.get_property<fpga_cluster_key>() == use_stall_free_clusters);
  ...
  static_assert(TaskSequence2.get_property<response_capacity_key>() == response_capacity<2>);
  static_assert(TaskSequence2.get_property<invocation_capacity_key>() == invocation_capacity<3>);
  static_assert(TaskSequence2.get_property<pipelined_key>() == pipelined<1>);
  static_assert(TaskSequence2.get_property<fpga_cluster_key>() == use_stall_enable_clusters);
