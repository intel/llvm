...
// RUN: %if asserts %{sycl-post-link -split=none -properties -debug-only=SpecConst %t.bc -spec-const=native -o %t.txt 2>&1 | FileCheck %s -check-prefixes=CHECK-LOG %} %else %{sycl-post-link -split=none %t.bc -properties -spec-const=native -o %t.txt 2>&1 %}
