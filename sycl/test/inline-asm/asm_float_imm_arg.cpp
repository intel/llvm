...
using arr_t = std::vector<cl::sycl::cl_double>;

// class is used for kernel name
template <typename T>
class float_imm_arg_kernel;
...
int main() {
  ...
  try {
    ...
    for (int i = 0; i < LIST_SIZE; i++)
      A[i] = (double)1 / std::pow(2, i);

    ct.process_buffers(A.data(), B.data(), LIST_SIZE);

    for (int i = 0; i < LIST_SIZE; ++i) {
      if (B[i] != A[i] * IMM_ARGUMENT) {
        std::cerr << "At index: " << i << ". ";
        std::cerr << B[i] << " != " << A[i] * IMM_ARGUMENT << "\n";
        abort();
      }
    }
...
void ocl_ctx_t::process_buffers(const T *pa, T *pb, size_t sz) {
  ...
  deviceQueue.submit([&](cl::sycl::handler &cgh) {
    ...
    cgh.parallel_for<class float_imm_arg_kernel<T>>(numOfItems, kern);
  });
}
