...
//
// CHECK: %struct.AccWrapper = type { %"class.__sycl_internal::__v1::accessor.1" }
// CHECK: %"class.__sycl_internal::__v1::accessor.1" = type { %"class{{.*}}AccessorImplDevice", %[[UNION:.*]] }
// CHECK-DISABLE: %[[UNION]] = type { i32 addrspace(1)* }
// CHECK-ENABLE: %[[UNION]] = type { i32 addrspace(5)* }
// CHECK: %struct.AccWrapper.{{[0-9]+}} = type { %"class.__sycl_internal::__v1::accessor.[[NUM:[0-9]+]]" }
// CHECK-NEXT: %"class.__sycl_internal::__v1::accessor.[[NUM]]" = type { %"class{{.*}}LocalAccessorBaseDevice", i32 addrspace(3)* }
//
