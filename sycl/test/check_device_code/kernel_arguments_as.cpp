...
//
// CHECK: %struct{{.*}}AccWrapper = type { %"class{{.*}}::accessor" }
// CHECK: %"class{{.*}}::accessor" = type { %"class{{.*}}AccessorImplDevice", %[[UNION:.*]] }
// CHECK-DISABLE: %[[UNION]] = type { i32 addrspace(1)* }
// CHECK-ENABLE: %[[UNION]] = type { i32 addrspace(5)* }
// CHECK: %struct{{.*}}AccWrapper = type { %"class{{.*}}::accessor" }
// CHECK-NEXT: %"class{{.*}}::accessor" = type { %"class{{.*}}LocalAccessorBaseDevice", i32 addrspace(3)* }
//
