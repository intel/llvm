...
int main() {
  ...
  Q.submit([](sycl::handler &CGH) {
    CGH.parallel_for(sycl::nd_range{sycl::range{1}, sycl::range{1}}, [](sycl::nd_item<1> item) {
      auto G = item.get_group();
      TestLambda(G);
    });
  });
  Q.submit([](sycl::handler &CGH) {
    CGH.parallel_for(sycl::nd_range{sycl::range{1, 1}, sycl::range{1, 1}}, [](sycl::nd_item<2> item) {
      auto G = item.get_group();
      TestLambda(G);
    });
  });
  Q.submit([](sycl::handler &CGH) {
    CGH.parallel_for(sycl::nd_range{sycl::range{1, 1, 1}, sycl::range{1, 1, 1}}, [](sycl::nd_item<3> item) {
      auto G = item.get_group();
      TestLambda(G);
    });
  });
...
// CHECK: tail call spir_func void @_Z22__spirv_ControlBarrierjjj(i32 2, i32 3, i32 904) #2
// CHECK: tail call spir_func void @_Z22__spirv_ControlBarrierjjj(i32 2, i32 2, i32 904) #2
