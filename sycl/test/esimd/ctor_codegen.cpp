...
// CHECK-NEXT: store <2 x double> %[[V1]], ptr %[[RES]]
// CHECK-NEXT: ret void
// CHECK-NEXT: }
}
...
SYCL_EXTERNAL auto double_base_step_var(double base, double step) SYCL_ESIMD_FUNCTION {
  ...
  // CHECK: store <32 x double> %[[FMA_VEC]], ptr %[[RES]]
  // CHECK-NEXT: ret void
}
...
SYCL_EXTERNAL auto int_base_step_const() SYCL_ESIMD_FUNCTION {
  ...
  return val;
  // CHECK: store <16 x i32> <i32 17, i32 20, i32 23, i32 26, i32 29, i32 32, i32 35, i32 38, i32 41, i32 44, i32 47, i32 50, i32 53, i32 56, i32 59, i32 62>, ptr %[[RES]]
  // CHECK-NEXT: ret void
  // CHECK-NEXT: }
}
...
SYCL_EXTERNAL auto int_base_step_var(int base, int step) SYCL_ESIMD_FUNCTION {
  ...
  // CHECK: store <32 x i32> %[[ADD_VEC]], ptr %[[RES]]
  // CHECK-NEXT: ret void
}
...
SYCL_EXTERNAL auto int_base_step_var_n2(int base, int step) SYCL_ESIMD_FUNCTION {
  ...
  // CHECK: store <2 x i32> %[[RESULT_VEC]], ptr %[[RES]]
  // CHECK-NEXT: ret void
}
...
// CHECK: define dso_local spir_func void @_Z3geev({{.*}} %[[RES:[a-zA-Z0-9_\.]+]]){{.*}} {
  simd<float, 2> val(-7);
  return val;
// CHECK: store <2 x float> splat (float -7.000000e+00), ptr %[[RES]]
// CHECK-NEXT: ret void
// CHECK-NEXT: }
}
...
// CHECK: define dso_local spir_func void @_Z7foomaskv({{.*}} %[[RES:[a-zA-Z0-9_\.]+]]){{.*}} {
  simd_mask<2> val({ 1, 0 });
  return val;
// CHECK: store <2 x i16> <i16 1, i16 0>, ptr %[[RES]]
// CHECK-NEXT: ret void
// CHECK-NEXT: }
}
...
// CHECK: define dso_local spir_func void @_Z7geemaskv({{.*}} %[[RES:[a-zA-Z0-9_\.]+]]){{.*}} {
  simd_mask<2> val(1);
  return val;
// CHECK: store <2 x i16> splat (i16 1), ptr %[[RES]]
// CHECK-NEXT: ret void
// CHECK-NEXT: }
}
...
// CHECK: define dso_local spir_func void @_Z7geehalfv({{.*}} %[[RES:[a-zA-Z0-9_\.]+]]){{.*}} {
  simd<half, 2> val(-7);
  return val;
// CHECK: store <2 x half> splat (half 0xHC700), ptr %[[RES]]
// CHECK-NEXT: ret void
// CHECK-NEXT: }
}
