...
bool test_2d_accessor_array(queue &myQueue)
{
  ...
  myQueue.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<class t_2d_accessor_array>(num_items, [=](cl::sycl::id<1> index) {
      output_accessor[index] = a_array[0][0][index] + a_array[1][2][index];
    });
  });
  const auto HostAccessor = out_buffer.get_access<cl::sycl::access::mode::read>();
...
bool test_array_struct_accessor_array(queue &myQueue)
{
  ...
  myQueue.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<class array_struct_accessor_array>(num_items, [=](cl::sycl::id<1> index) {
      output_accessor[index] = array_s_a[0].a_array[0][index] + array_s_a[1].a_array[1][index] + 3;
    });
  });
  const auto HostAccessor = out_buffer.get_access<cl::sycl::access::mode::read>();
...
bool run_tests() {
  ...
  if (test_2d_accessor_array(Q)) {
    ++passCount;
  }
  if (test_array_struct_accessor_array(Q)) {
    ++passCount;
  }
