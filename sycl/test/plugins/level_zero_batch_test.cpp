...

  {
    for (size_t j = 0; j < 3; j++) {
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy1>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z1[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy2>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z2[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy3>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z3[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy4>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z4[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy5>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z5[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy6>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z6[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy7>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z7[m * N + n] = Y1[m * N + n];
          });
      ...
      q.submit([&](sycl::handler& h) {
          h.parallel_for<class u32_copy8>( sycl::range<2>{M, N}, [=] (sycl::id<2> it) {
              const int m = it[0];
              const int n = it[1];
              Z8[m * N + n] = Y1[m * N + n];
          });
