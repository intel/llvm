...
constexpr int M = 16; // number of rows of "C"/"D" (Accumulator) sub-matrices,
                     // number of cols of "B" sub-matrix.
constexpr int N = 16; // number of cols of "C"/"D" (Accumulator) sub-matrices,
                     // number of rows of "A" sub-matrix.
constexpr int K =
    16; // number of cols of "A"/number of rows of "B" sub-matrices.
...
constexpr int SUB_TILES_M =
    7; // number of submatrices per row of accumulator ("C", "D") matrices.
constexpr int SUB_TILES_N =
    2; // number of submatrices per col of accumulator matrices.
constexpr int SUB_TILES_K =
    2; // number of submatrices per col of "A"/per row of "B", matrices.
...
float matrix_ref_mn(const int &m, const int &n) {
  float res = C[m * BIG_N + n];
...
int main() {
  ...
  buffer<float, 1> bufA(A, range<1>(BIG_M * BIG_K));
  buffer<float, 1> bufB(B, range<1>(BIG_K * BIG_N));
  buffer<float, 1> bufC(C, range<1>(BIG_M * BIG_N));
  buffer<float, 1> bufD(D, range<1>(BIG_M * BIG_N));
  ...
  q.submit([&](handler &cgh) {
    ...
    cgh.parallel_for<class imatrix>(
        nd_range<2>(GlobalRange, LocalRange), [=
          ...
          joint_matrix<matrix_type::bf16, matrix_use::a, 16, 16, matrix_layout::row_major>
              sub_a;

          joint_matrix<matrix_type::bf16, matrix_use::b, 16, 16, matrix_layout::row_major>
              sub_b;

          joint_matrix<matrix_type::fp32, matrix_use::accumulator, 16, 16, matrix_layout::row_major>
              sub_c;
