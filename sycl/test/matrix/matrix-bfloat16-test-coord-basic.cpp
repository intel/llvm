...
auto pA = sycl::multi_ptr<bfloat16, sycl::access::address_space::global_space>(A);
auto pB = sycl::multi_ptr<bfloat16, sycl::access::address_space::global_space>(B);
auto pC = sycl::multi_ptr<float, sycl::access::address_space::global_space>(C);
         ...
         [=](sycl::nd_item<2> spmd_item) [[intel::reqd_sub_group_size(SG_SZ)]]

          {
           ...
           for (int k = 0; k < K / TK; k += 1) { //
             joint_matrix_load(
                 sg, sub_a, pA + (sg_startx * TM) * K + k * TK,
                 K, layout::row_major);
             auto tAData = sub_a.get_wi_data();
             for (int i = 0; i < tAData.length(); ++i) {
              auto [row, col] = tAData[i].get_coord();
        ...
        for (int mm = 0; mm < TM; mm++) {
          for (int nn = 0; nn < TN; nn++) {
            ...
            for (int kk = 0; kk < TK; kk++) {
              res_local_row_origA[k*TK + kk]+= A_ref[m*TM+mm][k*TK+kk];
              res_local_col_origB[n*TN + nn]+= B_ref[k*TK+kk][n*TN+nn];
            }
          }
        }
      }
