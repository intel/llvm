  ...
  q.submit([&](handler &cgh) {
     ...
     cgh.parallel_for<class imatrix>(
...

         {
           ...
           joint_matrix_load(
               sg, sub_c,
               accC.template get_multi_ptr<access::decorated::legacy>() +
                   (sg_startx * TM) * N + sg_starty / SG_SZ * TN,
               N, matrix_layout::row_major);
           for (int k = 0; k < K / TK; k += 1) { //
             joint_matrix_load(
                 sg, sub_a,
                 accA.template get_multi_ptr<access::decorated::legacy>() +
                     (sg_startx * TM) * K + k * TK,
                 K, matrix_layout::row_major);
             // Assuming B data is already in VNNI format.
             joint_matrix_load(
                 sg, sub_b,
                 accB.template get_multi_ptr<access::decorated::legacy>() +
