...
#ifndef _WIN32
  void *mem = syclexp::alloc_exportable_memory(
      0, size, syclexp::external_mem_handle_type::opaque_fd, device, context);
#else
  void *mem = syclexp::alloc_exportable_memory(
      0, size, syclexp::external_mem_handle_type::win32_nt_handle, device,
      context);
#endif

  // Export the memory handle.
#ifndef _WIN32
  auto exportableMemoryHandle =
      syclexp::export_memory_handle<syclexp::resource_fd>(mem, device, context);
  std::cout << "exportableMemoryHandle.file_descriptor == "
            << exportableMemoryHandle.file_descriptor << "\n";
#else
  auto exportableMemoryHandle =
      syclexp::export_memory_handle<syclexp::resource_win32_handle>(mem, device,
                                                                    context);
  std::cout << "exportableMemoryHandle.handle == "
            << exportableMemoryHandle.handle << "\n";
#endif // _WIN32
