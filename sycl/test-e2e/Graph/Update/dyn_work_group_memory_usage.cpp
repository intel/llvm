...
int main() {
  ...
  int *PtrA = malloc_device<T>(N, Queue);
  std::vector<T> HostDataA(N);

  exp_ext::dynamic_work_group_memory<T[]> DynLocalMem{Graph, LocalSize};

  Queue.memset(PtrA, 0, N * sizeof(T)).wait();

  nd_range<1> NDRange{range{N}, range{LocalSize}};
  ...
  KernelNode.update_nd_range(nd_range<1>{range{N}, range{NewLocalSize}});
