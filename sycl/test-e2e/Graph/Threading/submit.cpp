...
// According to spec: If graph is submitted multiple times, dependencies are automatically added by the runtime to prevent concurrent executions of an identical graph.
// and so the result is deterministic and we can check the results.
...
int main() {
  ...
  calculate_reference_data(NumThreads * SubmitsPerThread, Size, ReferenceA, ReferenceB,
                           ReferenceC);
