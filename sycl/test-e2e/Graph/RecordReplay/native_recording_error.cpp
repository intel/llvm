...
int main() {
  ...
  // This should throw an exception because native recording requires immediate
  // command lists (unless the global environment variable
  // SYCL_PI_LEVEL_ZERO_USE_IMMEDIATE_COMMANDLISTS=1 is set)
  ...
  try {
    ...
    if (errorMsg.find(
      ...
      std::cout << "Expected exception thrown for batched queue: " << errorMsg
                << std::endl;
    }
  }

  if (!exceptionThrown2) {
    std::cerr << "ERROR: Expected exception was not thrown for batched queue!"
              << std::endl;
    return 1;
  }

  std::cout << "All tests passed - native recording correctly validates queue "
               "command list mode"
            << std::endl;
  return 0;
}