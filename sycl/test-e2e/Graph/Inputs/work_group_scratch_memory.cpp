...
int main() {
  ...
  queue.copy(a_host.data(), a, size);

  queue.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(
        sycl::nd_range<1>({size}, {size}),
        KernelFunctor(sycl_ext::properties{sycl_ext::work_group_scratch_size(
                          size * sizeof(DataType))},
                      a, b));
  });

  queue.copy(b, b_host.data(), size);
  graph.end_recording();
  auto exec_graph = graph.finalize();

  queue.submit([&](sycl::handler &cgh) { cgh.ext_oneapi_graph(exec_graph); })
      .wait_and_throw();
