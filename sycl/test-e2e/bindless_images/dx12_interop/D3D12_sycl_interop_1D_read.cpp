...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int channels, bool useSampled, bool useSemaphores,
    ...
    // Setup D3D12 (Using 1D specific creation, but passing height=1 to upload/verify)
    ...
    if (!verifyD3D12Data<T>(ctx, imgRes, width, 1, channels)) {
        std::cerr << "Aborting SYCL test. Native D3D12 readback failed." << std::endl;
    ...
    try {
            ...
            h.parallel_for(sycl::range<1>(width), [=](sycl::item<1> item) {
                ...
                if (useSampled) {
                    ...
                    if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, sycl::half>) {
                        sycl::float4 pixel = syclexp::sample_image<sycl::float4>(sampledHandle, coordX);
                        ...
                        sycl::int4 pixel = syclexp::sample_image<sycl::int4>(sampledHandle, coordX);
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
                        ...
                        sycl::uint4 pixel = syclexp::sample_image<sycl::uint4>(sampledHandle, coordX);
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
                    ...
                    if (channels == 1) {
                        ...
                        sycl::vec<T, 2> pixel = syclexp::fetch_image<sycl::vec<T, 2>>(unsampledHandle, x);
                        outAcc[base + 0] = pixel.x();
                        outAcc[base + 1] = pixel.y();
                    } else if (channels == 4) {
                        sycl::vec<T, 4> pixel = syclexp::fetch_image<sycl::vec<T, 4>>(unsampledHandle, x);
                        outAcc[base + 0] = pixel.x();
                        outAcc[base + 1] = pixel.y();
        ...
        for (size_t i = 0; i < totalValues; ++i) {
            ...
            T expected = generateTestValue<T>(pixelIdx, channelIdx, width); // totalPixels is just width in 1D

            if (!checkValue(hostAcc[i], expected)) {
                if (errorCount < 10) {
                    std::cerr << "Mismatch at index " << i << ": Expected " << expected << ", Got " << hostAcc[i] << std::endl;
                }
                errorCount++;
                passed = false;
            }
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running " << (useSampled ? "SAMPLED" : "UNSAMPLED")
              << " D3D12 1D Read Test | Type: " << type << " | Size: " << width << "x"
              << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;

    if (type == "float") return runTest<float>(width, channels, useSampled, useSemaphores);
    if (type == "half") return runTest<sycl::half>(width, channels, useSampled, useSemaphores);
    if (type == "int32") return runTest<int32_t>(width, channels, useSampled, useSemaphores);
    if (type == "uint32") return runTest<uint32_t>(width, channels, useSampled, useSemaphores);
    if (type == "int16") return runTest<int16_t>(width, channels, useSampled, useSemaphores);
    if (type == "uint16") return runTest<uint16_t>(width, channels, useSampled, useSemaphores);
    if (type == "uint8") return runTest<uint8_t>(width, channels, useSampled, useSemaphores);
    if (type == "int8") return runTest<int8_t>(width, channels, useSampled, useSemaphores);
    
    if (type == "unorm8") {
        return runTest<uint8_t>(width, channels, useSampled, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8);
    }
