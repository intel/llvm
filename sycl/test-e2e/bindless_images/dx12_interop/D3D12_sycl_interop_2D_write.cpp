...
// UAV IMAGE CREATION
// ---------------------------------------------------------
inline D3D12ImageResources createExportableImageWrite(D3D12Context& ctx, uint32_t width, uint32_t height, DXGI_FORMAT format) {
    ...
    ThrowIfFailed(ctx.device->CreateCommittedResource(
        ...
        nullptr,
        IID_PPV_ARGS(&imgRes.resource)), "Failed to create Shared UAV Texture");
...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int height, int channels, bool useSemaphores,
    ...
    D3D12ImageResources imgRes = createExportableImageWrite(ctx, width, height, dxgiFormat);
    ...
    // Tell D3D12 queue to wait for SYCL to finish writing before doing the readback verify
    if (useSemaphores) {
        ctx.cmdQueue->Wait(extFence.fence.Get(), extFence.fenceValue + 1);
        extFence.fenceValue++;
    }

    // Native D3D12 Verification (Checks the image memory against generateTestValue)
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running UNSAMPLED D3D12 2D Write Test | Type: " << type << " | Size: " << width << "x"
              << height << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;

    if (type == "float") return runTest<float>(width, height, channels, useSemaphores);
    if (type == "half") return runTest<sycl::half>(width, height, channels, useSemaphores);
    if (type == "int32") return runTest<int32_t>(width, height, channels, useSemaphores);
    if (type == "uint32") return runTest<uint32_t>(width, height, channels, useSemaphores);
    if (type == "int16") return runTest<int16_t>(width, height, channels, useSemaphores);
    if (type == "uint16") return runTest<uint16_t>(width, height, channels, useSemaphores);
    if (type == "uint8") return runTest<uint8_t>(width, height, channels, useSemaphores);
    if (type == "int8") return runTest<int8_t>(width, height, channels, useSemaphores);
    
    if (type == "unorm8") {
        return runTest<uint8_t>(width, height, channels, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8);
    }
