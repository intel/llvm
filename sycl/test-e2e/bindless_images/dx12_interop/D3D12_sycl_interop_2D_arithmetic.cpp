...
bool uploadCustomData(D3D12Context& ctx, D3D12ImageResources& imgRes, uint32_t width, uint32_t height, int channels, Func generator) {
    ...
    ThrowIfFailed(ctx.device->CreateCommittedResource(&uploadHeap, D3D12_HEAP_FLAG_NONE, &bufDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&uploadBuffer)));
...
template <typename T, typename Func>
bool verifyCustomData(D3D12Context& ctx, D3D12ImageResources& imgRes, uint32_t width, uint32_t height, int channels, Func expectedGenerator) {
    D3D12_RESOURCE_DESC desc = imgRes.resource->GetDesc();
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint;
    UINT numRows;
    UINT64 rowSizeInBytes, totalBytes;
    ...
    ThrowIfFailed(ctx.device->CreateCommittedResource(&readbackHeap, D3D12_HEAP_FLAG_NONE, &bufDesc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&readbackBuffer)));
    ...
    if (passed) std::cout << "[D3D12 Only] Readback Verified Successfully." << std::endl;
    else std::cerr << "[D3D12 Only] Readback FAILED with " << errorCount << " errors." << std::endl;
...
inline D3D12ImageResources createExportableImageWrite(D3D12Context& ctx, uint32_t width, uint32_t height, DXGI_FORMAT format) {
    ...
    ThrowIfFailed(ctx.device->CreateCommittedResource(&defaultHeap, D3D12_HEAP_FLAG_SHARED, &texDesc, D3D12_RESOURCE_STATE_COMMON, nullptr, IID_PPV_ARGS(&imgRes.resource)));
    
    auto allocInfo = ctx.device->GetResourceAllocationInfo(0, 1, &texDesc);
    imgRes.allocationSize = allocInfo.SizeInBytes;
    ThrowIfFailed(ctx.device->CreateSharedHandle(imgRes.resource.Get(), nullptr, GENERIC_ALL, nullptr, &imgRes.sharedHandle));
...
int runTest(int width, int height, int channels, bool useSampled, bool useSemaphores,
    ...
    D3D12ImageResources imgA = createExportableImage(ctx, width, height, dxgiFormat);
    D3D12ImageResources imgB = createExportableImage(ctx, width, height, dxgiFormat);
    D3D12ImageResources imgOut = createExportableImageWrite(ctx, width, height, dxgiFormat);
    ...
    uploadCustomData<T>(ctx, imgA, width, height, channels, [](size_t x, size_t y, int c) { return generateValueA<T>(x, y, c); });
    if (useSemaphores) signalExportableFence(ctx, extFenceA);

    uploadCustomData<T>(ctx, imgB, width, height, channels, [](size_t x, size_t y, int c) { return generateValueB<T>(x, y, c); });
    if (useSemaphores) signalExportableFence(ctx, extFenceB);

    try {
        sycl::queue q;

        auto extMemA = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{imgA.sharedHandle, syclexp::external_mem_handle_type::win32_nt_handle, imgA.allocationSize}, q.get_device(), q.get_context());
        auto extMemB = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{imgB.sharedHandle, syclexp::external_mem_handle_type::win32_nt_handle, imgB.allocationSize}, q.get_device(), q.get_context());
        auto extMemOut = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{imgOut.sharedHandle, syclexp::external_mem_handle_type::win32_nt_handle, imgOut.allocationSize}, q.get_device(), q.get_context());

        syclexp::external_semaphore extSemA, extSemB, extSemOut;
        if (useSemaphores) {
            extSemA = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{extFenceA.sharedHandle, syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
            extSemB = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{extFenceB.sharedHandle, syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
            extSemOut = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{extFenceOut.sharedHandle, syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
        }
        ...
        if (useSemaphores) {
            waitEvents.push_back(q.submit([&](sycl::handler& h) { h.ext_oneapi_wait_external_semaphore(extSemA, extFenceA.fenceValue); }));
            waitEvents.push_back(q.submit([&](sycl::handler& h) { h.ext_oneapi_wait_external_semaphore(extSemB, extFenceB.fenceValue); }));
        }
        ...
        if (useSemaphores) {
            q.submit([&](sycl::handler& h) {
                ...
                h.ext_oneapi_signal_external_semaphore(extSemOut, extFenceOut.fenceValue + 1);
            });
        }
        q.wait();
        ...
        cleanupD3D12(ctx, imgA); cleanupD3D12(ctx, imgB); cleanupD3D12(ctx, imgOut);
        if (useSemaphores) { cleanupExportableFence(extFenceA); cleanupExportableFence(extFenceB); cleanupExportableFence(extFenceOut); }
        return 1;
    }
    ...
    bool passed = verifyCustomData<T>(ctx, imgOut, width, height, channels, [&](size_t x, size_t y, int c) {
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--sampled") useSampled = true;
    ...
    std::cout << "Running " << (useSampled ? "SAMPLED" : "UNSAMPLED") << " D3D12 2D ARITHMETIC Test (C = A + B) | Type: " << type 
              << " | Size: " << width << "x" << height << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;
