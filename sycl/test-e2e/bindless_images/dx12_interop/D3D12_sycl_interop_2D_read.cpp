...
/*
    ...
    DG2 $ sycl-ls
    [level_zero:gpu][level_zero:0] Intel(R) oneAPI Unified Runtime over Level-Zero, Intel(R) Arc(TM) A770 Graphics 12.55.8 [1.6.34938]
    [opencl:gpu][opencl:0] Intel(R) OpenCL Graphics, Intel(R) Arc(TM) A770 Graphics OpenCL 3.0 NEO  [32.0.101.8132]
...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int height, int channels, bool useSampled, bool useSemaphores,
    ...
    D3D12ImageResources imgRes = createExportableImage(ctx, width, height, dxgiFormat);
    ...
    try {
        ...
        // Note: For Phase 2, we stick to unsampled logic as a baseline. We will branch for sampled in Phase 4.
        ...
        if (useSemaphores) {
            dependencyEvent = q.submit([&](sycl::handler& h) {
                // Note: D3D12 timeline fences require passing the specific fenceValue to wait on
            ...
            h.parallel_for(sycl::range<2>(width, height), [=](sycl::item<2> item) {
                ...
                if (useSampled) {
                    ...
                    if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, sycl::half>) {
                        sycl::float4 pixel = syclexp::sample_image<sycl::float4>(sampledHandle, sycl::float2(coordX, coordY));
                        ...
                        sycl::int4 pixel = syclexp::sample_image<sycl::int4>(sampledHandle, sycl::float2(coordX, coordY));
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
                        ...
                        sycl::uint4 pixel = syclexp::sample_image<sycl::uint4>(sampledHandle, sycl::float2(coordX, coordY));
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running " << (useSampled ? "SAMPLED" : "UNSAMPLED")
              << " D3D12 2D Read Test | Type: " << type << " | Size: " << width << "x"
              << height << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;

    if (type == "float") return runTest<float>(width, height, channels, useSampled, useSemaphores);
    if (type == "half") return runTest<sycl::half>(width, height, channels, useSampled, useSemaphores);
    if (type == "int32") return runTest<int32_t>(width, height, channels, useSampled, useSemaphores);
    if (type == "uint32") return runTest<uint32_t>(width, height, channels, useSampled, useSemaphores);
    if (type == "int16") return runTest<int16_t>(width, height, channels, useSampled, useSemaphores);
    if (type == "uint16") return runTest<uint16_t>(width, height, channels, useSampled, useSemaphores);
    if (type == "uint8") return runTest<uint8_t>(width, height, channels, useSampled, useSemaphores);
    if (type == "int8") return runTest<int8_t>(width, height, channels, useSampled, useSemaphores);
    
    if (type == "unorm8") {
        return runTest<uint8_t>(width, height, channels, useSampled, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8);
    }
