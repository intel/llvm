...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int height, int depth, int channels, bool useSampled, bool useSemaphores,
    ...
    D3D12ImageResources imgRes = createExportableImage3D(ctx, width, height, depth, dxgiFormat);
        ...
        q.submit([&](sycl::handler& h) {
            ...
            h.parallel_for(sycl::range<3>(width, height, depth), [=](sycl::item<3> item) {
                ...
                if (useSampled) {
                    ...
                    sycl::float3 coords((float)x + 0.5f, (float)y + 0.5f, (float)z + 0.5f);

                    if constexpr (std::is_floating_point_v<T> || std::is_same_v<T, sycl::half>) {
                        sycl::float4 pixel = syclexp::sample_image<sycl::float4>(sampledHandle, coords);
                        ...
                        sycl::int4 pixel = syclexp::sample_image<sycl::int4>(sampledHandle, coords);
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
                        ...
                        sycl::uint4 pixel = syclexp::sample_image<sycl::uint4>(sampledHandle, coords);
                        if (channels >= 1) outAcc[base + 0] = static_cast<T>(pixel.x());
                        if (channels >= 2) outAcc[base + 1] = static_cast<T>(pixel.y());
                        if (channels == 4) {
                            outAcc[base + 2] = static_cast<T>(pixel.z());
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running " << (useSampled ? "SAMPLED" : "UNSAMPLED")
              << " D3D12 3D Read Test | Type: " << type << " | Size: " << width << "x"
              << height << "x" << depth << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;

    if (type == "float") return runTest<float>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "half") return runTest<sycl::half>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "int32") return runTest<int32_t>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "uint32") return runTest<uint32_t>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "int16") return runTest<int16_t>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "uint16") return runTest<uint16_t>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "uint8") return runTest<uint8_t>(width, height, depth, channels, useSampled, useSemaphores);
    if (type == "int8") return runTest<int8_t>(width, height, depth, channels, useSampled, useSemaphores);
    
    if (type == "unorm8") {
        return runTest<uint8_t>(width, height, depth, channels, useSampled, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8);
    }
