...
// ---------------------------------------------------------
template <typename T>
int runTest(int width, int height, int depth, int channels, bool useSemaphores,
    ...
    D3D12ImageResources imgRes = createExportableImageWrite3D(ctx, width, height, depth, dxgiFormat);
            ...
            h.parallel_for(sycl::range<3>(width, height, depth), [=](sycl::item<3> item) {
                ...
                if (isUnorm) {
                    if (channels == 1) {
                        ...
                        syclexp::write_image(unsampledHandle, coords, sycl::float2(v1, v2));
                    } else { 
                        float v1 = (float)generateTestValue<T>(index, 0, totalPixels) / 255.0f;
                        float v2 = (float)generateTestValue<T>(index, 1, totalPixels) / 255.0f;
                        ...
                        syclexp::write_image(unsampledHandle, coords, sycl::float4(v1, v2, v3, v4));
                    }
                    return;
                }
    ...
    bool passed = verifyD3D12Data3D<T>(ctx, imgRes, width, height, depth, channels);
...
int main(int argc, char** argv) {
    ...
    std::cout << "Running UNSAMPLED D3D12 3D Write Test | Type: " << type << " | Size: " << width << "x"
              << height << "x" << depth << " | Channels: " << channels << " | Semaphores: " << (useSemaphores ? "ON" : "OFF") << std::endl;

    if (type == "float") return runTest<float>(width, height, depth, channels, useSemaphores);
    if (type == "half") return runTest<sycl::half>(width, height, depth, channels, useSemaphores);
    if (type == "int32") return runTest<int32_t>(width, height, depth, channels, useSemaphores);
    if (type == "uint32") return runTest<uint32_t>(width, height, depth, channels, useSemaphores);
    if (type == "int16") return runTest<int16_t>(width, height, depth, channels, useSemaphores);
    if (type == "uint16") return runTest<uint16_t>(width, height, depth, channels, useSemaphores);
    if (type == "uint8") return runTest<uint8_t>(width, height, depth, channels, useSemaphores);
    if (type == "int8") return runTest<int8_t>(width, height, depth, channels, useSemaphores);
    
    if (type == "unorm8") {
        return runTest<uint8_t>(width, height, depth, channels, useSemaphores, getUnorm8Format(channels), sycl::image_channel_type::unorm_int8);
    }
