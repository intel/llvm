...
/*
    ...
    This entire test takes less than 30 seconds on a slow machine.  MUCH faster (and more complete coveraage) than SFINAE based approach.

    IF a particular variant is having problems on some platform, please do NOT just disable the whole test,
    instead use   // RUN-IF: (SOMETHING) yadda-yadda
    to enable/disable that variant. 

    For semaphore testing, we run just a sampling. Note, that on Linux if there is a failure in the first section,
    then likely ALL semaphore tests afterwards will fail. This is being tracked as a separate issue.
...
int runTest(int width, int height, int channels, bool useLinear, bool useSemaphores, bool useSampled,
    ...
    uploadImage(vkCtx, imgA,  channels, semA, [&](size_t i, int c) { return generateValueA<T>(i % width, i / width, c); });
    uploadImage(vkCtx, imgB,  channels, semB, [&](size_t i, int c) { return generateValueB<T>(i % width, i / width, c); });

    try {
        sycl::queue q;
#ifdef _WIN32
        auto extMemA = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{getMemHandle(vkCtx, imgA.memory), syclexp::external_mem_handle_type::win32_nt_handle, imgA.allocationSize}, q.get_device(), q.get_context());
        auto extMemB = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{getMemHandle(vkCtx, imgB.memory), syclexp::external_mem_handle_type::win32_nt_handle, imgB.allocationSize}, q.get_device(), q.get_context());
        auto extMemOut = syclexp::import_external_memory(syclexp::external_mem_descriptor<syclexp::resource_win32_handle>{getMemHandle(vkCtx, imgOut.memory), syclexp::external_mem_handle_type::win32_nt_handle, imgOut.allocationSize}, q.get_device(), q.get_context());
#else
...
#ifdef _WIN32
            extSemA = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{getSemaphoreHandle(vkCtx, semA), syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
            extSemB = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{getSemaphoreHandle(vkCtx, semB), syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
            extSemOut = syclexp::import_external_semaphore(syclexp::external_semaphore_descriptor<syclexp::resource_win32_handle>{getSemaphoreHandle(vkCtx, semOutVk), syclexp::external_semaphore_handle_type::win32_nt_handle}, q.get_device(), q.get_context());
#else
        ...
        if (useSampled) {
            ...
            kernelEvent = q.submit([&](sycl::handler& h) {
                ...
                h.parallel_for(sycl::range<2>(width, height), [=](sycl::item<2> item) {
                    ...
                    if (isUnorm) {
                          valA = syclexp::sample_image<Vec4>(handleA, sycl::float2(x + 0.5f, y + 0.5f));
                          valB = syclexp::sample_image<Vec4>(handleB, sycl::float2(x + 0.5f, y + 0.5f));
                    } else {
                          using SampleT = sycl::vec<T, 4>;
                          auto rawA = syclexp::sample_image<SampleT>(handleA, sycl::float2(x + 0.5f, y + 0.5f));
                          auto rawB = syclexp::sample_image<SampleT>(handleB, sycl::float2(x + 0.5f, y + 0.5f));
                          valA = { (float)rawA.x(), (float)rawA.y(), (float)rawA.z(), (float)rawA.w() };
                          valB = { (float)rawB.x(), (float)rawB.y(), (float)rawB.z(), (float)rawB.w() };
                    }
