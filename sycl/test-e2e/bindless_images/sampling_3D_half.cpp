...
int main() {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      for (int k = 0; k < depth; k++) {
        auto const index = i + width * (j + height * k);
        expected[index] = sycl::half(index);
        dataIn[index] = {sycl::half(index), 0, 0, 0};
      }
    }
  }
  ...
  try {
    ...
    sycl::ext::oneapi::experimental::image_descriptor desc(
        {width, height, depth}, 4, sycl::image_channel_type::fp16);
    ...
    sycl::buffer<sycl::half, 3> buf((sycl::half *)out.data(),
