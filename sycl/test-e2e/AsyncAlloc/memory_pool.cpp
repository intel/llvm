...
int main() {
  ...
  try {
    ...
    assert(initialThreshold.get_initial_threshold() ==
               memPool1.get_threshold() &&
           "Stored pool release threshold is incorrect!");
    assert(maximumSize.get_maximum_size() == memPool1.get_max_size() &&
           "Stored pool maximum size is incorrect!");
    ...
    assert(newThreshold == memPool1.get_threshold() &&
           "Newly set pool threshold is stored incorrectly!");
