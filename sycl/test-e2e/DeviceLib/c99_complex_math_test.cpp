...
int c99_complex_classify_f(float _Complex x) {
...
int c99_complex_compare_mul_f(float _Complex x, float _Complex y,
                              float _Complex z) {
  int cx = c99_complex_classify_f(x);
  int cy = c99_complex_classify_f(y);
  int cz = c99_complex_classify_f(z);

  // Expected classification of the product based on input classifications.
  int expected = -1;
  switch (cx) {
  case c99_zero:
    switch (cy) {
  ...
  // For non_zero * non_zero, also check numerical accuracy.
  if (cx == c99_non_zero && cy == c99_non_zero) {
    float a = __real__ x, b = __imag__ x, c = __real__ y, d = __imag__ y;
    float _Complex t = CMPLXF(a * c - b * d, a * d + b * c);
    float _Complex diff = z - t;
    float abs_diff = sqrtf(__real__ diff * __real__ diff +
                           __imag__ diff * __imag__ diff);
    float abs_z = sqrtf(__real__ z * __real__ z + __imag__ z * __imag__ z);
    if (abs_diff / abs_z > 1.e-6f)
      return 1;
  }
  return 0;
}

int c99_complex_compare_div_f(float _Complex x, float _Complex y,
                              float _Complex z) {
  int cx = c99_complex_classify_f(x);
  int cy = c99_complex_classify_f(y);
  int cz = c99_complex_classify_f(z);
...
static float _Complex complex_input[] = {
    CMPLXF(1.e-6f, 1.e-6f),    CMPLXF(-1.e-6f, 1.e-6f),
    CMPLXF(-1.e-6f, -1.e-6f),  CMPLXF(1.e-6f, -1.e-6f),
    CMPLXF(1.e+6f, 1.e-6f),    CMPLXF(-1.e+6f, 1.e-6f),
    CMPLXF(-1.e+6f, -1.e-6f),  CMPLXF(1.e+6f, -1.e-6f),
    ...
    CMPLXF(NAN, NAN),           CMPLXF(-INFINITY, NAN),
    CMPLXF(-2.f, NAN),          CMPLXF(-1.f, NAN),
    CMPLXF(-0.5f, NAN),         CMPLXF(-0.f, NAN),
    CMPLXF(+0.f, NAN),          CMPLXF(0.5f, NAN),
    CMPLXF(1.f, NAN),           CMPLXF(2.f, NAN),
    CMPLXF(INFINITY, NAN),

    CMPLXF(NAN, -INFINITY),     CMPLXF(-INFINITY, -INFINITY),
    CMPLXF(-2.f, -INFINITY),    CMPLXF(-1.f, -INFINITY),
    CMPLXF(-0.5f, -INFINITY),   CMPLXF(-0.f, -INFINITY),
    CMPLXF(+0.f, -INFINITY),    CMPLXF(0.5f, -INFINITY),
    CMPLXF(1.f, -INFINITY),     CMPLXF(2.f, -INFINITY),
    CMPLXF(INFINITY, -INFINITY),

    CMPLXF(NAN, -2.f),          CMPLXF(-INFINITY, -2.f),
    CMPLXF(-2.f, -2.f),         CMPLXF(-1.f, -2.f),
    CMPLXF(-0.5f, -2.f),        CMPLXF(-0.f, -2.f),
    CMPLXF(+0.f, -2.f),         CMPLXF(0.5f, -2.f),
    CMPLXF(1.f, -2.f),          CMPLXF(2.f, -2.f),
    CMPLXF(INFINITY, -2.f),

    CMPLXF(NAN, -1.f),          CMPLXF(-INFINITY, -1.f),
    CMPLXF(-2.f, -1.f),         CMPLXF(-1.f, -1.f),
    CMPLXF(-0.5f, -1.f),        CMPLXF(-0.f, -1.f),
    CMPLXF(+0.f, -1.f),         CMPLXF(0.5f, -1.f),
    CMPLXF(1.f, -1.f),          CMPLXF(2.f, -1.f),
    CMPLXF(INFINITY, -1.f),

    CMPLXF(NAN, -0.5f),         CMPLXF(-INFINITY, -0.5f),
    CMPLXF(-2.f, -0.5f),        CMPLXF(-1.f, -0.5f),
    CMPLXF(-0.5f, -0.5f),       CMPLXF(-0.f, -0.5f),
    CMPLXF(+0.f, -0.5f),        CMPLXF(0.5f, -0.5f),
    CMPLXF(1.f, -0.5f),         CMPLXF(2.f, -0.5f),
    CMPLXF(INFINITY, -0.5f),

    CMPLXF(NAN, -0.f),          CMPLXF(-INFINITY, -0.f),
    CMPLXF(-2.f, -0.f),         CMPLXF(-1.f, -0.f),
    CMPLXF(-0.5f, -0.f),        CMPLXF(-0.f, -0.f),
    CMPLXF(+0.f, -0.f),         CMPLXF(0.5f, -0.f),
    CMPLXF(1.f, -0.f),          CMPLXF(2.f, -0.f),
    CMPLXF(INFINITY, -0.f),
...
int device_complex_test_mul(s::queue &deviceQueue) {
  ...
  std::array<float _Complex, OutputSize> complex_mul_result;
  {
    s::buffer<float _Complex, 1> buffer_complex_mul(complex_input,
                                                    numOfMulInput);
    s::buffer<float _Complex, 1> buffer_complex_mul_res(
    ...
    deviceQueue.submit([&](s::handler &cgh) {
      ...
      auto complex_mul_res_access =
          buffer_complex_mul_res.template get_access<s::access::mode::write>(
              cgh);
      cgh.single_task<class C99DeviceComplexMulTest>([=]() {
        size_t i, j;
...
int device_complex_test_div(s::queue &deviceQueue) {
  ...
  std::array<float _Complex, OutputSize> complex_div_result;
  {
    s::buffer<float _Complex, 1> buffer_complex_div(complex_input,
                                                    numOfDivInput);
    s::buffer<float _Complex, 1> buffer_complex_div_res(
    ...
    deviceQueue.submit([&](s::handler &cgh) {
      ...
      auto complex_div_res_access =
          buffer_complex_div_res.template get_access<s::access::mode::write>(
              cgh);
      cgh.single_task<class C99DeviceComplexDivTest>([=]() {
        size_t i, j;
        for (i = 0; i < TestComplexSize; ++i) {
          for (j = 0; j < TestComplexSize; ++j)
