...
int main(){
  ...
  // Check that device type of USM allocation is supported inside free function kernel.
  {
    int* HostDataPtr =  new int[NumOfElements];
    std::fill(HostDataPtr, HostDataPtr + NumOfElements,0);
    ...
    Queue.copy(HostDataPtr,DeviceDataPtr,NumOfElements).wait();
    Queue
    ...
    Failed += performResultCheck(
        NumOfElements, HostDataPtr, "ns::nsNdRangeFreeFunc with USM device allocation", ExpectedResultValue);
      sycl::free(DeviceDataPtr, Queue);
    }

    // Check that host type of USM allocation is supported inside free function kernel.
    {
      constexpr int ExpectedResultValue = 222;
      int *DataPtr = sycl::malloc_host<int>(NumOfElements, Queue);
      std::fill(DataPtr, DataPtr + NumOfElements,0);
      ...
      Queue
          ...
          .wait();  
      Failed += performResultCheck(
          NumOfElements, DataPtr, "globalScopeSingleFreeFunc with USM host allocation", ExpectedResultValue);
      sycl::free(DataPtr, Queue);
    }

 // Check that shared type of USM allocation is supported inside free function kernel.

{
  ...
  std::fill(DataPtr, DataPtr + NumOfElements,0);
    ...
    Failed += performResultCheck(
        NumOfElements, DataPtr, "ns::nsNdRangeFreeFunc with USM device allocation", ExpectedResultValue);
      sycl::free(DataPtr, Queue);
  }
