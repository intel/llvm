...
// clang-format off
// RUN: %clangxx -fsycl -fsycl-targets=spir64,%{intel_gpu_aot_targets} %s -o %t3.out
...
int main() {
  ...
  // CHECK-ENABLED: <--- urEnqueueKernelLaunchWithArgsExp{{.*}}KERNEL_ARG_TYPE_MEM_OBJ,{{.*}}.hMem = nullptr{{.*}} -> UR_RESULT_SUCCESS;
  // CHECK-ENABLED: Default value of specialization constant was used.

  // CHECK-ENABLED: Submission 1
  // CHECK-ENABLED: <--- urEnqueueKernelLaunchWithArgsExp{{.*}}KERNEL_ARG_TYPE_MEM_OBJ,{{.*}}.hMem = {{(0x)?[0-9,a-f,A-F]+}}{{.*}}-> UR_RESULT_SUCCESS;
  ...
  // CHECK-DEFAULT-EXPLICIT-SET: <--- urEnqueueKernelLaunchWithArgsExp{{.*}}KERNEL_ARG_TYPE_MEM_OBJ,{{.*}}.hMem = nullptr{{.*}} -> UR_RESULT_SUCCESS;
  ...
  // CHECK-DEFAULT-BACK-TO-DEFAULT: <--- urEnqueueKernelLaunchWithArgsExp{{.*}}KERNEL_ARG_TYPE_MEM_OBJ,{{.*}}.hMem = nullptr{{.*}} -> UR_RESULT_SUCCESS;
