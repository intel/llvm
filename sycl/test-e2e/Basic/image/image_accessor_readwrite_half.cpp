...
void check_read_data(s::vec<s::half, 4> ReadData, s::vec<s::half, 4> ExpectedColor) {
  s::vec<float, 4> ReadDatafloat = ReadData.convert<float>();
  s::vec<float, 4> ExpectedColorfloat = ExpectedColor.convert<float>();
...
void check_half4(char *HostPtr) {
  ...
  // Calling only valid channel types with s::vec<s::half, 4>.
  // s::image_channel_type::snorm_int8,
  write_type_order<s::vec<s::half, 4>, s::image_channel_type::snorm_int8>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(2, -2, 0.375f, 0));
  check_read_type_order<s::vec<s::half, 4>, s::image_channel_type::snorm_int8>(
      HostPtr, s::image_channel_order::rgba,
      s::vec<s::half, 4>(1, -1, ((float)48 / 127) /*0.3779527544975280762f*/, 0));

  // s::image_channel_type::snorm_int16,
  write_type_order<s::vec<s::half, 4>, s::image_channel_type::snorm_int16>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(2, -2, 0.375f, 0));
  check_read_type_order<s::vec<s::half, 4>, s::image_channel_type::snorm_int16>(
      HostPtr, s::image_channel_order::rgba,
      s::vec<s::half, 4>(1, -1, ((float)12288 / 32767) /*0.375011444091796875f*/, 0));

  // s::image_channel_type::unorm_int8,
  write_type_order<s::vec<s::half, 4>, s::image_channel_type::unorm_int8>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(2, -2, 0.375f, 0));
  check_read_type_order<s::vec<s::half, 4>, s::image_channel_type::unorm_int8>(
      HostPtr, s::image_channel_order::rgba,
      s::vec<s::half, 4>(1, 0, ((float)96 / 255) /*0.3764705955982208252f*/, 0));

  // s::image_channel_type::unorm_int16
  write_type_order<s::vec<s::half, 4>, s::image_channel_type::unorm_int16>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(1, -1, 0.375f, 0));
  check_read_type_order<s::vec<s::half, 4>, s::image_channel_type::unorm_int16>(
      HostPtr, s::image_channel_order::rgba,
      s::vec<s::half, 4>(1, 0, ((float)24576 / 65535) /*0.3750057220458984375f*/, 0));

  // s::image_channel_type::fp16
  write_type_order<s::vec<s::half, 4>, s::image_channel_type::fp16>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(2, -2, 0.375f, 0));
  check_read_type_order<s::vec<s::half, 4>, s::image_channel_type::fp16>(
      HostPtr, s::image_channel_order::rgba, s::vec<s::half, 4>(2, -2, 0.375f, 0));
};
