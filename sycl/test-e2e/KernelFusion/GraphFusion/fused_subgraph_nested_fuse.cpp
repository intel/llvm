...
// REQUIRES: linux, fusion, level_zero

// Test a nested fusing of fused graph added as a subgraph.
...
// and another kernel that should be fused with the subgraph fused kernel.
...
// Note that this test has been disabled for CUDA backend because nested fusion
// is not yet supported when using the CUDA backend.
...
int main() {
  ...
  bOutFinal.set_write_back(false);
  {
    ...
    auto exec_main_graph =
        main_graph.finalize({sycl_ext::property::graph::require_fusion{}});
