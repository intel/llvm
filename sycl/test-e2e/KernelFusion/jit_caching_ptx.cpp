...
// RUN: env SYCL_RT_WARNING_LEVEL=1 env ONEAPI_DEVICE_SELECTOR=opencl:cpu;cuda:gpu %{run-unfiltered-devices} %t.out 2>&1 | FileCheck %s --implicit-check-not "COMPUTATION ERROR" --implicit-check-not "WRONG INTERNALIZATION"
