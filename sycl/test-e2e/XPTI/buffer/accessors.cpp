...
int main() {
  ...
  Queue.submit([&](sycl::handler &cgh) {
    ...
    auto A4 = Buf.get_access(cgh, sycl::write_only, sycl::no_init);
    // CHECK: {{[0-9]+}}|Construct accessor|[[BUFFERID]]|[[ACCID5:.*]]|2014|1028|{{.*}}accessors.cpp:[[# @LINE + 2]]:13
    auto A5 =
        Buf.get_access<target::device>(cgh, sycl::read_write, sycl::no_init);
  ...
  {
    auto HA = Buf.get_access(sycl::write_only, sycl::no_init);
  }
  // CHECK: {{[0-9]+}}|Construct accessor|[[BUFFERID]]|[[ACCID11:.*]]|2018|1028|{{.*}}accessors.cpp:[[# @LINE + 2]]:19
  {
    auto HA = Buf.get_access(sycl::read_write, sycl::no_init);
  }
