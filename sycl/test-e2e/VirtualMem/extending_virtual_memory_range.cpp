// This test checks whether memory accesses to contiguous virtual memory ranges are performed correctly
...
struct PhysicalMemoryMapping{
    PhysicalMemoryMapping(syclext::physical_mem&& PhysicalMem, void* MappingPtr) : MPhysicalMem(std::move(PhysicalMem)), MMappingPtr(MappingPtr){}
...
int main(){
  ...
  size_t AlignedByteSizePerRange = GetAlignedByteSize(BytesRequiredPerRange, UsedGranularity);
  ...
  for(size_t Index =0; Index<NumberOfVirtualMemoryRanges; ++Index){
    ...
    void *MappedPtr = PhysicalMem.map(VirtualMemoryPtr, AlignedByteSizePerRange, syclext::address_access_mode::read_write);
  ...
  for (size_t Index =0; Index<NumberOfVirtualMemoryRanges; ++Index){
    int* DataRangePtr = reinterpret_cast<int *>(PhysicalMemoryMappings[Index].MMappingPtr);
...

    {
      ...
      Q.submit([&](sycl::handler &Handle) {
        ...
        Handle.parallel_for(ElementsInRange,
                          [=](sycl::id<1> Idx) { A[Idx] = DataRangePtr[Idx]; });
      });
    }
    
    for (size_t i = 0; i < ElementsInRange; i++) {
      if (ResultHostData[i] != ValueSetInKernel) {
      std::cout << "Comparison failed with virtual range " << Index+1 << " at index " << i<<": "
                << ResultHostData[i] << " != " << ValueSetInKernel
                << std::endl;
      ++Failed;
      }
    } 
  ...
  for (auto PhysMemMap: PhysicalMemoryMappings){
     syclext::unmap(PhysMemMap.MMappingPtr, PhysMemMap.MPhysicalMem.size(), Context);
  }
  for (auto VirtualMemRange: VirtualMemoryRanges) {
    syclext::free_virtual_mem(VirtualMemRange.MPtr, VirtualMemRange.MSize, Context);
  }
