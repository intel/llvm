...
// RUN: %clangxx -fsycl -fsycl-targets=%{sycl_triple} %{arch_flag} -O3 %t3.o %t.a -Wno-unused-command-line-argument -o %t1.exe
// RUN: %{run} %t1.exe
...
// RUN: %clangxx -fsycl -fsycl-targets=%{sycl_triple} %{arch_flag} -O3 %t3.o %t_repacked.a -Wno-unused-command-line-argument -o %t2.exe
// RUN: %{run} %t2.exe
