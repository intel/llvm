...
// RUN: %clangxx -fsycl -fsycl-targets=%{sycl_triple} %{arch_flag} %t1.o %t2.o -Wno-unused-command-line-argument -o %t1.exe
// RUN: %{run} %t1.exe
//
...
// RUN: %clangxx -fsycl -fsycl-targets=%{sycl_triple} %{arch_flag} %t2.o %t.a -Wno-unused-command-line-argument -o %t2.exe
// RUN: %{run} %t2.exe
