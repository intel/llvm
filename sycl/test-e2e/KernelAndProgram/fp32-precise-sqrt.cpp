// RUN: %{build} -Wno-error=unused-command-line-argument -foffload-fp32-prec-sqrt -o %t_with.out
...
// RUN: env SYCL_UR_TRACE=2 %{run} %t_without.out 2>&1 | FileCheck --implicit-check-not=fp32-correctly-rounded-divide-sqrt

// CHECK-INTEL-WITH: <--- urProgramBuild
// CHECK-INTEL-WITH-SAME: fp32-correctly-rounded-divide-sqrt
