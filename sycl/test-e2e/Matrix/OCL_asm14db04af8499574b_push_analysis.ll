; ------------------------------------------------
; OCL_asm14db04af8499574b_push_analysis.ll
...
.precompiled_u32divrem_sp.exit.i_crit_edge:       ; preds = %100
  br label %precompiled_u32divrem_sp.exit.i

if.end.i.i:                                       ; preds = %100
  %101 = call float @llvm.genx.GenISA.uitof.rtz.f32.i32(i32 %11) #4
  %conv.i.i = fptoui float %101 to i32
  %sub.i.i = sub i32 %11, %conv.i.i
  %102 = call float @llvm.genx.GenISA.uitof.rtz.f32.i32(i32 %sub.i.i) #4
  %103 = call float @llvm.genx.GenISA.uitof.rtz.f32.i32(i32 %86) #4
  %conv3.i.i = fptoui float %103 to i32
  %sub4.i.i = sub i32 %86, %conv3.i.i
  %104 = call float @llvm.genx.GenISA.uitof.rtz.f32.i32(i32 %sub4.i.i) #4
  %div.i.i = fdiv float 1.000000e+00, %101, !fpmath !523
  %105 = call float @llvm.genx.GenISA.fma.rtz.f32.f32.f32.f32(float %div.i.i, float 0xBE98000000000000, float %div.i.i) #4
  %106 = call float @llvm.genx.GenISA.mul.rtz.f32.f32.f32(float %103, float %105) #4
  %conv8.i.i = fptoui float %106 to i32
  %107 = call float @llvm.genx.GenISA.uitof.rtz.f32.i32(i32 %conv8.i.i) #4
  %108 = fsub float 0.000000e+00, %101
  %109 = call float @llvm.genx.GenISA.fma.rtz.f32.f32.f32.f32(float %108, float %107, float %103) #4
  %110 = fsub float 0.000000e+00, %102
  %111 = call float @llvm.genx.GenISA.fma.rtz.f32.f32.f32.f32(float %110, float %107, float %104) #4
  %112 = call float @llvm.genx.GenISA.add.rtz.f32.f32.f32(float %109, float %111) #4
  %113 = call float @llvm.genx.GenISA.mul.rtz.f32.f32.f32(float %105, float %112) #4
  %conv16.i.i = fptoui float %113 to i32
  ...
  %114 = sext i1 %cmp.i.i to i32
  %115 = sub i32 0, %114
  %add19.i.i = add i32 %add.i.i, %115
  br label %precompiled_u32divrem_sp.exit.i
...
116:                                              ; preds = %__builtin_spriv_OpJointMatrixLoadINTEL_Accumulator_RowMajor_8x8_i32_8_global_v8i8_pi32_i32.24.exit
  %117 = lshr i32 %11, 20
  %118 = shl i32 %12, 12
  %119 = lshr i32 %11, 20
  %120 = or i32 %119, %118
  ...
  %123 = and i32 %120, 1044480
  %124 = or i32 %123, %117
  %125 = uitofp i32 %124 to float
  %126 = and i32 %11, 1048575
  %127 = uitofp i32 %126 to float
  %128 = call float @llvm.fma.f32(float %125, float 0x4130000000000000, float %127) #4
  %129 = call float @llvm.fma.f32(float %122, float 0x4270000000000000, float %128) #4
  %130 = lshr i32 %87, 8
  %131 = uitofp i32 %130 to float
  %132 = fdiv float 1.000000e+00, %129
  %133 = call float @llvm.fma.f32(float %132, float 0xBE9C000000000000, float %132) #4
  %134 = fmul float %133, %131
  %135 = call float @llvm.floor.f32(float %134) #4
  %136 = fsub float 0.000000e+00, %127
  %137 = call float @llvm.fma.f32(float %136, float %135, float %131) #4
  %138 = fmul float %125, 0xC130000000000000
  %139 = call float @llvm.fma.f32(float %138, float %135, float %137) #4
  %140 = lshr i32 %86, 20
  %141 = shl i32 %87, 12
  %142 = lshr i32 %86, 20
  %143 = or i32 %142, %141
  %144 = and i32 %143, 1044480
  %145 = or i32 %144, %140
  %146 = uitofp i32 %145 to float
  %147 = call float @llvm.fma.f32(float %139, float 0x4130000000000000, float %146) #4
  %148 = fmul float %133, %147
  %149 = call float @llvm.floor.f32(float %148) #4
  %150 = fsub float 0.000000e+00, %125
  %151 = call float @llvm.fma.f32(float %150, float %149, float %139) #4
  %152 = fmul float %122, 0xC130000000000000
  %153 = call float @llvm.fma.f32(float %152, float %149, float %151) #4
  %154 = fmul float %127, 0x3EB0000000000000
  %155 = fmul float %154, %149
  %156 = call float @llvm.floor.f32(float %155) #4
  %157 = fsub float 0.000000e+00, %156
  %158 = call float @llvm.fma.f32(float %149, float %154, float %157) #4
  %159 = call float @llvm.fma.f32(float %156, float -1.000000e+00, float %153) #4
  %160 = call float @llvm.fma.f32(float %158, float 0xC130000000000000, float %146) #4
  %161 = and i32 %86, 1048575
  %162 = uitofp i32 %161 to float
  %163 = call float @llvm.fma.f32(float %160, float 0x4130000000000000, float %162) #4
  %164 = call float @llvm.fma.f32(float %159, float 0x4270000000000000, float %163) #4
  %165 = fmul float %133, %164
  %166 = call float @llvm.floor.f32(float %165) #4
  %167 = fsub float 0.000000e+00, %122
  %168 = call float @llvm.fma.f32(float %167, float %166, float %159) #4
  %169 = fmul float %154, %166
  %170 = call float @llvm.floor.f32(float %169) #4
  %171 = fsub float 0.000000e+00, %170
  %172 = call float @llvm.fma.f32(float %166, float %154, float %171) #4
  %173 = call float @llvm.fma.f32(float %170, float -1.000000e+00, float %160) #4
  %174 = call float @llvm.fma.f32(float %172, float 0xC130000000000000, float %162) #4
  %175 = fmul float %125, 0x3EB0000000000000
  %176 = fmul float %175, %166
  %177 = call float @llvm.floor.f32(float %176) #4
  %178 = fsub float 0.000000e+00, %177
  %179 = call float @llvm.fma.f32(float %166, float %175, float %178) #4
  %180 = call float @llvm.fma.f32(float %177, float -1.000000e+00, float %168) #4
  %181 = call float @llvm.fma.f32(float %179, float 0xC130000000000000, float %173) #4
  %182 = fptosi float %135 to i32
  %183 = fptosi float %149 to i32
  %184 = fptosi float %166 to i32
  %185 = call float @llvm.fma.f32(float %180, float 0x4130000000000000, float %181) #4
  %186 = call float @llvm.fma.f32(float %185, float 0x4130000000000000, float %174) #4
  %187 = fmul float %133, %186
  %188 = call float @llvm.floor.f32(float %187) #4
  %189 = fsub float 0.000000e+00, %188
  %190 = call float @llvm.fma.f32(float %189, float %122, float %180) #4
  %191 = call float @llvm.fma.f32(float %189, float %125, float %181) #4
  %192 = call float @llvm.fma.f32(float %189, float %127, float %174) #4
  %193 = fptosi float %188 to i32
  %194 = add i32 %193, %184
  ...
  %201 = or i32 %200, %199
  %202 = ashr i32 %196, 31
  %203 = fmul float %190, 0x3DF0000000000000
  %204 = call float @llvm.trunc.f32(float %203)
  %205 = call float @llvm.fma.f32(float %204, float 0xC1F0000000000000, float %190)
  %206 = fptoui float %205 to i32
  %207 = shl i32 %206, 8
  %208 = call { i32, i32 } @llvm.genx.GenISA.add.pair(i32 0, i32 %207, i32 %196, i32 %202)
  %209 = extractvalue { i32, i32 } %208, 0
  %210 = extractvalue { i32, i32 } %208, 1
  %211 = call { i32, i32 } @llvm.genx.GenISA.add.pair(i32 %209, i32 %210, i32 %198, i32 %201)
  %212 = extractvalue { i32, i32 } %211, 0
  %213 = extractvalue { i32, i32 } %211, 1
  %214 = shl i32 %182, 8
  %215 = shl i32 %183, 20
  %216 = lshr i32 %183, 12
  %217 = call { i32, i32 } @llvm.genx.GenISA.add.pair(i32 %215, i32 %216, i32 0, i32 %214)
  %218 = extractvalue { i32, i32 } %217, 0
  %219 = extractvalue { i32, i32 } %217, 1
  %220 = icmp uge i32 %212, %11
  %221 = icmp eq i32 %213, %12
  %222 = and i1 %221, %220
  %223 = icmp ugt i32 %213, %12
  %224 = or i1 %222, %223
  %225 = sext i1 %224 to i32
  %226 = sub i32 0, %225
  %227 = add i32 %194, %226
  %228 = call { i32, i32 } @llvm.genx.GenISA.add.pair(i32 %218, i32 %219, i32 %227, i32 0)
  %229 = extractvalue { i32, i32 } %228, 0
  br label %__igcbuiltin_u64_udiv_sp.exit

__igcbuiltin_u64_udiv_sp.exit:                    ; preds = %precompiled_u32divrem_sp.exit.i, %116
  %230 = phi i32 [ %retval.0.i.i, %precompiled_u32divrem_sp.exit.i ], [ %229, %116 ]
  %231 = shl i32 %230, 3
  %232 = shl i32 %231, 2
  %233 = add i32 %97, %232
