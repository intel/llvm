...
// RUN: %{build} -o %t_gpu_ocl_vnni.out -ffp-model=precise -DSLM -DOCL -DVNNI 
// RUN: %{run} %t_gpu_ocl_vnni.out

// RUN: %{build} -o %t_gpu_ocl.out -ffp-model=precise -DSLM -DOCL
// RUN: %{run} %t_gpu_ocl.out
