...
void matrix_multiply(big_matrix<T1, M, N> &C, big_matrix<T2, M, K> &A,
                     ...
                     big_matrix<T2, M, N> &AC) {
  size_t NDRangeM = M / TM;
  size_t NDRangeN = N / TN;
  buffer<bfloat16, 2> bufA(A.get_data(), range<2>(M, K));
  buffer<bfloat16, 2> bufB(B.get_data(), range<2>(K, N));
  buffer<float, 2> bufC((float *)C.get_data(), range<2>(M, N));
  buffer<bfloat16, 2> bufAC((bfloat16 *)AC.get_data(), range<2>(M, N));
  ...
  q.submit([&](handler &cgh) {
     ...
     auto accAC = bufAC.get_access<access::mode::write>(cgh);
     ...
     cgh.parallel_for<class imatrix>(
...

         {
           ...
           auto wi_slice_ac =
               sycl::ext::intel::experimental::matrix::get_wi_data(sg, sub_ac);
           for (int i = 0; i < wi_slice_c.length(); i++) {
             wi_slice_ac[i] = (bfloat16)wi_slice_c[i];
           }
           ext::intel::experimental::matrix::joint_matrix_store(
               sg, sub_ac,
               accAC.template get_multi_ptr<access::decorated::no>() +
                   (sg_startx * TM) * N + sg_starty / SG_SZ * TN,
               N);
         }); // parallel for
   }).wait();
...
int main() {
  ...
  big_matrix<bfloat16, MATRIX_M, MATRIX_N> MAC((bfloat16 *)&AC);
  big_matrix<bfloat16, MATRIX_K / 2, MATRIX_N * 2> MB((bfloat16 *)&B);
  matrix_multiply(MC, MA, MB, MAC);
  ...
  for (int i = 0; i < MATRIX_M; i++) {
    for (int j = 0; j < MATRIX_N; j++) {
      if (fabs(AC[i][j] - (bfloat16)D[i][j]) > BF16_EPSILON) {
        res = false;
      }
    }
  }
