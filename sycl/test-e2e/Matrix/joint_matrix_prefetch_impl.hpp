...
void joint_matrix_gemm_vnni(sub_group sg, size_t sg_startx, size_t sg_starty,
  ...
  joint_matrix<sub_group, T2, use::b, TK, TN, layout::ext_intel_packed> sub_b;
  joint_matrix<sub_group, T, use::accumulator, TM, TN> sub_c;
  joint_matrix_prefetch(sg, A + (sg_startx * TM) * K, K, layout::row_major, TM,
                        TK, syclex::properties{syclex::prefetch_hint_L1});
  joint_matrix_prefetch(sg, B + sg_starty / SG_SZ * TN * vnniFactor,
                        N * vnniFactor, layout::ext_intel_packed,
                        TK / vnniFactor, TN * vnniFactor,
                        syclex::properties{syclex::prefetch_hint_L1});
  joint_matrix_prefetch(sg, C + (sg_startx * TM) * N + sg_starty / SG_SZ * TN,
...
template <typename T1, typename T2, typename T, size_t M, size_t N, size_t K>
void joint_matrix_gemm(sub_group sg, size_t sg_startx, size_t sg_starty, T1 *A,
                       T2 *B, T *C) {
  auto pA = address_space_cast<sycl::access::address_space::global_space,
                               access::decorated::no>(A);
  auto pB = address_space_cast<sycl::access::address_space::global_space,
  ...
  joint_matrix<sub_group, T1, use::a, TM, TK, layout::row_major> sub_a;
  joint_matrix<sub_group, T2, use::b, TK, TN, layout::row_major> sub_b;
  joint_matrix<sub_group, T, use::accumulator, TM, TN> sub_c;
  joint_matrix_prefetch(sg, A + (sg_startx * TM) * K, K, layout::row_major, TM,
                        TK, syclex::properties{syclex::prefetch_hint_L1});
  joint_matrix_prefetch(sg, B + sg_starty / SG_SZ * TN, N, layout::row_major,
                        TK, TN, syclex::properties{syclex::prefetch_hint_L2});
  joint_matrix_prefetch(sg, C + (sg_startx * TM) * N + sg_starty / SG_SZ * TN,
                        N, layout::row_major, TM, TN,
                        syclex::properties{syclex::prefetch_hint_L1});
  joint_matrix_fill(sg, sub_c, 1);
  for (int k = 0; k < K; k += TK) {
    joint_matrix_load(sg, sub_a, pA + (sg_startx * TM) * K + k, K);
    joint_matrix_load(sg, sub_b, pB + k * N + sg_starty / SG_SZ * TN, N);
    joint_matrix_mad(sg, sub_c, sub_a, sub_b, sub_c);
  }
  joint_matrix_store(sg, sub_c,
                     pC + (sg_startx * TM) * N + sg_starty / SG_SZ * TN, N,
     ...
     cgh.parallel_for(
...

         {
           ...
           if constexpr (vnniFactor > 1)
             joint_matrix_gemm_vnni<T1, T2, T, M, N, K, vnniFactor>(
                 sg, sg_startx, sg_starty, A, B, C);
           if constexpr (vnniFactor == 1)
             joint_matrix_gemm<T1, T2, T, M, N, K>(sg, sg_startx, sg_starty, A,
                                                   B, C);
         }); // parallel for
   }).wait();
}
...
int main() {
  ...
  if (support_p) {
    ...
    matrix_multiply<float, bfloat16, bfloat16, M, N, K, 1>(C, A, B, q);

    bool res = matrix_compare(M, N, C, D);

    matrix_multiply<float, bfloat16, bfloat16, M, N, K, 2>(C, A, vnniB, q);

    bool vres = matrix_compare(M, N, C, D);
    std::cout << ((res && vres) ? "passed" : "failed") << std::endl;
    return !res;
  }
  return 0;
}
