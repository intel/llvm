  ...
  q.submit([&](handler &cgh)
           {
             ...
             cgh.parallel_for(
                 ...
                 [=](nd_item<2> spmd_item) [[intel::reqd_sub_group_size(SG_SZ)]]
                 {
                   ...
                   joint_matrix<sub_group, bfloat16, use::a, TM, TK, layout::row_major> sub_a[JM_ARRAY_SZ];

                   // For B, we assume B has been already VNNIed.
                   joint_matrix<sub_group, bfloat16, use::b, TK, TN,
                                ext::intel::experimental::matrix::layout::packed>
                       sub_b;
                   joint_matrix<sub_group, float, use::accumulator, TM, TN> sub_c[JM_ARRAY_SZ];
                   ...
                   for (int k = 0; k < K / TK; ++k)
                   {
                     ...
                     for (int i = 0; i < JM_ARRAY_SZ; ++i)
                     {
                       joint_matrix_load(sg, sub_a[i],
                                         accA.get_pointer() +
                                             (sg_startx * TM * JM_ARRAY_SZ + TM * i) * K +
                                             k * TK,
                                         K);
