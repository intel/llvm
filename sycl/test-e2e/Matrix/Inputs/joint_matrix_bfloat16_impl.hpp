...
void test() {
  ...
  if ((std::is_same_v<T, bfloat16> &&
       (std::is_same_v<TAcc, bfloat16> || std::is_same_v<TResult, bfloat16>))) {
    matrix_rand<T>(MATRIX_M, MATRIX_K, (T*)A, T(1));
    matrix_rand<T>(MATRIX_K / 2, MATRIX_N * 2, (T*)B, T(1));


    // matrix_fill(MATRIX_M, MATRIX_K, (T *)A,
    //             [](int i, int j) { return (i + j) % 3 - 1; });
    // matrix_fill(MATRIX_K / 2, MATRIX_N * 2, (T *)B,
    //             [](int i, int j) { return (i * j) % 3 - 1; });


    // matrix_fill(MATRIX_M, MATRIX_K, (T *)A,
    //             [](int i, int j) { return T(1) * i + T(2) * j; });
    // matrix_fill(MATRIX_K / 2, MATRIX_N * 2, (T *)B, [](int i, int j) {
    //   if (i % 2 == 0) {
    //     if (j % 2 == 0) {
    //       return T(1) * (i * 2) * MATRIX_N + T(1) * j / 2 + 1;
    //     }
    //     else {
    //       return T(1) * (i * 2 + 1) * MATRIX_N + T(1) * ((j - 1) / 2) + 1;
    //     }
    //   }
    //   else {
    //     if (j % 2 == 0) {
    ...
    //     }
    //     else {
    //       return T(1) * (i * 2 + 1) * MATRIX_N + T(1) * (j / 2) + 1;
    //     }
    //   }
    // });
  } else {
  ...
  matrix_multiply_ref<T, T, TResult, 2>((T *)A, (T *)B, (TResult *)D, MATRIX_M,
                                        MATRIX_N, MATRIX_K / 2);
  matrix_compare(MATRIX_M, MATRIX_N, (TAcc *)C, (TResult *)D);
}
...
int main() {
  ...
  for (unsigned int i = 0; i < combinations.size(); i++) {
    ...
    if (combinations[i].nsize == 16) { // architecture::intel_gpu_pvc
      ...
      // These combinations are not currently supported for sub group size = 32
      // in IGC
      ...
      // 1x64x16
      test<bfloat16, bfloat16, float, /*TM*/ 1, /*TN*/ 64, /*TK*/ 16>(); // PASS
      test<bfloat16, float,    bfloat16, /*TM*/ 1, /*TN*/ 64, /*TK*/ 16>(); // PASS
      test<bfloat16, bfloat16, bfloat16, /*TM*/ 1, /*TN*/ 64, /*TK*/ 16>(); // PASS

      // 1x64x32
      test<bfloat16, bfloat16, float, /*TM*/ 1, /*TN*/ 64, /*TK*/ 32>(); // PASS 
      test<bfloat16, float,    bfloat16, /*TM*/ 1, /*TN*/ 64, /*TK*/ 32>(); // PASS
      test<bfloat16, bfloat16, bfloat16, /*TM*/ 1, /*TN*/ 64, /*TK*/ 32>(); // PASS 

      // 32x64x16
      test<bfloat16, bfloat16, float, /*TM*/ 32, /*TN*/ 64, /*TK*/ 16>(); // VAL DIFF 
      test<bfloat16, float,    bfloat16, /*TM*/ 32, /*TN*/ 64, /*TK*/ 16>(); // BUILTIN
      test<bfloat16, bfloat16, bfloat16, /*TM*/ 32, /*TN*/ 64, /*TK*/ 16>(); // VAL DIFF

      // 32x64x32
      test<bfloat16, bfloat16, float, /*TM*/ 32, /*TN*/ 64, /*TK*/ 32>(); // VAL DIFF
      test<bfloat16, float,    bfloat16, /*TM*/ 32, /*TN*/ 64, /*TK*/ 32>(); // PASS
      test<bfloat16, bfloat16, bfloat16, /*TM*/ 32, /*TN*/ 64, /*TK*/ 32>(); // VAL DIFF
#endif
#endif
      break;
    }
