...
int main() {
  ...
  assert(actual_combinations.size() == expected_combinations.size() && "Number of combinations is not equal.");

  for (int i = 0; i < actual_combinations.size(); i++) {
    assert(compare(actual_combinations[i], expected_combinations[i]) && "Some values in matrix runtime query for PVC are not expected.");
  }
