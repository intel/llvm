...
ESIMD_CALLEE_vadd(esimd::simd<float, VL> a,
                  esimd::simd<float, VL> b) SYCL_ESIMD_FUNCTION {
  return a + b;
}

__attribute__((always_inline)) esimd::simd<float, VL>
ESIMD_CALLEE_vsub(esimd::simd<float, VL> a,
                  esimd::simd<float, VL> b) SYCL_ESIMD_FUNCTION {
  return a - b;
}

[[intel::device_indirectly_callable]] SYCL_EXTERNAL
    simd<float, VL> __regcall SIMD_CALLEE_vadd(
        simd<float, VL> a, simd<float, VL> b) SYCL_ESIMD_FUNCTION;

[[intel::device_indirectly_callable]] SYCL_EXTERNAL
    simd<float, VL> __regcall SIMD_CALLEE_vsub(
        simd<float, VL> a, simd<float, VL> b) SYCL_ESIMD_FUNCTION;
...
int main(void) {
  ...
  /* Kernel 1: This kernel does the following vector addition: C = A + B. */
  try {
    auto e = q.submit([&](handler &cgh) {
      cgh.parallel_for<class Test1>(Range, [=](nd_item<1> ndi) SUBGROUP_ATTR {
        ...
        if constexpr (use_invoke_simd) {
          res = invoke_simd(sg, SIMD_CALLEE_vadd, A[wi_id], B[wi_id]);
        } else {
  ...
  /* Kernel 2: In this second kernel, we do a vector subtraction of A from C
   * (remember that C contains the sum of vector addition from the first kernel:
   * A + B). Effectively, this program computes C = (A + B) - B, or C = A. */
  try {
    auto e = q.submit([&](handler &cgh) {
      cgh.parallel_for<class Test2>(Range, [=](nd_item<1> ndi) SUBGROUP_ATTR {
        ...
        if constexpr (use_invoke_simd) {
          res = invoke_simd(sg, SIMD_CALLEE_vsub, C[wi_id], B[wi_id]);
        } else {
...
[[intel::device_indirectly_callable]] SYCL_EXTERNAL
    simd<float, VL> __regcall SIMD_CALLEE_vadd(
        simd<float, VL> a, simd<float, VL> b) SYCL_ESIMD_FUNCTION {
  esimd::simd<float, VL> res = ESIMD_CALLEE_vadd(a, b);
  return res;
}
...
[[intel::device_indirectly_callable]] SYCL_EXTERNAL
    simd<float, VL> __regcall SIMD_CALLEE_vsub(
        simd<float, VL> a, simd<float, VL> b) SYCL_ESIMD_FUNCTION {
  esimd::simd<float, VL> res = ESIMD_CALLEE_vsub(a, b);
  return res;
}
