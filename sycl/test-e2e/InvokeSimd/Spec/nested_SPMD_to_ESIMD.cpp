...
/* Tests nested SPMD-to-ESIMD functionality, specifically, if invoke_simd()
 * can be called from within an SPMD function nested inside the orginal SPMD
...
/* Tests nested SPMD-to-ESIMD functionality. This function is called from
 * the normal SPMD context of the kernel, and it, in turn, calls the
 * invoke_simd() function. This serves to test if invoke_simd() can be
 * called from within an SPMD function nested inside the orginal SPMD
 * context of the kernel. */
float nested_SPMD_to_ESIMD(sycl::sub_group sg, float va, float vb) {
  // Invoke SIMD function:
  // va values from each work-item are combined into a simd<float, VL>.
  // vb values from each work-item are combined into a simd<float, VL>.
