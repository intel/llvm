    ...
    auto e = q.submit([&](handler &cgh) {
      ...
      cgh.parallel_for<TestID<Size, VL, use_invoke_simd, CaseNum>>(
          ...
          [=](nd_item<1> item) SUBGROUP_ATTR {
            ...
            float va = sg.load(PA.get_multi_ptr<access::decorated::no>().get() + offset);
            float vb = sg.load(PB.get_multi_ptr<access::decorated::no>().get() + offset);
            float vp = sg.load(PP.get_multi_ptr<access::decorated::no>().get() + offset);
            float vq = sg.load(PQ.get_multi_ptr<access::decorated::no>().get() + offset);
            float vr = sg.load(PR.get_multi_ptr<access::decorated::no>().get() + offset);
            float vx = sg.load(PX.get_multi_ptr<access::decorated::no>().get() + offset);
            float vy = sg.load(PY.get_multi_ptr<access::decorated::no>().get() + offset);
            float vz = sg.load(PZ.get_multi_ptr<access::decorated::no>().get() + offset);
            ...
            sg.store(PC.get_multi_ptr<access::decorated::no>().get() + offset, vc);
          });
    });
    e.wait();
