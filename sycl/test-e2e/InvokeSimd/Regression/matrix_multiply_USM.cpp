 ...
 *
 * This test is mostly the same as matrix_multiply_accessor_get_pointer.cpp, but
 * was modified to use USM instead of buffers/accessors and now it passes.
 *
...
ESIMD_CALLEE_computeDotProducts(esimd::simd<int, VL> row_indices,
                                esimd::simd<int, VL> column_indices, int *X,
                                int *Y) SYCL_ESIMD_FUNCTION {
  ...
  for (int i = 0; i < VL; ++i) {
    ...
    for (int k = 0; k < N; ++k) {
      dot_product += X[row_index * N + k] * Y[(k * N) + column_index];
    }
...
[[intel::device_indirectly_callable]] SYCL_EXTERNAL
    simd<int, VL> __regcall SIMD_CALLEE_computeDotProducts(
        simd<int, VL> row_indices, simd<int, VL> column_indices, int *X,
        int *Y) SYCL_ESIMD_FUNCTION;
...
int main(void) {
  ...
  int *X = static_cast<int *>(malloc_shared(N * N * sizeof(int), dev, ctxt));
  int *Y = static_cast<int *>(malloc_shared(N * N * sizeof(int), dev, ctxt));
  int *Z = static_cast<int *>(malloc_shared(N * N * sizeof(int), dev, ctxt));
        ...
        h.parallel_for<class Test>(
            nd_range{global, local}, [=](nd_item<2> ndi) SUBGROUP_ATTR {
              ...
              if constexpr (use_invoke_simd) {
                int res = invoke_simd(ndi.get_sub_group(),
                                      SIMD_CALLEE_computeDotProducts, row_index,
                                      column_index, uniform{X}, uniform{Y});
                Z[row_index * N + column_index] = res;
              } else {
                for (int k = 0; k < N; ++k) {
  ...
  for (int j = 0; j < N; ++j) {
    for (int i = 0; i < N; ++i) {
      float gold = 0;
      for (int k = 0; k < N; ++k) {
        gold += X[j * N + k] * Y[k * N + i];
      }
      if (std::abs(gold - Z[j * N + i]) / gold > 1.0E-06) {
        passed = false;
      }
    }
  }

  // Debug: Print out matrices.
  if (!passed) {
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < N; j++) {
        std::cout << Z[i * N + j] << ' ';
...
SIMD_CALLEE_computeDotProducts(simd<int, VL> row_indices,
                               simd<int, VL> column_indices, int *X,
                               int *Y) SYCL_ESIMD_FUNCTION {
  esimd::simd<int, VL> res =
      ESIMD_CALLEE_computeDotProducts(row_indices, column_indices, X, Y);
  return res;
}
