...
// Single test instance consisting of (i) calling malloc function `f` (ii) checking
// alignment
// Note that the annotated_ptr is not freed in alignment tests to avoid the same base
// address being returned every time
...
void testUsmKind(sycl::queue &q) {
  ...
  if (dev.has(sycl::aspect::usm_shared_allocations)) {
    TEST_USM_KIND(malloc_annotated, alloc::shared, N, q, properties{usm_kind_shared});
    TEST_USM_KIND(malloc_annotated<T>, alloc::shared, N, q, properties{usm_kind_shared});
