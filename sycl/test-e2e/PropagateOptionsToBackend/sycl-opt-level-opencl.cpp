...
// RUN: %if !acc %{ env SYCL_UR_TRACE=1 %{run} %t1.out 2>&1 | FileCheck %s --check-prefixes=CHECKOCL1 %}
// RUN: %{build} -O2 -o %t2.out
// RUN: %if !acc %{ env SYCL_UR_TRACE=1 %{run} %t2.out 2>&1 | FileCheck %s --check-prefixes=CHECKOCL2 %}
// RUN: %{build} -O3 -o %t3.out
// RUN: %if !acc %{ env SYCL_UR_TRACE=1 %{run} %t3.out 2>&1 | FileCheck %s --check-prefixes=CHECKOCL3 %}
