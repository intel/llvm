#ifndef JOINT_REDUCE_HPP
#define JOINT_REDUCE_HPP
...
bool joint_reduce(sycl::queue q, T input, BinaryOperation binary_op) {
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      *output_with_init = sycl::ext::oneapi::experimental::joint_reduce(g, in, in + N, init, binary_op);
      *output_without_init = sycl::ext::oneapi::experimental::joint_reduce(g, in, in + N, binary_op);
    });
  });
...
bool test_scalar_joint_reduce() {
  ...
  if (is_type_supported<T>(q)) {
    for (const auto &test_case : test_cases) {
      result &= joint_reduce(q, test_case, binary_op);
    }
  }
