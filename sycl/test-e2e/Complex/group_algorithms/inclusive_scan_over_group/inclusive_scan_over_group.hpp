#ifndef INCLUSIVE_SCAN_OVER_GROUP_HPP
#define INCLUSIVE_SCAN_OVER_GROUP_HPP
...
bool inclusive_scan_over_group(sycl::queue q, T input,
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      output_with_init[lid] = sycl::ext::oneapi::experimental::inclusive_scan_over_group(g, in[gid], binary_op, init);
      output_without_init[lid] = sycl::ext::oneapi::experimental::inclusive_scan_over_group(g, in[gid], binary_op);
    });
  });
  ...
  std::inclusive_scan(input.begin(), input.end(), expected.begin(), binary_op);
...
bool test_scalar_inclusive_scan_over_group() {
  ...
  if (is_type_supported<T>(q)) {
    for (const auto &test_case : test_cases) {
      result &= inclusive_scan_over_group(q, test_case, binary_op);
    }
  }
...
template <typename T, std::size_t N, typename BinaryOperation>
bool test_marray_inclusive_scan_over_group() {
  using Complex = typename sycl::ext::oneapi::experimental::complex<T>;
