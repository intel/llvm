...
bool reduce_over_group(sycl::queue q, T input, BinaryOperation binary_op) {
  ...
  const auto init = sycl::ext::oneapi::experimental::cplx::detail::get_init<
      V, BinaryOperation>();
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      *output_with_init = sycl::ext::oneapi::experimental::reduce_over_group(
          g, in[gid], init, binary_op);
      *output_without_init = sycl::ext::oneapi::experimental::reduce_over_group(
          g, in[gid], binary_op);
    });
  });
