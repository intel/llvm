#ifndef REDUCE_OVER_GROUP_HPP
#define REDUCE_OVER_GROUP_HPP
...
bool reduce_over_group(sycl::queue q, T input, BinaryOperation binary_op) {
  ...
  const auto init = sycl::ext::oneapi::experimental::cplx::detail::get_init<V, BinaryOperation>();
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      *output_with_init = sycl::ext::oneapi::experimental::reduce_over_group(g, in[gid], init, binary_op);
      *output_without_init = sycl::ext::oneapi::experimental::reduce_over_group(g, in[gid], binary_op);
    });
  });

  q.wait();

  const auto expected = std::reduce(input.begin(), input.end(), init, binary_op);

  result &= check_results(*output_with_init, expected, true);
  result &= check_results(*output_without_init, expected, true);
...
bool test_scalar_reduce_over_group() {
  ...
  if (is_type_supported<T>(q)) {
    for (const auto &test_case : test_cases) {
      result &= reduce_over_group(q, test_case, binary_op);
    }
  }
...
template <typename T, std::size_t N, typename BinaryOperation>
bool test_marray_reduce_over_group() {
  using Complex = typename sycl::ext::oneapi::experimental::complex<T>;
