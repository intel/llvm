#ifndef JOINT_INCLUSIVE_SCAN_HPP
#define JOINT_INCLUSIVE_SCAN_HPP
...
bool test_scalar_joint_inclusive_scan() {
  ...
  if (is_type_supported<T>(q)) {
    for (const auto &test_case : test_cases) {
      result &= joint_inclusive_scan(q, test_case, binary_op);
    }
  }
...
template <typename T, std::size_t N, typename BinaryOperation>
bool test_marray_joint_inclusive_scan() {
  using Complex = typename sycl::ext::oneapi::experimental::complex<T>;
