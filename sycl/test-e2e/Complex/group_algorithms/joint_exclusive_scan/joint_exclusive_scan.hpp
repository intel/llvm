#ifndef JOINT_EXCLUSIVE_SCAN_HPP
#define JOINT_EXCLUSIVE_SCAN_HPP
...
bool joint_exclusive_scan(sycl::queue q, T input,
  ...
  q.submit([&](sycl::handler &cgh) {
    cgh.parallel_for(sycl::nd_range<1>(N, N), [=](sycl::nd_item<1> it) {
      ...
      sycl::ext::oneapi::experimental::joint_exclusive_scan(g, in, in + N, output_with_init,
                                            init, binary_op);
      sycl::ext::oneapi::experimental::joint_exclusive_scan(g, in, in + N, output_without_init,
                                            binary_op);
    });
  });
...
bool test_scalar_joint_exclusive_scan() {
  ...
  if (is_type_supported<T>(q)) {
    for (const auto &test_case : test_cases) {
      result &= joint_exclusive_scan(q, test_case, binary_op);
    }
  }
...
template <typename T, std::size_t N, typename BinaryOperation>
bool test_marray_joint_exclusive_scan() {
  using Complex = typename sycl::ext::oneapi::experimental::complex<T>;
