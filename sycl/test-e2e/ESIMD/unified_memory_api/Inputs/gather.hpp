...
bool testUSM(queue Q, uint32_t MaskStride, PropertiesT Props) {

  static_assert(VS > 0 && N % VS == 0,
                "Incorrect VS parameter. N must be divisible by VS.");
  constexpr int NOffsets = N / VS;
#if 0 // !! TODO: check if this restriction is actual:  
  if constexpr (VS > 1) {
    static_assert(NOffsets == 16 || NOffsets == 32,
  ...
  try {
    Q.parallel_for(Range, [=](sycl::nd_item<1> NDI) SYCL_ESIMD_KERNEL {
...
#else
       //simd_mask<NOffsets> Pred = unpack_mask<32>(0x49249249);
       simd_mask<NOffsets> Pred = unpack_mask<32>(0xffffffff - 2);
#endif
       ...
       if constexpr (VS > 1) { // VS > 1 requires specifying <T, N, VS>
         ...
         // if (VS == 1) then <T, N, VS> can be omitted - test it here.
         // The variants accepting simd_view for 'PassThru' operand though
         // still require <T, N> to be specified explicitly because 'N' cannot
         // be auto-deduced.
         if constexpr (UsePassThru) {
           if constexpr (UseProperties) {
             if (GlobalID % 4 == 0) // ByteOffset - simd, PassThru - simd
...
template <typename T, bool TestPVCFeatures> bool testUSM(queue Q) {
  ...
  if constexpr (TestPVCFeatures) {
    properties PVCProps{cache_hint_L1<cache_hint::streaming>,
                        ...
                        alignment<sizeof(T)>};
    Passed &=
        testUSM<T, 1, 1, !UseMask, !UsePassThru, UseProperties>(Q, 2, PVCProps);
    Passed &=
        testUSM<T, 2, 1, UseMask, !UsePassThru, UseProperties>(Q, 2, PVCProps);
    Passed &=
        testUSM<T, 4, 1, UseMask, UsePassThru, UseProperties>(Q, 2, PVCProps);
    Passed &=
        testUSM<T, 8, 1, UseMask, UsePassThru, UseProperties>(Q, 3, PVCProps);

    Passed &=
        testUSM<T, 32, 1, UseMask, UsePassThru, UseProperties>(Q, 2, PVCProps);

    // Check VS > 1
    // TODO: calling this for bytes/shorts causes SegFault during JIT
    // compilation. Enable it for bytes/shorts after the issue is resolved.
    if constexpr (sizeof(T) >= 4) {
      Passed &= testUSM<T, 32, 2, !UseMask, !UsePassThru, UseProperties>(
          Q, 3, AlignElemProps);

      // TODO: This test case causes flaky fail of it and all consequent cases.
