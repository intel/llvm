...
bool testSLM(queue Q, uint32_t MaskStride, PropertiesT) {
  ...
  try {
    Q.parallel_for(Range, [=](sycl::nd_item<1> NDI) SYCL_ESIMD_KERNEL {
       ...
       constexpr uint32_t SLMSize = (Threads * N + 16) * sizeof(T);
       slm_init<SLMSize>();

       if (LocalID == 0) {
         for (int I = 0; I < Threads * N; I += 16) {
           simd<T, 16> InVec(In + GlobalElemOffset + I);
           simd<uint32_t, 16> offsets(I * sizeof(T), sizeof(T));
