//==-- lsc_local_accessor_gather_scatter.cpp - DPC++ ESIMD on-device test -==//
//
  ...
  try {
    ...
    q.submit([&](handler &cgh) {
       ...
       cgh.parallel_for(glob_range, [=](nd_item<1> ndi) SYCL_ESIMD_KERNEL {
         ...
         lsc_scatter<T>(LocalAcc, offsets, valsIn, pred);

         simd<T, VL> valsOut = lsc_gather<T>(LocalAcc, offsets);
