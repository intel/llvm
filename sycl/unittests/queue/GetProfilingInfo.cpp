//==------------------ GetProfilingInfo.cpp --- unit tests for check if needed exception is thrown 
// when get_profiling_info() called without queue::enable_profiling property in queue property list -----------------==//
//
...
template <typename T>
sycl::unittest::PiImage generateTestImage() {
...
static pi_result redefinedPiEventGetProfilingInfo(pi_event event,
...
TEST(GetProfilingInfo, normal_pass_without_exception) {
    ...
    Mock.redefine<sycl::detail::PiApiKind::piEventGetProfilingInfo>(redefinedPiEventGetProfilingInfo);
    const sycl::device Dev = Plt.get_devices()[0];
    static sycl::unittest::PiImage DevImage_1 = generateTestImage<InfoTestKernel>();

    static sycl::unittest::PiImageArray<1> DevImageArray = {&DevImage_1};
    auto KernelID_1 = sycl::get_kernel_id<InfoTestKernel>();
    sycl::queue Queue{Dev, sycl::property_list{sycl::property::queue::enable_profiling{}}};
    ...
    EXPECT_NO_THROW({
        ...
        (void)submit_time; (void)start_time; (void)end_time;
    });
}
...
TEST(GetProfilingInfo, command_exception_check) {
    ...
    { 
        ...
        try {
            ...
            EXPECT_STREQ(e.what(), "get_profiling_info() can't be used without set 'enable_profiling' queue property");
        }
    }
    { 
        sycl::queue Queue{Dev};
