...
void checkGenericExplicitConstConversions() {
  ...
  source_t Source =
      makePtr<const Record, address_space::generic_space, SourceDecoration>(
          &Value);
...
void checkNonLegacyConstSameSpaceConversions() {
  ...
  typed_legacy_t FromConstVoidLegacy =
      static_cast<typed_legacy_t>(ConstVoidLegacy);
...
template <address_space Space> void checkLegacyConversions() {
  ...
  EXPECT_EQ(ConstVoidFromVoid.get_raw(),
            static_cast<const void *>(&MutableValue));
...
void checkDefaultLegacyMakePtr() {
  int Value = 88;
  auto LegacyPtr = sycl::make_ptr<int, address_space::private_space>(&Value);
  EXPECT_TRUE(
      (std::is_same_v<
          decltype(LegacyPtr),
