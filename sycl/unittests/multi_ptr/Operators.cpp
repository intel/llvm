...
template <decorated IsDecorated> void checkOperatorTraits() {
  ...
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>()[diff_t{0}]),
                      typename ptr_t::reference>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>().operator->()),
  ...
  EXPECT_TRUE((std::is_same_v<decltype(std::declval<ptr_t &>()--), ptr_t>));
  EXPECT_TRUE((
      std::is_same_v<decltype(std::declval<ptr_t &>() += diff_t{0}), ptr_t &>));
  EXPECT_TRUE((
      std::is_same_v<decltype(std::declval<ptr_t &>() -= diff_t{0}), ptr_t &>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>() + diff_t{0}),
  ...
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>() == nullptr),
                      bool>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>() != nullptr),
                      bool>));
  EXPECT_TRUE((
      std::is_same_v<decltype(std::declval<const ptr_t &>() < nullptr), bool>));
  EXPECT_TRUE((
      std::is_same_v<decltype(std::declval<const ptr_t &>() > nullptr), bool>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>() <= nullptr),
  ...
  EXPECT_TRUE(
      (std::is_same_v<decltype(std::declval<const ptr_t &>() >= nullptr),
                      bool>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(nullptr == std::declval<const ptr_t &>()),
                      bool>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(nullptr != std::declval<const ptr_t &>()),
                      bool>));
  EXPECT_TRUE((
      std::is_same_v<decltype(nullptr < std::declval<const ptr_t &>()), bool>));
  EXPECT_TRUE((
      std::is_same_v<decltype(nullptr > std::declval<const ptr_t &>()), bool>));
  EXPECT_TRUE(
      (std::is_same_v<decltype(nullptr <= std::declval<const ptr_t &>()),
