...
// Error when mixing ND-range and range across the dynamic CG. Also check error
// when using incorrect update function for dynamic CG.
TEST_F(CommandGraphTest, DynamicCommandGroupMismatchNDRange) {
  size_t N = 32;
  experimental::command_graph Graph{Queue.get_context(), Queue.get_device()};

  auto CGFA = [&](handler &CGH) {
    CGH.parallel_for<TestKernel<>>(sycl::nd_range<1>({N * 2}, {N}),
                                   [&](sycl::nd_item<1> it) {});
  };

  auto CGFB = [&](handler &CGH) {
    CGH.parallel_for<TestKernel<>>(sycl::range<1>(N), [&](sycl::item<1> it) {});
  };
  ...
  ASSERT_THROW(experimental::dynamic_command_group DynCG(Graph, {CGFA, CGFB}),
               sycl::exception);

  experimental::dynamic_command_group DynCG(Graph, {CGFC, CGFB});
  auto DynCGNode = Graph.add(DynCG);

  ASSERT_THROW(DynCGNode.update_nd_range(sycl::nd_range<1>({N * 2}, {N})),
               sycl::exception);
}
