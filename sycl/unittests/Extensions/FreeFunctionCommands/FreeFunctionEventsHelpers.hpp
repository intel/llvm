...
inline ur_result_t redefined_urEnqueueKernelLaunch(void *pParams) {
  ...
  // TODO The no-handler scheduler submission includes a fix for the event
  // return, where the event is returned by the scheduler on every submission.
  // This fix is not yet applied to the handler-based path. #ifndef
