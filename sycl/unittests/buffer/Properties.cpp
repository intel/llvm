...
TEST(BufferProps, SetAndQueryMatch) {
   ...
   sycl::buffer<int, 1> buf{HostPtr, 1, {sycl::property::buffer::use_host_ptr{}, sycl::property::buffer::context_bound{Context}, sycl::property::buffer::use_mutex{Mutex}}};

   ASSERT_TRUE(buf.has_property<sycl::property::buffer::context_bound>());
   EXPECT_EQ(buf.get_property<sycl::property::buffer::context_bound>().get_context(), Context);
   ASSERT_TRUE(buf.has_property<sycl::property::buffer::use_mutex>());
   EXPECT_EQ(buf.get_property<sycl::property::buffer::use_mutex>().get_mutex_ptr(), &Mutex);
