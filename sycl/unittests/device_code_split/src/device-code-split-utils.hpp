...
pi_result piextDeviceSelectBinaryRedefine(pi_device device,
  ...
  for (pi_uint32 i = 0; i < num_images; ++i) {
    ...
    std::map<std::string, bool> hasKernel{
        {"File1Kern1", false}, {"File1Kern2", false}, {"File2Kern1", false}};
...
protected:
  ...
  DeviceCodeSplit() : Q(cl::sycl::cpu_selector()), Mock(Q) {}

  void SetUp() override {
    Mock.redefine<cl::sycl::detail::PiApiKind::piextDeviceSelectBinary>(
        piextDeviceSelectBinaryRedefine);
  }
};
