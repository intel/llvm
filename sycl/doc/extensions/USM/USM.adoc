...
[source,cpp]
----
std::tuple<usm::alloc, sycl::device> get_pointer_info(const void* ptr, const sycl::context& ctxt);
----
Parameters::
  * `const void* ptr` - the pointer to query
Return value:: Returns a `std::tuple` containing the type of the allocation and the SYCL `device` against which it was allocated (in the case of `device` and `shared` allocations).  The returned `device` is not assumed to be valid if the allocation is a `host` allocation.
  * `const sycl::context& ctxt` - the SYCL `context` against which the specified pointers were allocated
