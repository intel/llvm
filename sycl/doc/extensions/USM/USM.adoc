...
class handler {
 ...
 public:
  ...
  (1)
  void memfill(void* ptr, const void* pattern, size_t pattern_size, size_t count);
  (2)
  template <typename T>
  void memfill(void* ptr, const T* pattern, size_t count)
};
...
class queue {
 ...
 public:
  ...
  (1)
  event memfill(void* ptr, const void* pattern, size_t pattern_size, size_t count);
  (2)
  template <typename T>
  void memfill(void* ptr, const T* pattern, size_t count);
};
...
Parameters::
  ...
  * (1) `const void* pattern` - pattern to be filled
  * (1) `size_t pattern_size` - size of the pattern in bytes
  * (2) `const T* pattern` - pattern to be filled
...
Return value:: Returns the USM allocation type for the allocation pointed to by `ptr`.  If `ctxt` is a host `context`, returns `usm::alloc::host`.
 ...
 * `const void* ptr` - the pointer to query
 * `const sycl::context& ctxt` - the SYCL `context` to which the USM allocation belongs
 Return value:: Returns the `device` associated with the USM allocation.  If `ctxt` is a host `context`, returns the host `device` in `ctxt`.  If `ptr` is an allocation of type `usm::alloc::host`, returns the first device in `ctxt`.
