...
In the interop callable submitted to `ext_oneapi_enqueue_native_command` all
asynchronous interop work submitted to the native queue object returned by
`interop_handle::get_native_queue` will be correctly synchronized with the
command's dependent events.
