...
propagated to the native API by the SYCL implementation during the command
submission, meaning no further dependency analysis is needed within the user
submitted `interopCallable` in the native API. This happens, for example, if
the command group calls `handler::depends_on` or if it constructs an accessor.
Note, however, that the SYCL command is associated with the _asynchronous tasks
submitted by_ `interopCallable`, not the call to `interopCallable`. The call to
`interopCallable` may happen even before the dependencies are satisfied.
...
This example submits synchronous work within the
`ext_oneapi_enqueue_native_command` callable. Instead work must be submitted
asynchronously to the native queue obtained from
`interop_handle::get_native_queue`.
...
q.submit([&](sycl::handler &cgh) {
    ...
    cgh.ext_oneapi_enqueue_native_command([=](sycl::interop_handle &h) {
     ...
     // Undefined behaviour! Only async work submitted to ih.get_native_queue()
     // is allowed.
     hipMemcpy(target_ptr, native_mem, test_size * sizeof(int),
