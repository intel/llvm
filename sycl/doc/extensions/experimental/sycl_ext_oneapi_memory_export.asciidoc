...
only supports exporting memory with the `opaque_fd`, `win32_nt_handle`, and
`dma_buf` handle types. This enum is shared with the memory import functionality defined in the
...
The `dma_buf` handle type corresponds to a Linux DMA-BUF file descriptor, which
...
Only three values of `externalMemHandleType` are supported by this extension:
...
- `external_mem_handle_type::dma_buf` is supported when the host is Linux.
...
Constraints: `ExternalMemHandleType` is `external_mem_handle_type::opaque_fd`,
`external_mem_handle_type::win32_nt_handle`, or
...
When `ExternalMemHandleType` is `external_mem_handle_type::dma_buf`, the
`+__return_type__+` is `int`.
