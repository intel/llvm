...
virtual memory ranges previously mapped to `physical_mem`. `ptr` and
must be aligned to the minimum memory granularity of `syclContext` and
`numBytes` must be a multiple of the minimum memory granularity of
`syclContext`.

A virtual memory address cannot be simultaneously mapped to more than one
...
virtual memory ranges previously mapped to `physical_mem`. `ptr` and
must be aligned to the minimum memory granularity of `syclContext` and
`numBytes` must be a multiple of the minimum memory granularity of `syclContext`.
