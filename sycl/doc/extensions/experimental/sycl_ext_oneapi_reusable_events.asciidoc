...
When set, the event's `info::event_profiling::command_submit`,
`command_start`, and `command_end` timestamps are all equal to the device time
at which the tag operation executes.
...
elapsed device time between two tag operations is the difference of their
timestamps.
_{endnote}_]
