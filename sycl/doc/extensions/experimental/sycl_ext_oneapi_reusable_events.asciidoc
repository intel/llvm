...
* When an event is passed to the `enqueue_signal_event` function and is also
  set as a dependency of a command - using `handler::depends_on`, one of the
  `queue` member functions which take event dependencies, or any other API
  which establishes an event dependency - the same limitations apply as
  described for the `enqueue_wait_event` function above. The behavior in this
  cases is undefined. Unlike the other cases in this section, this one is not
  reported with an exception as the implementation does not currently
  differentiate between events enqueued for signaling using
