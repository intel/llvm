...
The limitations below refer to the dependencies of an operation. An "event
signal" operation depends on the commands which were previously submitted to an
in-order queue, because it is ordered after them. An "event wait" operation
depends on the events which are passed to it. It is also ordered after the
commands previously submitted to an in-order queue, but that is not a
limitation for it: unlike the "event signal" operation, which has to be
submitted to the backend by the call itself, the "event wait" operation may be
left pending inside the SYCL runtime until those commands are submitted to the
backend.
...
* The `enqueue_signal_event`, `enqueue_wait_event` and `enqueue_wait_events`
  functions cannot be dependent on a host task, which has been submitted using
  the `handler::host_task` function. Submitting host tasks using the
...
* The `enqueue_signal_event`, `enqueue_wait_event` and `enqueue_wait_events`
  ...
  while a host accessor for that memory object exists. This applies to
  `host_accessor`, `buffer::get_host_access`, `host_unsampled_image_accessor`
  and `host_sampled_image_accessor`. Unlike a host task, which completes on its
  own, such a command is held until the host accessor is destroyed, which
  happens when the last copy of the host accessor is destroyed, so the
  application controls how long this limitation lasts.
* The `enqueue_signal_event`, `enqueue_wait_event` and `enqueue_wait_events`
