...
This property is a hint to the driver to optimize event host
synchronization calls to use the CPU more efficiently. For example, the
driver may sleep the waiting thread instead of polling. Using this hint
may impact completion latency.

_Effects_ (1): Creates a new `low_power` property with a boolean value
indicating whether the event uses the low-power flag.
