...
[_Note:_ This extension uses the `single_task` and `nd_launch` function names
from `sycl_ext_oneapi_enqueue_functions`, but it does not use that extension's
`launch_config` type in this version (see the note in "New free functions to
launch a kernel").
This dependency is expected to be revisited if and when an equivalent of those
launch functions is provided by a finalized SYCL extension.
_{endnote}_]

[_Note:_ This extension's compile-time properties (`nd_range_kernel` and
`single_task_kernel`) are still defined below using the `property_value` shape
and the `sycl::ext::oneapi::experimental` namespace from the experimental
`sycl_ext_oneapi_properties` extension.
Once `sycl_khr_properties` is finalized, these property definitions need to be
revisited to use the `sycl::khr::property` shape (reading a compile-time
property's value through its `value` member) and the `sycl::khr` namespace.
_{endnote}_]
...
The captures or member variables of a standard SYCL kernel have no defined
order.
A free function kernel instead exposes each argument as a function parameter,
which gives the arguments a defined order, so the application can set each
argument by its index via `handler::set_arg(index, value)`.
When the arguments are restricted to trivially copyable types (see the
restrictions below), each argument also has a well defined size and
representation, giving the kernel a stable argument ABI.

A defined argument order and a stable argument ABI allow a free function kernel
to be invoked directly through a SYCL backend's native kernel-invocation API,
using the backend interoperability interface in the core SYCL specification,
without going through the SYCL host runtime APIs.
They also provide a clean separation between host and device code, which makes
free function kernels a useful building block for software layers built on top
of this extension.

A free function kernel can also be enqueued through the normal SYCL host runtime
APIs, so the feature does not require any particular backend.
Kernels that need accessors or buffer-based data management are not a good fit
for this feature and should use standard SYCL kernels instead.
...
* Each of the function's arguments must have a type `T` for which
  `is_valid_free_function_kernel_arg_v<T>` is `true` (see below).
  In particular, the function must not be declared with parameters of type
  `accessor`, `local_accessor`, `reducer`, or `kernel_handler`.
...
* The way the function's body obtains its iteration ID must be consistent with
  the declared property: a function decorated with `nd_range_kernel<Dims>` must
  obtain its iteration ID for an `nd_range` of `Dims` dimensions, and a function
  decorated with `single_task_kernel` must not obtain an iteration ID.
  Otherwise the program is ill formed, no diagnostic required.

[_Note:_ An implementation may, but need not, diagnose such an inconsistency.
This is not always possible, for example when the body is defined in another
translation unit through a `SYCL_EXTERNAL` function or provided as a precompiled
device object.
_{endnote}_]

The set of types that may be passed as a free function kernel argument is
defined by the following trait.
...
template<typename T>
struct is_valid_free_function_kernel_arg;

template<typename T>
inline constexpr bool is_valid_free_function_kernel_arg_v =
    is_valid_free_function_kernel_arg<T>::value;

} // namespace sycl::ext::oneapi::experimental
----

If `is_valid_free_function_kernel_arg_v<T>` is `true`, then `T` is trivially
copyable.
The converse does not hold: an implementation may report `value` as `false` for
a trivially copyable type whose size or representation it cannot guarantee to be
stable, for example a type whose layout differs between the toolchains that
produce the host code and the device code.
A type may therefore be a valid argument for one implementation and not for
another; applications can query the trait to detect this at compile time.
The trait's `value` is always `false` for `accessor`, `local_accessor`, and any
other type that the implementation must decompose into multiple backend
arguments, since such types have no single defined size and representation.

Another SYCL extension may specialize this trait so that `value` is `true` for
an additional permitted argument type.
Specializing it to `true` for a type that does not meet these requirements is
undefined behavior.
...
A function decorated with one of these properties is a kernel entry point.
It must not be called as a normal function in either host or device code; doing
so results in undefined behavior.
The function's address may still be taken and is used to identify the kernel
(see the `Func` template parameter used by the APIs below).

[_Note:_ Many of the APIs below take a template parameter `Func` that identifies
a free function kernel.
The kernel need not be defined in the same translation unit as the instantiation
of the template taking the `Func` parameter.
_{endnote}_]

=== New traits for kernel functions

This extension defines the following traits that tell whether a function is
declared as a free function kernel.
...
[_Note:_ A future version of this extension is expected to add `nd_launch`
overloads that accept a `launch_config` argument, allowing launch-time
properties (such as a dynamic work-group local memory size) to be specified.
These overloads are omitted from this version because they depend on the
`launch_config` type, which is not yet provided by a finalized SYCL extension.
_{endnote}_]
...
The information descriptor `info::kernel::num_args` may be queried on a `kernel`
object that represents a free function kernel.
It returns the number of formal parameters in the function's definition.

=== Enqueuing a free function kernel and setting parameter values

There are two ways to enqueue a free function kernel.
When the kernel's declaration is visible, the `nd_launch` and `single_task`
functions taking a `kernel_function_s<Func>` (described above) are preferred:
they check the kernel's kind, dimensionality, and argument types at compile time.
This section describes the alternative, in which the application obtains a
`kernel` object and sets each argument by index through `handler::set_arg`.
This path performs no compile-time checking, but it sets arguments using the
same positional index, size, and representation contract used to invoke a kernel
through a native backend (see <<level-zero-and-opencl-compatibility>>), so it
lets an application exercise that contract through the SYCL runtime on any
backend.
...
It is undefined behavior to enqueue a free function kernel using a mechanism
that does not match its type, or with an `nd_range` whose dimensionality does
not match the kernel definition.
...
A standard SYCL kernel receives its iteration ID as a parameter to the kernel's
callable object.
A free function kernel cannot, because its parameters pass the kernel arguments
instead.
It therefore obtains the iteration ID through the functions specified in
`sycl_khr_work_item_queries`, such as `sycl::khr::this_nd_item`.
...
== Direct invocation through a native backend

A free function kernel can be invoked directly through a SYCL backend's native
kernel-invocation API, using the backend interoperability interface in the core
SYCL specification, without going through the SYCL host runtime APIs.
This is guaranteed only when the kernel adheres to the following restrictions:

* The kernel is written in the free function kernel syntax.
* Each formal argument to the kernel is a {cpp} trivially copyable type, or a
  type that another SYCL extension specifies as a permitted free function kernel
  argument.

A native backend identifies a kernel by a _name_ string.
When a kernel adheres to the restrictions above, this name is the name of the
kernel's function as it appears in the native device code module.
For a kernel that is not declared with C language linkage, this is the {cpp}
mangled name of the function.
For a kernel that is declared `extern "C"`, this is the function's name without
{cpp} name mangling.

[_Note:_ Because a function with C language linkage cannot be a template,
declaring a kernel `extern "C"` gives it a predictable, unmangled name at the
cost of not being able to define it as a function template.
A templated free function kernel is still invocable through a native backend
using the mangled name of the desired specialization.
_{endnote}_]

A native backend sets a kernel argument value using three pieces of information:
the index of the argument, the size (in bytes) of the value, and a pointer to
the start of the value.
When a kernel adheres to the restrictions above, the argument indices are the
same as the positions of the formal kernel arguments in the {cpp} source code:
the first argument has index 0, the next has index 1, etc.

If an argument has a trivially copyable type, the size is the size of that type,
and the pointer must point to a memory region that has the same size and
representation as that trivially copyable type.
An extension that permits an additional free function kernel argument type
specifies how that argument's size and pointer are set when the kernel is
invoked through a native backend.

A kernel that is invoked through a native backend must not use specialization
constants as defined in section 4.9.5 "Specialization constants" of the core
SYCL specification.

[_Note:_ The OpenCL backend is an example of a backend that provides such a
native kernel-invocation interface, via `clCreateKernel` and `clSetKernelArg`.
Other extensions may specify additional permitted argument types and how their
size and pointer are set; see the documentation of those extensions.
_{endnote}_]
...
`sycl_khr_work_item_queries` to get the iteration index.
