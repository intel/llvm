...
Because the kernel arguments of a standard SYCL kernel are captures or member
variables of a callable object, they have no defined order.
A free function kernel instead exposes each kernel argument as a function
parameter.
This gives the kernel arguments a defined order, so the application can set the
value of each argument by its index via `handler::set_arg(index, value)`.
...
representation, which gives the kernel a stable argument ABI.
...
`sycl_khr_work_item_queries` (such as `sycl::khr::this_nd_item`) for this
purpose.
