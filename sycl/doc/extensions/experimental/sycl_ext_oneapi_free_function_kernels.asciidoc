...
The trait `is_valid_free_function_kernel_arg<T>` provides the member constant
`value` equal to `true` if `T` is a trivially copyable type, except that `value`
is `false` for `accessor`, `local_accessor`, and any other type that requires
the implementation to decompose the argument into multiple backend arguments.
Otherwise `value` is `false`.

Restricting free function kernel arguments to such types gives each argument a
defined size and representation, which is what provides the stable argument ABI
described in the Overview.
Kernel argument types such as `accessor` are excluded because they require the
implementation to decompose a single argument into several backend arguments,
which the application cannot reason about.

Another SYCL extension may specialize this trait so that `value` is `true` for
an additional type that the extension specifies as a permitted free function
kernel argument.
Specializing this trait so that `value` is `true` for a type that does not meet
the requirements of a permitted free function kernel argument is undefined
behavior.
...
[_Note:_ Work-group local memory is not addressed by this version of the
extension.
Because a `local_accessor` cannot be passed as a free function kernel argument,
a future version is expected to describe how a free function kernel obtains
work-group local memory through a mechanism provided by another SYCL extension.
_{endnote}_]
...
A function with C language linkage cannot be a template, so this guarantee is
not available for free function kernels that are function templates.
Such kernels can still be enqueued through the SYCL host runtime APIs.
...
kernel's function in the {cpp} source code with "++__sycl_kernel_++" prefixed.
For example, if the function name is "foo", the kernel's name in the native
device code module is "++__sycl_kernel_foo++".
Because the kernel is declared `extern "C"`, this name is not affected by {cpp}
name mangling.
...
Some kernel features are disallowed when invoking a kernel through a native
backend, and others require the kernel to be invoked in a special way:

* A kernel may use work-group local memory whose size is specified by a launch
  property rather than fixed at compile time, through a mechanism provided by
  another SYCL extension.
  When a kernel does so, it has an additional "hidden" argument whose value must
  be set when the kernel is invoked.
  This argument appears after all other kernel arguments.
  Its value corresponds to the size (in bytes) of the dynamic device local
  memory, and the kernel argument's pointer must be NULL.
