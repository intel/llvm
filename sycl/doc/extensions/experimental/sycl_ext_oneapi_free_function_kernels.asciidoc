...
[_Note:_ On the OpenCL and Level Zero backends, the native kernel-invocation API
described above corresponds to `clCreateKernel` / `clSetKernelArg` and
`zeKernelCreate` / `zeKernelSetArgumentValue` respectively.
The `work_group_memory` type (see
link:../experimental/sycl_ext_oneapi_work_group_memory.asciidoc[
sycl_ext_oneapi_work_group_memory]) is an example of an additional permitted
argument type; its native size is the size of the device local memory it
represents and its pointer must be NULL.
The dynamic local memory mechanism described above is provided by
link:../experimental/sycl_ext_oneapi_work_group_scratch_memory.asciidoc[
sycl_ext_oneapi_work_group_scratch_memory], whose hidden argument corresponds to
its `work_group_scratch_size` launch property.
To obtain the raw backend content of the device image that contains the kernel,
an application can use
link:../experimental/sycl_ext_oneapi_device_image_backend_content.asciidoc[
sycl_ext_oneapi_device_image_backend_content], or compile in AOT mode via the
`-fsycl-targets` compiler option and extract the device image's backend content
from the executable file.
The {dpcpp} implementation additionally requires the translation unit containing
the kernel to be compiled with the `-fno-sycl-dead-args-optimization` option.
_{endnote}_]
