...
* `#include <sycl/ext/oneapi/free_function_kernel_properties.hpp>` is
  sufficient for `SYCL_EXT_ONEAPI_FUNCTION_PROPERTY`,
  `nd_range_kernel<Dims>`, `single_task_kernel`, and the standalone
  compile-time kernel properties that may be applied directly to a free
  function kernel declaration, such as `work_group_size`,
  `work_group_size_hint`, `sub_group_size`, `max_work_group_size`, and
  `max_linear_work_group_size`.

* `#include <sycl/ext/oneapi/kernel_properties.hpp>` currently provides the
  broader kernel properties interface, including property lists such as
  `properties{...}` and embedded kernel properties via `properties_tag`.

Applications may include the umbrella header in all cases. The lightweight
header is intended for code that defines free-function kernels and only needs
the free-function annotation path.

Code that forms property lists such as `properties{...}`, uses
`properties_tag`, or otherwise needs the broader kernel-properties interface
will generally find `sycl/ext/oneapi/kernel_properties.hpp` to be the more
convenient header to include.
...
In the {dpcpp} implementation, code that defines free-function kernels may
include `sycl/ext/oneapi/free_function_kernel_properties.hpp` instead of the
full `sycl/ext/oneapi/kernel_properties.hpp` header when it only needs the
free-function annotation path. Applications that use property-list semantics,
including `properties{...}` or `properties_tag`, will generally prefer the
umbrella header.
