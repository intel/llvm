...
DataT gather_image(const sampled_image_handle &ImageHandle,
                   const sycl::float2 &Coords, const int Component = 0);
...
exactly as is in device memory.
The `sample_image` and `gather_image` APIs are only applicable to
sampled images. For both APIs the image data will be sampled according to the
...
[NOTE]
=====
`gather_image` is only currently supported in the Cuda backend.
=====
...
members. The `image_descriptor::depth` member must be zero.
The return values, `DataT`, consist of the four texture, `T`, coordinates that
would be used for linear filtering of `ImageHandle` at the coordinate specified
by `Coords`, and for channel component `Component`.
...
For `sycl::coordinate_normalization_mode::unnormalized` (in units of the image
...
(Consult the latest backend specific documentation for more information).
As such, for guaranteed correctness in the Cuda backend, it is also necessary
to construct `ImageHandle` using an `image_descriptor` with member
`image_type::gather`.
Using a value `image_descriptor::num_channels < 4` to construct `ImageHandle`
