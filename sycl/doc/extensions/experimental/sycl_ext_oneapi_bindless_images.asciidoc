...
The L0 backend only supports using external semaphores on immediate queues. This
extension also defines the `sycl::info::queue::ext_oneapi_supports_semaphores`
queue info query to determine if a queue suppports external semaphores. External
semaphores should not be used on queues for which this query returns false.
...
implementation opens the named object on the caller's behalf.
