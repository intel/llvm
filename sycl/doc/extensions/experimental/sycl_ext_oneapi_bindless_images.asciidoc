...
[NOTE]
====
The `sycl::range<N>` constructors map dimensions in **X, Y, Z order** (width, height, depth), which differs from the SYCL 2020 specification's general `range` convention of "fastest-incrementing" dimension order. This follows standard image/graphics conventions where:
