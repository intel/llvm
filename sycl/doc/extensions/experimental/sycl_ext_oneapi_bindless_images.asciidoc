...
template <typename DataT,
          sycl::image_channel_order COrder = sycl::image_channel_order::rgba,
          typename CoordT>
DataT fetch_image(const unsampled_image_handle &imageHandle, CoordT &&coords) {
  return fetch_image<DataT, DataT, COrder, CoordT>(imageHandle, coords);
}
...
template <typename DataT,
          sycl::image_channel_order COrder = sycl::image_channel_order::rgba,
          typename CoordT>
DataT fetch_image(const sampled_image_handle &imageHandle, CoordT &&coords) {
  return fetch_image<DataT, DataT, COrder, CoordT>(imageHandle, coords);
}
...
template <typename DataT,
          sycl::image_channel_order COrder = sycl::image_channel_order::rgba,
          typename CoordT>
void write_image(unsampled_image_handle ImageHandle, const CoordT &Coords,
                 const DataT &Color);
...
DataT sample_mipmap(const sampled_image_handle &imageHandle,
                    const CoordT &coords, const float level);
...
template <typename DataT, typename HintT = DataT,
          ...
          typename CoordT>
DataT sample_mipmap(const sampled_image_handle &imageHandle,
                    const CoordT &coords, const CoordT &dX, const CoordT &dY);
...
template <typename DataT,
          ...
          typename CoordT>
DataT fetch_image_array(const unsampled_image_handle &imageHandle,
                        const CoordT &&coords, const int arrayLayer) {
...
Our extension supports the reordering of image channels at the time of fetch,
sample, and write invocations, where the following image channel orders are
supported:
...
When sampling, fetching, or writing to an image, an optional
`image_channel_order` template argument may be passed to reorder pixel channels.
...
```cpp
// Reorder to argb
fetch_image<int, int, image_channel_order::argb>(handle, coords);

// No channel reordering occurs
fetch_image<int, int>(handle, coords);

// Reorder to argb
fetch_image<int, image_channel_order::argb>(handle, coords);

// No channel reordering occurs
fetch_image<int>(handle, coords);
```

The following template argument layouts are valid for `write_image`,
`write_image_array` and `write_cubemap`:

```cpp
// Reorder to argb
write_image<int, image_channel_order::argb>(handle, coords, color);

// No channel reordering occurs
write_image<int>(handle, coords, color);
```

When channel reordering occurs, pixel channels are treated as already being in
`rgba` order. Reordering swaps pixel channels based on that order to match the
order passed via template argument.

Attempting to reorder pixel data with a different number of channels than the
image was created with will result in undefined behavior.
