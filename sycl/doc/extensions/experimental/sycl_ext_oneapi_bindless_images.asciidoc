...
meaningful for 2D and 3D images and is ignored for 1D images. `slice_pitch` is
meaningful for 3D images and for 2D image arrays (where each array layer is a
"slice"); it is ignored for plain 2D and 1D images. Both members are accepted
on the `range<2>` constructor to cover the 2D-array case uniformly.
