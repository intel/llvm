...
enum class image_color_space : /* unspecified */ {
  linear,
  srgb,
};
...
[NOTE]
====
Certain combinations of image format and type may not be supported on all platforms
====
...
[NOTE]
====
The `sycl::range<N>` constructors map dimensions in **X, Y, Z order** (width, height, depth), 
which differs from the SYCL 2020 specification's general `range` convention of "fastest-incrementing" dimension order
 (https://registry.khronos.org/SYCL/specs/sycl-2020/html/sycl-2020.html#sec:multi-dim-linearization).
This means that users should not attempt to use the same `sycl::range<N>` instance for both their `parallel_for` calls 
and their bindless image `image_descriptor` construction.

This follows standard image/graphics conventions where:

* `range<1>{width}` => `width` = `dims[0]`
* `range<2>{width, height}` => `width` = `dims[0]`, `height` = `dims[1]`
* `range<3>{width, height, depth}` => `width` = `dims[0]`, `height` = `dims[1]`, `depth` = `dims[2]`

This ordering is consistent throughout the bindless images API, including coordinate parameters for read/write operations where `x`, `y`, `z` correspond to indices `0`, `1`, `2` respectively (as detailed in the copy API preconditions).
====
...
The `color_space` member specifies whether the image's raw pixel data is
stored in sRGB or linear color space, and defaults to `linear`.
When set to `srgb`, hardware applies the IEC 61966-2-1 sRGB decode on fetch
or sample, returning linear values to the kernel.

The `srgb` color space is only valid for images that will be read via `fetch_image`, `fetch_image_array`,
`sample_image`, or `sample_image_array`. Writing to an image with `color_space` set to `srgb` (via `write_image` or
`write_image_array`) is not supported.

The `verify` member function will reject a `color_space` of `srgb` unless
`num_channels` is `4` and `channel_type` is `unorm_int8`.

[NOTE]
====
`image_color_space` and hardware sRGB decode are only available when the
`-fpreview-breaking-changes` flag is enabled, and are currently only
implemented in the Level Zero backend.
====
...
for Windows NT resource handles.  Use `resource_win32_name` instead when the
external API has produced a named NT object (for example via
`ID3D12Device::CreateSharedHandle` with a name argument, or `CreateEvent` /
`CreateSemaphore`); the SYCL implementation opens the named object on the
caller's behalf, so the application does not pass an already-opened `HANDLE`.

The `name` field of `resource_win32_name` is a pointer to a null-terminated
wide-character string (`LPCWSTR` / `const wchar_t *`), matching the string type
used by `ID3D12Device::CreateSharedHandle`,
`VkImportSemaphoreWin32HandleInfoKHR::name`, and
`CUDA_EXTERNAL_SEMAPHORE_HANDLE_DESC::name`. The string's storage must remain
...
`resource_win32_handle` for Windows NT resource handles.  Use
`resource_win32_name` instead when the external API has produced a named NT
object (for example a D3D12 fence created via
`ID3D12Device::CreateSharedHandle` with a name argument); the SYCL
implementation opens the named object on the caller's behalf. The `name` field
is a null-terminated wide-character string (`LPCWSTR` / `const wchar_t *`).
