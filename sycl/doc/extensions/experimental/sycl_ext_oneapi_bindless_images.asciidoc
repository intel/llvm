...
extension also defines the
`sycl::info::queue::ext_oneapi_immediate_command_list` queue info query to
determine if a queue uses immediate or non-immediate (i.e., batched) command
submissions. External semaphores should not be used on queues for which this
query returns false.
