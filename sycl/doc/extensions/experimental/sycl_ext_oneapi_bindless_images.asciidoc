...
has additional padding to meet device alignment requirements. `row_pitch` and
`slice_pitch` are only meaningful for 2D and 3D images; they are ignored for 1D
images.
