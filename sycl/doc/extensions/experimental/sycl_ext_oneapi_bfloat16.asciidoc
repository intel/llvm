...
The following functions are only available only when "T" is `bfloat16` `or `sycl::marray<bfloat16, {N}>`, where {N} means any positive value of size_t type.
...
sum of `c` with the infinitely precise
product of `a` and `b`. Rounding of
intermediate products shall not
occur.
...
2. The CUDA backend has no use case that would necessitate support of the `vec` class in bfloat16 math builtins, since `marray` is always preferred. Support for the `vec` class can be added if other backends require it.

3. Decide on a roadmap to extend support of `bfloat16` to more SYCL 2020 math functions.
