...
1. The CUDA backend does not have a use case that would necessitate support of the `vec` class in bfloat16 math builtins, and `marray` would always be preferred over `vec` if there were to be added `vec` support in the CUDA backend. For portability reasons, support for the `vec` class can be easily added if other backends require it.
