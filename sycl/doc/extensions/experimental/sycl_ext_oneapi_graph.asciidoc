...
`exception` with error code `invalid`.
...
property, calls to `queue::wait()` or `event::wait()` during recording insert a
node of type `node_type::host_sync` into the graph. These nodes are not
enqueued to the device; instead, they act as partition boundaries.
