...
Events returned from queue submissions when a queue is not in the recording
state should not be used as parameters to `handler::depends_on()` or as dependent
events for queue shortcuts like `queue::parallel_for()` for submissions which
are being recorded.
