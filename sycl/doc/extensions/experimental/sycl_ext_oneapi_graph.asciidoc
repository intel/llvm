...
takes a graph in the modifiable state and updates the nodes in the executable
state graph to reflect any changes made to the nodes in the modifiable state
graph. The characteristics which will be updated are detailed in the section on
<<executable-graph-update, Executable Graph Update>>.

Both the graph being updated and the graph being used as the source for the
update must have been constructed with the same device and context.

The modifiable graph must have the same topology as the graph originally used to
create the executable graph. Additionally, all nodes and edges must be added in
the same order, and all Graph-related API calls and recorded queue submissions
...
identical modifiable graph. A topologically identical graph is one with the same
structure of nodes and edges. Additionally, all nodes and edges must be added in
the same order, and all Graph-related API calls and recorded queue submissions
must occur in the same order.
