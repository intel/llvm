...
2. `property::queue::enable_profiling` - This property must be set on the queue
   ...
   the graph. This property must also be set on the queue the queue an
   executable graph is submitted to. It allows profiling information to be
   obtained from the event returned by a graph submission. 
