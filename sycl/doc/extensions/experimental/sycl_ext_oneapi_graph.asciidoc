...
* An `exception` with error code `invalid` if `nodeEvent` is not associated with a
  graph node.
...
node to a `command_graph`. `graph` to be associated with this `dynamic_command_group`.
The list of command-group functions `cgfList` that can be activated for this dynamic command-group.
...
* An `exception` with error code `invalid` if `cgfIndex` is not a valid index.
...
* `syclDevice` must be associated with `syclContext`.
...
* Synchronously with error code `invalid` if `syclDevice` is not
associated with `syclContext`.

* Synchronously with error code `invalid` if `syclDevice`
...
_Effects:_ Simplified constructor form where `syclQueue` provides the device and context.
...
* Queue only provides the SYCL device and context for the graph,
...
* Synchronously with error code `invalid` if the device associated with
...
* Synchronously with error code `invalid` if a queue is recording
...
* Synchronously with error code `invalid` if a queue is recording
  commands to the graph.
* Synchronously with error code `invalid` if the graph wasn't created with
...
* Synchronously with error code `invalid` if the graph does not match
...
_Effects:_ Creates a dependency between two nodes representing a happens-before relationship.
Node `dest` will be dependent on `src`
...
* Synchronously with error code `invalid` if `src` or `dest`
  are not valid nodes assigned to the graph object.

* Synchronously with error code `invalid` if `src` and `dest`
  are the same node.

* Synchronously with error code `invalid` if the resulting dependency would
...
* Synchronously with error code `feature_not_supported` if the graph
...
* Synchronously with error code `invalid` if the path is invalid or
...
* Synchronously with error code `invalid` if
...
* Synchronously with error code `invalid` if `source` contains any node
...
* Synchronously with error code `invalid` if the context or device
...
* Synchronously with error code `invalid` if `recordingQueue` is
...
* Synchronously with error code `invalid` if any queue in
  `recordingQueues` is already recording to a graph.

* Synchronously with error code `invalid` if any of `recordingQueues`
...
_Effects:_Synchronously changes the state of `recordingQueue` to the
...
* Synchronously with error code `invalid` if the queue is not in `queue_state::recording`
state.
...
* Synchronously with error code `invalid` if the handler is submitted
...
* Synchronously with error code `invalid` if this function is called from
...
|3|2025-05-09|Pablo Reble|Update format
