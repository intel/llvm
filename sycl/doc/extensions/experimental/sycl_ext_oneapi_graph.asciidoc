...
waited on or used as dependencies for eager SYCL operations. See the section on
...
include eager submissions to a SYCL queue and events returned from submitting an
executable `command_graph` for execution.
...
The functions `node::get_event_from_node()` and `node::get_node_from_event()`
allow a user to take either an `event` or `node` object obtained via either API
and convert it to the other. This allows for mixed usage of both APIs to form a
single graph. For example, a node added via the explicit API can be used as a
dependency for a captured queue submission by obtaining the equivalent
<<event-terminology, graph-limited event>> and using that as a dependency to a
recorded queue submission:

[source, c++]
----
node SomeNode = Graph.add(...);

event SomeEvent = node::get_event_from_node(SomeNode);

Graph.begin_recording(Queue);
Queue.submit([&](handler& CGH){
  CGH.depends_on(SomeEvent);
  ...
});
----

Events or nodes obtained via these functions can be used with any other API
functions that take those objects, and function the same as events or nodes
obtained directly from recorded queue submissions or calls to
`command_graph::add()`.

The <<depends-on-property, `depends_on` property>> can also take events in
addition to nodes to define dependencies, without the need for first calling
`node::get_node_from_event()`.
...
public:
  ...
  static event get_event_from_node(node graphNode);
...
static event get_event_from_node(node graphNode);
----
|Finds the <<event-terminology, graph-limited event>> associated with a graph
node. This can be used for interop between the explicit and queue recording
APIs.

If `graphNode` was created via the explicit graph creation API then an event may
be created for it during this call.

Parameters:

* `graphNode` - A graph node.

Returns: A graph-limited event which represents `graphNode`.
...
* Passing nodes from another `command_graph` which will create an external
dependency between the node being added and the dependent nodes in other
graphs.
...
These events represent only the current execution of a given executable graph on
a device. These events are not unique per execution of a graph. If a graph is
enqueued multiple times before using one of these events (for example as a
dependency to an eager SYCL operation or a host wait), it is undefined which
specific execution of a graph the event will represent. If a dependency on a
specific graph execution is required this ordering must be enforced by the
application to ensure there is only a single graph execution in flight when
using these events.
...
These events cannot be used to define dependencies between graphs and attempting
to do so will result in an error being thrown. These should instead be defined
as described in <<inter-graph-dependencies, this section>>.
