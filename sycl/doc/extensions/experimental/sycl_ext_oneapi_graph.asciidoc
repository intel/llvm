...
3. **Native recording** - An optional variant of the queue recording API in
which graph capture is performed directly at the backend level rather than
through the SYCL runtime node model. Enabled by passing
`property::graph::enable_native_recording` to the `command_graph`
constructor. See <<native-recording, Native Recording>> for details.
...
It is valid to combine mechanisms 1 and 2, however it is invalid to modify
...
|2     |Add `property::graph::enable_native_recording` to support backend-level graph capture.
|===
...
| Native Recording
| A graph-building mode in which the underlying backend
performs the command capture directly, bypassing the SYCL runtime node model.
...
===== Allow-Wait-Recording Property [[allow-wait-recording-property]]

[source,c++]
----
namespace sycl::ext::oneapi::experimental::property::graph {
class allow_wait_recording {
  public:
    allow_wait_recording() = default;
};
}
----

The `property::graph::allow_wait_recording` property enables recording of
`queue::wait()` and `event::wait()` calls during graph recording. When this
property is set on a modifiable `command_graph`, calling `queue::wait()` or
`event::wait()` while the queue is in the recording state will insert a
`node_type::host_sync` node into the graph instead of throwing an exception.

Additionally, this property allows commands submitted during recording to depend
on events from outside the graph (i.e. events created before recording began or
on non-recording queues). Without this property, such dependencies throw a
synchronous `exception` with error code `invalid` and the message "Graph nodes
cannot depend on events from outside the graph." When this property is set, the
runtime inserts a `node_type::host_sync` node as a partition boundary instead.

At finalization, `host_sync` nodes act as partition boundaries that split the
graph into separately-submitted sub-graphs. During execution, the runtime
submits each partition to the device independently, waiting for the previous
partition to complete before submitting the next one.

When this property is *not* set, the existing behavior is preserved:
`queue::wait()` and `event::wait()` during recording throw a synchronous
`exception` with error code `invalid`, and external event dependencies are
rejected.

NOTE: Recording a wait silently changes semantics. Users calling
`queue::wait()` in handwritten SYCL code often have host code after the wait
that reads device data. Silently converting the wait to a partition boundary
would produce incorrect results that are difficult to debug. The opt-in
property makes the behavior change explicit.

NOTE: This property is not compatible with
<<enable-native-recording-property, `property::graph::enable_native_recording`>>.
Native recording delegates graph capture to the backend, which does not support
synchronization calls during capture. Passing both properties results in
undefined behavior.

==== Enable-Native-Recording Property [[enable-native-recording-property]]

[source,c++]
----
namespace sycl::ext::oneapi::experimental::property::graph {
class enable_native_recording {
  public:
    enable_native_recording() = default;
};
}
----

The `property::graph::enable_native_recording` property requests that graph
...
This property is optional. If the backend does not support native graph
capture the constructor will throw `feature_not_supported`.

See the <<native-recording, Native Recording>> section for a full description
of the feature.
...
* Synchronously `exception` with error code `feature_not_supported` if
  `property::graph::enable_native_recording` is in `propList` and the backend
  does not support native graph capture.
...
* Synchronously `exception` with error code `invalid` if the device
  associated with `syclQueue` <<device-info-query, reports this extension as unsupported>>.
...
* Synchronous `exception` with error code `feature_not_supported` if
  `property::graph::enable_native_recording` was set on the graph and
  `propList` contains `property::graph::updatable`. Graph update is not
  supported for natively recorded graphs.
...
_Returns:_ `true` if the graph contains no nodes, otherwise `false`.
...
* Synchronously `exception` with error code `feature_not_supported` if the
  graph was created with `property::graph::enable_native_recording`.
...
In this mode `begin_recording` may only be called once per graph, and only a
single queue may be passed.
...
* Synchronously `exception` with error code `invalid` if
  `property::graph::enable_native_recording` was set on the graph and
  `recordingQueue` is already in native graph capture mode.
...
When `property::graph::enable_native_recording` was set on the graph, this
call ends the backend-level capture. Because no SYCL node model is built
during native capture, calling `get_nodes()` or `get_root_nodes()` on the
graph will throw `invalid`.
...
=== Host Sync Nodes and Graph Partitioning [[host-sync-nodes]]

When a `command_graph` is constructed with the
<<allow-wait-recording-property, `property::graph::allow_wait_recording`>>
property, the following operations during recording insert a node of type
`node_type::host_sync` into the graph instead of throwing an exception:

* Calling `queue::wait()` or `event::wait()` while the queue is recording.
* Submitting a command that depends on an event from outside the graph (i.e. an
  event created before recording began or on a non-recording queue).

These `host_sync` nodes are not enqueued to the device; instead, they act as
partition boundaries.

At finalization, the runtime splits the graph into partitions at each
`host_sync` node (and each `host_task` node). During execution of the
executable graph, the runtime submits each partition to the device independently,
waiting for the previous partition to complete before submitting the next one.

[source,c++]
----
// Construct a graph with allow_wait_recording
...
queue.submit([&](handler& h) { h.parallel_for(N, kernel_A); });
queue.submit([&](handler& h) { h.parallel_for(N, kernel_B); });

queue.wait();  // Inserts a host_sync node (partition boundary)

queue.submit([&](handler& h) { h.parallel_for(N, kernel_C); });

graph.end_recording();
auto exec = graph.finalize();

// Execution submits [kernel_A, kernel_B], waits, then submits [kernel_C]
...
code `invalid`, unless the graph was constructed with the
<<allow-wait-recording-property, `property::graph::allow_wait_recording`>>
property.
...
throw synchronously with error code `invalid`, unless the graph was
...
Native recording is an optional variant of the <<queue-recording,
Queue Recording API>> in which graph capture is delegated to the backend
rather than being managed by the SYCL runtime. When enabled, the SYCL runtime
calls the backend's dedicated graph-capture APIs.
Because the capture occurs at the backend level, the SYCL runtime has no
visibility into the individual nodes or edges of the captured graph.
Any implementation of native recording must strongly interoperate with command
submission directly through the native backend API, so that commands submitted
via both SYCL and the native API (e.g., Level Zero) are captured together into
the same graph.

=== Enabling Native Recording

Native recording is enabled by passing
`property::graph::enable_native_recording` (see
<<enable-native-recording-property, Enable-Native-Recording Property>>) to
the `command_graph` constructor:
...
const size_t N = 1024;
int *data = sycl::malloc_device<int>(N, queue);
...
queue.ext_oneapi_graph(exec_graph).wait();
...
[cols="1,3,1"]
|===
| Feature | Reason | Exception code

| `graph.add(...)` / `graph.make_edge(...)` during recording
| Backend performs capture; SYCL runtime cannot inject explicit nodes.
| `invalid`

| `sycl::handler::host_task`
| Host tasks execute on the host and cannot be captured by the backend.
| `invalid`

| `ext_oneapi_async_malloc` / `ext_oneapi_async_free`
| Async memory allocation is not representable in native backend graphs.
| `invalid`

| `property::graph::allow_wait_recording`
| Backend capture does not support synchronization during recording.
| _(behavior undefined)_

| `sycl::buffer` accessors
| SYCL buffers are not supported with native recording.
| `feature_not_supported`

| `update()` on the executable graph
| Backend graphs do not expose a node model suitable for parameter updates.
| `feature_not_supported`

| `property::graph::updatable` with `finalize()`
...
| Events signaled during native capture are scoped to the graph and may not be used for synchronization outside it.
| _(behavior undefined)_

| `get_nodes()` / `get_root_nodes()`
| No SYCL node model is built during native capture.
| `invalid`

| Out-of-order queue
| Native recording requires an in-order queue backed by an immediate command list.
| `invalid`
...
Native recording requires backend support for native graph
capture. If the backend does not support this capability the
`command_graph` constructor will throw `feature_not_supported`.

The following requirements apply specifically to the Level Zero V2 (L0v2)
adapter. Other backend implementations may have different requirements.

Native recording requires the queue to be backed by an immediate command list
(i.e. `property::queue::in_order` with an underlying immediate command list).
On compatible devices, immediate command lists are used by default with the
L0v2 adapter and no explicit user configuration is required.
