...
* `get_nodes()` and `get_root_nodes()` return empty vectors.
* The explicit graph-building API (`graph.add(...)`, `graph.make_edge(...)`)
  cannot be used while native recording is active.
* Graph update (`update()`) is not supported and will throw
  `feature_not_supported`.
* Host tasks and asynchronous memory allocations (`ext_oneapi_async_malloc`,
  `ext_oneapi_async_free`) cannot be recorded and will throw `invalid`.
* `command_graph<graph_state::modifiable>::begin_recording` may be called at most once per graph and only with a single queue.
...
call ends the backend-level capture. Because the capture was performed at the backend level,
`get_nodes()` and `get_root_nodes()` will return empty vectors after recording
ends.
...
recording happens at the backend level and transitive recording is not
supported. See <<native-recording, Native Recording>> for constraints and
behavior differences compared to standard queue recording.
...
`property::graph::enable_native_recording` to the `command_graph` constructor:

[source,c++]
----
namespace sycl::ext::oneapi::experimental {
namespace property::graph {
  class enable_native_recording {};
} // namespace property::graph
} // namespace sycl::ext::oneapi::experimental
----
...
=== Limitations of Native Recording
...
| Host tasks
| Host tasks scheduled through the sycl::handler::host_task API cannot be captured by the backend.
| `invalid`
...
| Backend capture is tied to a single command list at a time.
| `invalid`
...
immediate command list (i.e. `property::queue::in_order` with an underlying
immediate command list). Using native recording with an out-of-order queue
may result in `invalid` being thrown at `begin_recording` time if the backend
...
. Using host tasks in a graph node when `property::graph::enable_native_recording` is set.
