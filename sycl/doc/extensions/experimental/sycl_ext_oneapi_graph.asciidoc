...
public:
  ...
  dynamic_parameter(const ValueT &initialValue, command_graph<graph_state::modifiable> graph);
...
----
|Constructs a dynamic parameter object that can be registered with node
arguments, while providing an intiial value which is used solely for
deducing ValueT.

Parameters:

* `initialValue` - Initial value used for CTAD.
* `graph` - Graph containing the nodes which will have dynamic parameters.
