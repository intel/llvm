...
`ext_oneapi_enqueue_custom_operation`, defined in
link:../experimental/sycl_ext_oneapi_enqueue_custom_operation.asciidoc[sycl_ext_oneapi_enqueue_custom_operation]
cannot be used in graph nodes. A synchronous exception will be thrown with error
code `invalid` if a user tries to add them to a graph.
