...
enum class aspect {
  ...
  sycl_ext_oneapi_limited_graph
}
}
----

If a SYCL device has the `sycl_ext_oneapi_limited_graph` aspect, then it
supports all graph functionality except that which is described in the
<<executable-graph-update, Executable Graph Update>> section.
