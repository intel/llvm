...
_Throws:_ An exception with the `errc::feature_not_supported` error code if
event `evt` was not created with the `enable_ipc` property.
...
`handle_data`. The returned event is associated with context `ctxt`. The
...
[_Note:_ The `open` function can be called multiple times on the same handle
within the same process. Each call to the `open` function may return a unique
event object even for the same handle, therefore each call to the `open`
function must have a matching call to the `close` function.
_{endnote}_]

[_Note:_ The event returned from a call to the `open` function is no longer
valid if the event associated with `handle_data` is deallocated.
_{endnote}_]
...
in this same process. This `evt` value has not yet been closed by calling the
`close` function.
...
[source,c++,indent=2]
----
namespace [[deprecated]] sycl::ext::oneapi::experimental::ipc_memory {
  using namespace ipc;
