...
`errc::feature_not_supported` error code if inter-process sharing is enabled
via `enable_ipc` and the event's device does not have the aspect
...
sycl::event open(const handle_data_t &handle_data, const sycl::device &dev);
 ...
 * `ctx` is a context associated with the `evt` object, which was passed to the
...
`handle_data`. The returned event is associated with context `ctx` and device
`dev`. The returned event and the event identified by `handle_data` share the
same state. When one is signaled, the state of the other event automatically
becomes signaled also.
 ...
 * An exception with the `errc::feature_not_supported` error code if device
   `dev` does not have `aspect::ext_oneapi_ipc_event`.
 * An exception with the `errc::invalid` error code if the handle data
   `handle_data` has an unexpected number of bytes.
 * An exception with the `errc::invalid` error code if `ctx` does not contain
  `dev`.
...
----
sycl::context ctxt = dev.get_platform().khr_get_default_context();
return ipc::event::open(handle_data, ctxt, dev);
----
...
sycl::device d;
sycl::context ctxt = d.get_platform().khr_get_default_context();
return ipc::event::open(handle_data, ctxt, d);
----
...
----
handle_data_t handle_data{handle_data_view.begin(), handle_data_view.end()};
return ipc::event::open(handle_data, ctx, dev);
----
...
----
sycl::context ctxt = dev.get_platform().khr_get_default_context();
return ipc::event::open(handle_data_view, ctxt, dev);
----
...
sycl::device d;
sycl::context ctxt = d.get_platform().khr_get_default_context();
return ipc::event::open(handle_data_view, ctxt, d);
----
