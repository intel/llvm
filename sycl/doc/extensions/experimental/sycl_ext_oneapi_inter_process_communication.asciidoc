...
syclext::ipc::handle get(void *ptr, const sycl::context &ctx);

syclext::ipc::handle get(void *ptr);

void put(syclext::ipc::handle &ipc_handle, const sycl::context &ctx);

void put(syclext::ipc::handle &ipc_handle);

void *open(const syclext::ipc::handle_data_t &handle_data,
...
// Requires C++20
void *open(const syclext::ipc::handle_data_view_t &handle_data,
           const sycl::context &ctx, const sycl::device &dev);
...
[source]
----
void *open(const syclext::ipc::handle_data_view_t &handle_data_view,
           const sycl::context &ctx, const sycl::device &dev)
----
!====
...
[source,c++,indent=2]
----
syclext::ipc::handle_data_t handle_data{handle_data_view.begin(),
                                        handle_data_view.end()};
...
syclext::ipc::handle get(const syclext::physical_mem &phys_mem);
...
// Requires C++20
syclext::physical_mem open(const syclext::ipc::handle_data_view_t &handle_data_view,
                           const sycl::context &ctx, const sycl::device &dev);
 ...
 * An exception with the `errc::feature_not_supported` error code if `phys_mem`
