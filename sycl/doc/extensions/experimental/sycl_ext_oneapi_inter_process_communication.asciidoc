...
This extension adds generic types to be used by the IPC features going forward.

It adds `handle_data_t` and `handle_data_view_t` aliases as well as a new
handle type `handle` under the `ipc` experimental namespace.

For backward compatibility, the USM memory IPC API continues to define and use
the `handle` type and related aliases from the
`sycl::ext::oneapi::experimental::ipc_memory` namespace until the next
ABI-breaking window.
...
// This struct is defined for backward compatibility and will be replaced by
// the sycl::ext::oneapi::experimental::ipc::handle struct in the next
// ABI-breaking window.
struct handle {
  handle_data_t data() const;
...
the aspect `aspect::ext_oneapi_ipc_event`.

==== New physical_mem member function

This extension adds a new physical_mem type member function, which can be used
to check if a given physical memory allcation can be passed between processes.
...
[source]
----
syclext::ipc::handle get(syclext::physical_mem &phys_mem)
----
!====
