 ...
 * `ipc_handle` is the IPC "handle" to an event that was returned from a call
   to `get` in this same process. The `put` function has not been previously
...
signaled also. The returned `event` is reusable (see the
sycl_ext_oneapi_reusable_events extension).
