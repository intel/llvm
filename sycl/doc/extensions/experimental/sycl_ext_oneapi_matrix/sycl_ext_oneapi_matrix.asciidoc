...
```c++
joint_matrix<int8_t, use::a, tM, tN, layout::row_major> tA();
```
...
q.parallel_for(nd_range<2>(G, L), [=](nd_item<2> item)                            
  [[sycl::reqd_sub_group_size(SG_SIZE)]] {
   ...
   joint_matrix<int8_t, use::a, tM, tK, layout::row_major> tA();
   joint_matrix<int8_t, use::b, tK, tN, layout::row_major> tB();
   joint_matrix<int32_t, use::accumulator, tM, tN> tC();
...
using myparams = tpu_params<tpu::xmx, int8_t, int8_t, int, 16, 8, 32>;  
...
```c++
using myparams = tpu_params_both<tpu::xmx, int8_t, int8_t, int>;
...
myparams::joint_matrix_a sub_a();
myparams::joint_matrix_b sub_b();
myparams::joint_matrix_accumulator sub_c();
...
// device code:
joint_matrix<int8_t, use::a, msize, ksize> sub_a();
joint_matrix<int8_t, use::b, ksize, nsize> sub_b();
joint_matrix<int, use::accumulator, msize, nsize> sub_c();
//Remainder handling
```
...
- Add a second constructor to `joint_matrix` that takes no argument. The `Group` argument in the current constructor is unused in the current implementations. Having a second constructor will simplify performance code where one SG is assigned multiple joint matrices.
...
|5   |2022-11-07 |Dounia Khaldi |Update the matrix spec by moving the Intel-specifics to a separate document.  
|======================
