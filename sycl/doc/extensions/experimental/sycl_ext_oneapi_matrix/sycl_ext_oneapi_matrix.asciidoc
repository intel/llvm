...
template <typename T, use Use,
          ...
          layout Layout = (Use == use::accumulator) ? layout::dynamic : layout::invalid, 
          typename Group = sub_group>
struct joint_matrix {
...
The second overload without a memory layout uses static_assert must not used with a `joint_matrix` type that was declared with `layout::dynamic`.
