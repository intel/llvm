...
class annotated_ref {
  public:
    ...
    // OP is: +=, -=, *=, /=, %=, <<=, >>=, &=, |=, ^=
    // is_ann_ref_v<O> returns true if O is a annotated_ref<T2,P2> type
    // where T2 and P2 can be any annotated_ref supported types
...
[source,c++]
----
T operatorOP(const annotated_ref &a) const;
----
|
Writes an object referenced by another `annotated_ref` to the location referenced by this wrapper,
applying the annotations when the object is stored to memory.
...
Compound assignment operators for type `O`. `O` cannnot be a type of `annotated_ref<T2, P2>`
while T2 and P2 can be any `annotated_ref` supported types.
...
Defines a hidden friend operator `OP` overload for type `annotated_ref` and `O`. `O` cannnot be
a type of `annotated_ref<T2, P2>` while T2 and P2 can be any `annotated_ref` supported types.
