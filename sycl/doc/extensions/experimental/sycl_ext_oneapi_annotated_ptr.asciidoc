...
class annotated_ref {
  public:
    ...
    annotated_ref& operator=(const annotated_ref&) const;
    // OP is: +=, -=, *=, /=, %=, <<=, >>=, &=, |=, ^=
    annotated_ref& operatorOP(const T &) const;
...
[source,c++]
----
annotated_ref& operator=(const annotated_ref& other);
----
a|
Equivalent to:
```c++
...
applying the annotations when the object is loaded/stored to memory.
...
[source,c++]
----
annotated_ref& operatorOP(const T &);
----
a|
