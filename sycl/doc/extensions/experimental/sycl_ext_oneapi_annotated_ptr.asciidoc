...
----
It is not recommended to apply `annotated_ptr` to a non-trivially-copyable type `T`. After
dereferencing an `annotated_ptr` to an `annotated_ref`, many operators in `annotated_ref` are
return-by-value and pass-by-valuem, and thus might result in inefficient memory copies.
...
----
|
Writes an object of type `T` to the location referenced by this wrapper via perfect forwarding,
