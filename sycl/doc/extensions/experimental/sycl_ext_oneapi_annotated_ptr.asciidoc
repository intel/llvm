...
class annotated_ref {
  public:
    ...
    auto friend operatorOP(const A& a, const B& b) ->
               decltype(remove_ann_ref<A> OP remove_ann_ref<B>);

    // OP is: +, -, !
    template <class A> auto friend operatorOP(const A& a) ->
               decltype(OP remove_ann_ref<A>);
  };
...
and `B` is an `annotated_ref<T, PropertyListT>`.

Let `operatorOP` denotes the operator used and `X`, `Y` denote the type returned by `remove_ann_ref<A>`
and `remove_ann_ref<B>`.

The overloaded operator `operatorOP` utilizes `operatorOP(X, Y)` and
is available only if `operatorOP(X, Y)` is well formed. The value and result
is the same as the result of `operatorOP(X, Y)` applied to the objects of
type `X` and `Y`.

If type `A` or `B` is an `annotated_ref<T, PropertyListT>`, the annotations from `PropertyListT`
are applied when the object `a` or `b` is loaded from memory. 
...
Defines a hidden friend operator `OP` overload for types `A`.

When `A` is an `annotated_ref<T, PropertyListT>`, `remove_ann_ref<A>` returns `T`, otherwise
returns `A`. This operator is invoked only when `A` is an `annotated_ref<T, PropertyListT>`.

Let `operatorOP` denotes the operator used and `T` denote the type returned by `remove_ann_ref<A>`.

The overloaded operator `operatorOP` utilizes `operatorOP(T)` and
is available only if `operatorOP(T)` is well formed. The value and result
is the same as the result of `operatorOP(T)` applied to the objects of
type `T`.
