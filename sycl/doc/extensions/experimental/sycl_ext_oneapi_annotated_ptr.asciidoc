...
class annotated_ref {
  public:
    ...
    auto friend operatorOP(const A& a, const B& b) ->
               decltype(remove_ann_ref<A>(a) OP remove_ann_ref<B>(b));

    // OP is: +, -, !, ~
    auto friend operatorOP(const annotated_ref& a) -> decltype(OP std::declval<T>());
  };
} // namespace sycl::ext::oneapi::experimental
```
...
Defines a hidden friend operator `OP` overload for types `T`.

Let `operatorOP` denotes the operator used. The overloaded operator
`operatorOP` utilizes `operatorOP(T)` and is available only if `operatorOP(T)`
is well formed. The value and result is the same as the result of `operatorOP(T)`
applied to the objects of type `T`.
