...
Only applicable properties will be passed to `annotated_ref`.  Unless a property's description states otherwise it will not be passed on.  
...
Only applicable properties will be passed to `annotated_ref`.
...
Only applicable properties will be passed to the returned `annotated_ptr`.
...
If property list contains properties that are not sustainable through pointer
arithemic, this operator errors out.
...
This property is passed to the `annotated_ref` class when an `annotated_ptr` is
deferenced with `operator*`.
|====
--
