...
Each formal parameter of the invoked `Callable` must be a scalar type, or a SIMD type.
The length of each SIMD type must match the sub-group size of the kernel (or be a
multiple for `flatten`); calling a SIMD function with mismatched lengths
...
`sycl::ext::oneapi::experimental::flatten`.  Arguments may not be pointers or references,
...
- Arguments of type `sycl::ext::oneapi::experimental::flatten<std::array<T, M>>`
  are converted to type `sycl::ext::oneapi::experimental::simd<T, N*M>`,
  where `N` is the sub-group size of the calling kernel. The m-th element from each
  work-item is stored consecutive.  
...
- Return value of type `sycl::ext::oneapi::experimental::simd<T, N*M>` is converted
  to `sycl::ext::oneapi::experimental::flatten<array<T, M>>`.
...
type, and `N` must be equal to the the sub-group size (or multiple for `flatten`).
...
=== `flatten` wrapper type

`flatten` is template in `sycl::ext::oneapi::experimental` namespace and can only be instantiated for `std::array`.
It can be implicitly converted to and from `std::array`. Synapsis:

[source, c++]
----
template<class T> //only well-formed if T is std::array<>
struct flatten
{
  flatten(T);         // conversion constructor
