...
The `nd_launch` overloads that take the kernel arguments as a `std::span`
depend on
version 2 of
...
Those overloads use `std::span`, so they are available only when the application
is compiled with C++20 or later.
