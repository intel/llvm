...
The `nd_launch` overloads that take the kernel arguments as a `std::span` depend
on version 2 of link:../experimental/sycl_ext_oneapi_raw_kernel_arg.asciidoc[
...
The arguments of a `sycl::kernel` can be passed either as a parameter pack or as
a sequence, for an application that only learns its argument list at run time and
would otherwise have to instantiate a parameter pack overload for each number of
arguments it may encounter. The sequence is a `std::span`, so those overloads
require C++20.
...
|Adds the `nd_launch` overloads that take the arguments of a `sycl::kernel` as
 a `std::span` of `raw_kernel_arg`, which require C++20, and specifies that a
 parameter pack overload handed such a sequence as its only argument passes it
 on as the argument list.
|===
...
whose type is convertible to `std::span<const raw_kernel_arg>`, that argument is
the kernel argument list and the effects are those of the overload taking a
`std::span`.
...
An argument list that is only known at run time is held in a container and passed
as the sequence of arguments: a `std::vector<raw_kernel_arg>`, a `std::array` of
them and a `std::span` of them all convert to `std::span<const raw_kernel_arg>`.
These overloads require C++20.
...
int count = 1024;
float *data = sycl::malloc_device<float>(count, q);

std::vector<syclex::raw_kernel_arg> args;
args.emplace_back(&data, syclex::pointer_arg);
args.emplace_back(&count, sizeof(count));

syclex::nd_launch(q, sycl::nd_range<1>{count, 256}, k, args);
----
