...
The overloads that take the kernel arguments as a `std::span` depend on
version 2 of link:../experimental/sycl_ext_oneapi_raw_kernel_arg.asciidoc[
sycl_ext_oneapi_raw_kernel_arg], which is what lets an element of that sequence
represent a pointer argument.  Those overloads use `std::span`, so they are
available only when the application is compiled with C++20 or later.
...
|2
|Adds the overloads that take the arguments of a `sycl::kernel` as
 a `std::span` of `raw_kernel_arg`.
|===
...
----
!====
_Constraints_: `+sizeof...(Args)+` is not one, or the single argument in
`args` is not convertible to `std::span<const raw_kernel_arg>`.
...
void single_task(sycl::queue q, const sycl::kernel& k, // Requires C++20
                 std::span<const raw_kernel_arg> args);

void single_task(sycl::handler &h, const sycl::kernel& k, // Requires C++20
                 std::span<const raw_kernel_arg> args);
...
!====
_Effects_: Enqueues a kernel object to the `sycl::queue` or `sycl::handler` as
a single task. The element `args[i]` is passed to the kernel as the argument at
index `i`, as if it had been passed to `set_arg`. The sequence referenced by
...
template <int Dimensions> // Requires C++20
void parallel_for(sycl::queue q, sycl::range<Dimensions> r,
                  const sycl::kernel& k,
                  std::span<const raw_kernel_arg> args);
...
_Effects_: Enqueues a kernel object to the `sycl::queue` or `sycl::handler`
as a basic kernel, using the number of work-items specified by a `sycl::range`.
The element `args[i]` is passed to the kernel as the argument at index `i`, as
if it had been passed to `set_arg`. The sequence referenced by `args` need only
...
template <int Dimensions, typename Properties> // Requires C++20
void parallel_for(sycl::queue q,
                  launch_config<sycl::range<Dimensions>, Properties> c,
                  const sycl::kernel& k,
                  std::span<const raw_kernel_arg> args);
...
_Effects_: Enqueues a kernel object to the `sycl::queue` or `sycl::handler`
as a basic kernel, using the launch configuration specified by a
`launch_config`. The element `args[i]` is passed to the kernel as the argument
...
template <int Dimensions> // Requires C++20
void nd_launch(sycl::queue q, sycl::nd_range<Dimensions> r,
               const sycl::kernel& k, std::span<const raw_kernel_arg> args);
...
template <typename T>
void host_task(sycl::handler &h, T&& hostTaskCallable);

}
----
!====
_Preconditions_: The callable function `hostTaskCallable` does not call any
function in the `sycl` namespace.

_Effects_: Enqueues a command to the `sycl::queue` or `sycl::handler` that
executes `hostTaskCallable` exactly once on an unspecified host thread.
...
The following example launches a kernel whose argument list is assembled at run
time, as a launcher for kernels that are not known at compile time would do, so
the arguments are passed as a sequence rather than as a parameter pack.  A
`std::vector<raw_kernel_arg>`, a `std::array` of them and a `std::span` of them
all convert to `std::span<const raw_kernel_arg>`.
...
void scale(float *data, int factor) { /* kernel body */ }

int main() {
  sycl::queue q;

  auto bundle =
      syclex::get_kernel_bundle<scale, sycl::bundle_state::executable>(
          q.get_context());
  sycl::kernel k = bundle.ext_oneapi_get_kernel<scale>();

  float *data = sycl::malloc_device<float>(N, q);
  int factor = 2;

  std::vector<syclex::raw_kernel_arg> args;
  args.emplace_back(&data, syclex::pointer_arg);
  args.emplace_back(&factor, sizeof(factor));

  syclex::nd_launch(q, sycl::nd_range<1>{{N}, {256}}, k, args);
  q.wait();
