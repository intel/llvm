...
The `nd_launch` overloads that take the kernel arguments as a `span` depend on
version 2 of
...
void nd_launch(sycl::queue q, sycl::nd_range<Dimensions> r,
               const sycl::kernel& k, span<const raw_kernel_arg> args);
...
An application that calls one of the parameter pack overloads above with a
single argument whose type is convertible to `span<const raw_kernel_arg>` is
ill-formed.
...
pack overload for each number of arguments it may encounter. Since `span` has
no implicit conversion from a container, an application holding its arguments
in a `std::vector<raw_kernel_arg>` passes `{args.data(), args.size()}`. Passing
a single `raw_kernel_arg` still selects the parameter pack overload. _end note_]
