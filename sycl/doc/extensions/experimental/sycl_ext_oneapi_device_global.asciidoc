...
_T_ is restricted to types that have a trivial destructor in this revision of the specification. A trivial default constructor is also required when compiling with {cpp} versions before {cpp}20. When compiling with {cpp}20, and when used on a `device_global` with the `device_image_scope` property, no trivial default constructor is required (this constructor restriction may be partially relaxed in a future revision). _PropertyListT_ enables properties to be associated with a `device_global`.

If no trivial default constructor is required with _T_, a non-default `device_global` constructor can be called. This allows for the specification of initialization values for the contents of a `device_global` allocation on a device. The allocation will occur before the first access to that `device_global` on that device.

If a trivial default constructor is required, or the default `device_global` constructor has been called, the allocation of type _T_ for a given `device_global` is zero-initialized on a given device prior to the first access to that `device_global` on that device. For the purposes of this definition an access can be a direct access of the `device_global` in kernel code or a copy to or from that `device_global` enqueued to the given device.
...
The storage on each device for `T` is initialized with the contents of the arguments.
...
When compiling with {cpp}20, and when used on a `device_global` with the `device_image_scope` property, constant compile-time initialization for device_globals is supported. The following example shows a few examples of what this would look like:
...
In a future version of this extension, it is expected that when {cpp}20 support is available and enabled, the `consteval` keyword will be used to enable compile-time constant initialization of the device allocations backing `device_global` without the `device_image_scope` property.  This will simplify some coding patterns, compared with the current zero-initialization requirement.
