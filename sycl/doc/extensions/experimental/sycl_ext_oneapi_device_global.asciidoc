...
_T_ is restricted to types that have a trivial destructor. Furthermore, `T` must also have a trivial default constructor when compiling with {cpp} versions before {cpp}20. _PropertyListT_ enables properties to be associated with a `device_global`.

If no trivial default constructor is required with _T_, a non-default  `consteval` `device_global` constructor can be called. This allows for the specification of initialization values for the contents of a `device_global` allocation. The allocation will occur before the first access to that `device_global` on that device.
