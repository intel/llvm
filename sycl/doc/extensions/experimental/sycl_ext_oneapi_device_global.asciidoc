...
----
a|
Normally, a single instance of a device
...
property is specified, but the user must be aware
...
implementation may be able to perform certain optimizations.

The following values are supported:

* `read`: The user asserts that the host code may copy from (read) the
  variable, but it will never copy to (write) it.
* `write`: The user asserts that the host code may copy to (write) the
  variable, but it never copy from (read) it.
...
* A kernel that uses specialization constants may have a new instance in a new
  ...
  native specialization constants.
====
