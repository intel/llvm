...
----
|-
|Some architecture which is not supported by this extension.
|===
...
_Returns:_ The architecture of the device if architecture is supported, overwise 
...
The architecture enumeration `unknown` is not currently supported in AOT mode.
