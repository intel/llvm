...
many registers.
However, increasing this size may also reduce the maximum work-group size for
the kernel.
...
This extension adds the following properties that can be used when defining a
kernel.
However, using these properties to select a larger GRF size may impose a lower
