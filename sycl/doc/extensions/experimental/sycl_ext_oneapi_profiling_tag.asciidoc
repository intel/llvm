...
The event's `info::event_profiling::command_submit`, `command_start`, and
`command_end` timestamps are all equal to the device time at which the tag
command executes.

[_Note:_ A tag captures a single device timestamp, so all three fields report
the same value.
This value shares the same time base as other commands on the queue, so the
elapsed device time between two tags is the difference of their timestamps.
_{endnote}_]
