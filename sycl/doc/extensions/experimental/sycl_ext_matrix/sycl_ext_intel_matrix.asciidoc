...
functions checks that the 2d block starting at the address to be accessed
`[src,0]` and ending at `[src + Cols, src + Rows * stride]` does not
exceed the bigger 2d block starting at `[src, 0]` and ending at `[src
+ Width, src + Height * stride]`. For instance, load of `sub_b` defined as
`joint_matrix<sub_group, bfloat16, use::b, 2, 3, layout::row_major>
sub_b;` from `ptr + 9` checks that `ptr + 9` does not exceed `ptr +
10` where 10 is `Width` so it reads only 1 element for that
iteration. The 2 columns left that exceed the total `10` columns are
filled by zero.
