...
the original memory matrix excluding any offset calculations. The
coordinates are given separately here.
* The coordinates `coordX` and `coordY` to calculate the  pointer
offset to load/store are given as separate arguments.
* These variants take extra arguments to determine the global bounds
`Height` and `Width` of the two-dimensional region to load to or store
from.

The two-dimensional memory region is interpreted as follows, where
`Height` is number of rows in the memory matrix, `Width` is number of
columns in the memory matrix, `stride` is number of rows that include
...
of the picture above for each sub-group execution. The out of bounds
functions checks that the 2d block starting at the address to be
accessed `[0, src]` and ending at `[Cols, src + Rows * stride]` does
not exceed the bigger 2d block starting at `[0, src]` and ending at
`[Width, src + Height * stride]`. For instance, load of `sub_b`
defined as `joint_matrix<sub_group, bfloat16, use::b, 2, 3,
layout::row_major> sub_b;` from `ptr + 9` checks that `ptr + 9` does
not exceed `ptr + 10` where 10 is `Width` so it reads only 1 element
for that iteration. The 2 columns left that exceed the total `10`
columns are filled by zero.
...
void joint_matrix_store_checked(Group g,
   const joint_matrix<Group, T, Use, Rows, Cols, Layout> &res,
   ext::oneapi::experimental::annotated_ptr<T2, PropertyListT> base_dest,
   size_t Stride, size_t Height, size_t Width, size_t coordX, size_t coordY);
