...
`joint_matrix_store`. The only difference is that these new variants
take extra arguments to determine the global bounds `Height` and
`Width` of the two-dimensional region to load to or store from.
...
void joint_matrix_fill_checked(Group g, joint_matrix<Group, T, Use, Rows,
          Cols, Layout> &m, Tv v, size_t Stride, size_t Height, size_t Width);
