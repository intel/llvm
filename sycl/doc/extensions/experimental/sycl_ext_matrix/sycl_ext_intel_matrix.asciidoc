...
=== Load/Store Using Annotated Pointers
These are overloads of existing `joint_matrix_load` and
`joint_matrix_store` that take
...
represent the compile-time cach control constant properties included
...
auto aptr = annotated_ptr(ptr, syclex::properties{syclintelex::cache_control_read<cache_control_read_type::cached, cache_level::L2>});
q.parallel_for(nd_range<2>(G, L), [=](nd_item<2> it) {
  ...
  for (int k = 0; k < K; k += tileK) {
    ...
    joint_matrix_load(sg, tA, aptr + sg_startx * tM * K + k, K);
    ...
  }
});
```
...
template <typename Group, typename T, typename PropertyListT, size_t Rows, size_t Cols,
...
prefetch `Rows x cols` data in a 2d manner. This function is a group
...
SYCL extenion
* link:../../proposed/sycl_ext_oneapi_prefetch.asciidoc[sycl_ext_oneapi_prefetch.asciidoc]
...
q.parallel_for(nd_range<2>(G, L), [=](nd_item<2> it) {
  sub_group sg = it.get_sub_group();
  for (int k = 0; k < K; k += tileK) {
    syclex::joint_matrix_prefetch(sg, memA, K, layout::row_major, tileM, tileK,
                      properties{syclex::prefetch_hint_L2});
    ...
  }
});
```
...
|2   |2023-10-19 |Dounia Khaldi |Add Intel-specific prefetch and cache
control load/store APIs
|======================
