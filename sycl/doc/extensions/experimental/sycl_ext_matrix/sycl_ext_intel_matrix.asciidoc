...
* The pointer `src` designates the base pointer of the original memory
matrix excluding any offset calculations. The coordinates are given
separately here.

* The coordinates to calculate the base pointer offset to load/store are
...
of the picture above for each sub-group execution. At the last
iteration of the range of the launched kernel, the remainder that
should exclude the out-of-bounds data is checked against `Width` and
`Height`. For instance, load of `sub_b` defined as
`joint_matrix<sub_group, bfloat16, use::b, 2, 3, layout::row_major> sub_b;`
checks that `Height=10` is not multiple of `3` and the data (2
elements left) that exceeds getting the total `10` rows will be filled
by zero.
...
void joint_matrix_store_checked(Group g,
   ...
   multi_ptr<T, Space, IsDecorated> base_src, size_t stride,
   size_t Height, size_t Width, size_t coordX, size_t coordY);
...
void joint_matrix_store_checked(Group g,
   ...
   size_t stride, layout Layout, size_t Height, size_t Width, size_t coordX, size_t coordY);
