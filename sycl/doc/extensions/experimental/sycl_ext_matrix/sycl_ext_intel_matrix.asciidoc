...
`joint_matrix_store` to utilize
`sycl::ext::oneapi::experimental::annotated_ptr` instead of
`sycl::multi_ptr`.
...
auto aptr = annotated_ptr(ptr, syclex::properties{syclintelex::cache_control_read<cache_control_read_type::cached, 2>});
q.parallel_for(nd_range<2>(G, L), [=](nd_item<2> it) {
  sub_group sg = it.get_sub_group();
  for (int k = 0; k < K; k += tileK) {
    syclex::joint_matrix_prefetch(sg, ptr, K, layout::row_major, tileM, tileK, properties{syclex::prefetch_hint_L2});
    gemm_k(sg, aptr);
  }
});
```
