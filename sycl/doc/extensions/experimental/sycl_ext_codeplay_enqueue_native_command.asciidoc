...
With the exception of calling synchronous APIs required to append commands to a
native graph object, see
link:../experimental/sycl_ext_oneapi_graph.asciidoc[sycl_ext_oneapi_graph],
the call to `interopCallable` must not submit any
synchronous tasks to the native backend object. The call must also not block
