...
Currently, SYCL applications are linked with MSVC/GNU C/{cpp} runtimes on Win/Lin
...
distribute LLVM's {libcxx}/libc headers as part of the kernel compiler and
default to using them for SPIR-V based targets. That results in a restriction
not to pass any data that transitively uses any types defined in C/{cpp} headers
(with the exception of `[u]intN_t` type aliases) as those might be
ABI-incompatible between headers used to compiler device code and those used to
compile the host part of the SYCL application.
...
{libcxx}/libc headers. That implicitly removes CUDA/HIP SKDs and would break
online compilation for those targets, thus we do *not* employ that mechanism for
those targets. Instead, end user's system is required to have a C/{cpp} toolchain
installation.
