...
Finally, the compiler searches a set of implicitly available header files, as
defined in the section "Implicitly available headers".

==== Adding paths to the search list

Directories on the local filesystem can be appended to the search path by adding
one or more occurences of the `-I` option to the `build_options` property. This
...
==== Implicitly available headers

The following header files are implicitly available, and do not need to be
specified via the `include_files` property.
