...
** `B` is an enum value: The type of control from the set {`latency::exact`, `latency::min`, `latency::max`}.
...
class pipe {
  ...
  static dataT read( props p ); // p must be a property_lis

  static void write( const dataT &data );

  template <typename props>
  static void write( const dataT &data, props p ); // p must be a property_list

  // Non-blocking
  static dataT read( bool &success_code );

  template <typename props>
  static dataT read( bool &success_code, props p ); // p must be a property_list

  static void write( const dataT &data, bool &success_code );

  template <typename props>
  static void write( const dataT &data, bool &success_code, props p ); // p must be a property_list
}
----
