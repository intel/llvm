...
This non-normative section describes the conditions when the DPC++ implementation provides an optimization benefit* for the `discard_events` property.
 ...
 - A kernel submitted to the queue must not use the https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/supported/SYCL_EXT_ONEAPI_ASSERT.asciidoc[fallback assert feature].
