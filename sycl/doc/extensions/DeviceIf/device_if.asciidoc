...
feature.  For example, consider a library function that wants to conditionally
...
The first overload takes a single callable (typically a lamdba expression),
...
online compiler will not raise a compilation error from an unselected lamdba
...
```
if (__builtin_spec_constant(/*SPIR-V ID for aspect::fp16*/)) call lamdba
```
...
body of the "if" (the lamdba call) is discarded if the boolean expression is
`false`.
