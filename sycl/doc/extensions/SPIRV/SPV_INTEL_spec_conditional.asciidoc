...
* For each instruction `idef` that defines an SSA _<id>_, search for all other
  instructions `i` that use _<id>_.  Compute `G(iuse)` as:
+
--
  - Let `G(iuse) = G(i)`.
  - If instruction `i` is *OpPhiSpecConstantINTEL*, compute `G(iuse)`
    separately for each _Parent i_ as `G(iuse) = G(i) && S` where `S` is the
    specialization constant _Cond i_ associated with _Parent i_.
--
+
Validate that each `G(iuse)` can never be true unless `G(idef)` is also true.
