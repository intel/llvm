...
`sycl::ext::oneapi::experimental::annotated_arg` classes. They are only
meaningful on the kernel argument when the kernel argument is a pointer type or
a pointer wrapper type and are not meaningful within the kernel body.
