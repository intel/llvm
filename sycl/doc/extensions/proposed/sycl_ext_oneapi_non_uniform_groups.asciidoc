...
Two or more work-items are considered to have _simultaneous_ execution at a given
point if they execute together. The amount of and granularity of simultaneous
...
all work-items in the group must call exactly the same function under the same
set of conditions -- calling the same function under different conditions
(e.g. in different iterations of a loop, or different branches of a conditional
statement) results in undefined behavior.
...
if (will_branch)
{
  ...
  // further subdivide group into work-items that will branch further
  auto will_branch_further = inner.get_local_linear_id() < 8;
  auto inner_inner = sycl::ext::oneapi::experimental::get_ballot_group(inner, will_branch_further);

  if (will_branch_further)
  {
    // synchronize across the work-items that took both branches
...
if (will_branch)
{
  ...
  {
    // synchronize across the work-items that took both branches
    auto inner_inner = sycl::ext::oneapi::experimental::get_tangle_group(inner);
    sycl::group_barrier(inner_inner);
  }
}
----
...
int atomic_aggregate_inc(sycl::sub_group sg, sycl::atomic_ref<int, Order, Scope, AddressSpace> ptr) {
  ...
  auto active_group = sycl::ext::oneapi::experimental::get_opportunistic_group(sub_group);
