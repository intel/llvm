...
`ballot_group`, `partitioned_group`, `tangle_group`, or `opportunisic_group`.

Additionally, the `is_group<T>::value` trait from SYCL 2020 is `std::true_type`
if `T` is one of: `ballot_group`, `partitioned_group`, `tangle_group`, or
`opportunistic_group`.
...
The following group functions support the `ballot_group`, `partitioned_group`,
`tangle_group`, and `opportunistic_group` group types:

* `group_barrier`
* `group_broadcast`

The following group algorithms support `ballot_group`, `partitioned_group`,
`tangle_group`, and `opportunistic_group` group types:
...
A partitioned-group is a contiguous collection of work-items created by subdividing
a group into equally sized parts, such that each work-item is a member of
exactly one partition. The size of a partitioned-group is a static (compile-time)
property.


==== Creation

New partitioned-groups are created by partitioning an existing group, using the
`get_partitioned_group` free-function.

NOTE: Creating a partitioned-group does not require a barrier across all work-items
...
partitioned_group<PartitionSize, Group> get_partitioned_group(Group group);
...
_Returns_: A `partitioned_group<PartitionSize>` consisting of all work-items in
`group` that are in the same partition as the calling work-item.


==== `partitioned_group` Class
...
NOTE: `partitioned_group` is templated on a `ParentGroup` because it is expected
that it will eventually be possible to construct a partitioned-group from more
than only sub-groups.
...
id_type get_group_id() const;
----
_Returns_: An `id` representing the index of the partitioned-group.
...
range_type get_group_range() const;
----
_Returns_: A `range` representing the number of partitioned-groups.
...
partitioned-group, which is always equal to `PartitionSize`.
...
bool leader() const;
----
_Returns_: `true` for exactly one work-item in the partitioned-group, if the calling
work-item is the leader of the partitioned-group, and `false` for all other
work-items in the partitioned-group. The leader of the partitioned-group is guaranteed
to be the work-item for which `get_local_id()` returns 0.


==== Usage Examples

A `partitioned_group` can be used to apply group algorithms to subsets of data:
...
auto partition = sycl::ext::oneapi::experimental::get_partitioned_group<8>(sg);
...
A `partitioned_group` can be used to provide an interface accepting a specific
number of work-items:

[source, c++]
----
void func_that_needs_4_work_items(sycl::ext::oneapi::experimental::partitioned_group<4> group);
----
...
instructions]. `partitioned_group` functionality is expected to leverage the
