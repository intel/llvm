...
_Throws:_ An `exception` with the `errc::feature_not_supported` error code if
`evt` was created with the `enable_profiling` property that enables profiling
timestamps and if the platform associated with `q` does not support creation of
such events as reported by the `event_profiling` information descriptor.
