 ...
 * An `exception` with the `errc::invalid` error code if `evt` is non-reusable
   (`event::ext_oneapi_reusable() returns false) or any event in `evts` is
   non-reusable.
...
Reusable events can only be used with APIs defined in this extension.

Several of the `queue` class member functions from the core SYCL specification
return an event. These events are non-reusable. They cannot be passed to any
APIs defined in this extension.
