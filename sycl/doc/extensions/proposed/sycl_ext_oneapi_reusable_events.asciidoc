...
class event {
  // ...
  std::optional<device> ext_oneapi_get_device() const;
...
_Returns:_ The device object associated with this event. If the event is not
associated with a device, returns `std::nullopt`.
...
_Throws:_ If `evt` has an associated device, an `exception` with the
`errc::invalid` error code if the device associated with `evt` differs from the
device associated with `q` and the devices cannot P2P access each other. The
P2P access between devices can be queried using the
`device::ext_oneapi_can_access_peer` function and enabled using the
`device::ext_oneapi_enable_peer_access` function defined in the
