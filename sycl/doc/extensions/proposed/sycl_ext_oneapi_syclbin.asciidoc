...
`-fsycl-allow-device-image-dependencies` is required: it tells the
post-link stage to emit the exported and imported symbol metadata that the
runtime needs to resolve `SYCL_EXTERNAL` references across separately
compiled device images. Without it, `sycl::link` throws an `exception`
with the `errc::invalid` error code and the message
...
int main() {
  ...
  // The same option must be passed when compiling a runtime-compiled
  // source bundle that participates in a cross-image link.
