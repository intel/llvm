...
void group_load(Group gh, InputIteratorT in_iter,
...
in striped case:
`out_iter[g.get_local_linear_id() + gh.get_local_linear_range() * i]` = `in[i];`
...
there is a enum
(proposed in link:https://github.com/intel/llvm/blob/sycl/sycl/doc/extensions/proposed/sycl_ext_oneapi_group_sort.asciidoc[`group_sort extention`]):
