...
class annotated_ptr {
  public:
    ...
    annotated_ptr(const annotated_ptr&) noexcept;
    ...
    annotated_ptr& operator=(const annotated_ptr&) noexcept;
...
`PropertyListV` will be merged together to construct `PropertyListT`. If any
common properties have different values then an error will be issued.
...
6) Ctor should clarify whether when constructing the object from two different
property lists, duplicates will exist or not.
