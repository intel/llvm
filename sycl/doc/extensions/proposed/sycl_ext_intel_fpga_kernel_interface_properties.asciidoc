...
enum class streaming_interface_options_enum : /* unspecified */ {
...
enum class register_map_interface_options_enum : /* unspecified */ {
  wait_for_done_write,
...
|`fpga_cluster`
|The `fpga_cluster` property adds a request, to the extent possible, for the
 compiler to implement statically-scheduled clusters using the specified
 clustering method. The following values are supported:
 ...
 *Note*: Not all clusters can be implemented with the provided preference.

 If the `fpga_cluster` property is not specified, the default behavior is
 equivalent to one of the values listed above, but the choice is implementation
 defined.
