...
|Two sets of values are accepted as property parameters - a boolean, and another
 ...
 The second argument specifies the minimum number of cycles between successive
 invocations of the kernel:

 * `true, initiation_interval<N>`: The compiler will pipeline multiple kernel
   invocations such that an invocation can be launched every N cycles if one is available.
 
 If the `initiation_interval` argument is not specified, the behavior is
 implementation defined.
