...
=== Motivation
`fpga_mem` is a class template templated on type T, that represents an object of type T. It is a request to the compiler to implement that object, when present in FPGA device code, in off-datapath memory, as opposed in the datapath.
...
* The `fpga_mem` variable can only have accesses from a single kernel. This happens when `device_global` is templated on a `fpga_mem` type. 

* The `fpga_mem` variable cannot be templated on a `fpga_datapath` object, as this is a conflict of requirements on how the variable should be implemented, on or off the datapath. Diagnostic is required.
