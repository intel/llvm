...
`fpga_mem` initializes the underlying object `T` with the values passed into its constructor. If no values are passed into the constructor C++ default initialization rules will dictate the value of the underlying object `T`.
...
* The `fpga_datapath` and `fpga_mem` are tempate classes that have conflicting requirements on how a variable should be implemented, on or off the datapath. In situations where an object of one of these classes is templated on another, the less specific template will used by the compiler, and the rest will be ignored.

[source,c++]
----
namespace intelexp = sycl::ext::intel::experimental;

// 10 integers will be implemented off the datapath, in memory
// `fpga_datapath` is ignored.
intelexp::fpga_mem<intelexp::fpga_datapath<int[10]>> dm1;

----

* `fpga_mem` and `fpga_datapath` objects can be member variables of a struct. If the struct type itself is used as a template argument to `fpga_mem` or `fpga_datapath` any hints provided from the member variables are ignored. 
...
// `var` will be split into two variables
// `var.a` will be implemented in memory, off the datapath
// `var.b` will be implemented in the datapath
foo var;

// Hints applied on member variables ignored
// `dm1` implemented in memory, off the datapath
intelexp::fpga_mem<foo> dm1;

// Hints applied on member variables ignored
// `dm2` implemented in the datapath
intelexp::fpga_datapath<foo> dm2;
----
