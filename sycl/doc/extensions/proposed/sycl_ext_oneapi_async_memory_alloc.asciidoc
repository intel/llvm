...
+
--
*UNRESOLVED*: Currently this extension only support host USM allocations 
without a `memory_pool` via `async_alloc` and does not support shared USM allocations,
as these are much more complex to implement compared to device USM allocations.
shared allocations and full support for host allocations are expected to be supported in the future. 
--
