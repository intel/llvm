...
device from the queue `que``, with the allocation kind `kind` and applying any
...
|The `zero_init` property adds the requirement that all memory reserved by the
 ...
 the pool, so users must re-initialize memory to zero if they wish for later
 allocations to have this behavior.
...
is the allocation kind which is used. If a memory pool `pool` is provided, this
must be associated with the same SYCL context and device as `que` or the SYCL
queue associated with `cgh`. Memory can either be allocated directly from the
memory pool or allocated to the memory pool to provide enough memory in the
