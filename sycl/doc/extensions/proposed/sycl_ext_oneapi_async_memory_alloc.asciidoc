...
pool of memory managed by the SYCL runtime, and adheres to the SYCL common
reference semantics.
...
pool or allocated from the source to the memory pool to provide enough memory in
the memory pool for the allocation. Accessing the memory at the address of the
