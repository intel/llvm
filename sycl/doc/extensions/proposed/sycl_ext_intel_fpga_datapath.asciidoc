...
`fpga_datapath` initializes the underlying object of type `T` with the values passed into its constructor. If no values are passed into the constructor C++ default initialization rules will dictate the value of the underlying object `T`.
