...
=== Motivation
`fpga_datapath` is a wrapper around user variables that asserts the variable should be implemented in the datapath of the kernel and not use FPGA memory resources.
...
----
template<typename ... S>
constexpr fpga_datapath_base(S ... args);
----
|
...
* The `fpga_datapath` variable must not itself be wrapped in a `fpga_datapath` class
* The `fpga_datapath` variable must not have dynamic storage duration
* The `fpga_datapath` variable can only have accesses from a single kernel
