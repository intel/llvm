= sycl_ext_oneapi_device_specific
...
This extension introduces device-specific memory allocations into SYCL that can be
...
are provided for the host program to enqueue copies to or from device-specific
allocations.  Restrictions are placed on the types of data that can be
stored within `device_specific` allocations, particularly around copyability and
...
specification.*  See
link:../experimental/sycl_ext_oneapi_device_global.asciidoc[here] for the existing
...
[NOTE]
====
In this document, we use `device_specific` to indicate the proposed `sycl::ext::oneapi::experimental::device_specific`.
====

The purpose of this document is to clearly describe and specify `device_specific` and related
...
Two example `device_specific` variables can be declared at namespace scope, as follows:
...
device_specific<MyClass> dm1;
static device_specific<int[4]> dm2;
----
...
in {cpp} (access directly through the namespace scope identifier), and `device_specific` has
reference wrapper-like semantics on a device.  Of note, because {cpp} doesn't allow for
overloading of the "dot operator", a `get()` member of `device_specific` allows a reference
...
`SYCL_EXT_ONEAPI_DEVICE_SPECIFIC` to one of the values defined in the table below.
...
=== Representation of device-specific variables

`device_specific` provides a mechanism to allocate device scope memory - memory
...
that reference the same `device_specific` entity (either directly or via a pointer
...
`device_specific` allocations are in the global address space, as are any
underlying allocations of type `T` which are implicitly allocated on each device
as a result of a `device_specific` object. It is undefined behavior if the host
program directly accesses a `device_specific` or any address obtained from a
`device_specific` member function, and similarly it is undefined behavior if a
`device_specific` or address obtained on one device from a `device_specific` member
function is accessed on a different device or context.  There is no mechanism to
obtain addresses of or directly access a device's `device_specific` allocation
within the host program.

A `device_specific` on a given device and context maintains its state (address of
...
`handler::set_specialization_constant()`.  Additionally, a `device_specific`
...
class device_specific {
  ...
----

`device_specific` is a class template, parameterized by the type of the underlying allocation _T_, and a list of properties _PropertyListT_. The type of the allocation _T_ also encodes the size of the allocation for potentially multidimensional array types.

_T_ is restricted to types that have a trivial destructor and a trivial default constructor in this revision of the specification (the constructor restriction may be partially relaxed in a future revision). _PropertyListT_ enables properties to be associated with a `device_specific`.

Since _T_ is restricted to types with trivial default constructors in this version of the specification, there are no non-default `device_specific` constructors, and therefore no initialization values may be specified for the content of a `device_specific` allocation on a device.

The allocation of type _T_ for a given `device_specific` is zero-initialized on a given device prior to the first access to that `device_specific` on that device. For the purposes of this definition an access can be a direct access of the `device_specific` in kernel code or a copy to or from that `device_specific` enqueued to the given device.

Properties may be specified for a `device_specific` to provide semantic
...
On a device, `device_specific` has similar semantics to a reference wrapper.  The dot operator (`operator.`) cannot be overloaded, so a `get()` member is provided to allow a reference to be extracted directly when needed.  Some operators are declared in `device_specific` that must be members (e.g. `operator[]` and `+operator->+`).  Note that other operators can be overloaded by specific `T` as free functions, which will be selected through implicit conversion to `T` in device functions.

====


The section below and the table following describe the constructors, member functions and factory methods for `device_specific`.
...
public:
  ...
  device_specific(const device_specific &) = delete;
  device_specific(const device_specific &&) = delete;
  device_specific &operator=(const device_specific &) = delete;
  device_specific &operator=(const device_specific &&) = delete;
  ...
  device_specific& operator=(const T&) noexcept;
  ...
  // Note that there is no need for "device_specific" to define member functions for
  ...
  // from "device_specific" to "T&", the operations can be applied to objects of type
  // "device_specific<T>".
...
----
|
Constructs a `device_specific` object, and implicit storage for `T` in the global address space on each device that may access it.
...
Available only if `sycl::is_property_key_of_v<propertyT, sycl::ext::oneapi::experimental::device_specific>` is true.
...
=== Restrictions on creating device-specific objects

There are restrictions on how the application can create objects of type
`device_specific`.  Applications that violate these restrictions are ill-formed.

* The application may declare a variable of type `device_specific` in the
  following ways:
+
--
...
--
+
The application must not create an object of type `device_specific` in any other
way.  (E.g. variables with automatic storage duration or objects created via
`new` are not allowed.)

* The `device_specific` variable must not itself be an array.  The underlying
  type _T_ may be an array type, but the `device_specific` variable itself must
  not be an array.

* The `device_specific` variable must not be shadowed by another identifier _X_
  which has the same name and is declared in an inline namespace, such that the
  `device_specific` variable is no longer accessible after the declaration of
  _X_.

* If the `device_specific` variable is declared in a namespace, none of the
...
the end of a translation unit which references each `device_specific` variable
...
device_specific<int> a;           // OK
static device_specific<int> b;    // OK
inline device_specific<int> c;    // OK

struct Foo {
  static device_specific<int> d;  // OK
};
device_specific<int> Foo::d;

struct Bar {
  device_specific<int> e;         // ILLEGAL: non-static member variable not
};                              // allowed

struct Baz {
 private:
  static device_specific<int> f;  // ILLEGAL: not publicly accessible from
};                              // namespace scope
device_specific<int> Baz::f;

device_specific<int[4]> g;        // OK
device_specific<int> h[4];        // ILLEGAL: array of "device_specific" not
                                // allowed

device_specific<int> same_name;   // OK
namespace foo {
  device_specific<int> same_name; // OK
}
namespace {
...
inline namespace other {
  device_specific<int> same_name; // ILLEGAL: shadows "device_specific" variable
}                               // with same name in enclosing namespace scope
inline namespace other2 {
  namespace foo {               // ILLEGAL: namespace name shadows "::foo"
  }                             // namespace which contains "device_specific"
                                // variable.
}
----

=== Properties for device-specific variables

The `device_specific` class supports several compile-time-constant properties.
...
device_specific<MyClass, decltype(properties(device_image_scope))> dm1;
device_specific<int[4], decltype(properties(host_access_read))> dm2;
----
...
struct is_property_key_of<device_image_scope_key, device_specific<T, PropertyListT>>
  : std::true_type {};
template <typename T, typename PropertyListT>
struct is_property_key_of<host_access_key, device_specific<T, PropertyListT>>
  : std::true_type {};
template <typename T, typename PropertyListT>
struct is_property_key_of<init_mode_key, device_specific<T, PropertyListT>>
  : std::true_type {};
template <typename T, typename PropertyListT>
struct is_property_key_of<implement_in_csr_key, device_specific<T, PropertyListT>>
  : std::true_type {};
...
but it may be used with any kernel.  Normally, a single instance of a
device-specific variable is allocated for each device, and that instance is
shared by all kernels that belong to the same context and are submitted to the
same device, regardless of which _device image_ contains the kernel.  When this
property is specified, it is an assertion by the user that the device-specific
variable is referenced only from kernels that are contained by the same _device
image_.  An implementation may be able to optimize accesses to the
device-specific variable when this property is specified (especially on an FPGA
device), but the user must be aware of which _device image_ contains the
kernels that use the variable.

A device-specific variable that is decorated with this property may not be
accessed from kernels that reside in different _device images_, either by
direct reference to the variable or indirectly by passing the variable's
address to another kernel.  The implementation is required to diagnose an error
if the kernels that directly access a variable do not all reside in the same
_device image_, however no diagnostic is required for an indirect access from
another _device image_.

When a device-specific variable is decorated with this property, the
implementation re-initializes it whenever the _device image_ is loaded onto the
device.  As a result, the application can only be guaranteed that a
device-specific variable retains its value between kernel invocations if it
understands when the _device image_ is loaded onto the device.  For an FPGA,
this happens whenever the device is reprogrammed.  Other devices typically load
the _device image_ once before the first invocation of any kernel in that
_device image_, and then it remains loaded onto the device until the program
terminates.

The application may copy to or from a device-specific variable even before any
kernel in the _device image_ is submitted to the device.  Doing so causes the
device-specific variable to be initialized immediately before the copy happens.
(Typically, the copy operation causes the _device image_ to be loaded onto the
device also.) As a result, copying from a device-specific variable returns the
initial value if the _device image_ that contains the variable is not currently
...
whether the host code copies to or from the device-specific variable.  As a
result, the implementation may be able to perform certain optimizations.
Although this property may be used with any device, it is generally only
beneficial when used on FPGA devices.
...
SYCL 2020 restrictions must be relaxed to allow `device_specific` to be used within
device functions without being `const` or `constexpr` and without being zero-initialized
or constant-initialized.  This is achieved by adding `device_specific` exceptions to the
...
of type `device_specific` in which case it can be odr-used inside a device function without being
...
global variable that isn't `const` or `constexpr` unless the variable is of type `device_specific`.


=== Referencing a device-specific variable defined in another translation unit

This extension broadens the use of the `SYCL_EXTERNAL` macro to apply also to
device-specific variables.  If the implementation defines the `SYCL_EXTERNAL`
macro, device code in one translation unit may reference a device-specific
...
SYCL_EXTERNAL device_specific<int> Foo;  // definition (also a declaration)
...
SYCL_EXTERNAL extern device_specific<int> Foo;  // declaration
...
[NOTE]
====
A pointer to the allocation within a `device_specific` may not be obtained by the host program (can only be extracted in device functions because allocations are per device), so pointer arithmetic can therefore not be used in the host program to define `copy`/`memcpy` offsets into data.  `startIndex` and `offset` arguments are provided in these interfaces to allow offsetting without pointer arithmetic.
====
...
class queue {
public:
  // Copy to device_specific
  template <typename T, typename PropertyListT>
  event copy(const std::remove_all_extents_t<T> *src,
    device_specific<T, PropertyListT>& dest,
  ...
  // Copy from device_specific
  template <typename T, typename PropertyListT>
  event copy(const device_specific<T, PropertyListT>& src,
  ...
  // memcpy to device_specific
  template <typename T, typename PropertyListT>
  event memcpy(device_specific<T, PropertyListT>& dest, 
  ...
  // memcpy from device_specific
  template <typename T, typename PropertyListT>
  event memcpy(void *dest,
    const device_specific<T, PropertyListT>& src,
    size_t numBytes = sizeof(T), size_t offset = 0);

  template <typename T, typename PropertyListT>
  event memcpy(void *dest, 
    const device_specific<T, PropertyListT>& src, size_t numBytes,
    size_t offset, event depEvent);
...
Copies _count_ elements of type `std::remove_all_extents_t<T>` from the pointer _src_ to the `device_specific` _dest_, starting at _startIndex_ elements of _dest_. _src_ may be either a host or USM pointer.
...
----
template <typename T, typename PropertyListT>
void copy(const device_specific<T, PropertyListT>& src,
  std::remove_all_extents_t<T> *dest,
  size_t count = sizeof(T) / sizeof(std::remove_all_extents_t<T>),
  size_t startIndex = 0);
----
...
Copies _count_ elements of type `std::remove_all_extents_t<T>` from the `device_specific` _src_ to the pointer _dest_, starting at _startIndex_ elements of _src_. _dest_ may be either a host or USM pointer.
...
----
template <typename T, typename PropertyListT>
void memcpy(device_specific<T, PropertyListT>& dest,
  const void *src, size_t numBytes = sizeof(T), size_t offset = 0);
----
|`T` must be device copyable.
...
Copies _count_ bytes from the pointer _src_ to the `device_specific` _dest_, starting at _offset_ bytes. _src_ may be either a host or USM pointer.
...
Copies _count_ bytes from the `device_specific` _src_ to the pointer _dest_, starting at _offset_ bytes. _dest_ may be either a host or USM pointer.
...
device-specific variable has the `device_image_scope` property and exists in
more than one _device image_ for the queue's device.  This condition could
occur if the application submits a kernel referencing the variable to the same
device with different values for a specialization constant (when the device
supports specialization constants natively).  This condition could also occur
if the application submits the same kernel from more than one kernel bundle.
====


== Non-normative: Future anticipated changes, not enabled within this version of the specification

In a future version of this extension, it is expected that when {cpp}20 support is available and enabled, the `consteval` keyword will be used to enable compile-time constant initialization of the device allocations backing `device_specific`.  This will simplify some coding patterns, compared with the current zero-initialization requirement.
...
public:
  ...
  static_assert(std::is_trivially_destructible_v<T>,
    ...
    // device_specific initializes underlying T with the args argument
    template <typename... Args>
    consteval explicit device_specific(Args&&... args);
  #else
    static_assert(std::is_trivially_default_constructible_v<T>,
                  "Type T must be trivially default constructable (until C++20 "
                  "consteval is supported and enabled)");
...
The example below creates two global namespace scope `device_specific` objects named `dm1` and `dm2`. `dm1` is default constructed with external linkage, while `dm2` is initialized and has internal linkage.
...
device_specific<MyClass> dm1;
static device_specific<int[4]> dm2{1, 3, 5, 7};  // Requires C++20 to be enabled
...
`device_specific` prioritizes usability over simplicity of implementation, and therefore adds requirements such as (1) that contents and addresses of the allocation on each device remain stable across changes to specialization constant values, and (2) that the allocation be accessible across `device_image` on the same device.  These requirements mean that the semantics of `device_specific` do not match the semantics of SPIR-V module scope variables, and therefore may not be implementable exclusively using the SPIR-V feature in existing SPIR-V consuming implementations.

Also note that there are no restrictions on passing (and subsequent dereferencing) of pointers obtained on a device from a `device_specific`, between kernels on the same device, including through storage to memory.

== Issues

1) Can `sycl::atomic_ref` be used with `device_specific`? +
*Resolved*: Yes, but only on the device side.  There is no visibility/communication across devices because each device receives a unique allocation of type _T_ underlying the `device_specific`.  There is no way for an `atomic_ref` associated with the allocation to be created in host code because there is no way to extract a pointer or reference in host code (only copy/memcpy).

2) Should we restrict `device_specific` to static storage duration, and if so how? +
...
5) Are there important use cases that require arbitrary destructors to be supported by `device_specific`? +
