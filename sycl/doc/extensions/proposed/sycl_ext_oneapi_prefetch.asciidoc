...
// Only available if Dimensions == 1 && (AccessMode == read || AccessMode == read_write)
...
void prefetch(accessor<DataT, Dimensions, AccessMode, target::device, IsPlaceholder> acc,
              size_t offset, Properties properties = {});
----
_Constraints_: Available only if `Dimensions == 1 && (AccessMode == read ||
...
----
_Constraints_: Available only if `sycl::is_group_v<std::decay_t<Group>>` is
`true` and `Dimensions == 1 && (AccessMode == read || AccessMode ==
read_write)` is `true`.
