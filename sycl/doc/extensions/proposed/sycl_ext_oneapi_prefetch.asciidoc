...
[NOTE]
====
The current version of the above functions effectively ignores all annotations in the `annotated_ptr` when issuing a prefetch. This behavior may change in future, if other extensions introduce properties that interact with or modify prefetch behavior.
====
