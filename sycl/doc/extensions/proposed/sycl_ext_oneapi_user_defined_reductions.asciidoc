...
specification by allowing user-defined binary operators and non-fundamental
types.
...
NOTE: Implementation of both overaloads may use less memory than passed
...
q.submit([&](sycl::handler& h) {
  ...
  h.parallel_for(sycl::nd_range<1>{N, group_size}, [=](sycl::nd_item<1> it) {
    ...
    auto handle = sycl::ext::oneapi::experimental::group_with_scratchpad(
        it.get_group(), sycl::span(scratch, temp_memory_size));

    // Pass the handle as the first argument to the group algorithm
    T sum = sycl::reduce_over_group(handle, acc[it.get_global_id(0)], 0,
        UserDefinedSum<T>{});
