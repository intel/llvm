...
be used. The macro accepts a single optional argument, which is passed to the
property parameter `SetId` and therefore has all the same requirements.
...
// both declaration and definition should be annotated:
void SYCL_EXT_ONEAPI_INDIRECTLY_CALLABLE_PROPERTY(set_B) Foo::baz() {}
...
virtual functions. When the typename is omitted from the
`SYCL_EXT_ONEAPI_INDIRECTLY_CALLABLE_PROPERTY` macro, the type `void` is used by
default. Applications may also explicitly use the type `void` to denote this
default set of virtual functions.
...
struct Foo {
  ...
  virtual SYCL_EXT_ONEAPI_INDIRECTLY_CALLABLE_PROPERTY(set_fp64) void f64() {
    double d = 3.14;
  }

  // This function uses 'fp16' aspect
  virtual SYCL_EXT_ONEAPI_INDIRECTLY_CALLABLE_PROPERTY(set_fp16) void f16() {
    sycl::half h = 2.71f;
  }
};
