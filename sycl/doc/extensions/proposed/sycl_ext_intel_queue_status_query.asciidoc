...
This extension add `::queue::size()`, `sycl::queue::empty()`, `sycl::queue::get_wait_list()` menber functions.
...
macro `SYCL_EXT_ONEAPI_QUEUE_STATUS_QUERRY` to one of the values defined in the table
...
The semantic of the new function are: 
...
```
size_t size() const
   Return the number of enqueued `command` in the queue 
...
```
std::vector<event> get_wait_list() const
   Return the list of events such that waiting for all returned events
