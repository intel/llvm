...
Each value is converted using the `r` rounding mode and the `saturation::finite`
saturation mode.
...
_Effects:_ Initializes each element of this `fp8_e5m2_x` object from the
corresponding element in the array `vals`.
Each value is converted using the `r` rounding mode and the `s` saturation mode.
