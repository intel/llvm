...
This property is passed to `annotated_ref` class when an `annotated_ptr` is
dereferenced with `operator*` and `operator[]`

This property is sustainable through pointer arithmetic (i.e. + and -)
