...
can be passed to `sort_over_group` function only. If it's passed to `joint_sort`, it leads to a compilation error.
...
|`template<typename Group, typename T, std::size_t ElementsPerWorkItem, typename Properties = ext::oneapi::experimental::properties<>>
...
|`template<typename Group, typename T, typename U, std::size_t ElementsPerWorkItem, typename Properties = ext::oneapi::experimental::properties<>>
