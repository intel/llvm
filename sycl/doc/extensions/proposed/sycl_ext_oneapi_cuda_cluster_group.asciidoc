...
A device requires the `ext_oneapi_cuda_cluster_group` aspect to
...
class, via a member function to be introduced called `ext_oneapi_cuda_get_cluster_group()`.
...
`ext_oneapi_cuda_cluster_group` which indicates a memory ordering
...
----
    ...
    static constexpr memory_scope fence_scope = 
                memory_scope::ext_oneapi_cuda_cluster_fence;
    }
----
