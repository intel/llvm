...
Becuase of the special scheduling guarantees associated with a cluster launch,
...
Building upon the group heirarchy in sycl, this proposal adds another level
above `group`(for work-groups), to be called as `cluster-group`, which 
represents a collection of work-groups and will be accesible via the `nd_item`
...
|Returns the consituent `cluster_group` in the kernel, representing this
