...
proposal introduces a new launch property called as `ClusterRange` that will
...
* cluster_range sycl::range specifying the number of workgroups in the cluster
  in each dimension.
*/
template <int Dim>
struct ClusterRange {
  ClusterRange(const sycl::range<Dim>& cluster_range) : 
