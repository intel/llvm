...
execution and the assert message is already printed to `stderr`. Format of the
assert message is the following:
```
...
preprocessor macro at the point when `<sycl.hpp>` and `<cassert>/<assert.h>`
are included.
