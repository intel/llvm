...
In the experimental API version, the device side read/write methods take in a properties object as function argument, which can contain the latency control properties `latency_anchor_id` and/or `latency_constraint`.
