...
Although this specification places no constraints on the context from which queues may participate in the `waitList`, the current implementation does not support it. An event which belongs to another context is passed to the backend as it is, without being translated to the context of the queue the barrier is submitted to, so the behavior of such a barrier is undefined. This applies to both `handler::ext_oneapi_barrier` and `queue::ext_oneapi_submit_barrier`. The events of host tasks are not affected, because the SYCL runtime waits for them itself instead of passing them to the backend.
