...
Two example `device_global` can be declared at namespace scope, as follows:
...
static constexpr bool has_property();
----
| Returns true if the `property_listT` contains the property specified by `propertyT`. Returns false if it does not.
...
Must produce a compiler diagnostic if `property_listT` does not contain a `propertyT` property.
Available only if `sycl::is_property_of_v<propertyT, sycl::ext::oneapi::device_global>` is true.
