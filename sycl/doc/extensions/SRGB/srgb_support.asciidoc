...
== aspect::ext_intel_oneapi_srgb
Since not all devices and backends support sRGB images, `aspect::ext_intel_oneapi_srgb` is available to programmatically detect this.
[source]
----
 // someDevice is a sycl::device
 if(someDevice.has(aspect::ext_intel_oneapi_srgb)){ ... }
----
...
The values `ext_oneapi_srgb` and `ext_oneapi_srgbx` have also been added. `ext_oneapi_srgb` for 24-bit sRGB images, and `ext_oneapi_srgbx` for 24-bit images in 32-bit space (where the alpha channel is ignored). However, it should be noted that like their existing `rgb` and `rgbx` counterparts, neither is working at this time. They are added in hopes of future support

[source]
----
image_channel_order::ext_oneapi_srgba
image_channel_order::ext_oneapi_srgb
image_channel_order::ext_oneapi_srgbx
----
