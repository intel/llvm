...
_Constraints_: Only available if `GroupHelper` was created with a work group or a sub_group and
some associated scratch space.
...
Example of custom Sorter is below. This implementation is not recommended as a performant way
to implement bubble sort. It's just an example of a custom sorter.
