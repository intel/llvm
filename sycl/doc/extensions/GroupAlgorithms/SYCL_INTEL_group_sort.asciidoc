...
parameter SYCL introduces special class `group_executor`:
...
_Constraints_: Only available if `GroupExecutor` was created with a work group or sub_group and
...
. `group_executor` can be replaced with concepts that requires 2 methods.
