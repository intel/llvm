...
namespace sycl::ext::oneapi::experimental {
  ...
  {
  public:
    group_helper(Group group, sycl::span<uint8_t, Extent> scratch);
  ...
  // Deduction guides
  template<typename Group, std::size_t Extent>
  group_helper(Group, sycl::span<uint8_t, Extent>)
    -> group_helper<Group, Extent>;

}
----

NOTE: `group_helper` is in the `experimental` namespace: interfaces might be changed later.

Table 9. Constructors of the `group_helper` class.
|===
|Constructor|Description

|`group_helper(Group group, sycl::span<uint8_t, Extent> scratch)`
|Creates the `group_helper` object using `group` and `scratch`.
...
Table 10. Member functions of the `group_helper` class.
|===
|Member function|Description

|`Group get_group() const`
|Returns the `Group` class object that is handled by the `group_helper` object.

|`sycl::span<uint8_t, Extent>
get_memory() const`
|Returns `sycl::span` that represents an additional memory
that is handled by the `group_helper` object.
...
. `group_helper` can be replaced with concepts that requires 2 methods.
