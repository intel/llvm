...
_Result Type_ must be an <<OpTypePointer>> a <<vector>> with <<physical pointer type>> _Component Type_.
...
is specified by a mask instruction parameter. +
 +
'ResultType' is a type of the gathered vector. Its _Component Type_ must be the same as the base type of
'PtrVector'.
 +
