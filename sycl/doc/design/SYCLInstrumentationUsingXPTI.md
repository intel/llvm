...
| `offload_kernel_enqueue` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_kernel_enqueue_data_t` that marks offload kernel enqueue creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.buffer` layer.</li> <li> <**event**: `xpti::trace_event_data_t` - contains information about source location including kernel lambda used as kernel name./li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_kernel_enqueue_data_t` object, that includes kernel execution parameters (global size, local size, offset), number of kernel arguments. The kernel arguments are sent as event metadata packed in `xpti::offload_kernel_arg_data_t` structure. It contains argument kind (sycl::detail::kernel_param_kind_t), pointer to the value, size and index in the argument list.</li></div> | None     |
