...
| Trace Point Type           | Parameter Description | Metadata |
| :------------------------: | :-------------------- | :------- |
| `offload_alloc_sampled_image_construct` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_sampled_image_data_t` that marks offload sampled image creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.sampled_image` layer.</li> <li> **event**: `xpti::trace_event_data_t` - contains information about source location.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_sampled_image_data_t` object, that includes sampled image object ID, host pointer used to create/initialize sampled image, number of image dimensions, the image format and sampler information (addressing mode, coordinate normalization mode, filtering mode). </li></div> | None     |
| `offload_alloc_sampled_image_associate`   | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_association_data_t` that provides association between user level sampled image object and platform specific memory object</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.sampled_image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed.</li> <li> **user_data**: A pointer to `offload_association_data_t` object, that includes user object ID and platform-specific representation for offload sampled image. </li></div> | None     |
| `offload_alloc_sampled_image_destruct` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_sampled_image_data_t` that marks offload sampled image destruction point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.sampled_image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_sampled_image_data_t` object, that includes sampled image object ID. </li></div> | None     |
| `offload_alloc_sampled_image_release`   | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_association_data_t` that provides information about release of platform specific memory object</li> <li> **parent**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed.</li> <li> **user_data**: A pointer to `offload_association_data_t` object, that includes user object ID and platform-specific representation for offload image. </li></div> | None     |
| `offload_alloc_unsampled_image_construct` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_unsampled_image_data_t` that marks offload unsampled image creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `xpti::trace_event_data_t` - contains information about source location.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_unsampled_image_data_t` object, that includes unsampled image object ID, host pointer used to create/initialize unsampled image, number of image dimensions and the image format. </li></div> | None     |
| `offload_alloc_unsampled_image_associate`   | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_unsampled_image_association_data_t` that provides association between user level unsampled image object and platform specific memory object</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed.</li> <li> **user_data**: A pointer to `offload_unsampled_image_association_data_t` object, that includes user object ID and platform-specific representation for offload unsampled image. </li></div> | None     |
| `offload_alloc_unsampled_image_destruct` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_unsampled_image_data_t` that marks offload unsampled image destruction point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_unsampled_image_data_t` object, that includes unsampled image object ID. </li></div> | None     |
| `offload_alloc_unsampled_image_release`   | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_unsampled_image_association_data_t` that provides information about release of platform specific memory object</li> <li> **parent**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed.</li> <li> **user_data**: A pointer to `offload_unsampled_image_association_data_t` object, that includes user object ID and platform-specific representation for offload image. </li></div> | None     |
| `offload_alloc_sampled_image_accessor` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_sampled_image_accessor_data_t` that marks offload sampled image accessor creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_sampled_image_accessor_data_t` object, that includes image object ID, accessor handle created from specific image, access target and element information (type name, size). </li></div> | None     |
| `offload_alloc_host_sampled_image_accessor` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_host_sampled_image_accessor_data_t` that marks offload sampled image host accessor creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_sampled_image_accessor_data_t` object, that includes image object ID, accessor handle created from specific image and element information (type name, size). </li></div> | None     |
| `offload_alloc_unsampled_image_accessor` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_unsampled_image_accessor_data_t` that marks offload unsampled image accessor creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_unsampled_image_accessor_data_t` object, that includes image object ID, accessor handle created from specific image, accessor information (access target and mode) and element information (type name, size). </li></div> | None     |
| `offload_alloc_host_unsampled_image_accessor` | <div style="text-align: left"><li>**trace_type**: `xpti::trace_point_type_t::offload_host_unsampled_image_accessor_data_t` that marks offload unsampled image host accessor creation point</li> <li> **parent**: Event ID created for all functions in the `oneapi.experimental.image` layer.</li> <li> **event**: `nullptr` - since the stream of data just captures functions being called.</li> <li> **instance**: `nullptr` since no begin-end event alignment is needed. </li> <li> **user_data**: A pointer to `offload_unsampled_image_accessor_data_t` object, that includes image object ID, accessor handle created from specific image, access mode and element information (type name, size). </li></div> | None     |
