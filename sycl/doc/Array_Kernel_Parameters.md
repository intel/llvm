...
and special treatment of arrays of `sycl::accessor` objects.
...
As can be seen observed from the example above, standard-layout lambda capture
...
`sycl::accessor` and `sycl::sampler` are not standard-layout type
and would not normally be copyable from host to device.
However, they are required by the SYCL standard to be used
across host and devices. To transfer them from host to device, 
the parameters of their init functions are handled individually.
They are converted from host to device format and passed
as separate kernel arguments. On the device the values received
are used to call their init functions on local copies of the
accessor/sampler objects.
...
the extension to arrays of accessor/sampler within structs
