...
// Note: A possible solution is to add kernel names as metadata, like with
...
pi_result cuda_piProgramHasKernel(pi_program program, const char *kernel_name,
  ...
  CUfunction func;
  CUresult ret = cuModuleGetFunction(&func, program->get(), kernel_name);
  pi_result retError = PI_SUCCESS;
...
#define _PI_CL(pi_api, cuda_api)                                               \
  ...
  _PI_CL(piProgramHasKernel, cuda_piProgramHasKernel)
  // Kernel
