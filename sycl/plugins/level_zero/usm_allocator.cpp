...
public:
  ...
  SetLimits() {
    ...
    // SYCL_PI_LEVEL_ZERO_USM_ALLOCATOR=<memtype>:<limits>
    //  memtype: [host|device|shared]
    //  limits:  [SlabMinSize][,[MaxPoolableSize][,[Capacity][,[MaxPoolSize]]]]
    //
    //  e.g.: SYCL_PI_LEVEL_ZERO_USM_ALLOCATOR=host:64K,,2;device:2M;shared:1K
    //
    // SlabMinSize:     Minimum allocation size requested from USM.
    //                  Default is mimimum allocation size for the memory type.
    ...
    auto MemParser = [=](std::string &Params, SystemMemory::MemType M) {
      ...
      bool More = ParamParser(Params, SlabMinSize[LM]);
      if (More)
        More = ParamParser(Params, MaxPoolableSize[LM]);
      if (More)
        More = ParamParser(Params, Capacity[LM]);
      if (More)
        ParamParser(Params, MaxPoolSize);
      if (M == SystemMemory::All) {
        SlabMinSize[SystemMemory::Shared] = SlabMinSize[SystemMemory::Device] =
    ...
    printPoolSettings(nullptr);
  }

  void printPoolSettings(void *Ctx) {
    if (PoolTrace < 1)
      return;
    ...
    if (Ctx == nullptr)
      ...
      std::cout << "USM Pool Settings for Context " << Ctx
                << " Adjusted for Minimum Allocation Sizes" << std::endl;
...
void setSlabMinSizes(void *Ctx, size_t minAllocSizes[3]) {
  for (int M = 0; M < 3; M++) {
    // The minimum allocation size to be used for slabs is set to be at least
    // the minimum size allocated by the L0 layer for that memory type.
    USMSettings.SlabMinSize[M] =
        std::max(minAllocSizes[M], USMSettings.SlabMinSize[M]);

    // The max poolable size is then set to at least half the Slab minimum size
    // to ensure that each slab is fully utilized.
    USMSettings.MaxPoolableSize[M] = std::max(USMSettings.MaxPoolableSize[M],
                                              USMSettings.SlabMinSize[M] / 2);
  }
  USMSettings.printPoolSettings(Ctx);
}
