...
struct _pi_context : _pi_object {
  _pi_context(pi_device ArgDevice)
      : Device{ArgDevice}, HostSurfaceIndex{
  ...
  unsigned int generateSurfaceIndex() {
    // ESIMD_EMULATOR Plug-in generates its own surface index for
    // Buffer/Image independent from ones generated by CM.
    // Thread-safe decrementing generation beginning from UINT_MAX in
    // order to avoid collision with __SEIEED::SLM_BTI
    unsigned int NewIndex = HostSurfaceIndex--;
    return NewIndex;
  }
...
private:
  std::atomic<unsigned int> HostSurfaceIndex;
};
