...
// Mapping between surface index and surfaces from both Host and
// CM. No need to serialize accesses to this map as the SurfaceIndex
// generation for this map always generates unique key atomically and
// generated keys are never re-used for another instance of _pi_mem object
// REF : _pi_context::generateSurfaceIndex()
static std::unordered_map<unsigned int, _pi_mem *> *PiESimdSurfaceMap =
    new std::unordered_map<unsigned int, _pi_mem *>;

pi_result _pi_mem::addMapping(void *MappedTo) {
  // TODO: Runtime guarantees LIFO across multiple threads? No
  // duplicated mappings for same (base,offset)?
  Mappings.push(MappedTo);
  return PI_SUCCESS;
}

pi_result _pi_mem::removeMapping(void *MappedTo) {
  // TODO: Runtime guarantees LIFO across multiple threads? No
  // duplicated mappings for same (base, offset)?
  auto peak = Mappings.top();
  if (peak != MappedTo) {
    if (PrintPiTrace) {
...
// This Plug-In library generates its own Surface index independent
// from ones generated by CM
// REF : _pi_context::generateSurfaceIndex()
unsigned int sycl_get_surface_index(void *PtrInput) {
...
void sycl_get_buffer_params(unsigned int IndexInput, char **BaseAddr,
...
// Function to provide image info for kernel compilation using surface
// index without dependency on '_pi_image' definition
void sycl_get_image_params(unsigned int IndexInput, char **BaseAddr,
...
sycl::detail::ESIMDDeviceInterface::ESIMDDeviceInterface() {
  ...
  sycl_get_surface_index_ptr = sycl_get_surface_index;
  sycl_get_buffer_params_ptr = sycl_get_buffer_params;
  sycl_get_image_params_ptr = sycl_get_image_params;
...
pi_result piMemBufferCreate(pi_context Context, pi_mem_flags Flags, size_t Size,
  ...
  try {
    *RetMem = new _pi_buffer(Context, MapBasePtr, CmBuf, Size);
  } catch (const std::bad_alloc &) {
    return PI_OUT_OF_HOST_MEMORY;
  } catch (...) {
    return PI_ERROR_UNKNOWN;
  ...
  // SurfaceIndex is automatically generated by
  // _pi_context::generateSurfaceIndex() from UINT_MAX
  assert(PiESimdSurfaceMap->find((*RetMem)->SurfaceIndex) ==
             PiESimdSurfaceMap->end() &&
         "Failure from Plug-in-managed buffer creation");
...
pi_result piMemRelease(pi_mem Mem) {
  ...
  if (--(Mem->RefCount) == 0) {
    if (Mem->isCmSurface()) {
      std::lock_guard<std::mutex> Lock(Mem->Context->CmSurfaceManageLock);
...
pi_result piMemImageCreate(pi_context Context, pi_mem_flags Flags,
  ...
  try {
    *RetImage =
        new _pi_image(Context, MapBasePtr, CmSurface, ImageDesc->image_width,
                      ImageDesc->image_height, BytesPerPixel);
  } catch (const std::bad_alloc &) {
    return PI_OUT_OF_HOST_MEMORY;
  } catch (...) {
  ...
  // SurfaceIndex is automatically generated by
  // _pi_context::generateSurfaceIndex()
  assert(PiESimdSurfaceMap->find((*RetImage)->SurfaceIndex) ==
             PiESimdSurfaceMap->end() &&
         "Failure from Plug-in-managed image creation");
...
pi_result piEnqueueMemBufferMap(pi_queue Queue, pi_mem Buffer,
  ...
  *RetMap = Buffer->MapHostPtr + Offset;
  if (PI_SUCCESS != Buffer->addMapping(*RetMap)) {
    return PI_INVALID_VALUE;
  }
...
pi_result piEnqueueMemUnmap(pi_queue Queue, pi_mem MemObj, void *MappedPtr,
  ...
  if (PI_SUCCESS != MemObj->removeMapping(MappedPtr)) {
    return PI_INVALID_VALUE;
  }
...
pi_result piEnqueueMemImageRead(pi_queue CommandQueue, pi_mem Image,
  ...
  if (PiImg->isCmSurface()) {
    ...
    memcpy(Ptr, PiImg->MapHostPtr, RowPitch * (Region->height));
  }
