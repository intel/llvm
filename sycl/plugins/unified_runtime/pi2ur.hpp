...
inline pi_result piContextCreate(const pi_context_properties *properties,
  ...
  auto hDevicesConst = reinterpret_cast<const ur_device_handle_t *>(devices);
  // TODO(ur): This const cast should be removed when UR is moved to a newer
  // version
  auto hDevices = const_cast<ur_device_handle_t *>(hDevicesConst);
