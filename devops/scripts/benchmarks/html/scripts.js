...
// Shared URL query helper (used by updateURL, initializeCharts-from-URL, and dashboard-toggles.js)
...
function getLayerTags(metadata) {
    ...
    if (metadata?.tags) {
        metadata.tags.forEach(tag => {
            if (tag.startsWith('SYCL') || tag.startsWith('UR') || tag === 'L0' || tag === 'OFFLOAD') {
                layerTags.add(tag);
            }
        });
    }
