// RUN: %clang_cc1 -emit-llvm -o - -mllvm -opaque-pointers %s | FileCheck %s
// END.
...
// CHECK-NEXT: call ptr @llvm.ptr.annotation.p0({{.*}}str{{.*}}str{{.*}}i32 14, ptr null)
    gf.v = argc;
// CHECK: call ptr @llvm.ptr.annotation.p0(ptr getelementptr inbounds (%struct.foo, ptr @gf, i32 0, i32 0), {{.*}}str{{.*}}str{{.*}}i32 12, ptr null)
    gf.w = 42;
