...
// pointers by default
// RUN: %clang_cc1 -flto=thin -emit-llvm-bc < %s | llvm-dis -opaque-pointers -o - | FileCheck %s
...
// FIXME: Align with the community code when project is ready to enable opaque
// pointers by default
// RUN: %clang_cc1 -flto=thin -fsplit-lto-unit -emit-llvm-bc < %s | llvm-dis -opaque-pointers -o - | FileCheck %s --check-prefix=SPLIT
...
// FIXME: Align with the community code when project is ready to enable opaque
// pointers by default
// RUN: %clang_cc1 -flto -triple x86_64-pc-linux-gnu -emit-llvm-bc < %s | llvm-dis -opaque-pointers -o - | FileCheck %s --implicit-check-not="EnableSplitLTOUnit" --check-prefix=SPLIT
