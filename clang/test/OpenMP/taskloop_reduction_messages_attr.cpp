...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 10; ++i)
    m++;
  [[omp::directive(taskloop reduction(task, + : m))]] // omp45-error 2 {{expected expression}} omp45-warning {{missing ':' after reduction identifier - ignoring}} omp50-error {{'reduction' clause with 'task' modifier allowed only on non-simd parallel or worksharing constructs}}
  for (int i = 0; i < 10; ++i)
    m++;
