...
// RUN: %clang_cc1 -fno-legacy-pass-manager        -verify=all,safe  -Rpass=openmp-opt -Rpass-analysis=openmp-opt -fopenmp -O2 -x c++ -triple nvptx64-unknown-unknown -fopenmp-targets=nvptx64-nvidia-cuda -emit-llvm %s -fopenmp-is-device -fopenmp-host-ir-file-path %t-ppc-host.bc -o %t.out
...
#pragma omp parallel // #0
                     ...
                     // all-remark@#0 3 {{Value has potential side effects preventing SPMD-mode execution. [OMP121]}}
                     // all-remark@#0 {{Replaced globalized variable with 1 byte of shared memory. [OMP111]}}
  {
  }
}
void bar2(void) {
#pragma omp parallel // #1
                     ...
                     // all-remark@#1 2 {{Value has potential side effects preventing SPMD-mode execution. [OMP121]}}
                     // all-remark@#1 {{Replaced globalized variable with 1 byte of shared memory. [OMP111]}}
  {
  }
}
