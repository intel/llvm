...
// RUN: %clang -### -fsycl --offload-new-driver --no-offloadlib %s 2>&1 | FileCheck -check-prefix=CHECK-DEFAULT %s
...
// RUN: %clang -### -fsycl --offload-new-driver --no-offloadlib -fno-sycl-esimd-force-stateless-mem %s 2>&1 | FileCheck -check-prefix=CHECK-NEG %s
