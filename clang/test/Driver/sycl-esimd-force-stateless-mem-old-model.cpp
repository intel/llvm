...
// RUN: %clang -### -fsycl --no-offload-new-driver %s 2>&1 | FileCheck -check-prefix=CHECK-DEFAULT %s
...
// RUN: %clang -### -fsycl --no-offload-new-driver -fno-sycl-esimd-force-stateless-mem %s 2>&1 | FileCheck -check-prefix=CHECK-NEG %s
