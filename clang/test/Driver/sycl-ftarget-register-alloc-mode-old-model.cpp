...
// RUN: %clangxx -### -fsycl --no-offload-new-driver -fsycl-targets=spir64_gen -Xs "-device bdw" \
// RUN:          %s 2>&1 \
// RUN:   | FileCheck -check-prefix=NO_PVC %s

// RUN: %clangxx -### -fsycl --no-offload-new-driver -fsycl-targets=spir64_gen -Xs "-device *" \
// RUN:          %s 2>&1 \
// RUN:   | FileCheck -check-prefix=NO_PVC %s

// RUN: %clangxx -### -fsycl --no-offload-new-driver -fsycl-targets=spir64_gen -Xs "-device pvc:mtl-s" \
// RUN:          %s 2>&1 \
// RUN:   | FileCheck -check-prefix=NO_PVC %s
