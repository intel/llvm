...
/// Verify --offload-new-driver --no-offloadlib option phases
// RUN:  %clang --target=x86_64-unknown-linux-gnu -fsycl -fsycl-targets=nvptx64-nvidia-cuda,spir64 --offload-new-driver --no-offloadlib -ccc-print-phases %s 2>&1 \
...
// RUN: %clangxx -### --target=x86_64-unknown-linux-gnu -fsycl -fsycl-targets=spir64 --offload-new-driver --no-offloadlib %s 2>&1 | FileCheck -check-prefix=CHK-FLOW %s
...
// RUN: %clangxx -### -fsycl --offload-new-driver --no-offloadlib -target x86_64-unknown-linux-gnu \
// RUN:          -shared %s 2>&1 \
...
// RUN:          -fsycl-targets=spir64 --offload-new-driver --no-offloadlib %s 2>&1 \
// RUN:  | FileCheck -check-prefix=CHK_ARCH \
// RUN:              -DTRIPLE=spir64-unknown-unknown -DARCH= %s
// RUN: %clangxx -### --target=x86_64-unknown-linux-gnu -fsycl \
// RUN:          -fsycl-targets=intel_gpu_pvc --offload-new-driver --no-offloadlib %s 2>&1 \
// RUN:  | FileCheck -check-prefix=CHK_ARCH \
// RUN:              -DTRIPLE=spir64_gen-unknown-unknown -DARCH=pvc %s
// RUN: %clangxx -### --target=x86_64-unknown-linux-gnu -fsycl \
// RUN:          -fno-sycl-libspirv -fsycl-targets=amd_gpu_gfx900 \
// RUN:          -nogpulib --offload-new-driver --no-offloadlib %s 2>&1 \
// RUN:  | FileCheck -check-prefix=CHK_ARCH \
...
// RUN:          --offload-new-driver --no-offloadlib %s 2>&1 \
...
// RUN:          -nocudalib --offload-new-driver --no-offloadlib \
...
// RUN:          -fno-sycl-libspirv -nocudalib --offload-new-driver --no-offloadlib %s 2>&1 \
...
// RUN: %clangxx -fsycl -### -fsycl-targets=nvptx64-nvidia-cuda \
// RUN:          -fno-sycl-libspirv -nocudalib --offload-new-driver --no-offloadlib \
...
// RUN: %clangxx -fsycl -### -fsycl-targets=spir64_gen --offload-new-driver --no-offloadlib \
...
// RUN: --offload-new-driver --no-offloadlib %s 2>&1 \
// RUN:   | FileCheck -check-prefix ARCH_CHECK %s
// RUN: %clang_cl -fsycl -### -fsycl-targets=spir64_gen --offload-new-driver --no-offloadlib \
// RUN:   -Xsycl-target-backend=spir64_gen "-device bdw" %s 2>&1 \
// RUN:   | FileCheck -check-prefix ARCH_CHECK %s
// RUN: %clang_cl -fsycl -### -fsycl-targets=spir64_gen --offload-new-driver --no-offloadlib \
// RUN:   -Xsycl-target-backend "-device bdw" %s 2>&1 \
...
// RUN: %clangxx -fsycl -### --offload-new-driver --no-offloadlib -fno-sycl-libspirv \
...
// Verify that the driver --no-offloadlib correctly handles link-opt and compile-opt values with commas
// RUN: %clangxx -fsycl -### -fsycl-targets=spir64_gen --offload-new-driver --no-offloadlib \
...
// RUN:          --offload-new-driver --no-offloadlib 2>&1 \
...
// and AMD GPU targets.
// RUN:   %clang -### -fsycl -fsycl-targets=nvidia_gpu_sm_60,nvidia_gpu_sm_70 -fno-sycl-libspirv -nocudalib --offload-new-driver --no-offloadlib  %s 2>&1 \
...
// RUN:   %clang -### -fsycl -fsycl-targets=amd_gpu_gfx90a,amd_gpu_gfx90c -fno-sycl-libspirv -nogpulib --offload-new-driver --no-offloadlib  %s 2>&1 \
