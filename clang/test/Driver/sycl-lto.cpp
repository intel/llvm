...
// Verify we error when using the new offload driver --no-offloadlib but with device code split set to off.
// RUN: not %clangxx -fsycl --offload-new-driver --no-offloadlib -foffload-lto=thin -fsycl-device-code-split=off %s -### 2>&1 | FileCheck -check-prefix=CHECK-SPLIT-ERROR %s
...
// RUN: %clangxx -fsycl --offload-new-driver --no-offloadlib -foffload-lto=thin %s -### 2>&1 | \
