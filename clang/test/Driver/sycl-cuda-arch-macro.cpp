...
// RUN: %clangxx -E -dM -fsycl -fsycl-targets=nvptx64-nvidia-cuda -fsycl-cuda-compatibility -Xsycl-target-backend --offload-arch=sm_80 -nocudalib -fno-sycl-libspirv -nocudainc %s 2>&1 \
// RUN: | FileCheck --check-prefix=CHECK-CUDA-COMPAT-ARCH-MACRO %s
// CHECK-CUDA-COMPAT-ARCH-MACRO: #define __CUDA_ARCH__ {{[0-9]+}}
...
// RUN: %clangxx -### -fsycl -fsycl-targets=nvptx64-nvidia-cuda -fsycl-cuda-compatibility -Xsycl-target-backend --offload-arch=sm_80 -nocudalib -fno-sycl-libspirv %s -nocudainc 2>&1 \
// RUN: | FileCheck --check-prefix=CHECK-CUDA-IS-DEVICE %s
