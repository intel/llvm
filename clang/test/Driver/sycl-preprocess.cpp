...
// PREPROC_ONLY: clang{{.*}} "-include" "[[INTHEADER]]"{{.*}} "-fsycl-is-host"{{.*}} "-o" "[[HOST_TEMP:.+\.ii]]"
// PREPROC_ONLY: append-file{{.*}} "[[HOST_TEMP]]" "--append=[[INTFOOTER]]" "--output=[[HOST_APPENDED:.+\.cpp]]"
// PREPROC_ONLY: clang{{.*}} "-fsycl-is-host"{{.*}} "-o" "[[HOST_OUT:.+\.ii]]"{{.*}} "[[HOST_APPENDED]]"
...
// PREPROC_PHASES: 7: append-footer, {6}, c++, (host-sycl)
// PREPROC_PHASES: 8: preprocessor, {7}, c++-cpp-output, (host-sycl)
// PREPROC_PHASES: 9: clang-offload-bundler, {2, 8}, c++-cpp-output, (host-sycl)
