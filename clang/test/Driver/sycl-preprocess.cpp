...
/// creation step.
// RUN: %clangxx -fsycl --offload-new-driver --no-offloadlib -E -o %t_output.ii %s -### 2>&1 \
// RUN:  | FileCheck -check-prefix PREPROC_ONLY %s
// RUN: %clang_cl -fsycl --offload-new-driver --no-offloadlib -P %s -### 2>&1 \
// RUN:  | FileCheck -check-prefix PREPROC_ONLY %s
...
// RUN: touch %t.ii
// RUN: %clangxx -fsycl --offload-new-driver --no-offloadlib %t.ii -### 2>&1 | FileCheck -check-prefix PREPROC_IN %s
...
// RUN: %clangxx --target=x86_64-unknown-linux-gnu --offload-new-driver --no-offloadlib -fsycl -E %s -o %t.ii -ccc-print-phases 2>&1 \
// RUN: | FileCheck %s -check-prefix PREPROC_PHASES -DTARGET=x86_64-unknown-linux-gnu
// RUN: %clang_cl --target=x86_64-pc-windows-msvc --offload-new-driver --no-offloadlib -fsycl -P %s -Fi%t.ii -ccc-print-phases 2>&1 \
...
/// output file name.
// RUN: %clangxx --offload-new-driver --no-offloadlib -fsycl -E %s -o sycl-preprocess.ii -### 2>&1 \
// RUN: | FileCheck %s -check-prefix PREPROC_TOOLS
// RUN: %clang_cl --offload-new-driver --no-offloadlib -fsycl -P %s -Fisycl-preprocess.ii -### 2>&1 \
// RUN: | FileCheck %s -check-prefix PREPROC_TOOLS
// RUN: %clang_cl --offload-new-driver --no-offloadlib -fsycl -E %s -o sycl-preprocess.ii -### 2>&1 \
// RUN: | FileCheck %s -check-prefix PREPROC_TOOLS
