...
// CHK-FPGA-AOCO: sycl-post-link{{.*}} "-split-esimd"{{.*}} "-O2" "-spec-const=default" "-device-globals" "-host-pipes" "-o" "[[SPLTABLE:.+\.table]]" "[[LINKEDBC]]"
...
// CHK-FPGA-AOCO-EMU: sycl-post-link{{.*}} "-split-esimd"{{.*}} "-O2" "-spec-const=default" "-device-globals" "-host-pipes" "-o" "[[SPLTABLE:.+\.table]]" "[[LINKEDBC]]"
