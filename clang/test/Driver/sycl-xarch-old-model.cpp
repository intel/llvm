...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_device -fsanitize=address -### 2>&1 \
...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_device "-fsanitize=address -DXARCH_DEVICE_TEST -mllvm -enable-merge-functions" -### 2>&1 \
...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_host -fsanitize=address -### 2>&1 \
...
// SYCL_XARCH_HOST_OPTION: libclang_rt.asan
...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_host "-fsanitize=address -DXARCH_HOST_TEST -mllvm -enable-merge-functions" -### 2>&1 \
...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_device "-fsanitize=address -mllvm -enable-merge-functions" \
...
// SYCL_XARCH_COM_HOST_OPTIONS1: libclang_rt.msan
...
// RUN: %clangxx -fsycl --no-offload-new-driver %s -Xarch_host "-fsanitize=address -mllvm -enable-merge-functions" \
...
// SYCL_XARCH_HOST_MULTIPLE1: libclang_rt.asan
