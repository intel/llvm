...
// RUN: %clang --offload-new-driver --no-offloadlib -ccc-print-phases --target=x86_64-unknown-linux-gnu -fsycl %s 2>&1 \
// RUN:   | FileCheck -check-prefixes=CHK-PHASES %s
// RUN: %clang_cl --offload-new-driver --no-offloadlib -ccc-print-phases --target=x86_64-pc-windows-msvc -fsycl -- %s 2>&1 \
// RUN:   | FileCheck -check-prefixes=CHK-PHASES %s
...
// RUN: %clang --offload-new-driver --no-offloadlib -### -fsycl -c --target=x86_64-unknown-linux-gnu %s 2>&1 \
...
// RUN: %clang --offload-new-driver --no-offloadlib -### -fsycl -c %s 2>&1 \
// RUN:   | FileCheck -check-prefixes=CHK-FSYCL-IS-DEVICE,CHK-FSYCL-IS-HOST %s
// RUN: %clang --offload-new-driver --no-offloadlib -### -fsycl -fsycl-device-only %s 2>&1 \
// RUN:   | FileCheck -check-prefix=CHK-FSYCL-IS-DEVICE %s
// RUN: %clang_cl --offload-new-driver --no-offloadlib -### -fsycl -c -- %s 2>&1 \
// RUN:   | FileCheck -check-prefixes=CHK-FSYCL-IS-DEVICE,CHK-FSYCL-IS-HOST %s
// RUN: %clang --offload-new-driver --no-offloadlib -### -fsycl -fsycl-host-only %s 2>&1 \
