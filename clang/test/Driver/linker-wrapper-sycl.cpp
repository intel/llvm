...
//
// RUN: %clang -cc1 %s -triple spir64-unknown-unknown -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t.bc
...
//
// RUN: clang-linker-wrapper -sycl-device-library-location= -sycl-device-libraries=%t.devicelib.o -sycl-post-link-options="SYCL_POST_LINK_OPTIONS" -llvm-spirv-options="LLVM_SPIRV_OPTIONS" "--host-triple=x86_64-unknown-linux-gnu" "--linker-path=/usr/bin/ld" "--" HOST_LINKER_FLAGS "-dynamic-linker" HOST_DYN_LIB "-o" "a.out" HOST_LIB_PATH HOST_STAT_LIB %t.o --dry-run 2>&1 | FileCheck -check-prefix=CHK-CMDS %s
...
// RUN: clang-linker-wrapper -sycl-device-library-location= -sycl-device-libraries=%t.devicelib.o -sycl-post-link-options="SYCL_POST_LINK_OPTIONS" -llvm-spirv-options="LLVM_SPIRV_OPTIONS" "--host-triple=x86_64-unknown-linux-gnu" "--triple=spir64" "--linker-path=/usr/bin/ld" -shared "--" HOST_LINKER_FLAGS "-dynamic-linker" HOST_DYN_LIB "-o" "a.out" HOST_LIB_PATH HOST_STAT_LIB %t.o --dry-run 2>&1 | FileCheck -check-prefix=CHK-SHARED %s
...
// Generate .o file as linker wrapper input.
//
// RUN: %clang -cc1 %s -triple spir64_gen-unknown-unknown -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t1.bc
...
// RUN: %clang -cc1 %t1.devicelib.cpp -triple spir64_gen-unknown-unknown -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t1.devicelib.bc
...
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t1.devicelib.o \
// RUN:   -fembed-offload-object=%t1.devicelib.out
//
// Run clang-linker-wrapper test
//
// RUN: clang-linker-wrapper -sycl-device-library-location= -sycl-device-libraries=%t1.devicelib.o -sycl-post-link-options="SYCL_POST_LINK_OPTIONS" -llvm-spirv-options="LLVM_SPIRV_OPTIONS" "--host-triple=x86_64-unknown-linux-gnu" "--linker-path=/usr/bin/ld" "--" HOST_LINKER_FLAGS "-dynamic-linker" HOST_DYN_LIB "-o" "a.out" HOST_LIB_PATH HOST_STAT_LIB %t1.o --dry-run 2>&1 | FileCheck -check-prefix=CHK-CMDS-AOT-GEN %s
...
// Generate .o file as linker wrapper input.
//
// RUN: %clang -cc1 %s -triple spir64_x86_64-unknown-unknown -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t2.bc
// RUN: clang-offload-packager -o %t2.out --image=file=%t2.bc,kind=sycl,triple=spir64_x86_64-unknown-unknown,arch=x86_64
...
// RUN: %clang -cc1 %t2.devicelib.cpp -triple spir64_x86_64-unknown-unknown -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t2.devicelib.bc
// RUN: clang-offload-packager -o %t2.devicelib.out --image=file=%t2.devicelib.bc,kind=sycl,triple=spir64_x86_64-unknown-unknown,arch=x86_64
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t2.devicelib.o \
...
// Generate .o file as linker wrapper input.
//
// RUN: %clang -cc1 %s -triple nvptx64-nvidia-cuda -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t3.bc
// RUN: clang-offload-packager -o %t3.out --image=file=%t3.bc,kind=sycl,triple=nvptx64-nvidia-cuda
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t3.o \
// RUN:   -fembed-offload-object=%t3.out
//
// Generate .o file as SYCL device library file.
//
// RUN: echo '' > %t3.devicelib.cpp
// RUN: %clang -cc1 %t3.devicelib.cpp -triple nvptx64-nvidia-cuda -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t3.devicelib.bc
// RUN: clang-offload-packager -o %t3.devicelib.out --image=file=%t3.devicelib.bc,kind=sycl,triple=nvptx64-nvidia-cuda
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t3.devicelib.o \
// RUN:   -fembed-offload-object=%t3.devicelib.out
//
// Run clang-linker-wrapper test
//
// RUN: clang-linker-wrapper -sycl-device-library-location= -sycl-device-libraries=%t3.devicelib.o -sycl-post-link-options="SYCL_POST_LINK_OPTIONS" -llvm-spirv-options="LLVM_SPIRV_OPTIONS" "--host-triple=x86_64-unknown-linux-gnu" "--linker-path=/usr/bin/ld" "--" HOST_LINKER_FLAGS "-dynamic-linker" HOST_DYN_LIB "-o" "a.out" HOST_LIB_PATH HOST_STAT_LIB %t3.o --dry-run 2>&1 | FileCheck -check-prefix=CHK-CMDS-AOT-NV %s
...
// Generate .o file as linker wrapper input.
//
// RUN: %clang -cc1 %s -triple amdgcn-amd-amdhsa -aux-triple x86_64-unknown-linux-gnu -fsycl-is-device -emit-llvm-bc -o %t4.bc
// RUN: clang-offload-packager -o %t4.out --image=file=%t4.bc,kind=sycl,triple=amdgcn-amd-amdhsa
// RUN: %clang -cc1 %s -triple x86_64-unknown-linux-gnu -emit-obj -o %t4.o \
// RUN:   -fembed-offload-object=%t4.out
//
// Run clang-linker-wrapper test
//
// RUN: clang-linker-wrapper -sycl-device-library-location= -sycl-post-link-options="SYCL_POST_LINK_OPTIONS" -llvm-spirv-options="LLVM_SPIRV_OPTIONS" "--host-triple=x86_64-unknown-linux-gnu" "--linker-path=/usr/bin/ld" "--" HOST_LINKER_FLAGS "-dynamic-linker" HOST_DYN_LIB "-o" "a.out" HOST_LIB_PATH HOST_STAT_LIB %t4.o --dry-run 2>&1 | FileCheck -check-prefix=CHK-CMDS-AOT-AMD %s
