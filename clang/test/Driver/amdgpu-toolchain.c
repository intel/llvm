// RUN: %clang -### --target=amdgcn-amd-amdhsa -x assembler -mcpu=kaveri %s 2>&1 | FileCheck -check-prefix=AS_LINK %s
// RUN: %clang -### -g --target=amdgcn-amd-amdhsa -mcpu=kaveri -nogpulib %s 2>&1 | FileCheck -check-prefix=DWARF_VER %s
...
// RUN: %clang -### --target=amdgcn-amd-amdhsa -x assembler \
