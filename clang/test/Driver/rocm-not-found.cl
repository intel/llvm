...
// RUN: not %clang -### --sysroot=%s/no-rocm-there -target amdgcn-amd-amdhsa %s 2>&1 | FileCheck %s --check-prefix ERR
// RUN: not %clang -### --rocm-path=%s/no-rocm-there -target amdgcn-amd-amdhsa %s 2>&1 | FileCheck %s --check-prefix ERR
