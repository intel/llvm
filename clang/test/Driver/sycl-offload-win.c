...
// RUN: touch %t.lib
// RUN: %clang --target=x86_64-pc-windows-msvc -fsycl -fno-sycl-devicelib -foffload-static-lib=%t.lib -ccc-print-phases %s 2>&1 \
// RUN:   | FileCheck -DLIB=%t.lib %s -check-prefix=FOFFLOAD_STATIC_LIB_SRC
// RUN: %clang_cl --target=x86_64-pc-windows-msvc -fsycl -fno-sycl-devicelib -foffload-static-lib=%t.lib -ccc-print-phases %s 2>&1 \
// RUN:   | FileCheck -DLIB=%t.lib %s -check-prefix=FOFFLOAD_STATIC_LIB_SRC
