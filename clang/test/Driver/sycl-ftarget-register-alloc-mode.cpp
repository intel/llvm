...
// RUN: not %clang -### -fsycl --offload-new-driver --no-offloadlib \
...
// RUN: %clangxx -### -fsycl --offload-new-driver --no-offloadlib -fsycl-targets=spir64_gen -Xs "-device bdw" \
// RUN:          %s 2>&1 \
...
// RUNx: %clangxx -### -fsycl --offload-new-driver --no-offloadlib -fsycl-targets=spir64_gen -Xs "-device *" \
// RUNx:          %s 2>&1 \
// RUNx:   | FileCheck -check-prefix=NO_PVC %s

// RUN: %clangxx -### -fsycl --offload-new-driver --no-offloadlib -fsycl-targets=spir64_gen -Xs "-device pvc:mtl-s" \
// RUN:          %s 2>&1 \
// RUN:   | FileCheck -check-prefix=NO_PVC %s
