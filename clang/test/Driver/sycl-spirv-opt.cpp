...
// RUN: %clangxx -fsycl --offload-new-driver --no-offloadlib -Xspirv-translator "foo" -### %s 2>&1 | \
// RUN:  FileCheck %s -check-prefix CHECK-SINGLE-TARGET

// RUN: %clangxx -fsycl --offload-new-driver --no-offloadlib -Xspirv-translator=spir64_gen "foo" -### %s 2>&1 | \
