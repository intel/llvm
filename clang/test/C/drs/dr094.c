/* INTEL RUN: %clang_cc1 -std=c89 -opaque-pointers -emit-llvm -o - %s | FileCheck %s
   INTEL RUN: %clang_cc1 -std=c99 -opaque-pointers -emit-llvm -o - %s | FileCheck %s
   INTEL RUN: %clang_cc1 -std=c11 -opaque-pointers -emit-llvm -o - %s | FileCheck %s
   INTEL RUN: %clang_cc1 -std=c17 -opaque-pointers -emit-llvm -o - %s | FileCheck %s
   INTEL RUN: %clang_cc1 -std=c2x -opaque-pointers -emit-llvm -o - %s | FileCheck %s
 */
