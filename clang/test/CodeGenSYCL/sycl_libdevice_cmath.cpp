...
//
// For SPIR-V compilation, libdevice funcs are used since LLVM intrinsics will
// not be used if IsMathErrnoDefault evaluates to true, which is the case for
// SPIR-V. Since IsMathErrnoDefault is a driver setting, we must invoke clangxx
// for spirv compilation to check that LLVM intrinsics are not being emitted.

// RUN: %clangxx %s -fsycl-device-only -emit-llvm -S -o - | FileCheck %s
