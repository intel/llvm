...
// RUN:   -fsycl-is-device -disable-llvm-passes -emit-llvm -o - %s | FileCheck %s
// RUN: %clang_cc1 -triple spir64-unknown-unknown -aux-triple arm64ec-pc-windows-msvc \
// RUN:   -fsycl-is-device -disable-llvm-passes -emit-llvm -o - %s | FileCheck %s

// When SYCL device code is compiled with a host that predefines _M_X64 (an
// x86_64 or ARM64EC MSVC host), MSVC STL headers take the x86 intrinsics
// path. The device target feature set must correspondingly carry
