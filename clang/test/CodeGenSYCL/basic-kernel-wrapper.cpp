...
// CHECK: call spir_func void @{{.*}}__init{{.*}}(%"class.cl::sycl::accessor" addrspace(4)* noundef [[ACCESSOR]], i32 addrspace(1)* noundef [[MEM_LOAD]], %"struct.cl::sycl::range"* noundef byval({{.*}}) align 4 [[ARANGE]], %"struct.cl::sycl::range"* noundef byval({{.*}}) align 4 [[MRANGE]], %"struct.cl::sycl::id"* noundef byval({{.*}}) align 4 [[OID]])
