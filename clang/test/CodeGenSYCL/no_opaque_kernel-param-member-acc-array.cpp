// RUN: %clang_cc1 -fsycl-is-device -fsycl-int-header=%t.h -triple spir64-unknown-unknown -disable-llvm-passes -no-opaque-pointers -emit-llvm %s -o - | FileCheck %s
...
// CHECK: call spir_func void @{{.*}}__init{{.*}}([[ACCESSOR]] addrspace(4)* {{[^,]*}} [[ARRAY_IDX2]], i32 addrspace(1)* noundef [[MEM_LOAD2]], %"struct{{.*}}.sycl::_V1::range"* noundef byval({{.*}}) align 4 [[ACC_RANGE2]], %"struct{{.*}}.sycl::_V1::range"* noundef byval({{.*}}) align 4 [[MEM_RANGE2]], %"struct{{.*}}.sycl::_V1::id"* noundef byval({{.*}}) align 4 [[OFFSET2]])
