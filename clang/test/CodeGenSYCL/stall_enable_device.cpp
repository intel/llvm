...
int main() {
  q.submit([&](handler &h) {
    ...
    // CHECK define {{.*}}spir_func void @{{.*}}FooclEv(%class._ZTS3Foo.Foo addrspace(4)* noundef %this) #3 comdat align 2 !stall_enable ![[NUM4]]
    Foo f;
    ...
    // CHECK-NOT: !stall_enable
    // CHECK-SAME: {
    // CHECK: define {{.*}}spir_func void @{{.*}}func1{{.*}}(%class.anon{{.*}} addrspace(4)* noundef %this) #4 align 2 !stall_enable ![[NUM4]]
