...
int main() {
  q.submit([&](handler &h) {
    ...
    // CHECK: define {{.*}}spir_func void @{{.*}}Functor10{{.*}}(%class.Functor10 addrspace(4)* noundef %this) #3 comdat align 2
    // CHECK-NOT: noalias
    // CHECK-SAME: {
    ...
    // CHECK: define {{.*}}spir_func void @{{.*}}Foo8{{.*}}(%class.Foo8 addrspace(4)* noalias noundef %this) #3 comdat align 2
    Foo8 boo8;
    h.single_task<class kernel_name33>(boo8);

    // CHECK: define {{.*}}spir_kernel void @{{.*}}kernel_name34() #0 !kernel_arg_buffer_location ![[NUM]]
    // CHECK: define {{.*}}spir_func void @{{.*}}(%class.anon{{.*}} addrspace(4)* noalias noundef %this) #4 align 2
