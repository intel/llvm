...
// CHECK: call spir_func void @{{.*}}__init{{.*}}([[ACCESSOR]] addrspace(4)* noundef [[ARRAY_IDX1]], i32 addrspace(1)* noundef [[MEM_LOAD1]], %"struct{{.*}}.cl::sycl::range"* noundef byval({{.*}}) align 4 [[ACC_RANGE1]], %"struct{{.*}}.cl::sycl::range"* noundef byval({{.*}}) align 4 [[MEM_RANGE1]], %"struct{{.*}}.cl::sycl::id"* noundef byval({{.*}}) align 4 [[OFFSET1]])
...
// CHECK: call spir_func void @{{.*}}__init{{.*}}([[ACCESSOR]] addrspace(4)* noundef [[ARRAY_IDX2]], i32 addrspace(1)* noundef [[MEM_LOAD2]], %"struct{{.*}}.cl::sycl::range"* noundef byval({{.*}}) align 4 [[ACC_RANGE2]], %"struct{{.*}}.cl::sycl::range"* noundef byval({{.*}}) align 4 [[MEM_RANGE2]], %"struct{{.*}}.cl::sycl::id"* noundef byval({{.*}}) align 4 [[OFFSET2]])
