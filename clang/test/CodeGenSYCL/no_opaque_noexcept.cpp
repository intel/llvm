...
// RUN:      -no-opaque-pointers -emit-llvm %s -o - | FileCheck %s -check-prefix CHECK-DEVICE
//
// RUN: %clang_cc1 -triple x86_64-unknown-linux-gnu -I%S \
// RUN:      -fcxx-exceptions -fexceptions -disable-llvm-passes \
// RUN:      -no-opaque-pointers -emit-llvm %s -o - | FileCheck %s -check-prefix CHECK-HOST-LIN
//
// RUN: %clang_cc1 -triple x86_64-pc-windows-msvc -I%S \
// RUN:      -fcxx-exceptions -fexceptions -disable-llvm-passes \
// RUN:      -no-opaque-pointers -emit-llvm %s -o - | FileCheck %s -check-prefix CHECK-HOST-WIN
