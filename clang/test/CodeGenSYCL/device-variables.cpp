// RUN: %clang_cc1 -I %S/Inputs -triple spir64-unknown-unknown-sycldevice -fsycl-is-device -disable-llvm-passes -emit-llvm %s -o - | FileCheck %s
...
// Global variables used directly
    foo(global_value);
// CHECK: call spir_func void @{{.*}}foo{{.*}}(i32 addrspace(4)* dereferenceable(4) addrspacecast (i32 addrspace(1)* @{{.*}}global_value to i32 addrspace(4)*))
    int a = my_array[0];
...
// CHECK: call spir_func void @{{.*}}foo{{.*}}(i32 addrspace(4)* dereferenceable(4) [[GEP]])
...
// CHECK:  store i32 [[LOAD1]], i32* %some_device_local_var
  });
