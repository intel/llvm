...
// RUN:   -disable-llvm-passes -opaque-pointers -emit-llvm %s -o - | FileCheck %s
...
void test(int val) {
  ...
  // CHECK:   call spir_func void @{{.*}}__init_esimd{{.*}}(ptr addrspace(4) {{[^,]*}} %{{[a-zA-Z0-9_]+}}, ptr addrspace(1) noundef %{{[0-9]+}})
  // CHECK-LABEL: }
