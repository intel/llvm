...
int main() {
  ...
  deviceQueue.submit([&](sycl::handler &h) {
    ...
    h.single_task<class variables>([=]() {
      ...
      // expected-error@+2 {{'CapturedToDevice' requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{'__float128' is not supported on this target}}
      auto C = CapturedToDevice;
      // expected-note@+1 3{{used here}}
      Z<__float128> S;
      // expected-error@+2 {{'field1' requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{expression requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      S.field1 += 1;
      // expected-error@+2 {{'field' requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{expression requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      S.field = 1;
    });
  });
  ...
  deviceQueue.submit([&](sycl::handler &h) {
    ...
    h.single_task<class functions>([=]() {
      ...
      // expected-error@+2 2{{'foo' requires 128 bit size '__float128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{'BBBB' requires 128 bit size 'BIGTY' (aka '__float128') type support, but target 'spir64' does not support it}}
      auto A = foo(BBBB);
    });
  });
