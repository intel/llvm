// RUN: %clang_cc1 -triple spir64 -fsycl-is-device -internal-isystem %S/Inputs -Wno-double-in-device -sycl-std=2020 -verify -fsyntax-only %s
// RUN: %clang_cc1 -triple x86_64-linux-gnu -fsycl-is-device -internal-isystem %S/Inputs -Wno-double-in-device -sycl-std=2020 -fsyntax-only %s
