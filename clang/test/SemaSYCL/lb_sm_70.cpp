...
int main() {
  ...
  Q.submit([&](sycl::handler &cgh) {
    // expected-warning@+3 {{'maxclusterrank' requires sm_90 or higher, CUDA arch provided: sm_70, ignoring 'intel::max_work_groups_per_mp' attribute}}
    ...
    // expected-note@+2 {{value 2147483648 is outside the range of representable values of type 'int'}}
    cgh.single_task<class T3>(
        [=] [[intel::min_work_groups_per_cu(2147483647 + 1)]] () {
          volatile int A = 42;
        });

    // expected-warning@+4 {{attribute 'intel::min_work_groups_per_cu' is already applied with different arguments}}
    // expected-note@+2 {{previous attribute is here}}
    cgh.single_task<class T4>(
        [=] [[intel::min_work_groups_per_cu(4),
              intel::min_work_groups_per_cu(8)]] () { volatile int A = 42; });

    // expected-error@+2 {{'intel::min_work_groups_per_cu' attribute requires a non-negative integral compile time constant expression}}
