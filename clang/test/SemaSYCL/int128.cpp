...
int main() {
  ...
  deviceQueue.submit([&](sycl::handler &h) {
    ...
    h.single_task<class variables>([=]() {
      ...
      // expected-note@+1 3{{used here}}
      Z<__int128> S;
      // expected-error@+2 {{'field1' requires 128 bit size '__int128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{expression requires 128 bit size '__int128' type support, but target 'spir64' does not support it}}
      S.field1 += 1;
      // expected-error@+2 {{'field' requires 128 bit size '__int128' type support, but target 'spir64' does not support it}}
      // expected-error@+1 {{expression requires 128 bit size '__int128' type support, but target 'spir64' does not support it}}
      S.field = 1;
    });
  });
