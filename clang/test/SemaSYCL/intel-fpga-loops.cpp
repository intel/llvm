...
void foo() {
  ...
  // expected-error@+1 {{'intel::initiation_interval' attribute only applies to 'for', 'while', 'do' statements, and functions}}
  [[intel::initiation_interval(2)]] int c[10];
  // expected-error@+1 {{'intel::max_concurrency' attribute only applies to 'for', 'while', 'do' statements, and functions}}
  [[intel::max_concurrency(2)]] int d[10];
  // expected-error@+1 {{'intel::disable_loop_pipelining' attribute only applies to 'for', 'while', 'do' statements, and functions}}
  ...
  // expected-error@+1{{'intel::loop_count_avg' attribute cannot be applied to a declaration}}
  [[intel::loop_count_avg(6)]] int l[10];
  // expected-error@+1{{'intel::loop_count' attribute cannot be applied to a declaration}}
...
void boo() {
  ...
  [[intel::ivdep(2, 2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::initiation_interval' attribute takes one argument}}
  [[intel::initiation_interval]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::initiation_interval' attribute takes one argument}}
  [[intel::initiation_interval(2, 2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::max_concurrency' attribute takes one argument}}
  ...
  // expected-error@+1 {{'intel::disable_loop_pipelining' attribute takes no arguments}}
  ...
  [[intel::nofusion(0)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::loop_count_avg' attribute takes one argument}}
  [[intel::loop_count_avg(3, 6)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::loop_count' attribute takes one argument}}
  ...
  [[intel::max_reinvocation_delay(5, 2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::enable_loop_pipelining' attribute takes no arguments}}
...
void goo() {
  ...
  [[intel::ivdep(0)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::initiation_interval' attribute requires a positive integral compile time constant expression}}
  [[intel::initiation_interval(0)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::max_concurrency' attribute requires a non-negative integral compile time constant expression}}
  ...
  [[intel::max_interleaving(2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::speculated_iterations' attribute requires a non-negative integral compile time constant expression}}
  ...
  [[intel::loop_count_avg(0)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::loop_count_avg' attribute requires a non-negative integral compile time constant expression}}
  ...
  [[intel::loop_count(0)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  // expected-error@+1 {{'intel::loop_count' attribute requires a non-negative integral compile time constant expression}}
...
void zoo() {
  ...
  // expected-error@+1 {{conflicting loop attribute 'intel::max_concurrency'}}
  ...
  // expected-error@+1 {{conflicting loop attribute 'intel::initiation_interval'}}
  [[intel::initiation_interval(20)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
  [[intel::disable_loop_pipelining]]
  // expected-error@+1 {{duplicate Intel FPGA loop attribute 'intel::disable_loop_pipelining'}}
  [[intel::disable_loop_pipelining]] for (int i = 0; i != 10; ++i)
    a[i] = 0;
  ...
  // expected-error@+2 {{conflicting loop attribute 'intel::speculated_iterations'}}
  [[intel::loop_coalesce]]
  ...
  // expected-error@+1 {{conflicting loop attribute 'intel::speculated_iterations'}}
  [[intel::speculated_iterations(2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;

  [[intel::speculated_iterations(1)]] // expected-note 2{{previous attribute is here}}
  [[intel::speculated_iterations(1)]] // OK
  [[intel::speculated_iterations(2)]] // expected-error {{conflicting loop attribute 'intel::speculated_iterations'}}
  [[intel::speculated_iterations(4)]] // expected-error {{conflicting loop attribute 'intel::speculated_iterations'}}
  ...
  [[intel::max_concurrency(10)]] // OK
  [[intel::max_concurrency(20)]] // expected-error {{conflicting loop attribute 'intel::max_concurrency'}}
  [[intel::max_concurrency(40)]] // expected-error {{conflicting loop attribute 'intel::max_concurrency'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }

  [[intel::initiation_interval(10)]] // expected-note 2{{previous attribute is here}}
  [[intel::initiation_interval(10)]] // OK
  [[intel::initiation_interval(20)]] // expected-error {{conflicting loop attribute 'intel::initiation_interval'}}
  [[intel::initiation_interval(40)]] // expected-error {{conflicting loop attribute 'intel::initiation_interval'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }
  ...
  // expected-error@+1{{duplicate Intel FPGA loop attribute 'intel::loop_count_avg'}}
  [[intel::loop_count_avg(2)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;

  [[intel::loop_count(2)]]
  // expected-error@+1{{duplicate Intel FPGA loop attribute 'intel::loop_count'}}
  ...
  // expected-error@+1 {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  ...
  [[intel::max_reinvocation_delay(10)]] // OK
  [[intel::max_reinvocation_delay(20)]] // expected-error {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  [[intel::max_reinvocation_delay(40)]] // expected-error {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }

  [[intel::enable_loop_pipelining]]
  // expected-error@+1 {{duplicate Intel FPGA loop attribute 'intel::enable_loop_pipelining'}}
  [[intel::enable_loop_pipelining]] for (int i = 0; i != 10; ++i)
      a[i] = 0;
}
...
void loop_attrs_compatibility() {
  ...
  [[intel::disable_loop_pipelining]] [[intel::max_concurrency(0)]] for (int i = 0; i != 10; ++i)
    a[i] = 0;
  // expected-error@+2 {{'intel::disable_loop_pipelining' and 'intel::speculated_iterations' attributes are not compatible}}
  // expected-note@+1 {{conflicting attribute is here}}
  ...
  [[intel::enable_loop_pipelining]] [[intel::loop_coalesce]] for (int i = 0; i != 10; ++i)
    a[i] = 0;
  // expected-error@+2 {{'intel::disable_loop_pipelining' and 'intel::enable_loop_pipelining' attributes are not compatible}}
  // expected-note@+1 {{conflicting attribute is here}}
...
void ii_dependent() {
  ...
  // expected-error@+2 {{conflicting loop attribute 'intel::initiation_interval'}}
  ...
  [[intel::initiation_interval(B)]] // expected-error {{conflicting loop attribute 'intel::initiation_interval'}}
  [[intel::initiation_interval(D)]] // expected-error {{conflicting loop attribute 'intel::initiation_interval'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }
...
void max_concurrency_dependent() {
  ...
  // expected-error@+2 {{conflicting loop attribute 'intel::max_concurrency'}}
  ...
  [[intel::max_concurrency(A)]] // expected-error {{conflicting loop attribute 'intel::max_concurrency'}}
  [[intel::max_concurrency(B)]] // expected-error {{conflicting loop attribute 'intel::max_concurrency'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }
...
void speculated_iterations_dependent() {
  ...
  [[intel::speculated_iterations(B)]] // expected-error {{conflicting loop attribute 'intel::speculated_iterations'}}
  [[intel::speculated_iterations(E)]] // expected-error {{conflicting loop attribute 'intel::speculated_iterations'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }
...
void loop_count_control_dependent() {
  ...
  //expected-error@+1{{'intel::loop_count_min' attribute requires a non-negative integral compile time constant expression}}
  [[intel::loop_count_min(C)]]
  for (int i = 0; i != 10; ++i)
      a[i] = 0;

  //expected-error@+1{{'intel::loop_count_max' attribute requires a non-negative integral compile time constant expression}}
  ...
  //expected-error@+1{{duplicate Intel FPGA loop attribute 'intel::loop_count_min'}}
  [[intel::loop_count_min(B)]] for (int i = 0; i != 10; ++i)
      a[i] = 0;

  [[intel::loop_count_max(A)]]
  //expected-error@+1{{duplicate Intel FPGA loop attribute 'intel::loop_count_max'}}
...
void max_reinvocation_delay_dependent() {
  ...
  // expected-error@+2 {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  ...
  [[intel::max_reinvocation_delay(B)]] // expected-error {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  [[intel::max_reinvocation_delay(D)]] // expected-error {{conflicting loop attribute 'intel::max_reinvocation_delay'}}
  for (int i = 0; i != 10; ++i) { a[i] = 0; }
...
void parse_order_error() {
  ...
  [[clang::nomerge, intel::max_concurrency(1)]] // expected-error {{'intel::max_concurrency' attribute only applies to 'for', 'while', 'do' statements, and functions}}
