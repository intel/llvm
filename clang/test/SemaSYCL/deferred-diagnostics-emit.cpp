...
void setup_sycl_operation(const T VA[]) {
  ...
  deviceQueue.submit([&](sycl::handler &h) {
    // expected-note@Inputs/sycl.hpp:225 {{called by 'kernel_single_task<AName, (lambda}}
...
int main(int argc, char **argv) {
  ...
  deviceQueue.submit([&](sycl::handler &h) {
    // expected-note@Inputs/sycl.hpp:225 2 {{called by 'kernel_single_task<AName, (lambda}}
    h.single_task<class AName>([]() {
      // expected-error@+1 {{zero-length arrays are not permitted in C++}}
      int BadArray[0];
