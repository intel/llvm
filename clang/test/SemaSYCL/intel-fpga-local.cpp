...
void diagnostics()
{
  ...
  //expected-warning@+2{{attribute 'intel::doublepump' is already applied}}
  ...
  //expected-warning@+2{{attribute 'intel::singlepump' is already applied}}
  ...
  //expected-warning@+2{{attribute 'intel::fpga_register' is already applied}}
  ...
  //expected-error@+2{{'intel::fpga_memory' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  [[intel::fpga_memory]]
  //expected-note@-2 {{conflicting attribute is here}}
  unsigned int reg_memory[64];

  //expected-error@+2{{'intel::fpga_register' and 'intel::fpga_memory' attributes are not compatible}}
  [[intel::fpga_memory]]
  [[intel::fpga_register]]
  //expected-note@-2 {{conflicting attribute is here}}
  unsigned int reg_fp_memory[64];

  //expected-error@+2{{'intel::bank_bits' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  [[intel::bank_bits(4, 5)]]
  //expected-note@-2 {{conflicting attribute is here}}
  ...
  //expected-error@+2{{'intel::bankwidth' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  [[intel::bankwidth(16)]]
  //expected-note@-2 {{conflicting attribute is here}}
  ...
  //expected-warning@+2{{attribute 'intel::numbanks' is already applied with different arguments}}
  ...
  //expected-error@+3{{'intel::max_replicates' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  //expected-note@-1 {{conflicting attribute is here}}
  [[intel::max_replicates(2)]] unsigned int reg_maxrepl[64];

  //expected-error@+3{{'intel::simple_dual_port' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  //expected-note@-1 {{conflicting attribute is here}}
  [[intel::simple_dual_port]] unsigned int reg_dualport[64];

  //expected-error@+3{{'intel::force_pow2_depth' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  //expected-note@-1 {{conflicting attribute is here}}
  [[intel::force_pow2_depth(0)]] unsigned int reg_force_p2d[64];

  //expected-error@+3{{'intel::fpga_register' and 'intel::force_pow2_depth' attributes are not compatible}}
  ...
  //expected-warning@+2{{attribute 'intel::fpga_memory' is already applied with different arguments}}
  ...
  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::max_replicates' is already applied with different arguments}}
  [[intel::max_replicates(8)]] //expected-note{{previous attribute is here}}
  [[intel::max_replicates(16)]] unsigned int max_repl[64];

  //expected-error@+1{{'intel::max_replicates' attribute requires a positive integral compile time constant expression}}
  ...
  // **simple_dual_port
  //expected-error@+1{{'intel::simple_dual_port' attribute takes no arguments}}
  ...
  //expected-note@+1 {{conflicting attribute is here}}
  [[intel::fpga_register]]
  //expected-error@+1{{'intel::simple_dual_port' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::simple_dual_port]] unsigned int sdp_reg[64];

  //expected-warning@+2{{attribute 'intel::simple_dual_port' is already applied}}
  [[intel::simple_dual_port]] //expected-note{{previous attribute is here}}
  [[intel::simple_dual_port]] unsigned int dual_port_var_dup[64];

  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::bankwidth' is already applied}}
  ...
  //expected-error@+1{{'intel::bankwidth' attribute takes one argument}}
  ...
  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::private_copies' is already applied with different arguments}}
  [[intel::private_copies(8)]] //expected-note{{previous attribute is here}}
  [[intel::private_copies(16)]] unsigned int pc_pc[64];

  //expected-error@+1{{'intel::private_copies' attribute requires a non-negative integral compile time constant expression}}
  ...
  //expected-error@+1{{'intel::private_copies' attribute takes one argument}}
  ...
  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::numbanks' is already applied}}
  [[intel::numbanks(8)]] // expected-note {{previous attribute is here}}
  ...
  //expected-error@+1{{'intel::numbanks' attribute takes one argument}}
  ...
  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::merge' is already applied with different arguments}}
  ...
  // **bank_bits
  //expected-error@+2 1{{'intel::fpga_register' and 'intel::bank_bits' attributes are not compatible}}
  [[intel::bank_bits(2, 3)]]
  [[intel::fpga_register]]
  ...
  // Checking of different argument values.
  //expected-warning@+2{{attribute 'intel::bank_bits' is already applied}}
  ...
  //expected-error@+1{{'intel::bank_bits' attribute requires a non-negative integral compile time constant expression}}
  ...
  //expected-error@+1{{'intel::force_pow2_depth' attribute requires integer constant value 0 or 1}}
  [[intel::force_pow2_depth(-1)]] unsigned int force_p2d_below_min[64];
  //expected-error@+1{{'intel::force_pow2_depth' attribute requires integer constant value 0 or 1}}
  [[intel::force_pow2_depth(2)]] unsigned int force_p2d_above_max[64];

  //expected-error@+1{{'intel::force_pow2_depth' attribute takes one argument}}
  ...
  //expected-warning@+1{{attribute 'intel::force_pow2_depth' is already applied with different arguments}}
...
void check_template_parameters() {
  ...
  // expected-warning@+1 {{attribute 'intel::max_replicates' is already applied with different arguments}}
  [[intel::max_replicates(C)]] unsigned int max_repl_duplicate[64];

  // Test that checks template instantiations for different arg values.
  [[intel::private_copies(4)]] // expected-note {{previous attribute is here}}
  // expected-warning@+1 {{attribute 'intel::private_copies' is already applied with different arguments}}
  ...
  // expected-warning@+1 {{attribute 'intel::numbanks' is already applied with different arguments}}
  [[intel::numbanks(C)]] unsigned int num_banks[64];

  //expected-error@+3{{'intel::numbanks' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  //expected-note@-1 {{conflicting attribute is here}}
  ...
  // expected-warning@+1 {{attribute 'intel::bankwidth' is already applied with different arguments}}
  [[intel::bankwidth(C)]] unsigned int bank_width[64];

  //expected-error@+3{{'intel::bankwidth' and 'intel::fpga_register' attributes are not compatible}}
  [[intel::fpga_register]]
  //expected-note@-1 {{conflicting attribute is here}}
...
//expected-error@+3{{'intel::fpga_register' and 'intel::max_replicates' attributes are not compatible}}
//expected-note@+1{{conflicting attribute is here}}
[[intel::max_replicates(12)]] extern const int var_max_replicates_2;
[[intel::fpga_register]] const int var_max_replicates_2 =0;

// Merging of different arg values.
//expected-warning@+2{{attribute 'intel::force_pow2_depth' is already applied with different arguments}}
...
// Merging of incompatible attributes.
//expected-error@+3{{'intel::fpga_register' and 'intel::numbanks' attributes are not compatible}}
//expected-note@+1{{conflicting attribute is here}}
[[intel::numbanks(16)]] extern const int var_numbanks_2;
[[intel::fpga_register]] const int var_numbanks_2 =0;

// Merging of different arg values.
//expected-warning@+2{{attribute 'intel::bankwidth' is already applied with different arguments}}
[[intel::bankwidth(8)]] extern const int var_bankwidth_1;
[[intel::bankwidth(16)]] const int var_bankwidth_1 = 0;
//expected-note@-2{{previous attribute is here}}

// Merging of incompatible attributes.
//expected-error@+3{{'intel::fpga_register' and 'intel::bankwidth' attributes are not compatible}}
//expected-note@+1{{conflicting attribute is here}}
