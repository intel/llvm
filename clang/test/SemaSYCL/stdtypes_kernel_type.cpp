...
int main() {
#ifdef CHECK_ERROR
  q.submit([&](handler &h) {
    // expected-error@Inputs/sycl.hpp:233 {{'nullptr_t' is an invalid kernel name, ‘std::nullptr_t’ is declared in std namespace }}
  ...
  q.submit([&](handler &h) {
    // expected-error@Inputs/sycl.hpp:233 {{'Templated_kernel_name<nullptr_t>' is an invalid kernel name, ‘std::nullptr_t’ is declared in std namespace }}
    // expected-note@+1{{in instantiation of function template specialization}}
  ...
  q.submit([&](handler &cgh) {
    // expected-error@Inputs/sycl.hpp:233 {{'TemplParamPack<int, float, nullptr_t, double>' is an invalid kernel name, ‘std::nullptr_t’ is declared in std namespace }}
    // expected-note@+1{{in instantiation of function template specialization}}
