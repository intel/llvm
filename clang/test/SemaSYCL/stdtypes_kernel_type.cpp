...
int main() {
  ...
  q.submit([&](handler &h) {
    // expected-error@Inputs/sycl.hpp:220 {{'std::T' is an invalid kernel name, 'std::T' is declared in the 'std' namespace}}
    // expected-note@+1{{in instantiation of function template specialization}}
    h.single_task<std::T>([=] {});
  });
  q.submit([&](handler &h) {
  ...
  q.submit([&](handler &h) {
    // expected-error@Inputs/sycl.hpp:220 {{'Templated_kernel_name<std::U>' is an invalid kernel name, 'std::U' is declared in the 'std' namespace}}
    // expected-note@+1{{in instantiation of function template specialization}}
    h.single_task<Templated_kernel_name<std::U>>([=] {});
  });
  q.submit([&](handler &cgh) {
    // expected-error@Inputs/sycl.hpp:220 {{'Templated_kernel_name2<Templated_kernel_name<std::Foo>>' is an invalid kernel name, 'std::Foo' is declared in the 'std' namespace}}
    // expected-note@+1{{in instantiation of function template specialization}}
