// RUN: %clang_cc1 -fsyntax-only -verify=precision-loss,precision-gain,size-change -fsycl-is-host -x c++ -Wimplicit-float-conversion -Wdouble-promotion -Wsycl-implicit-float-size-conversion \
// RUN:   -triple x86_64-apple-darwin %s

// RUN: %clang_cc1 -fsyntax-only -verify=size-only,precision-gain,size-change -fsycl-is-host -x c++ -Wdouble-promotion -Wsycl-implicit-float-size-conversion \
// RUN:   -triple x86_64-apple-darwin %s

// RUN: %clang_cc1 -fsyntax-only -verify=precision-increase -fsycl-is-host -x c++ -Wdouble-promotion \
// RUN:   -triple x86_64-apple-darwin %s

// RUN: %clang_cc1 -fsyntax-only -verify=precision-loss,size-change -fsycl-is-host -x c++ -Wimplicit-float-conversion \
// RUN:   -triple x86_64-apple-darwin %s

// RUN: %clang_cc1 -fsyntax-only -verify=precision-loss -Wimplicit-float-conversion \
// RUN:   -triple x86_64-apple-darwin %s

// RUN: %clang_cc1 -fsyntax-only -verify -fsycl-is-host -x c++ \
// RUN:   -triple x86_64-apple-darwin %s
