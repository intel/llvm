...
ABIArgInfo SPIRABIInfo::classifyRegcallReturnType(QualType RetTy) const {
  if (isAggregateTypeForABI(RetTy)) {
    // Records with non-trivial destructors/copy-constructors should not be
    // returned by value.
    if (!getRecordArgABI(RetTy, getCXXABI())) {
...
ABIArgInfo SPIRABIInfo::classifyRegcallArgumentType(QualType Ty) const {
  Ty = useFirstFieldIfTransparentUnion(Ty);
...
void SPIRABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  if (!getCXXABI().classifyReturnType(FI)) {
    ...
    FI.getReturnInfo() = IsRegCall ? classifyRegcallReturnType(RetT) : classifyReturnType(RetT);
  }
  ...
  for (auto &Arg : FI.arguments()) {
    if (CC == llvm::CallingConv::SPIR_KERNEL) {
      ...
      Arg.info = IsRegCall ? classifyRegcallArgumentType(Arg.type) : classifyArgumentType(Arg.type);
    }
  }
}
