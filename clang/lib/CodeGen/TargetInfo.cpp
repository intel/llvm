...
bool isStdTupleOfTriviallyCopyableTypes(QualType T) {
...
ABIArgInfo getStdTupleSPIRVRegcallABI(ASTContext& Ctx, llvm::LLVMContext& LLVMCtx, QualType Ty) {
  ...
  return ABIArgInfo::getDirect(llvm::ArrayType::get(I32Ty, NElems));
}
...
void CommonSPIRABIInfo::computeInfo(CGFunctionInfo &FI) const {
  ...
  if (IsRegCall) {
    assert((CC != llvm::CallingConv::SPIR_KERNEL) && "kernels can't have __regcall calling convention");
    auto RetTy = FI.getReturnType();

    if (isStdTupleOfTriviallyCopyableTypes(RetTy)) {
      FI.getReturnInfo() = getStdTupleSPIRVRegcallABI(getContext(), getVMContext(), RetTy);
    } else {
      if (!getCXXABI().classifyReturnType(FI))
        FI.getReturnInfo() = classifyRegcallReturnType(RetTy);
    }
    for (auto& Arg : FI.arguments()) {
      bool IsTuple = isStdTupleOfTriviallyCopyableTypes(Arg.type);
      Arg.info = IsTuple ? getStdTupleSPIRVRegcallABI(getContext(), getVMContext(), Arg.type) : classifyRegcallArgumentType(Arg.type);
    }
    return;
  }
...
ABIArgInfo CommonSPIRABIInfo::classifyRegcallArgumentType(QualType Ty) const {
  ...
  if (isAggregateTypeForABI(Ty)) {
    if (isStdTupleOfTriviallyCopyableTypes(Ty))
