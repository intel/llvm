...
static FuncNamePair EmitMaybeIntrinsic(CodeGenFunction &CGF, const CallExpr *E,
  ...
  if (GenerateFPMathIntrinsics) {
    ...
    case Builtin::BI__builtin_elementwise_sqrt: {
      llvm::Value *Call = emitUnaryMaybeConstrainedFPBuiltin(
          *this, E, Intrinsic::sqrt, Intrinsic::experimental_constrained_sqrt, Intrinsic::fpbuiltin_sqrt);
...
RValue CodeGenFunction::EmitIntelFPGARegBuiltin(const CallExpr *E,
  ...
  llvm::Value *AnnotatedV = EmitAnnotationCall(F, V, AnnotStr, E->getExprLoc());
...
llvm::CallInst *CodeGenFunction::MaybeEmitFPBuiltinofFD(
  ...
  if (hasFuncNameRequestedFPAccuracy(Name, LangOpts) ||
    ...
    llvm::FunctionType *FPAccuracyIntrinsicFT = llvm::Intrinsic::getType(
        CGM.getLLVMContext(), FPAccuracyIntrinsicID, {IRArgs[0]->getType()});
