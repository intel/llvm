...
renderDebugOptions(const ToolChain &TC, const Driver &D, const llvm::Triple &T,
  ...
  if (Args.hasFlag(options::OPT_fno_system_debug, options::OPT_fsystem_debug,
                   false))
    if (checkDebugInfoOption(Args.getLastArg(options::OPT_fno_system_debug), Args,
                             D, TC))
      CmdArgs.push_back("-no-system-debug");
