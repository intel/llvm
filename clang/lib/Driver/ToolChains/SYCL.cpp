...
StringRef SYCL::gen::resolveGenDevice(StringRef DeviceName) {
  return clang::SYCLTargetToOffloadArchName(DeviceName);
}

StringRef clang::driver::mapIntelGPUArchName(StringRef ArchName) {
  return clang::NormalizeIntelGPUOffloadArch(ArchName);
}

SmallString<64> SYCL::gen::getGenDeviceMacro(StringRef DeviceName) {
  SmallString<64> Macro;
  StringRef Suffix = clang::GetOffloadArchMacroSuffix(DeviceName);
  if (!Suffix.empty()) {
    ...
    if (A->getOption().matches(Opt_EQ)) {
      ...
      if (IsGenTriple) {
        ...
        if (OptTargetTriple == Triple && !Device.empty())
          ...
          continue;
        // In the new offload model, -Xsycl-target-backend=spir64_gen is no
        // longer supported; users must switch to intel_gpu_<device>.
        // (GenDevice empty means the qualifier was "spir64_gen", not
        // "intel_gpu_*".)
        if (GenDevice.empty() && OptTargetTriple == Triple && !A->isClaimed() &&
            getDriver().getUseNewOffloadingDriver())
          getDriver().Diag(diag::err_drv_sycl_target_spir64_gen_deprecated);
