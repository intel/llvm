  ...
  if (NeedLibs && Triple.getSubArch() == llvm::Triple::SPIRSubArch_gen) {
    ...
    auto checkIntelGPUBF16 = [&](StringRef Target) {
      if (!Target.starts_with("intel_gpu_"))
        return false;
      StringRef IntelGPUDevice = llvm::sycl_target::resolveGenDevice(Target);
...
void SYCLToolChain::TranslateTargetOpt(const llvm::Triple &Triple,
  ...
  for (auto *A : Args) {
    ...
    if (A->getOption().matches(Opt_EQ)) {
      ...
      StringRef GenDevice = llvm::sycl_target::resolveGenDevice(A->getValue());
...
void SYCLToolChain::AddSPIRVImpliedTargetArgs(const llvm::Triple &Triple,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_ftarget_register_alloc_mode_EQ)) {
    ...
    auto ProcessElement = [&](StringRef Ele) {
      ...
      StringRef BackendOptName = llvm::sycl_target::getGenGRFFlag(RegAllocMode);
    ...
    if (IsJIT || Device == "pvc" || hasPVCDevice(TargArgs, DevArg)) {
      ...
      if (!DevArg.empty())
        DeviceName = DevArg;
      StringRef BackendOptName = llvm::sycl_target::getGenGRFFlag("auto");
      if (IsGen)
