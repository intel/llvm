...
class OffloadingActionBuilder final {
  ...
  class SYCLActionBuilder final : public DeviceActionBuilder {
    ...
    SmallVector<std::pair<Action*, SmallVector<std::string, 4>>> SYCLFinalDeviceList;
  ...
  public:
    ...
    SmallVector<std::string, 4> deviceBinarySections(Compilation &C, const StringRef &Input) {
    ...
    bool initialize() override {
      ...
      auto makeInputAction = [&](const StringRef Name, types::ID Type) -> Action* {
      ...
      if (HasValidSYCLRuntime) {
        ...
        for (StringRef LA : LinkArgs) {
          SmallVector<std::string, 4> DeviceTargets(deviceBinarySections(C, LA));
          if (DeviceTargets.size()) {
            bool IsArchive = isStaticArchiveFile(LA);
            types::ID FileType = IsArchive ? types::TY_Archive : types::TY_Object;
            SYCLFinalDeviceList.push_back(std::make_pair(makeInputAction(LA, FileType), DeviceTargets));
          }
        }
      }
...
public:
  ...
  bool appendTopLevelActions(ActionList &AL, Action *HostAction,
    ...
    if (CanUseBundler && HostAction &&
      ...
      // qualifier.
      if (LinkDevice) {
        std::string DeviceStr = makeDeviceArchString();
        if (!DeviceStr.empty())
