      ...
      for (auto *A : Args) {
        ...
        if (ParsedArg &&
          ...
          if (TargetBE->isNVPTX()) {
            ...
            const llvm::Triple AMDTriple("amdgcn-amd-amdhsa");
            StringRef TargetIDStr = ArchStr;
            if (llvm::Triple::parseSubArch(ArchStr) != llvm::Triple::NoSubArch)
              TargetIDStr = getProcessorFromTargetID(AMDTriple, ArchStr);
            std::optional<llvm::AMDGPU::TargetID> ID =
                llvm::AMDGPU::TargetID::parse(AMDTriple, TargetIDStr);
            if (!ID) {
              C.getDriver().Diag(clang::diag::err_drv_bad_target_id) << ArchStr;
              continue;
            }
            ArchStr = Args.MakeArgStringRef(ID->getCanonicalTargetIDString());
          }
          ParsedArg->claim();
    ...
    for (auto OA : OffloadActions) {
      if (const OffloadAction *CurOA = dyn_cast<OffloadAction>(OA)) {
        CurOA->doOnEachDependence(
            [&](Action *A, const ToolChain *TC, BoundArch BA) {
              ...
              if (isa<PrecompileJobAction>(A)) {
                ...
                DDeps.add(*CompileAction, *TC, BA, Action::OFK_SYCL);
              }
            });
      }
    }
    ...
    for (auto &UI : UA->getDependentActionsInfo()) {
      ...
      if (C.getDriver().getOffloadStaticLibSeen() &&
        ...
        std::string OffloadingPrefix = Action::GetOffloadingFileNamePrefix(
          ...
          UI.DependentToolChain->getTripleString(),
...
const char *Driver::GetNamedOutputPath(Compilation &C, const JobAction &JA,
  ...
  if (JA.getType() == types::TY_PCH &&
    ...
    return CreateTempFile(C, Split.first, Suffix, MultipleArchs, BoundArchStr,
                          JA.getType(), false);
  }
