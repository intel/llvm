...
bool Qualifiers::isTargetAddressSpaceSupersetOf(LangAS A, LangAS B,
  ...
  // allocated on the device. This holds for any combination of the OpenCL and
  // SYCL spellings, as `__attribute__((opencl_global_device))` and
  // `__attribute__((opencl_global_host))` are mapped onto the SYCL address
  // spaces in SYCL mode while `__attribute__((opencl_global))` is not.
  if (getMemoryRegion(A) == MemoryRegion::Global &&
      (getMemoryRegion(B) == MemoryRegion::GlobalDevice ||
       getMemoryRegion(B) == MemoryRegion::GlobalHost))
    return true;
  ...
  // FIXME: In SYCL mode, the OpenCL address space attributes are treated as
  // supersets of the default address space just like their SYCL counterparts
  // above. The SYCL headers and libdevice spell the address spaces with
  // `__attribute__((opencl_*))`, and until they are migrated to the SYCL
  // address space attributes the generic address space has to overlap them.
  // See https://github.com/intel/llvm/issues/3062 for more details.
  if (Ctx.getLangOpts().isSYCL() &&
      (A == LangAS::Default || A == LangAS::sycl_generic) &&
      (B == LangAS::opencl_private || B == LangAS::opencl_local ||
       B == LangAS::opencl_global || B == LangAS::opencl_global_device ||
