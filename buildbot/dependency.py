...
def do_dependency(args):
    ...
    if args.pr_number is not None and not args.clean_build:
        if args.branch is None or args.base_branch is None:
            print("branch ({}) and base branch ({}) is required for pull request #{}".format(args.branch, args.base_branch, args.pr_number))
            return ret
...
def main():
    ...
    parser.add_argument("-w", "--builder-dir", metavar="BUILDER_DIR", help="builder directory, which is the directory contains source and build directories")
    ...
    parser.add_argument("-c", "--clean-build", action="store_true", default=False, help="true if the build is clean build which has clobber step")
