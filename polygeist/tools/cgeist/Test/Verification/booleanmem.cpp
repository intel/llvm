...
// CHECK-NEXT:      %[[VAL_3:.*]] = llvm.alloca %[[VAL_2]] x !polygeist.struct<(i8, memref<?xi8>)> : (i64) -> !llvm.ptr
// CHECK-NEXT:      %[[VAL_4:.*]] = llvm.alloca %[[VAL_2]] x !polygeist.struct<(i8, memref<?xi8>)> : (i64) -> !llvm.ptr
// CHECK-NEXT:      %[[VAL_5:.*]] = llvm.getelementptr inbounds %[[VAL_4]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(i8, memref<?xi8>)>
...
// CHECK-NEXT:      %[[VAL_10:.*]] = llvm.getelementptr inbounds %[[VAL_4]][0, 1] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(i8, memref<?xi8>)>
// CHECK-NEXT:      llvm.store %[[VAL_9]], %[[VAL_10]] : memref<?xi8>, !llvm.ptr
// CHECK-NEXT:      %[[VAL_11:.*]] = llvm.load %[[VAL_4]] : !llvm.ptr -> !polygeist.struct<(i8, memref<?xi8>)>
// CHECK-NEXT:      llvm.store %[[VAL_11]], %[[VAL_3]] : !polygeist.struct<(i8, memref<?xi8>)>, !llvm.ptr
