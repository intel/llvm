...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.alloca %[[VAL_0]] x !polygeist.struct<(!llvm.struct<packed (ptr, i32, array<4 x i8>)>, !llvm.struct<(ptr)>)> : (i64) -> !llvm.ptr
...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(!llvm.struct<packed (ptr, i32, array<4 x i8>)>, !llvm.struct<(ptr)>)>
// CHECK-NEXT:      call @_ZN1AC1Ev(%[[VAL_1]]) : (!llvm.ptr) -> ()
// CHECK-NEXT:      %[[VAL_2:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 1] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(!llvm.struct<packed (ptr, i32, array<4 x i8>)>, !llvm.struct<(ptr)>)>
