...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.alloca %[[VAL_0]] x !polygeist.struct<(!llvm.struct<(i8)>)> : (i64) -> !llvm.ptr
...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(!llvm.struct<(i8)>)>
