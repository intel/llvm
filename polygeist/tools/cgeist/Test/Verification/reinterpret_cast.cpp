...
// CHECK-SAME:                                              %[[VAL_0:.*]]: !polygeist.struct<(f32)>) -> !polygeist.struct<(i32)>
// CHECK:           %[[VAL_1:.*]] = arith.constant 1 : i64
// CHECK:           %[[VAL_2:.*]] = llvm.alloca %[[VAL_1]] x !polygeist.struct<(i32)> : (i64) -> !llvm.ptr
// CHECK:           %[[VAL_3:.*]] = llvm.alloca %[[VAL_1]] x !polygeist.struct<(f32)> : (i64) -> !llvm.ptr
// CHECK:           llvm.store %[[VAL_0]], %[[VAL_3]] : !polygeist.struct<(f32)>, !llvm.ptr
// CHECK:           call @_ZN8fooint32C1ERKS_(%[[VAL_2]], %[[VAL_3]]) : (!llvm.ptr, !llvm.ptr) -> ()
// CHECK:           %[[VAL_4:.*]] = llvm.load %[[VAL_2]] : !llvm.ptr -> !polygeist.struct<(i32)>
// CHECK:           return %[[VAL_4]] : !polygeist.struct<(i32)>
// CHECK:         }

// CHECK-LABEL:   func.func @_ZN8fooint32C1ERKS_(
// CHECK-SAME:                                   %[[VAL_0:.*]]: !llvm.ptr, %[[VAL_1:.*]]: !llvm.ptr)
// CHECK:           %[[VAL_2:.*]] = llvm.getelementptr inbounds %[[VAL_1]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(i32)>
// CHECK:           %[[VAL_3:.*]] = llvm.load %[[VAL_2]] : !llvm.ptr -> i32
// CHECK:           %[[VAL_4:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(i32)>
