...
// CHECK-NEXT:      %[[VAL_2:.*]] = llvm.alloca %[[VAL_1]] x !polygeist.struct<(i16)> : (i64) -> !llvm.ptr
// CHECK-NEXT:      call @_ZN4HalfC1ERKS_(%[[VAL_2]], %[[VAL_0]]) : (!llvm.ptr, !llvm.ptr) -> ()
// CHECK-NEXT:      %[[VAL_3:.*]] = llvm.load %[[VAL_2]] : !llvm.ptr -> !polygeist.struct<(i16)>
// CHECK-NEXT:      %[[VAL_4:.*]] = call @thing(%[[VAL_3]]) : (!polygeist.struct<(i16)>) -> f32
