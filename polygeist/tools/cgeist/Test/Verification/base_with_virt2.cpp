...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.alloca %[[VAL_0]] x !polygeist.struct<(!llvm.struct<(ptr)>, !llvm.struct<(ptr)>)> : (i64) -> !llvm.ptr
...
// CHECK-NEXT:      %[[VAL_1:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 0] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(!llvm.struct<(ptr)>, !llvm.struct<(ptr)>)>
// CHECK-NEXT:      call @_ZN15basic_streambufC1Ev(%[[VAL_1]]) : (!llvm.ptr) -> ()
// CHECK-NEXT:      %[[VAL_2:.*]] = llvm.getelementptr inbounds %[[VAL_0]][0, 1] : (!llvm.ptr) -> !llvm.ptr, !polygeist.struct<(!llvm.struct<(ptr)>, !llvm.struct<(ptr)>)>
