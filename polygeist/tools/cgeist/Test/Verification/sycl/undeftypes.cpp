...
// CHECK:          sycl.call @VecWrapper(%{{.*}}, %[[SPAN]]) {MangledFunctionName = @{{.*}}, TypeName = @VecWrapper} : (memref<?x!polygeist.struct<(!sycl_vec_f32_4_)>, 4>, f32) -> ()
// CHECK:          %{{.*}} = sycl.call @"operator[]"(%{{.*}}, %[[INDEX]]) {MangledFunctionName = @{{.*}}, TypeName = @VecWrapper} : (memref<?x!polygeist.struct<(!sycl_vec_f32_4_)>, 4>, i64) -> f32
