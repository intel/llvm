...
// CHECK-MLIR-SAME:      %arg1: !llvm.ptr {llvm.align = 8 : i64, llvm.byval = !polygeist.struct<(memref<?xi32, 1>)>, llvm.noundef}) 
...
// CHECK-MLIR:         %[[VAL_15:.*]] = "polygeist.subindex"(%[[VAL_9:.*]], %[[VAL_1:.*]]) : (memref<?x!polygeist.struct<(!sycl_range_1_, !polygeist.struct<(memref<?xi32, 4>)>)>>, index) -> memref<?x!polygeist.struct<(memref<?xi32, 4>)>>
...
// CHECK-MLIR:         %[[VAL_19:.*]] = llvm.load %[[VAL_5]] : !llvm.ptr -> !polygeist.struct<(memref<?xi32, 4>)>
// CHECK-MLIR:         affine.store %[[VAL_19]], %[[VAL_15]][0] : memref<?x!polygeist.struct<(memref<?xi32, 4>)>>
...
// CHECK-MLIR:         %[[VAL_22:.*]] = memref.memory_space_cast %[[VAL_9]] : memref<?x!polygeist.struct<(!sycl_range_1_, !polygeist.struct<(memref<?xi32, 4>)>)>> to memref<?x!polygeist.struct<(!sycl_range_1_, !polygeist.struct<(memref<?xi32, 4>)>)>, 4>
// CHECK-MLIR:         affine.store %[[VAL_21]], %[[VAL_3:.*]][0] : memref<1x!sycl_item_1_>
// CHECK-MLIR:         sycl.call @"operator()"(%[[VAL_22]], %[[VAL_4:.*]]) {MangledFunctionName = @_ZNK4sycl3_V16detail18RoundedRangeKernelINS0_4itemILi1ELb1EEELi1EZ4testRNS0_5queueEEUlNS0_2idILi1EEEE_EclES4_, TypeName = @RoundedRangeKernel} : (memref<?x!polygeist.struct<(!sycl_range_1_, !polygeist.struct<(memref<?xi32, 4>)>)>, 4>, memref<?x!sycl_item_1_>) -> ()
