...
func.func @test1(%arg0 : memref<?x!sycl_accessor_1_f32_rw_gb, 4>) {
  ...
  affine.for %i = 0 to 64 {
    ...
    affine.store %1, %alloca_0[0] {tag_name = "test13_store2"}: memref<1x!sycl_id_1>
    %2 = sycl.accessor.subscript %arg0[%cast_0] {tag = "test13_sub", ArgumentTypes = [memref<?x!sycl_accessor_1_f32_rw_gb, 4>, memref<?x!sycl_id_1>], FunctionName = @"operator[]", MangledFunctionName = @subscript, TypeName = @accessor} : (memref<?x!sycl_accessor_1_f32_rw_gb, 4>, memref<?x!sycl_id_1>) -> memref<?xf32, 4>
    %3 = affine.load %2[0] {tag = "test13_load3"} : memref<?xf32, 4>
  }
