...
#define _CLC_DEFINE_BUILTIN_CAST_VEC4_TO_VEC3(vec4_elem_t, to_t)                                      \
  _CLC_INLINE to_t##3 __clc_cast_from_##vec4_elem_t##4_to_##to_t##3(vec4_elem_t##4 from) {            \
...
#define _CLC_DEFINE_BUILTIN_CAST_VEC4_TO_VEC2(vec4_elem_t, to_t)                                      \
  _CLC_INLINE to_t##2 __clc_cast_from_##vec4_elem_t##4_to_##to_t##2(vec4_elem_t##4 from) {            \
...
#define _CLC_DEFINE_BUILTIN_CAST_VEC4_TO_SCALAR(vec4_elem_t, to_t)                                    \
  _CLC_INLINE to_t __clc_cast_from_##vec4_elem_t##4_to_##to_t(vec4_elem_t##4 from) {                  \
    vec4_elem_t##4 casted = as_##vec4_elem_t##4(from);                                                \
    return as_##to_t(casted.x);                                                                       \
  }
#define _CLC_DEFINE_BUILTIN_CAST_VEC3_TO_VEC4(from_t, vec4_elem_t)                                    \
  _CLC_INLINE vec4_elem_t##4 __clc_cast_from_##from_t##3_to_##vec4_elem_t##4(from_t##3 from) {        \
    vec4_elem_t##3 casted = as_##vec4_elem_t##3(from);                                                \
    return as_##vec4_elem_t##4(casted);                                                               \
  }
#define _CLC_DEFINE_BUILTIN_CAST_VEC2_TO_VEC4(from_t, vec4_elem_t)                                    \
  _CLC_INLINE vec4_elem_t##4 __clc_cast_from_##from_t##2_to_##vec4_elem_t##4(from_t##2 from) {        \
...
#define _CLC_DEFINE_BUILTIN_CAST_SCALAR_TO_VEC4(from_t, vec4_elem_t)                                  \
  _CLC_INLINE vec4_elem_t##4 __clc_cast_from_##from_t##_to_##vec4_elem_t##4(from_t from) {            \
...
#define _CLC_DEFINE_CAST_VEC4(vec4_elem_t, to_t)                                                      \
  _CLC_INLINE to_t##4 __clc_cast_from_##vec4_elem_t##4_to_##to_t##4(vec4_elem_t##4 from) {            \
...
// The ockl functions/builtins expect the resource in constant addrspace.
...
void __ockl_image_store_1D(_CLC_CONST_AS unsigned int *tex, int coord, float4 color);
void __ockl_image_store_2D(_CLC_CONST_AS unsigned int *tex, int2 coord, float4 color);
void __ockl_image_store_3D(_CLC_CONST_AS unsigned int *tex, int3 coord, float4 color);
void __ockl_image_storeh_1D(_CLC_CONST_AS unsigned int *tex, int coord, half4 color);
void __ockl_image_storeh_2D(_CLC_CONST_AS unsigned int *tex, int2 coord, half4 color);
void __ockl_image_storeh_3D(_CLC_CONST_AS unsigned int *tex, int3 coord, half4 color);

void __ockl_image_store_1Da(_CLC_CONST_AS unsigned int *tex, int2 coord, float4 color);
void __ockl_image_store_2Da(_CLC_CONST_AS unsigned int *tex, int4 coord, float4 color);
void __ockl_image_storeh_1Da(_CLC_CONST_AS unsigned int *tex, int2 coord, half4 color);
void __ockl_image_storeh_2Da(_CLC_CONST_AS unsigned int *tex, int4 coord, half4 color);

float4 __ockl_image_sample_1D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float coord);
float4 __ockl_image_sample_2D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float2 coord);
float4 __ockl_image_sample_3D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float3 coord);
half4 __ockl_image_sampleh_1D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float coord);
half4 __ockl_image_sampleh_2D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float2 coord);
half4 __ockl_image_sampleh_3D(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float3 coord);

float4 __ockl_image_sample_1Da(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float2 coord);
float4 __ockl_image_sample_2Da(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float4 coord);
half4 __ockl_image_sampleh_1Da(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float2 coord);
half4 __ockl_image_sampleh_2Da(_CLC_CONST_AS unsigned int *tex, _CLC_CONST_AS unsigned int *samp, float4 coord);
...
#define _CLC_DEFINE_IMAGE_BINDLESS_FETCH_BUILTIN(                                     \
  ...
  _CLC_DEF elem_t _CLC_MANGLE_FUNC_IMG_HANDLE(                                        \
    ...
    _CLC_CONST_AS unsigned int *tex = (_CLC_CONST_AS unsigned int *)imageHandle;      \
...
#define _CLC_DEFINE_IMAGE_BINDLESS_FETCH_8_BUILTIN(                                   \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled)                        \
  _CLC_DEFINE_IMAGE_BINDLESS_FETCH_16_BUILTIN(dimension, elem_t, elem_t_mangled,      \
                                              coord_t, coord_mangled)
...
#define _CLC_DEFINE_IMAGE_BINDLESS_WRITE_BUILTIN(                                    \
  ...
  _CLC_DEF void _CLC_MANGLE_FUNC_IMG_HANDLE(                                         \
    ...
    __ockl_image_store##builtin_ret_postfix##_##dimension##D(tex, coord, outColor);  \
  }
...
#define _CLC_DEFINE_IMAGE_BINDLESS_WRITE_8_BUILTIN(                                   \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled)                        \
  _CLC_DEFINE_IMAGE_BINDLESS_WRITE_16_BUILTIN(dimension, elem_t, elem_t_mangled,      \
                                              coord_t, coord_mangled)
...
#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_BUILTIN(                               \
    ...
    builtin_ret_t, builtin_ret_postfix)                                               \
  _CLC_DEF elem_t _CLC_MANGLE_FUNC_IMG_HANDLE(                                        \
      23, __spirv_ImageArrayFetch, I##elem_t_mangled,                                 \
      coord_mangled##ET_T0_T1_i)(ulong imageHandle, coord_t coord, int layer) {       \
    _CLC_CONST_AS unsigned int *tex = (_CLC_CONST_AS unsigned int *)imageHandle;      \
    int##vec_size arrayCoord =                                                        \
      (int##vec_size)(_CLC_ARRAY_COORD_PARAMS_##dimension##D(coord, layer));          \
    builtin_ret_t##4 color =                                                          \
      __ockl_image_load##builtin_ret_postfix##_##dimension##Da(tex, arrayCoord);      \
    return __clc_cast_from_##builtin_ret_t##4_to_##elem_t(color);                     \
  }

#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(                            \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_BUILTIN(dimension, elem_t,                   \
                                                 elem_t_mangled, coord_t,             \
                                                 coord_mangled, vec_size,             \
                                                 float,)
...
#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(                             \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(dimension, elem_t,                \
                                                    elem_t_mangled, coord_t,          \
                                                    coord_mangled, vec_size)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, float2, Dv2_f, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(1, float4, Dv4_f, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, float4, Dv4_f, int2, Dv2_i, 4)

// Half
#ifdef cl_khr_fp16
// return 1-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(1, half, DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, half, DF16_, int2, Dv2_i, 4)
// return 2-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(1, half2, Dv2_DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, half2, Dv2_DF16_, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(1, half4, Dv4_DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, half4, Dv4_DF16_, int2, Dv2_i, 4)
#endif
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, int2, Dv2_i, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(1, int4, Dv4_i, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, int4, Dv4_i, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, uint2, Dv2_j, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(1, uint4, Dv4_j, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_32_BUILTIN(2, uint4, Dv4_j, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, short2, Dv2_s, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(1, short4, Dv4_s, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, short4, Dv4_s, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, ushort2, Dv2_t, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(1, ushort4, Dv4_t, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_16_BUILTIN(2, ushort4, Dv4_t, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(2, char2, Dv2_a, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(1, char4, Dv4_a, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(2, char4, Dv4_a, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(2, uchar2, Dv2_h, int2, Dv2_i, 4)
// return 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(1, uchar4, Dv4_h, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_FETCH_8_BUILTIN(2, uchar4, Dv4_h, int2, Dv2_i, 4)
...
#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_BUILTIN(                               \
    ...
    builtin_ret_t, builtin_ret_postfix)                                               \
  _CLC_DEF void _CLC_MANGLE_FUNC_IMG_HANDLE(                                          \
      23, __spirv_ImageArrayWrite, I, coord_mangled##elem_t_mangled##EvT_T0_iT1_)(    \
...
#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(                            \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_BUILTIN(dimension, elem_t,                   \
                                                 elem_t_mangled, coord_t,             \
                                                 coord_mangled, vec_size,             \
                                                 float,)
...
#define _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(                             \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(dimension, elem_t,                \
                                                    elem_t_mangled, coord_t,          \
                                                    coord_mangled, vec_size)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, float2, Dv2_f, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(1, float4, Dv4_f, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, float4, Dv4_f, int2, Dv2_i, 4)

// Half
#ifdef cl_khr_fp16
// write 1-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(1, half, DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, half, DF16_, int2, Dv2_i, 4)
// write 2-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(1, half2, Dv2_DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, half2, Dv2_DF16_, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(1, half4, Dv4_DF16_, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, half4, Dv4_DF16_, int2, Dv2_i, 4)
#endif
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, int2, Dv2_i, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(1, int4, Dv4_i, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, int4, Dv4_i, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, uint2, Dv2_j, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(1, uint4, Dv4_j, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_32_BUILTIN(2, uint4, Dv4_j, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, short2, Dv2_s, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(1, short4, Dv4_s, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, short4, Dv4_s, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, ushort2, Dv2_t, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(1, ushort4, Dv4_t, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_16_BUILTIN(2, ushort4, Dv4_t, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(2, char2, Dv2_a, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(1, char4, Dv4_a, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(2, char4, Dv4_a, int2, Dv2_i, 4)
...
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(2, uchar2, Dv2_h, int2, Dv2_i, 4)
// write 4-channel color data
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(1, uchar4, Dv4_h, int, i, 2)
_CLC_DEFINE_IMAGE_ARRAY_BINDLESS_WRITE_8_BUILTIN(2, uchar4, Dv4_h, int2, Dv2_i, 4)
...
//
// SAMPLED IMAGES
// Note: consider splitting it off in sampled_image.cl when it grows larger.
//

// From
// https://github.com/ROCm/clr/tree/amd-staging/hipamd/include/hip/amd_detail/texture_fetch_functions.h
static _CLC_CONST_AS const unsigned int SAMPLER_OBJECT_OFFSET_DWORD = 12;
...
#define _CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_BUILTIN(                               \
  ...
  _CLC_DEF elem_t _CLC_MANGLE_FUNC_IMG_HANDLE(                                        \
    ...
    builtin_ret_t##4 color =                                                          \
      __ockl_image_sample##builtin_ret_postfix##_##dimension##D(tex, samp, coord);    \
    return __clc_cast_from_##builtin_ret_t##4_to_##elem_t(color);                     \
  }

#define _CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(                            \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled)                        \
  _CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_BUILTIN(dimension, elem_t, elem_t_mangled,   \
                                           coord_t, coord_mangled, float,)
...
#define _CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(                             \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled)                        \
  _CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(dimension, elem_t,                \
                                                    elem_t_mangled, coord_t,          \
                                                    coord_mangled)
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(2, float2, Dv2_f, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(3, float2, Dv2_f, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(1, float4, Dv4_f, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(2, float4, Dv4_f, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(3, float4, Dv4_f, float3, Dv3_f)
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, half2, Dv2_DF16_, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, half2, Dv2_DF16_, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(1, half4, Dv4_DF16_, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, half4, Dv4_DF16_, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, half4, Dv4_DF16_, float3, Dv3_f)
#endif
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(2, uint2, Dv2_j, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(3, uint2, Dv2_j, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(1, uint4, Dv4_j, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(2, uint4, Dv4_j, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_32_BUILTIN(3, uint4, Dv4_j, float3, Dv3_f)
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, short2, Dv2_s, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, short2, Dv2_s, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(1, short4, Dv4_s, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, short4, Dv4_s, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, short4, Dv4_s, float3, Dv3_f)
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, ushort2, Dv2_t, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, ushort2, Dv2_t, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(1, ushort4, Dv4_t, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(2, ushort4, Dv4_t, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_16_BUILTIN(3, ushort4, Dv4_t, float3, Dv3_f)
...
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(2, uchar2, Dv2_h, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(3, uchar2, Dv2_h, float3, Dv3_f)
// return 4-channel color data
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(1, uchar4, Dv4_h, float, f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(2, uchar4, Dv4_h, float2, Dv2_f)
_CLC_DEFINE_SAMPLEDIMAGE_BINDLESS_READ_8_BUILTIN(3, uchar4, Dv4_h, float3, Dv3_f)
...
#define _CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_BUILTIN(                         \
    ...
    builtin_ret_t, builtin_ret_postfix)                                               \
  _CLC_DEF elem_t _CLC_MANGLE_FUNC_IMG_HANDLE(                                        \
      22, __spirv_ImageArrayRead, I##elem_t_mangled,                                  \
      coord_mangled##ET_T0_T1_i)(ulong imageHandle, coord_t coord, int layer) {       \
    _CLC_CONST_AS unsigned int *tex = (_CLC_CONST_AS unsigned int *)imageHandle;      \
    _CLC_CONST_AS unsigned int *samp = tex + SAMPLER_OBJECT_OFFSET_DWORD;             \
    float##vec_size arrayCoord =                                                      \
      (float##vec_size)(_CLC_ARRAY_COORD_PARAMS_##dimension##D(coord, (float)layer)); \
    builtin_ret_t##4 color =                                                          \
      __ockl_image_sample##builtin_ret_postfix##_##dimension##Da(tex, samp,           \
                                                                 arrayCoord);         \
    return __clc_cast_from_##builtin_ret_t##4_to_##elem_t(color);                     \
  }
...
#define _CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(                      \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_BUILTIN(dimension, elem_t,             \
                                                       elem_t_mangled, coord_t,       \
                                                       coord_mangled, vec_size,       \
                                                       float,)
...
#define _CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(                       \
    dimension, elem_t, elem_t_mangled, coord_t, coord_mangled, vec_size)              \
  _CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(dimension, elem_t,          \
                                                          elem_t_mangled, coord_t,    \
                                                          coord_mangled, vec_size)

// Float
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, float, f, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, float, f, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, float2, Dv2_f, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, float2, Dv2_f, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, float4, Dv4_f, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, float4, Dv4_f, float2, Dv2_f, 4)

// Half
#ifdef cl_khr_fp16
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, half, DF16_, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, half, DF16_, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, half2, Dv2_DF16_, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, half2, Dv2_DF16_, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, half4, Dv4_DF16_, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, half4, Dv4_DF16_, float2, Dv2_f, 4)
#endif

// Int
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, int, i, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, int, i, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, int2, Dv2_i, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, int2, Dv2_i, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, int4, Dv4_i, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, int4, Dv4_i, float2, Dv2_f, 4)

// Unsigned Int
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, uint, j, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, uint, j, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, uint2, Dv2_j, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, uint2, Dv2_j, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(1, uint4, Dv4_j, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_32_BUILTIN(2, uint4, Dv4_j, float2, Dv2_f, 4)

// Short
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, short, s, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, short, s, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, short2, Dv2_s, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, short2, Dv2_s, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, short4, Dv4_s, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, short4, Dv4_s, float2, Dv2_f, 4)

// Unsigned Short
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, ushort, t, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, ushort, t, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, ushort2, Dv2_t, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, ushort2, Dv2_t, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(1, ushort4, Dv4_t, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_16_BUILTIN(2, ushort4, Dv4_t, float2, Dv2_f, 4)

// Char
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, char, a, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, char, a, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, char2, Dv2_a, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, char2, Dv2_a, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, char4, Dv4_a, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, char4, Dv4_a, float2, Dv2_f, 4)

// Unsigned Char
// return 1 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, uchar, h, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, uchar, h, float2, Dv2_f, 4)
// return 2 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, uchar2, Dv2_h, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, uchar2, Dv2_h, float2, Dv2_f, 4)
// return 4 channel color data
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(1, uchar4, Dv4_h, float, f, 2)
_CLC_DEFINE_SAMPLEDIMAGE_ARRAY_BINDLESS_READ_8_BUILTIN(2, uchar4, Dv4_h, float2, Dv2_f, 4)
