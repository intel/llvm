...
#define AMDGPU_ATOMIC_FP64_ADD_IMPL(AS, CHECK,                     \
                                    NEW_BUILTIN)                   \
  _CLC_OVERLOAD _CLC_DECL double                                   \
      __spirv_AtomicFAddEXT(                                       \
          AS double *p, int scope,                                 \
          int semantics, double val) {                             \
    if (CHECK)                                                     \
      return NEW_BUILTIN(p, val);                                  \
    int atomic_scope = 0, memory_order = 0;                        \
...
AMDGPU_ATOMIC_FP64_ADD_IMPL(local,
                            AMDGPU_ARCH_BETWEEN(9010, 10000),
