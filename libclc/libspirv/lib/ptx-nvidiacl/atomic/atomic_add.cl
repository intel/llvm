...
#define __CLC_NVVM_ATOMIC_ADD_DOUBLE_IMPL(ADDR_SPACE,                         \
                                          ADDR_SPACE_NV)                      \
  __attribute__((always_inline)) _CLC_OVERLOAD _CLC_DECL double               \
      __spirv_AtomicFAddEXT(ADDR_SPACE double *pointer, int scope,            \
                            int semantics, double value) {                    \
...
Memory order is stored in the lowest 5 bits */                                \
    ...
    if (__clc_nvvm_reflect_arch() >= 600) {                                   \
      ...
      ADDR_SPACE long *pointer_int =                                          \
          (ADDR_SPACE long *)pointer;                                         \
      long old_int;                                                           \
      long new_val_int;                                                       \
      do {                                                                    \
        old_int =                                                             \
