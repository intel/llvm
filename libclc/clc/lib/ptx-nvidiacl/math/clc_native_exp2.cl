...
_CLC_DEF _CLC_OVERLOAD float __clc_native_exp2(float x) {
...
_CLC_DEF _CLC_OVERLOAD half __clc_native_exp2(half x) {
  return (__USE_HALF_EXP2_APPROX) ? __nvvm_ex2_approx_f16(x)
                                  : __clc_native_exp2((float)x);
}

_CLC_DEF _CLC_OVERLOAD half2 __clc_native_exp2(half2 x) {
  return (__USE_HALF_EXP2_APPROX) ? __nvvm_ex2_approx_f16x2(x)
                                  : (half2)(__clc_native_exp2((float)x.x),
                                            __clc_native_exp2((float)x.y));
}
