...
def main():
    ...
    parser.add_argument(
        "-o", "--output", type=str, required=True, help="Output C++ file"
    )
    parser.add_argument(
        "-i",
        "--toolchain-dir",
    ...
    parser.add_argument(
        "--prefix", type=str, required=True, help="Prefix for virtual file locations"
    )
    parser.add_argument(
        "-m",
        "--manifest-input",
        type=str,
        help="Build from this whitelist manifest (read-only).",
    )
    parser.add_argument(
        "--manifest-output",
        type=str,
        help="Glob for files and write them to this capture manifest.",
    )
    parser.add_argument(
        "--blacklist",
        type=str,
    ...
    if args.manifest_input and args.manifest_output:
        print(
            "Error: --manifest-input and --manifest-output are mutually exclusive.",
            file=sys.stderr,
        )
        sys.exit(1)
...
const resource_file ToolchainFiles[] = {"""
        ...
        if args.manifest_input:
            print(
                f"Reading resource list from whitelist manifest: {args.manifest_input}"
            )
            ...
            if args.manifest_output:
                print(
                    f"Globbing for resources and writing capture manifest to: {args.manifest_output}"
                )
            else:
